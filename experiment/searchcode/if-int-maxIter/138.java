...
public class Settings {
    ...
    public static Configuration addPathToDC( Configuration conf , String path ) throws IOException {
        ...
        for( Path p : listedPaths ) {   
        		//BugBug: Hack to fix
                if(!p.toString().toString().endsWith("_SUCCESS") && !p.toString().endsWith("_logs")){
                	System.out.println("Filtering out files beginning with _");
                	System.out.println(" Add File to DDDDCC " + p.toUri().toString() );
