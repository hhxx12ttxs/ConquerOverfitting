...
public class NPCEntity extends EntityPlayer {
    ...
    public void lookAtPlayer(Player player, NPCEntity npc) {
        if (npc.getBukkitEntity().getWorld() != player.getWorld()) {
            return;
        }
        Location npcLoc = npc.getBukkitEntity().getLocation(), target = player.getLocation();
        double xDiff = target.getX() - npcLoc.getX();
        double yDiff = target.getY() - npcLoc.getY();
        double zDiff = target.getZ() - npcLoc.getZ();
        ...
        npc.yaw = (float) (newYaw - 90);
        npc.pitch = (float) newPitch;
    }

    public void attackPlayer(Player player, NPCEntity npc, int damageAmount) {
        npc.lookAtPlayer((Player) player, npc);
        npc.animateArmSwing();
        npc.pathFindTo(player.getLocation(), maxIter);
        LivingEntity other = (LivingEntity) player;
        other.damage(damageAmount, player);
    }
