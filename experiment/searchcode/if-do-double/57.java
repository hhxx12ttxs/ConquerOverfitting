...
public abstract class EntityLiving extends Entity {
	protected HashMap activeSkillsMap;
	 ...
	 * (Raytrace)
	 * MODIF EntityLiving: canEntityBeSeen(Entity par1Entity)
	 */
	public boolean canEntityBeSeen(Entity par1Entity) {
		return (par1Entity instanceof EntityLiving && ((EntityLiving)par1Entity).isSkillActive(TKHSkill.invisibility))?
				false : worldObj.rayTraceBlocks(
	 ...
	 * MODIF EntityLiving: fall(float par1)
	 */
	protected void fall(float par1) {
		...
		if (i > 0) {
			if (i > 4 || (this.entityType != null && this.entityType.equals("Sismeroan"))) {
				worldObj.playSoundAtEntity(this, "damage.fallbig", 1.0F, 1.0F);
			} else {
				worldObj.playSoundAtEntity(this, "damage.fallsmall", 1.0F, 1.0F);
			}
			attackEntityFrom(DamageSource.fall, isSkillActive(TKHSkill.decFallDmg) && isSneaking()?(int)(i*0.5):i);
	 ...
	 * MODIF EntityLiving: moveEntityWithHeading(float par1, float par2)
	 */
	public void moveEntityWithHeading(float par1, float par2) {
		if (isInWater()) {

			if (isSkillActive(TKHSkill.speedWater)) {
				//FIXME deplacement aquatique
				...
				if (!isInsideOfMaterial(Material.water) && isCollidedHorizontally 
						&& isOffsetPositionInLiquid(motionX, ((motionY + 0.60000002384185791D) - posY) + d, motionZ)) {
					motionY = 0.40000001192092896D;
				}
				...
				if (isCollidedHorizontally && isOffsetPositionInLiquid(motionX,
						((motionY + 0.60000002384185791D) - posY) + d, motionZ)) {
					motionY = 0.30000001192092896D;
				}
			}
		} else if (handleLavaMovement()) {
			if (isSkillActive(TKHSkill.speedLava)) {
				double d = posY;
				moveFlying(par1, par2, 0.02F);
				if (isSprinting()) {
					motionX *= 9;
		...
		if (Reflector.hasMethod(50)) {
			...
			if (block != null) {
				return Reflector.callBoolean(block, 50, new Object[] {
						worldObj, Integer.valueOf(i), Integer.valueOf(j),
						Integer.valueOf(k) });
			}
		}
	...
	/**
	 * MODIF EntityLiving: canBreatheUnderwater()
	 */
	public boolean canBreatheUnderwater() {
		if (isSkillActive(TKHSkill.airWater))
			return true;
		return false;
	}

	public void setMoveStrafing(float par1) {
		moveStrafing = par1;
	}

	public float setMoveForward(float par1) {
		moveForward = par1;
		return moveForward;
	}
	...
	public void onLivingUpdate() {
		...
		setMoveStrafing(getMoveStrafing() * 0.98F);
		setMoveForward(getMoveForward() * 0.98F);
		randomYawVelocity *= 0.9F;
		float f = landMovementFactor;
		landMovementFactor *= getSpeedModifier();
		moveEntityWithHeading(getMoveStrafing(), getMoveForward());
	 ...
	 * MODIF EntityLiving: isMovementBlocked()
	 */
	protected boolean isMovementBlocked() {
		return health <= 0 || isSkillActive(TKHSkill.isStun);
	}
	 ...
	 * MODIF EntityLiving: jump()
	 */
	protected void jump() {
		motionY = isSkillActive(TKHSkill.incJump)?0.4199998688697815*1.35:0.4199998688697815D;
	...
	/**
	 * MODIF EntityLiving: isPotionApplicable(PotionEffect par1PotionEffect)
	 */
	public boolean isPotionApplicable(PotionEffect par1PotionEffect) {
		if (par1PotionEffect.getPotionID() == Potion.poison.id
				&& isSkillActive(TKHSkill.imunPoison))
			return false;
	...
	/** MODIF EntityLiving: Rajout methods skill **/
	protected void updateSkillEffects() {
		if (isInWater() && isSkillActive(TKHSkill.walkWater))
			TKHSkill.walkWater.SkillWalkWater(this);
		if (handleLavaMovement() && isSkillActive(TKHSkill.walkLava))
			TKHSkill.walkLava.SkillWalkLava(this);

		Iterator iterator = activeSkillsMap.keySet().iterator();
		do {
			if (!iterator.hasNext()) {
				break;
			}
			...
			Integer integer = (Integer) iterator.next();
			TKHSkillEffect skillEffect = (TKHSkillEffect) activeSkillsMap
					.get(integer);

			if (!skillEffect.onUpdate(this)) {
				iterator.remove();
				onFinishedSkillEffect(skillEffect);
			}
			
		} while (true);
	}

	public void addSkillEffect(TKHSkillEffect par1SkillEffect) {
		if (!isSkillApplicable(par1SkillEffect)) {
			return;
		}

		if (activeSkillsMap.containsKey(Integer.valueOf(par1SkillEffect
				.GetSkillID()))) {
			((TKHSkillEffect) activeSkillsMap.get(Integer
					.valueOf(par1SkillEffect.GetSkillID())))
					.combine(par1SkillEffect);
			onChangedSkillEffect((TKHSkillEffect) activeSkillsMap.get(Integer
					.valueOf(par1SkillEffect.GetSkillID())));
		} else {
			activeSkillsMap.put(Integer.valueOf(par1SkillEffect.GetSkillID()),
					par1SkillEffect);
			onNewSkillEffect(par1SkillEffect);
		}
	}

	public boolean isSkillApplicable(TKHSkillEffect par1SkillEffect) {
		if (getCreatureAttribute() == EnumCreatureAttribute.UNDEAD) {
			int i = par1SkillEffect.GetSkillID();
		}

		return true;
	}

	public boolean isSkillActive(TKHSkill par1Skill) {
		return activeSkillsMap.containsKey(Integer.valueOf(par1Skill.skillID));
	}

	public void removeSkillEffect(int par1) {
		activeSkillsMap.remove(Integer.valueOf(par1));
	}

	protected void onNewSkillEffect(TKHSkillEffect par1SkillEffect) {
	}

	protected void onChangedSkillEffect(TKHSkillEffect par1SkillEffect) {
	}

	protected void onFinishedSkillEffect(TKHSkillEffect par1SkillEffect) {
	}
