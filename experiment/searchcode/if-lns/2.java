...
public class CompletionDialog3 extends JDialog {
	...
	public void showDialog() {
		...
		if (s.javaVersion.compareTo("1.4.0") >= 0) {
			...
			// decorated ?? how can I get exact border size?
			dim.height += 30; dim.width += 13;
		}
		if (dim.height > Opt.completionDialogMaxHeight()) {
			dim.width = (int)(dim.width+scrollPane.getVerticalScrollBar().getPreferredSize().getWidth());
			dim.height = Opt.completionDialogMaxHeight();
		}
		if (dim.width > Opt.completionDialogMaxWidth()) {
			dim.width = Opt.completionDialogMaxWidth();
			dim.height = (int)(dim.height+scrollPane.getHorizontalScrollBar().getPreferredSize().getHeight());
		}
		setSize(dim); 
