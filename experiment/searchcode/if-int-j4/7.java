 ...
 implements Runnable
{
 ...
 public boolean handleEvent (Event event)
 {
	...
	switch (event.id)
	{
		...
		showStatus ("Slime Volleyball 2-Player");
		break;
		...
		switch (event.key)
		{
		    ...
		    case 65: // 'A'
		    case 97: // 'a'
			p1XV = (scoringRun <= -scoringRunForSuper) ? -16:
			- 8;
			break label0;
		    ...
		    case 119: // 'w'
			if (p1Y == 0)
			    p1YV = (scoringRun <= -scoringRunForSuper) ? 45:
			    31;
			break label0;
		    ...
		    case 74: // 'J'
		    case 106: // 'j'
			p2XV = (scoringRun >= scoringRunForSuper) ? -16:
			- 8;
			break label0;
		    ...
		    case 105: // 'i'
			if (p2Y == 0)
			    p2YV = (scoringRun >= scoringRunForSuper) ? 45:
			    31;
			break label0;
 ...
 private void DrawSlimers ()
 {
	...
	screen.setColor ((scoringRun <= -scoringRunForSuper) ? slimeColours [frenzyCol = ((frenzyCol + 1) % slimeColours.length)]:
	slimeColours [p1Col]);
	...
	if (p2Blink == 0)
	{
	    screen.setColor (flag ? Color.gray:
	    Color.white);
	    screen.fillOval (i - i3, l - j3, i3, j3);
	    if (k4 > 0 && !flag)
	    {
 ...
 private void MoveSlimers ()
 {
	...
	if (p1YV != 0)
	{
	    p1Y += p1YV -= (scoringRun <= -scoringRunForSuper) ? 4:
	    2;
	    if (p1Y < 0)
	    {
		p1Y = 0;
	...
	if (p2YV != 0)
	{
	    p2Y += p2YV -= (scoringRun >= scoringRunForSuper) ? 4:
	    2;
	    if (p2Y < 0)
	    {
		p2Y = 0;
 ...
 public void run ()
 {
	...
	while (nScore != 0 && nScore != 10 && gameThread != null)
	{
		...
		if ((ballX <= 500) && (scoringRun >= 0))
		    scoringRun++;
		else if ((ballX > 500) && (scoringRun <= 0))
		    scoringRun--;
		else if ((ballX <= 500) && (scoringRun <= 0))
		    scoringRun = 1;
		else if ((ballX > 500) && (scoringRun >= 0))
		    scoringRun = -1;
		promptMsg = ballX <= 500 ? slimeColText [p2Col]:
		slimeColText [p1Col];
		if (!fP1Touched && !fP2Touched)
		    promptMsg = "What can I say?";
		else
		    if ((scoringRun < 0 ? -scoringRun:
		    scoringRun) == scoringRunForSuper)
