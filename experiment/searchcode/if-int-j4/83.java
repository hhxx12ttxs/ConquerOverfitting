...
public class TexturePersonFinderFX extends TextureFX
{
    private Minecraft mc;
    private int compassIconImageData[], compassImageData[];
    private ByteBuffer buffer;
    private double field_4229_i;
    private double field_4228_j;
    private byte[] data = new byte[256];

    public TexturePersonFinderFX(Minecraft minecraft)
    {
        super(mod_MyPeople.personFinder.getIconFromDamage(0));
        buffer = GLAllocation.createDirectByteBuffer(0x1000000);
        compassIconImageData = new int[256];
        compassImageData = new int[256];
        mc = minecraft;
        tileImage = 1;
        try
        {
            BufferedImage bufferedimage = ImageIO.read((net.minecraft.client.Minecraft.class).getResource("/gui/items.png"));
            int i = (Item.compass.iconIndex % 16) * 16;
            int j = (Item.compass.iconIndex / 16) * 16;
    ...
    public void onTick()
    {
        ...
        if (mc.theWorld != null && mc.thePlayer != null)
        {
        	...
        	if(mod_MyPeople.entityToFind != null){
        		chunkcoordinates = new ChunkCoordinates((int)Math.floor(mod_MyPeople.entityToFind.posX), (int)Math.floor(mod_MyPeople.entityToFind.posY), (int)Math.floor(mod_MyPeople.entityToFind.posZ));
        	}
        	...
        	if(mc.theWorld.worldProvider.worldType!=0){
