...
import fr.cemagref.prima.regionalmodel.altmark.parameters.EmployedSectorChoice;
...
public class GermanySectorChoice extends Scenario {
    ...
    public final void init(final MunicipalitySet region) throws BadDataException {
        ...
        for (Municipality mun : municipalities) {
            ...
            SortedMap<Integer, Value<Integer, Individual>> dynamics = readDynamicsFile(mun);
            int i = beginYear;
    ...
    private SortedMap<Integer, Value<Integer,Individual>> readDynamicsFile(Municipality mun) throws BadDataException {
        SortedMap<Integer, Value<Integer, Individual>> dynamics = new TreeMap<Integer, Value<Integer, Individual>>();
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.length() > 0) {
                    ...
                    Value<Integer, Individual> dynTable = new EmployedSectorChoice(probaFile);
    ...
    protected void preStep() {
        this._region.initSectorChoiceDynamic();
    }

    private static class GermanySectorChoiceEvent implements Event<Municipality> {

        Value<Integer, Individual> distribution;

        public GermanySectorChoiceEvent(Value<Integer, Individual> newDistribution) {
            distribution = newDistribution;
        }

        @Override
        public void process(Municipality munic) throws ProcessingException {
            munic.getMyRegion().addSectorChoiceScenarioDynamic(munic, distribution);
        }
    }

    private GermanySectorChoiceEvent buildEvent(Value<Integer, Individual> newDistribution) {
        GermanySectorChoiceEvent event = new GermanySectorChoiceEvent(newDistribution);
        return event;
