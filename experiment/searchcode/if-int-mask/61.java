...
class InArchive extends Header {
    ...
    int GetNextFolderItem(Folder folder)  throws IOException {
        ...
        for (i = 0; i < numCoders; i++) {
            folder.Coders.add(new SevenZip_CoderInfo());
            SevenZip_CoderInfo coder = folder.Coders.Back();
    ...
    int ReadAndDecodePackedStreams(long baseOffset,
        ...
        int packIndex = 0;
        SevenZip_Decoder decoder = new SevenZip_Decoder(false); // _ST_MODE
        ...
        for(int i = 0; i < folders.size(); i++) {
            ...
            SevenZip.SequentialOutStreamImp2 outStreamSpec = new SevenZip.SequentialOutStreamImp2();
