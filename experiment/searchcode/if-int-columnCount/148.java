package com.pb.sandag.aquavis;

import com.pb.sandag.VehicleType;
...
public class SandagAquavisInputBuilder {
    ...
    public SandagAquavisInputBuilder(PropertyDeluxe properties) {
        connectionUrl = formConnectionUrl(properties.getString(REPORTS_DATABASE_IPADDRESS_PROPERTY),
                                          properties.getInt(REPORTS_DATABASE_PORT_PROPERTY),
                                          properties.getString(REPORTS_DATABASE_NAME_PROPERTY),
                                          properties.hasKey(REPORTS_DATABASE_USERNAME_PROPERTY) ? properties.getString(REPORTS_DATABASE_USERNAME_PROPERTY) : null,
                                          properties.hasKey(REPORTS_DATABASE_PASSWORD_PROPERTY) ? properties.getString(REPORTS_DATABASE_PASSWORD_PROPERTY) : null,
                                          properties.hasKey(REPORTS_DATABASE_INSTANCE_PROPERTY) ? properties.getString(REPORTS_DATABASE_INSTANCE_PROPERTY) : null);
        this.properties = properties;
    }

    public void createAquavisInputs(String aquavisNetworkPathProperty, String aquavisTripsPathProperty, String aquavisIntrazonalPathProperty) {
    ...
    private void createAquavisNetwork(Path outputFile) {
        ...
        detemplifyAndRunScript(properties.getPath(CREATE_AQUAVIS_NETWORK_TEMPLATE_PROPERTY),schema,
    ...
    private void createAquavisIntrazonal(Path outputFile) {
        String schema = properties.getString(SCHEMA_NAME_PROPERTY);
        detemplifyAndRunScript(properties.getPath(CREATE_AQUAVIS_INTRAZONAL_TEMPLATE_PROPERTY),schema,
                properties.getString(AQUAVIS_TEMPLATE_SCHEMA_TOKEN_PROPERTY));
        writeTableToCsv(AQUAVIS_INTRAZONAL_TABLE,schema,outputFile);
        addExternalIntrazonalData(outputFile.toString(),properties.getString(AQUAVIS_EXTERNAL_INTRAZONAL_TABLE_PROPERTY));
    }

    private void addExternalIntrazonalData(String intrazonalTablePath, String externalIntrazonalTablePath) {
    ...
    public void writeTableToCsv(String table, String schema, Path outputFile) { //convenient to use outside of this...
    ...
    private String formConnectionUrl(String ipAddress, int port, String databaseName, String username, String password, String instance) {
        ...
        if (username != null)
            url += ";user=" + username + ";" + password; //not super secure, btu ok for now - probably will use SSO normally
        if (instance != null)
