		...
		checkBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (checkBox.isSelected()) {
					...
					float mb = (count * 70.0f) / 1024;
					bar.setString(String.format(count == 0 ? "No images to download!"
							: "%d of %d cards finished! Please wait! [%.1f Mb]", 0, DownloadPictures.this.cardsInGame.size(), mb));
				} else {
			...
			for (update(0); (checkBox.isSelected() ? cardIndex < cardsInGame.size() : cardIndex < cards.size() ) && !cancel; update(cardIndex + 1)) {
				try {
					...
					if (ignoreUrls.contains(card.set) || card.token) {
						// we have card in scripts, but we should ignore downloading image for it 
						// (e.g. for cards from custom or not released yet sets
						// such urls should come from card-pictures files instead
						if (card.collector != 0) {  
							continue;
						}
