...
//        final String welcomeUrl = (settings != null && settings.containsKey(Const.PARAM_WELCOME_URL)) ? settings.get(Const.PARAM_WELCOME_URL) :  Const.PATH_WELCOME_URL;
        final boolean loadConnections = (settings != null && settings.containsKey(Const.PARAM_ENABLE_CONNECTIONS)
                && settings.get(Const.PARAM_ENABLE_CONNECTIONS).equals("1"));
        ...
        mainPanel = new MainPanel(loginInfo, loadConnections);
...
//    private void showDiagram(final Diagram d) {
...
//        final DiagramPanel diagramPanel = new DiagramPanel(d, Double.valueOf(h * .65).intValue(), Double.valueOf(w * .65).intValue());
//
//        diagramPanel.addPointClickedListeners(new NavigationEventProvider.PointClickedListener() {
//
//            @Override
//            public void onPointClicked(final Point p) {
//                nimbits.this.mainPanel.addPoint(p);
//            }
//
//        });
//        diagramPanel.addDiagramClickedListeners(new NavigationEventProvider.DiagramClickedListener() {
...
//
//            @Override
//            public void onDiagramClicked(final Diagram d, final String target) {
//                window.hide();
//                showDiagram(d);
//            }
//        });
//
//        window.setHeight(Double.valueOf(h * .65).intValue());
//        window.setWidth(Double.valueOf(w * .65).intValue());
//        window.setHeading(d.getName().getValue());
//        window.add(diagramPanel);
//        window.getHeader().addTool(
...
//    }
    ...
    public void onModuleLoad() {
        ...
        // final String point = Location.getParameter(Const.PARAM_POINT);
        // final String user = Location.getParameter("user");
        final String uuid = Location.getParameter(Const.PARAM_UUID);
        // final String source = Location.getParameter("source");
        ...
        if (doDiagram) {
            ...
            try {
                loadPortalView(uuid, code, oauth_token, doFacebook, doTwitter, doTwitterFinish);
    ...
    private void loadPortalView(final String uuid, final String code, final String oauth_token, final boolean doFacebook, final boolean doTwitter, final boolean doTwitterFinish) throws NimbitsException {
    ...
    private void loadSinglePointDisplay(final String uuid) throws NimbitsException {
        PointServiceAsync pointService;
        pointService = GWT.create(PointService.class);

        pointService.getPointByUUID(uuid, new AsyncCallback<Point>() {
            @Override
            public void onFailure(Throwable throwable) {
                handleError(throwable);
            }
    ...
    private void loadPortal(final boolean doTwitter, final Map<String, String> settings) throws NimbitsException {
