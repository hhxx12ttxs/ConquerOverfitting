...
public class OptionsController extends DefaultHandler {
	...
	public final static File BACKUP_OPTIONS_FILE = new File("calc.dat");
	...
	public static SimulatorOptions loadOptions() {
		...
		try {
			...
			if (OPTIONS_FILE.exists() && OPTIONS_FILE.canRead()) {
				importFile = OPTIONS_FILE;
			} else if (BACKUP_OPTIONS_FILE.exists() && BACKUP_OPTIONS_FILE.canRead()) {
				importFile = BACKUP_OPTIONS_FILE;
			} else if (OLD_OPTIONS_FILE.exists() && OLD_OPTIONS_FILE.canRead()) {
				importFile = OLD_OPTIONS_FILE;
			}

			SAXParser parser = SAXParserFactory.newInstance().newSAXParser();
			parser.parse(importFile, controller);
			if (OLD_OPTIONS_FILE.exists() && !OLD_OPTIONS_FILE.delete()) {
	...
	public static void saveOptions(SimulatorOptions options) 
		...
		File exportFile = BACKUP_OPTIONS_FILE;
		try {
			if (!OPTIONS_FILE.exists()) {
			 	if (OPTIONS_FILE.getParentFile().mkdirs()) {
					if (OPTIONS_FILE.createNewFile()) {
						exportFile = BACKUP_OPTIONS_FILE;
					}
				}
			} else {
				exportFile = OPTIONS_FILE;
			}
		} catch (SecurityException ex) {
			// Could not save to the main location, so safe
			// to ignore.
		}
		
		Transformer trans = TransformerFactory.newInstance().newTransformer();
		trans.transform(new DOMSource(doc), new StreamResult(exportFile));
	}
}
