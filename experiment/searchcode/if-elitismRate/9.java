...
import com.arm.fanucci.ui.Config;
...
public class OptionsController extends DefaultHandler {
	...
	public static SimulatorOptions loadOptions() {
		...
		try {
			...
			parser.parse(Config.OPTIONS.getFile(), controller);
		} catch (Exception ex) {
	...
	public static void saveOptions(SimulatorOptions options) 
		...
		trans.transform(new DOMSource(doc), new StreamResult(Config.OPTIONS.getFile()));
	}
}
