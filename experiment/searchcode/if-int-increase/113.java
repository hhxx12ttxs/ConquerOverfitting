...
public class MutateDemotePipeline extends GPBreedingPipeline {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (maxDepth == 0)
            state.output.fatal("The MutateDemotePipeline " + base + "has an invalid maximum depth (it must be >= 1).",
    ...
    private boolean demotable(final GPInitializer initializer, final GPNode node, final GPFunctionSet set) {
        ...
        for (int x = 0; x < set.nonterminals[t.type].length; x++)
            for (int y = 0; y < set.nonterminals[t.type][x].constraints(initializer).childtypes.length; y++)
                if (set.nonterminals[t.type][x].constraints(initializer).childtypes[y].compatibleWith(initializer,
                        node.constraints(initializer).returntype))
                    return true;
        return false;
    }

    private void demoteSomething(final GPNode node, final EvolutionState state, final int thread,
    ...
    private void _demoteSomething(final GPNode node, final EvolutionState state, final int thread,
            final GPFunctionSet set) {
        int numDemotable = 0;
                        ...
                        for (int z = 0; z < cnode.children.length; z++)
                            if (chityp[z].compatibleWith(initializer, retyp)) {
                                if (numSpots == choice) {
                                    ...
                                    // cnode
                                    GPNode term = (GPNode) (set.terminals[chityp[z].type][state.random[thread]
                                            .nextInt(set.terminals[chityp[z].type].length)].lightClone());
    ...
    private void _demoteSomethingTypeless(final GPNode node, final EvolutionState state, final int thread,
            final GPFunctionSet set) {
        int numDemotable = 0;
    ...
    private int _numDemotableNodes(final GPInitializer initializer, final GPNode root, int soFar,
            final GPFunctionSet set) {
    ...
    private int pickDemotableNode(final GPInitializer initializer, final GPNode root, int num, final GPFunctionSet set) {
        ...
        if (initializer.numAtomicTypes + initializer.numSetTypes == 1) {
            ...
            if (gatherer.node == null) // uh oh
                throw new InternalError("Internal error in pickDemotableNode, nodeInPosition didn't find a node!"); // should
                                                                                                                    // never
                                                                                                                    // happen
    ...
    public int produce(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
        ...
        for (int q = start; q < n + start; q++) {
            ...
            for (int x = 0; x < numTries; x++) {
                ...
                int numdemote = numDemotableNodes(initializer, j.trees[t].child, 0,
