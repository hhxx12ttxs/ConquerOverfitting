...
public class Search implements ISearch {
	...
	private OntModel getOntModelForOntologyURI(String uri,
		...
		System.out.println("osmancan:::" + uri);
	...
	private List<OntModel> getIndexedOntModels(boolean retrieveReasonedModel) {
		...
		if (connectionInstance == null) {
			...
			System.out.println("ONT MODEL regenerated from fresh connection");
		} else {
			try {
				...
				indexOntModels(ontModelsToIndex);
				System.out
						.println("ONT MODEL regenerated from fresh connection");
			}
		}
				...
				if (rdfNode.isURIResource()) {
					...
					if (!containsStringInIgnoreList(uri)) {
						...
						double similarity = computeSimilarity(uriPostfix,
								...
								* MAX_SCORE;
						if (similarity > 0) {
							System.out.println(uri + ", " + similarity);
	...
	private List<ScoredOntResource> retrieveRelatedIndividuals(
			...
			double degradingCoefficient) {
		List<ScoredOntResource> result = new Vector<ScoredOntResource>();
		System.out.println("RETRIEVE_RELATED_INDIVIDUALS individual="
				+ individual.getURI());
		/**
		 * FIXME::
		 * 
		...
		if (!individual.hasProperty(objProp)) {
			System.out
					.println("RETRIEVE_RELATED_INDIVIDUALS individual does not have object property");
			return result;
		}
		...
		while (iter1.hasNext()) {
			...
			System.out.println("RETRIEVE_RELATED_INDIVIDUALS in curStmt1="
					+ curStmt1.toString());
				...
				if (ontologyURI1 != null
					...
					if (relatedIndividual1 != null) {
						...
						while (iter2.hasNext()) {
							...
							System.out.println("FOUND STATEMENTS2:"
									+ curStmt2.toString());
							if (curStmt2.getObject().isURIResource()) {
								...
								System.out.println("relatedIndividual2::"
										+ relatedIndividualURI2);
						...
						while (iter3.hasNext()) {
							...
							System.out.println("FOUND STATEMENTS3:"
									+ curStmt3.toString());
							...
							System.out.println("relatedIndividual3::"
									+ relatedIndividualURI3);
						...
						for (int i = 0; i < intermediateIndividuals.size(); i++) {
							...
							while (iter4.hasNext()) {
								...
								if (ontologyURI4 != null
									...
									System.out.println("relatedIndividual4::"
											+ relatedIndividual4.getURI());
	...
	private List<ScoredOntResource> computeClosureForOntClass(
		...
		if (ontClass != null) {
			...
			while (closureItr.hasNext()) {
				...
				System.out
						.println("cur ont resource = " + curOntClass.getURI());
	...
	private List<ScoredOntResource> getPathRelatedResources(
		...
		for (int x = 0; x < searchResults.size(); x++) {
			...
			for (int y = 0; y < individuals.size(); y++) {
				...
				while (sortedOntClassesBasedOnIndividualCountIterator.hasNext()) {
					...
					if (ontClass == null) {
						System.out
								.println("most relevant container class of individual "
					...
					if (!isClassExcludedForCategorization(ontClass.getURI())
						...
						System.out.println("----\tintegerCountForClass="
								+ integerCountForClass);
		...
		while (ontClasses.hasMoreElements()) {
			...
			double initialScore = totalIndividualScoreForClasses.get(ontClass)
					.doubleValue();
			System.out.println(ontClass.getURI() + " score = " + initialScore);
			if (initialScore > 0) {
				List<ScoredOntResource> subResult = computeClosureForOntClass(
	...
	public eu.iksproject.fise.stores.persistencestore.rest.model.search.Result performQuery(
		...
		try {
			System.out.println("IN PERFORM QUERY:::");
			...
			if (structuralQueryPart.getSPARQLQuery() != null) {
				System.out.println("IN PERFORM QUERY::: getSPARQLQuery");
				...
				System.out
						.println("IN PERFORM QUERY::: getSelectionRelatedExcludedResources");
				...
				for (int i = 0; i < resourceList.getSelectiveResource().size(); i++) {
					SelectiveResource sr = resourceList.getSelectiveResource()
							.get(i);
					System.out.println("OPERATOR:::" + sr.getOperator());
					System.out.println("CLASSURI:::" + sr.getResourceURI());
				}
			...
			System.out.println("Score Threshold set to = " + scoreThreshold);
	...
	private String mergeKeywords(Vector<String> keywords) {
		...
		System.out.println("IN MERGEKEYWORDS::: mergedResult="
				+ sb.toString().trim());
	...
	public static void main(String args[]) {
		try {
			...
			/*
			 * JenaPersistenceStore jps = JenaPersistenceStore.getInstance();
			 * List<String> uris = new Vector<String>();uris.add(
			 * "http://iks-project.org/companyname/repositoryname/workspacename"
			 * ); Search search = new Search(jps, uris);
			 * 
			 * eu.iksproject.fise.stores.persistencestore.rest.model.search.
			 * ObjectFactory objectFactory = new
			 * eu.iksproject.fise.stores.persistencestore
			 * .rest.model.search.ObjectFactory();
			 * eu.iksproject.fise.stores.persistencestore
			 * .rest.model.search.Query query = objectFactory.createQuery();
			 * query.setUseSynonymsInOntologyLookup(new Boolean(false));
			 * KeywordList keywordList = objectFactory.createKeywordList();
			 * keywordList.setOperator(OperatorType.AND);
			 * //keywordList.getKeywordListOrKeyword().add("air berlin");
			 * //keywordList.getKeywordListOrKeyword().add("cannabis");
			 * query.setKeywordList(keywordList);
			 * query.setFullTextSearchResultList
			 * (objectFactory.createFullTextSearchResultList());
			 * 
			 * FullTextSearchResult ftsr1 =
			 * objectFactory.createFullTextSearchResult();
			 * ftsr1.setDocumentXPath("/NewsArticles/AirBerlin");
			 * ftsr1.setScore(45);
			 * query.getFullTextSearchResultList().getFullTextSearchResult
			 * ().add(ftsr1);
			 */
			/*
			 * FullTextSearchResult ftsr2 =
			 * objectFactory.createFullTextSearchResult();
			 * ftsr2.setDocumentXPath
			 * ("/NewsArticles/SwineFluCoupleFearedDying"); ftsr2.setScore(35);
			 * query
			 * .getFullTextSearchResultList().getFullTextSearchResult().add(
			 * ftsr2); FullTextSearchResult ftsr3 =
			 * objectFactory.createFullTextSearchResult();
			 * ftsr3.setDocumentXPath
			 * ("/NewsArticles/TwoMoreCasesOfSwineFluInUK"); ftsr3.setScore(25);
			 * query
			 * .getFullTextSearchResultList().getFullTextSearchResult().add(
			 * ftsr3);
			 */
			...
			/*
			 * StructuralQueryPart sqp =
			 * objectFactory.createStructuralQueryPart(); ResourceList
			 * resourceList = objectFactory.createResourceList();
			 * SelectiveResource selectiveResource = new
			 * ResourceList.SelectiveResource();
			 * selectiveResource.setOperator(OperatorType.OR);
			 * selectiveResource.setResourceURI(
			 * "http://iks-project.org/companyname/repositoryname/workspacename#Conservation_257c6f28-0f2d-40a5-b90e-7e05b74033d4"
			 * ); //resourceList.getSelectiveResource().add(selectiveResource);
			 * sqp.setResourceList(resourceList);
			 * query.setStructuralQueryPart(sqp);
			 */

			// http://iks-project.org/companyname/repositoryname/workspacename#research_6b04f8e4-61c7-4796-aab4-ce13e025be16

			/*
			 * Result result = search.performQuery(query); String
			 * schemaLocations[] = {"model/xlinks.xsd",
			 * "model/SearchRESTfulInterface.xsd"}; org.w3c.dom.Document
			 * resultNode = XMLUtil.marshall(result,
			 * "eu.iksproject.fise.stores.persistencestore.rest.model.search",
			 * schemaLocations);
			 * System.out.println(XMLUtil.convertToString(resultNode));
			 */

			String schemaLocations[] = { "model/SearchRESTfulInterface.xsd" };
			/*
			 * org.w3c.dom.Document queryNode = XMLUtil.marshall(query,
			 * "eu.iksproject.fise.stores.persistencestore.rest.model.search",
			 * schemaLocations);
			 */
			/*
			 ...
			 * client.resource(
			 * "http://144.122.230.117:8080/PersistenceLayerService/ontologies/search"
			 * );
			 * 
			 * String result = webResource.queryParam("query",
			 * FileUtil.readWholeFile
			 * ("D:/codes/IKS/PersistenceStore/resources/samples/hata-1.txt"
			 * )).accept("application/xml").get(String.class);
			 * //System.out.println(result); Result resultObject = (Result)
			 * XMLUtil.unmarshall(
			 * "eu.iksproject.fise.stores.persistencestore.rest.model.search",
			 * schemaLocations, result); List<Document> list =
			 * resultObject.getReturnedDocuments().getDocument(); for (int i=0;
			 * i<list.size(); i++){
			 * System.out.println(list.get(i).getDocumentXPath());
			 * System.out.println(list.get(i).getScore()); }
			 * 
			 * //System.out.println(search.getOntClassesByKeyword(
			 * "http://iks-project.org/companyname/repositoryname/workspacename2"
			 * , "drug").get(0).getURI());
			 * 
			 * //search.deneme(
			 * "http://iks-project.org/companyname/repositoryname/workspacename2"
			 ...
			 * search.findIndividualsGivenContentRepositoryPath
			 * ("http://iks-project.org/companyname/repositoryname/workspacename"
			 * , "/myBlog/CakeRecipe"); for (int i=0; i<individuals.size();
			 * i++){ Individual individual = individuals.get(i);
			 * System.out.println(search.getContentRepositoryPathForIndividual(
			 * "http://iks-project.org/companyname/repositoryname/workspacename"
			 * , individual)); }
			 */
			 ...
			 * keywordList.add("drug"); keywordList.add("collapse");
			 */

			/*
			 * eu.iksproject.fise.stores.persistencestore.rest.model.search.ObjectFactory
			 * objectFactory = new
			 * eu.iksproject.fise.stores.persistencestore.rest
			 * .model.search.ObjectFactory(); KeywordList keywordList =
			 * objectFactory.createKeywordList();
			 * keywordList.setOperator(OperatorType.AND);
			 * keywordList.getKeywordListOrKeyword().add("drug");
			 * keywordList.getKeywordListOrKeyword().add("cannabis");
			 * List<ScoredOntResource> relatedResources =
			 * search.getKeywordRelatedResources
			 * ("http://iks-project.org/companyname/repositoryname/workspacename2"
			 * , keywordList, 1); for (int i=0; i<relatedResources.size(); i++){
			 * ScoredOntResource scoredOntResource = relatedResources.get(i);
			 * OntResource ontResource = scoredOntResource.getOntResource();
			 * double rank = scoredOntResource.getScore();
			 * System.out.println(ontResource.getURI() + " :: " + rank); }
			 */

			/*
			 * Vector<ScoredKeywordBasedSearchResult> list = new
			 * Vector<ScoredKeywordBasedSearchResult>();
			 * ScoredKeywordBasedSearchResult searchResult = search.new
			 * ScoredKeywordBasedSearchResult("/myBlog/CakeRecipe", 50);
			 * list.add(searchResult);
			 * System.out.println("****************************");
			 * relatedResources =search.getPathRelatedResources(
			 * "http://iks-project.org/companyname/repositoryname/workspacename"
			 * , list, 1); for (int i=0; i<relatedResources.size(); i++){
			 * ScoredOntResource scoredOntResource = relatedResources.get(i);
			 * OntResource ontResource = scoredOntResource.getOntResource();
			 * double rank = scoredOntResource.getScore();
			 * System.out.println(ontResource.getURI() + " :: " + rank); }
