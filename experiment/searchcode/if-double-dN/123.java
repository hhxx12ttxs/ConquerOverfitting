...
/**
 * <p>This class represents a Sample2 (cXYAPoint) as defined by newer 
 * SBP_XYZA_PING2 created by SWATH Processor versions 3.7.N and newer (after
 * January 2010). The main change was the addition of a Total Propagated
 * Uncertainty field (TPU). The class provides additional fields for
 ...
 * HISTORY
 * 
 * 2011-02-25 - Copied and modified from Sample.java
 * 
...
public final class Sample2
{
    ...
    private static final double MIN_TPU = -10000;
    private static final double MAX_TPU = 10000.0;

    /** Sample number (original cXYZAPoint structure)*/
    ...
    /** Total propagated uncertainty of sample position */
    private final double tpu;
    
    /* additional fields added for processing */
     ...
     * @return a new sample
     */
    public static Sample2 acceptSample(Sample2 sample)
    {
        Sample2.Builder builder = new Sample2.Builder(sample);
        builder.status(Sample2.SAMPLE_ACCEPTED);
    ...
    public static class Builder
    {
         ...
         * @param tpu sample total propagated uncertainty in position (m)
         */
        public Builder(int sampNum, double x, double y, double z, int amp,
                int procAmp, int status, double tpu)
        {
            sampNum(sampNum);
            x(x);
            y(y);
        ...
        public Builder tpu(double value)
        {
            if (value < MIN_TPU || MAX_TPU < value) {
                throw new IllegalStateException("MIN_TPU: " + MIN_TPU +
                        " MAX_TPU: " + MAX_TPU + " tpu: " + value);
            }
            this.tpu = value;
            return this;
        }
    ...
    private Sample2(Builder builder)
    {
    ...
    /**
     * Get the total propagated uncertainty in position of this sample
     * @return tpu (m)
     */
    public double tpu()
    {
    ...
    {
        return status == Sample2.SAMPLE_REJECTED;
    }
    ...
    public String toString()
    {
        return String.format(
                "%d %.2f %.2f %.2f %d %d %d %.2f%n", 
                    sampNum, x, y, z, amp, procAmp, status, tpu);
    }
     ...
     * @return a new sample
     */
    public Sample2 newRejectedSample()
    {
        Sample2.Builder builder = new Sample2.Builder(this);
        builder.status(Sample2.SAMPLE_REJECTED);
        return builder.build();
    }
     ...
     */
    public double distanceTo(Sample2 sample)
    {
