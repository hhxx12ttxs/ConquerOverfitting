...
public class Workspace extends SpaceImpl {
	...
	public static final String[] WORKSPACE_COLOR_VALUES = { "555555", "0000FF",
			"003399", "009900", "009933", "660033", "663300", "990000",
			"996699", "99CCFF", "99FF00", "CC3300", "CC9900", "CC9999",
			"CC99FF", "CCCC66", "FF99CC", "FFCC33" };
	...
	public Workspace(String spaceName, String spaceTitle, boolean create,
			SpaceManagerImpl manager, XWikiContext context)
	...
	public void updateSpaceFromRequest() throws SpaceManagerException {
		...
		try {
			...
			doc.updateObjectFromRequest(((WorkspacesManagerExtension) manager
					.getSpaceManagerExtension()).getWorkspaceSpaceClassName(),
					context);
	...
	public void setAccessLevel(String level) {
		getDoc().setStringValue(
				((WorkspacesManagerExtension) manager
	...
	public String getColor() {
		String color = getDoc().getStringValue(
				((WorkspacesManagerExtension) manager
						.getSpaceManagerExtension())
			...
			List list = context
					...
					.searchDocumentsNames(
							",BaseObject as obj, StringProperty as tprop where doc.fullName=obj.name and obj.className='"
									+ manager.getSpaceClassName()
									+ "' and obj.id=tprop.id.id and tprop.id.name='"
									+ SPACE_DISPLAYTITLE
									+ "' and tprop.value='"
									+ this.getDisplayTitle().replaceAll("'",
											"''")
									+ "' and doc.web<>'"
