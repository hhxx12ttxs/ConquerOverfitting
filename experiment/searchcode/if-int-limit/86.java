...
public class SimpleFacets {
  ...
  public NamedList getTermCounts(String field) throws IOException {
    ...
    if (method.equals("enumtermfreq")) {
       counts = getFacetTermEnumFreqCounts(searcher, base, field, offset, limit, mincount,missing,sort,prefix);
    } else {
  ...
  public NamedList getFacetTermEnumFreqCounts(SolrIndexSearcher searcher, DocSet docs, String field, int offset, int limit, int mincount, boolean missing, String sort, String prefix)
    ...
    // Check which parameters have been set in "q"
    // for ngram fields
    String filtererField = "";
    String givenTerm = "";
    IndexReader rFilterer = searcher.getReader();
    TermEnum teFilterer = rFilterer.terms(new Term(filtererField, givenTerm));

    NamedList namedParams = req.getParams().toNamedList();
    // Remove leading and trailing parentheses.
    String[] q = req.getParams().get("q").replaceAll("^\\(", "").replaceAll("\\)$", "").split("\\sAND\\s");
    for (String param : q) {
        String[] kv = param.split(":", 2);
        if (kv[0].endsWith("grams")) {
            filtererField = kv[0];
            givenTerm = kv[1].replaceAll("\"", "");

            teFilterer = rFilterer.terms(new Term(filtererField, givenTerm));
            break;
        }
    }
    TermDocs tdFilterer = rFilterer.termDocs();

    /* Loop through every "term" in the index and, for each one,
     * loop through every document in the index. If the document
     * is part of the docset, get the frequency of the
     * term in the document. (When faceting on a non-ngram field,
     * such as date or bioguide_id, the value of that field is
     * considered the term, so the term frequency for it will be
     * 1 for every document.)
     *
     * This works when generating lists of ngrams and their counts,
     * because the frequency of the ngram in a document is what
     * we're looking for and what we get. However, this method does
     * not work for faceting on other fields, such as date or party,
     * because those fields appear ONLY ONCE in each document, so
     * the "term frequency" for those fields in every document is 1.
     *
