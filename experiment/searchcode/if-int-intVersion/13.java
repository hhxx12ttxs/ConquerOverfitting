...
//			this.model.setIcon(ResourceCache.getInstance().getResourceByName("NCL_MAINFRAME_TITLE"));
	...
	private void loadUserRights() {
		final Collection<MasterDataVO<?>> colRole = MasterDataCache.getInstance().get(E.ROLE.getUID());
		final Collection<MasterDataVO<?>> colRoleMasterdata;
		if(this.model.isStateModel()) {
			colRoleMasterdata = MasterDataCache.getInstance().get(E.ROLEMODULE.getUID());
		} else {
			colRoleMasterdata = MasterDataCache.getInstance().get(E.ROLEMASTERDATA.getUID());
		}
		this.model.getUserRights().clear();
		if(this.model.getUID() == null) {
			for(MasterDataVO<?> vo : colRole) {
				final MasterDataVO<?> voAdd = NuclosWizardUtils.setFieldsForUserRight(vo, null, this.model);
			...
			for(MasterDataVO<?> vo : colRole) {
				...
				for(MasterDataVO<?> voRole : colRoleMasterdata) {
					if(this.model.isStateModel()) {
						if(voRole.getFieldUid(E.ROLEMODULE.module).equals(this.model.getUID())) {
							voAdd = voRole;
							break;
						}
					}
					else {
						final UID entity = voRole.getFieldUid(E.ROLEMASTERDATA.entity);
						try {
							if(entity.equals(this.model.getUID())) {
								voAdd = voRole;
								break;
							}
						}
				...
				voAdd = NuclosWizardUtils.setFieldsForUserRight(vo, voAdd, this.model);
				this.model.getUserRights().add(voAdd);
			}
		}
	}
	...
	private boolean hasDocumentType() {
		for(FieldMeta voField : MetaDataDelegate.getInstance().getAllEntityFieldsByEntity(model.getUID()).values()) {
					...
					SwingUtilities.invokeLater(new Runnable() {
						...
						public void run() {
							...
							if(obj instanceof MasterDataVO) {
								MasterDataVO<UID> vo = (MasterDataVO<UID>)obj;
								NuclosEntityCommonPropertiesStep.this.model.setResourceUID(vo.getPrimaryKey());
								NuclosEntityCommonPropertiesStep.this.model.setResourceName(vo.getFieldValue(E.RESOURCE.name));
	    ...
	    for(EntityMeta<?> vo : MetaProvider.getInstance().getAllEntities()) {
	    	if(sTable.equalsIgnoreCase(vo.getDbTable())) {
	    		...
	    		JOptionPane.showMessageDialog(this, SpringLocaleDelegate.getInstance().getMessage(
	    				...
	    				"Der vergebene Tabellenname ist schon vorhanden. Bitte ?ndern Sie ihn in den erweiterten Einstellungen!"),
