...
import org.lifeform.pricer.Commission;
...
public class PerformanceManager {
	...
	private double tradeProfit, grossProfit, grossLoss, netProfit,
			netProfitAsOfPreviousTrade;
	...
	public PerformanceManager(final Strategy strategy, final int multiplier,
	...
	public double getProfitFactor() {
		...
		if (grossProfit > 0) {
			profitFactor = (grossLoss == 0) ? Double.POSITIVE_INFINITY
	...
	public double getKellyCriterion() {
		...
		if (profitableTrades > 0) {
			if (unprofitableTrades > 0) {
				...
				double kellyCriterion = probabilityOfWin
						- (1 - probabilityOfWin) / winLossRatio;
				kellyCriterion *= 100;
	...
	public double getPerformanceIndex() {
		double pi = 0;
		if (trades > 0) {
			double stdev = Math.sqrt(trades * sumTradeProfitSquared
					- sumTradeProfit * sumTradeProfit)
					/ trades;
			if (stdev != 0) {
	...
	public void updatePositionValue(final double price, final int position) {
		positionValue = position * price * multiplier;
	}

	public void updateOnTrade(final int quantity, final double avgFillPrice, final int position) {
		...
		isCompletedTrade = isCompletedTrade
				|| (previousPosition < 0 && position > previousPosition);

		if (isCompletedTrade) {
			trades++;

			netProfit = totalSold - totalBought + positionValue
					- totalCommission;
		...
		// performanceChartData.updateNetProfit(new
		// TimedValue(strategy.getTime(), netProfit));
		// }
