...
import org.abrantix.rockon.rockonnggl.equalizer.MediaPlayerUtil;
import org.abrantix.rockon.rockonnggl.prefs.PrefKeys;
import org.abrantix.rockon.rockonnggl.util.ApiLevelUtil;
import org.abrantix.rockon.rockonnggl.widget.WidgetProvider2x2;
import org.abrantix.rockon.rockonnggl.widget.WidgetProvider3x3;
import org.abrantix.rockon.rockonnggl.widget.WidgetProvider4x1;
import org.abrantix.rockon.rockonnggl.widget.WidgetProvider4x4;
import org.abrantix.rockon.rockonnggl.widget.WidgetProviderBase;
...
public class RockOnNextGenService extends Service {
	...
	private WidgetProviderBase[] widgetProviders = new WidgetProviderBase[] {
			new WidgetProvider4x1(), new WidgetProvider2x2(),
			new WidgetProvider3x3(), new WidgetProvider4x4() };
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Constants.CMDNEXT.equals(cmd) || Constants.NEXT_ACTION.equals(action)) {
            	...
            	long value = intent.getLongExtra(Constants.CMDSEEKAMOUNT, 0);
            	seek(position() - value); 	
            } else if (WidgetProviderBase.CMD_APP_WIDGET_UPDATE.equals(cmd)) {
                // Someone asked us to refresh a set of specific widgets, probably
                // because they were just added.
                int[] appWidgetIds = intent.getIntArrayExtra(AppWidgetManager.EXTRA_APPWIDGET_IDS);
                for (WidgetProviderBase provider : widgetProviders) {
                	provider.performUpdate(RockOnNextGenService.this, appWidgetIds);
        		}
            }
        }
    };
    ...
    public void onCreate() {
        ...
        if(ApiLevelUtil.isEqualizerSupported()) {
    ...
    private void loadEqualizerWrapper() {
    	if(mEqualizerWrapper == null) {
    		mEqualizerWrapper = new EqualizerWrapper(readEqSettingsFromPreferences());
    	}
    	
        if(mEqualizerWrapper.mSettings.isEnabled()) {
        	mEqualizerWrapper.enable(0, mPlayer.getAudioSessionId());
    	}
    }
    
    private EqSettings readEqSettingsFromPreferences() {
    	String eqSettingsString = mPreferences.getString(PrefKeys.EQUALIZER_SETTINGS, null);
...
//                }
    ...
    public IBinder onBind(Intent intent) {
    	if(mAnalytics != null) {
	    	...
	    	mBindingTimes.add(Double.valueOf(System.currentTimeMillis()));
	...
	private void onStartImpl(Intent intent, int startId) {
...
//                        mCardId = FileUtils.getFatVolumeId(intent.getData().getPath());
    ...
    private void notifyChange(String what) {
        ...
        for (WidgetProviderBase provider : widgetProviders) {
        	provider.notifyChange(this, what);
		}
    }
...
//    }
     ...
     */
    boolean isEqEnabled() {
    	return mEqualizerWrapper != null && mEqualizerWrapper.isEnabled();
    }
    ...
    void enableEq() {
    	...
    	try {
			mPreferences.edit().putString(PrefKeys.EQUALIZER_SETTINGS, mEqualizerWrapper.getSettings()).commit();
			mPreferences.edit().putBoolean(PrefKeys.EQUALIZER_ENABLED, true); // a little dupe
		} catch (IOException e) {
			e.printStackTrace();
		}
    }
	...
	void disableEq() {
    	...
    	try {
			mPreferences.edit().putString(PrefKeys.EQUALIZER_SETTINGS, mEqualizerWrapper.getSettings()).commit();
			mPreferences.edit().putBoolean(PrefKeys.EQUALIZER_ENABLED, false); // a little dupe
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
...
//            mMediaPlayer.start();
		...
		public int getAudioSessionId() {
			return MediaPlayerUtil.getAudioSessionId(mMediaPlayer);
		}
    }
