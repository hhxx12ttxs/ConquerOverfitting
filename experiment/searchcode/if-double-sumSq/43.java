...
/*
      ...
      SoundIn m;
      //m = new Mike(sampleRate);
      m = new DISoundIn(Cut.soundFile2());
      int frame=0;
      int prevIdx=-1;
      int prevFrame=-1;
      while(m!=null){
...
//    seg1.clusterSearch(100, movingAvgSize);
  ...
  void play() throws Exception{
    // sh ? to higher (*3/2 or *2) freq = sh' ?
    // ch ? to higher (*3/2 or *2) freq = ts ? - very distinct
    // ss ? to higher (*3/2 or *2) freq = ss ? - no change
    // sh ? to lower freq (/2) = mechanic sound
    // ch ? to lower freq (/2) = ? ch-strong
    // ss ? to lower freq (/2) = ? sh'
    // [sh - sh' - ss  ?-?-?] is lower-higher series of the same freq curve
