...
public final class StringHelper
{
	...
	*/
	...
	public static List<String> clean(Iterable<String> items)
	{
		...
		{
			item=trim(item);
			if (!StringHelper.isEmpty(item) && !list.contains(item))
				list.add(item);
		}
		return list;
	}
	...
	public static String formatScientificNotation(Double value, int numdecimals)
	{
		if (value==null)
			return "";
		String pattern="0."+StringHelper.repeatString("#",numdecimals)+"E0";
	...
	{
		return (StringHelper.containsHtml(str) || StringHelper.containsLinks(str));
	}
	...
	public static Map<String,Object> createMap(Object... args)
	{
		...
		if (args.length%2!=0)
			throw new CException("name/value args should be provided as a multiple of 2: "+StringHelper.join(args,","));
		Map<String,Object> map=new LinkedHashMap<String,Object>();
		for (int index=0;index<size;index++)
		{
			Object name=args[index*2];
			if (!(name instanceof String))
				throw new CException("parameter name at position "+index*2+" should be a String: "+StringHelper.join(args,","));	
	...
	{
		return (!StringHelper.hasContent(str) || "null".equals(str) || "{}".equals(str));
	}
	...
	*/
	...
	{
		return !(!StringHelper.hasContent(email) || email.indexOf('@')==-1 || email.indexOf('.')==-1);
	}
