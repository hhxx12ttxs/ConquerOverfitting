...
public class ISOCurrency implements java.io.Serializable {
	private static HashMap<String, Currency> currencies;

	/**
	 * Should be called like this: put("ALL", "008", 2); Note: the second
	 * parameter is zero padded to three digits
	 */
	private static void put(String alphaCode, String isoCode, int numDecimals) {
		Currency ccy = new Currency(alphaCode, Integer.parseInt(isoCode),
				numDecimals);
	 ...
	 * @param currency
	 *            - The ISO currency to be converted (eg. ISOField 49)
	 * @return result - A double representing the converted field
	 * @exception IllegalArgumentException
	 */
	public static double convertFromIsoMsg(String isoamount, String currency)
		...
		try {
			...
			if (decimals > 0)
				for (int x = 1; x <= decimals; x++)
					m *= 10;
			d = Double.valueOf(isoamount).doubleValue();
			d /= m;
		} catch (Exception e) {
	...
	/**
	 * Converts an amount to an ISO Amount taking in consideration the number of
	 * decimal digits according to currency
	 * 
	 * @param amount
	 ...
	 * @exception IllegalArgumentException
	 */
	public static String convertToIsoMsg(double amount, String currency)
			throws IllegalArgumentException {
		String z = null;
		try {
			Currency c = (Currency) currencies.get(currency.toUpperCase());
		...
		try {
			...
			if (isocode == null) {
				throw new IllegalArgumentException(
						"AlphaCode not found, or incorrectly specified");
			}
		} catch (Exception e) {
	...
	public static Currency getCurrency(int code) throws ISOException {
		return (Currency) currencies.get(ISOUtil.zeropad(
				Integer.toString(code), 3));
	}

	public static ArrayList<Currency> getCurrencies() {
		ArrayList<Currency> ar = new ArrayList<Currency>(185);
	...
	/*
	 * static public void main(String[] args) { try { String
	 * cychamount="uyu 25049.00"; Object[]
	 * d=decomposeComposedCurrency(cychamount); String curr=(String)d[0]; double
	 * amount=((Double)d[1]).doubleValue(); System.out.println("Cych: "
	 * +cychamount+" , CURR="+curr+" , AMOUNT="+amount); String
	 * isocurr=getIsoCodeFromAlphaCode(curr); String
	 * a1=convertToIsoMsg(amount,isocurr); double
	 * d2=convertFromIsoMsg(a1,isocurr); System.out.println("ISOCUrr: "+a1);
	 * System.out.println(amount+"="+d2); } catch(Exception e) {
	 * e.printStackTrace(); } }
	 */
}
