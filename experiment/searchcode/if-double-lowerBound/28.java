...
/**
 * This is a class representing a new candidate feature
 * Both maximal supergraphs and whole supergraphs are used here
 * 
 * One CandidateFeature have one gain scores (w.r.t the objective function)
 * The gain score can be calculated precisely or as a average (some ratio) of the upper and lower bound
 * A branch upper bound can also be derived for branch and bound prunning.
 * @author dayuyuan
 */
public class CandidateInfo implements Comparable<CandidateInfo>{
	private Graph graph;
	private boolean hitOtherIndexFeature; // true if this candidate feature hit on any indexing feature
	...
	// If the gain function is estimated instead of calculated, then the support is not generated
	private int[] supports; // the supports of the feature
	private List<Integer> maxSubgraphs; // the maximum subgraphs of this candidate feature
	private List<Integer> minSupgraphs; //  the minimal supergraph of this candidate feature
	...
	private boolean estimated; // the gain function can be either estimated or calculated
...
//	 * @param estimated
//	 */
//	public CandidateInfo(Graph featureGraph, ArrayList<QueryInfo> maxQueries, ArrayList<QueryInfo> allQueries,
//			QueryInfo hitQuery, SubgraphSearch_LindexUpdate lindex, boolean estimated){
...
//	}
	 ...
	 * @param estimated
	 */
	public CandidateInfo(Graph feature, ArrayList<QueryInfo> maxQueries,ArrayList<QueryInfo> allQueries,
			QueryInfo hitQuery, 
			SubgraphSearch_LindexUpdate lindex, 
			List<Integer> maxSubgraphs, 
			List<Integer> minSupergraphs,
			boolean estimated, double gamma, double alpha){
		this.graph = feature;
		this.maxQueries = maxQueries;
		this.queries = allQueries;
		this.hitQuery = hitQuery;
	...
	/**
	 * Given the Lindex, find the maximum subgraph indexing terms of the new feature
	 * If estimated, no need for finding the exact support. The lower bound will be set as the |maxSub(f)|
	 * Otherwise, the exact support is calculated. 
	 * @param lindex
	 * @return null if the new feature is found to be isomorphic to the any current indexing feature
	 *  
	 */
//	private int[] findSupport(SubgraphSearch_LindexUpdate searcher) {
//		long[] TimeComponent = new long[4];
//		// If the maximal subgraphs are generated here, then it may contain falst positives
//		this.maxSubgraphs = searcher.indexSearcher.maxSubgraphs(this.graph, TimeComponent);
//		return findSupport(searcher, maxSubgraphs);
//	}
	
	private int[] findSupport(SubgraphSearch_LindexUpdate searcher, List<Integer> maxSubgraphs) {
	...
	/**
	 * Return the branch upper bound functin
	 * Calculated on all queries
	 * @return
	 */
	...
	/**
	 * Return the upper bound function:
	 * Calculated on maximal queries
	 * @return
	 */
	...
	/**
	 * For the input gamma, calculate the lower bound
	 * The lower bound will change with gamma
	 * @param gamma
	 * @return
	 */
	public int getLowerBound(){
		if(this.lowerBound ==-1){
			...
			int queryCount = 0;
			for(QueryInfo oneQ: this.maxQueries){
				int oneLowerBound = oneQ.getGapCount() + oneQ.getSupport()-temp;
				if(oneLowerBound >0){
	...
	/**
	 * Return the estimated calculated gain function
	 * The estimated = true;
	 * @param lowerBound
	 * @return
	 */
	public int getGain(double alpha){
		int result =(int)( alpha * this.getUpperBound() + (1-alpha) * this.getLowerBound());
		return result;
	}
	/**
	 * Return the precise calculated gain function: 
	 * The estimated = false;
	 * @return
	 */
	public int getGain(){
		if(gain == -1){
			gain = 0;
			if(estimated){
				gain =(int)( alpha * this.getUpperBound() + (1-alpha) * this.getLowerBound());
			}
			else{
				for(QueryInfo oneQ : this.maxQueries){
					gain += oneQ.getFrequency()*
					(oneQ.getGapCount() - IntersectionSet.getInterSectionSize(oneQ.getGaps(), supports));
				}
				if(this.hitQuery!=null)
					gain += hitQuery.getFrequency() * (hitQuery.getSupport()+ hitQuery.getGapCount());
			}
		}
		return gain;
	}
	...
	/**
	 * The support may be null if the gain is estimated instead of calculated precisely
	 * @return
	 */
	...
	/**
	 * Return the maximum subgraphs of the feature
	 * return [0] = -1 means that a hiting
	 * @return
	 */
