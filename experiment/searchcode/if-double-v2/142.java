...
public final class SuryonoParser extends MathExpParser {
	...
	private final double number[]; // numeric constants in defined function
	...
	private final double[] stack = new double[STACK_SIZE]; // moved by W.
	// Christian
	// from evaluate to
	 ...
	 */
	public static String toErrorString(final int errorcode) {
		String s = "";
		switch (errorcode) {
		case NO_ERROR:
			s = "no error";
	...
	// built in functions
	private final String funcname[] = { "sin", "cos", "tan", "ln", "log",
			"abs", "int", "frac", "asin", "acos", "atan", "sinh", "cosh",
			"tanh", "asinh", "acosh", "atanh", "ceil", "floor", "round", "exp",
			"sqr", "sqrt", "sign", "step", "random" };

	// extended functions
	private final String extfunc[] = { "min", "max", "mod", "atan2" };
	 ...
	 */
	public SuryonoParser(final int variablecount) {
	 ...
	 * The constructor of <code>Parser</code>.
	 * 
	 * Added by W. Christian to make it easy to contruct a parser for with one
	 * variable.
	 * 
	 * @param f
	 *            function
	 ...
	 * @throws AppException
	 */
	public SuryonoParser(final String f, final String v) throws AppException {
		this(1);
	 ...
	 * The constructor of <code>Parser</code>.
	 * 
	 * Added by W. Christian to make it easy to contruct a parser for with two
	 * variables.
	 * 
	 * @param f
	 *            the function
	 ...
	 *            variable 2
	 */
	public SuryonoParser(final String f, final String v1, final String v2)
			throws AppException {
		this(2);
		defineVariable(1, v1);
		defineVariable(2, v2);
	 ...
	 * The constructor of <code>Parser</code>.
	 * 
	 * Added by W. Christian to make it easy to contruct a parser for with
	 * multiple variables.
	 * 
	 * @param f
	 *            the function
	 ...
	 * 
	 * @param code
	 *            the postfix code to append
	 */
	private void addCode(final char code) {
		postfix_code += code;
	}

	// W. Christian addition to automatically add variables
	private boolean append(final String stream) {
	 ...
	 */
	private void arithmeticLevel1() throws AppException {
		boolean negate;
		if (ISBOOLEAN) {
			throw new AppException(toErrorString(INVALID_OPERAND));
		}
		do {
	 ...
	 * @AppException AppException
	 */
	private void arithmeticLevel2() throws AppException {
		boolean negate;
		if (ISBOOLEAN) {
			throw new AppException(toErrorString(INVALID_OPERAND));
		}
	 ...
	 * @AppException AppException
	 */
	private void arithmeticLevel3() throws AppException {
		boolean negate;
		if (ISBOOLEAN) {
			throw new AppException(toErrorString(INVALID_OPERAND));
		}
	 ...
	 * @AppException AppException
	 */
	private void booleanLevel() throws AppException {
		if (!ISBOOLEAN) {
			throw new AppException(toErrorString(INVALID_OPERAND));
		}
		char operator = character;
	 ...
	 */
	private double builtInExtFunction(final int function, final double param1,
			final double param2) {
		switch (function) {
		case 0:
	 ...
	 */
	private double builtInFunction(final int function, final double parameter) {
		switch (function) {
		case 0:
			if (radian) {
		...
		case 25:
			return parameter * Math.random(); // added by W. Christian for
			// random function
		default:
			error = CODE_DAMAGED;
			return Double.NaN;
	 ...
	 */
	public void define(final String definition) {
		function = definition;
	 ...
	 * 
	 * @param index
	 *            the variable index (one based)
	 * @param name
	 *            the variable name
	 */
	public void defineVariable(final int index, final String name) {
		if (index > var_count) {
			return;
		}
	...
	public double evaluate(final double[] v)
	// added by Wolfgang Christian to make it easier to call parser with an
	// array.
	{
		if (var_value.length != v.length) {
			System.out
					.println("JEParser Error: incorrect number of variables.");
			return 0;
		}
	...
	private double evaluateSubFunction() {
		...
		int codeLength = postfix_code.length(); // added bt W. Christian to
		// check the length.
		while (true) {
			try {
				if (code_pointer == codeLength) {
					return stack[0]; // added by W. Christian. Do not use doing
					// an AppException!
				}
			...
			try {
				...
				case '/':
					if (stack[stack_pointer] != 0) {
						...
						stack[stack_pointer - 1] /= 1.0e-128; // added by
						// W.Christian
						// to trap for
						// divide by
						// zero.
					...
					break;
				case '^':
					stack[stack_pointer - 1] = Math.pow(
							stack[stack_pointer - 1], stack[stack_pointer]);
					stack_pointer--;
					break;
				case '_':
				...
				case JUMP_CODE:
					destination = code_pointer
							+ postfix_code.charAt(code_pointer++);
				...
				case LESS_THAN:
					stack_pointer--;
					stack[stack_pointer] = (stack[stack_pointer] < stack[stack_pointer + 1]) ? 1.0
							: 0.0;
					break;
				case GREATER_THAN:
					stack_pointer--;
				...
				case OR_CODE:
					...
					break;
				case NOT_CODE:
					stack[stack_pointer] = (stack[stack_pointer] == 0.0) ? 1.0
							: 0.0;
					break;
				case NUMERIC:
				...
				default:
					if (code >= REF_OFFSET) {
						...
						stack[stack_pointer - 1] = builtInExtFunction(code
								- EXT_FUNC_OFFSET, stack[stack_pointer - 1],
								stack[stack_pointer]);
						stack_pointer--;
					} else if (code >= FUNC_OFFSET) {
						stack[stack_pointer] = builtInFunction(code
								- FUNC_OFFSET, stack[stack_pointer]);
					} else {
						error = CODE_DAMAGED;
						return Double.NaN;
					}
	 ...
	 * @AppException AppException
	 */
	private boolean getIdentifier() throws AppException {
		...
		if (character == '!') {
			...
			skipSpaces();
			if (character != '(') {
				throw new AppException(toErrorString(PAREN_EXPECTED));
			}
			scanAndParse();
			if (character != ')') {
				throw new AppException(toErrorString(PAREN_EXPECTED));
		...
		start = position;
		// if ((character >= '0') && (character <= '9')) changed be W. Christian
		// to handle leanding zeros.
	 ...
	 * @AppException AppException
	 */
	private void getNextCharacter() throws AppException {
		position++;
		try {
			character = function.charAt(position - 1);
		} catch (StringIndexOutOfBoundsException e) {
			throw new AppException(toErrorString(PAREN_NOT_MATCH));
		}
	}
	 ...
	 * Parses defined function.
	 */
	public void parse(final String function) throws AppException {
		define(function);
		parse();
		if (getErrorCode() != NO_ERROR) {
	...
	private void parseSubFunction() {
		...
		try {
			...
			if ((error.equals(toErrorString(SYNTAX_ERROR)) && (postfix_code == ""))) {
				error = toErrorString(EXPRESSION_EXPECTED);
			}
		}
	...
	/**
	 * Parses a function looking for unknown variables. Unknown tokens are used
	 * to create the variable list in the order that they are found.
	 */
	public String[] parseUnknown(final String function) throws Exception {
	 ...
	 * @AppException AppException
	 */
	private void relationLevel() throws AppException {
		char code = (char) 0;
		if (INRELATION) {
			throw new AppException(toErrorString(INVALID_OPERATOR));
		}
		INRELATION = true;
		if (ISBOOLEAN) {
			throw new AppException(toErrorString(INVALID_OPERAND));
	...
	/**
	 * Remove any escape sequences such as \, and replace with the character. by
	 * W. Christian.
	 * 
	 * @param function
	 *            the function
	 ...
	 */
	private String removeEscapeCharacter(final String str) {
	 ...
	 * @AppException AppException
	 */
	private void scanAndParse() throws AppException {
		boolean negate;
		negate = getIdentifier();
		if ((character != '^') && (negate)) {
			addCode('_');
		...
		do {
			...
			case '|':
				...
				break;
			default:
				throw new AppException(toErrorString(OPERATOR_EXPECTED));
			}
		} while (true);
	}

	/**
	 * Scans a non-numerical identifier. Can be function call, variable,
	 * reference, etc.
	 * 
	 * @exception Exception
	 */
	private void scanNonNumeric() throws AppException {
		String stream = "";
		if ((character == '*') || (character == '/') || (character == '^')
				...
				|| (character == '=') || (character == '&')
				|| (character == '|')) {
			throw new AppException(toErrorString(SYNTAX_ERROR));
		}
		do {
			stream += character;
			getNextCharacter();
		...
		if (stream.equals("if")) {
			...
			scanAndParse();
			if (character != ',') {
				throw new AppException(toErrorString(COMMA_EXPECTED));
			}
			addCode(IF_CODE);
		...
		position = start;
		throw new AppException(toErrorString(UNKNOWN_IDENTIFIER));
	}
	 ...
	 * @AppException AppException
	 */
	private void scanNumber() throws AppException {
		// changed by W. Christian to parse numbers with leading zeros.
		String numstr = "";
		double value;
		if (num == MAX_NUM) {
			throw new AppException(toErrorString(TOO_MANY_CONSTS));
		}
		...
		// if(character=='e') {
		if (character == 'e' || character == 'E') { // changed by Doug Brown May
			// 2007
			numstr += character;
			getNextCharacter();
	 ...
	 */
	@Override
	public void setFunction(final String funcStr) throws AppException {
		function = funcStr;
		define(function);
		parse();
		if (error != NO_ERROR) {
	 ...
	 */
	@Override
	public void setFunction(final String funcStr, final String[] vars)
			throws AppException {
		function = funcStr;
	...
	/**
	 * Sets the variable value. The variable is accessed by index. Nothing
	 * happens if variable index > number of variables.
	 * 
	 * @param index
	 *            the variable index (one based)
	 * @param value
	...
	/**
	 * Sets the variable value. The variable is accessed by name. Nothing
	 * happens if variable could not be found.
	 * 
	 * @param name
	 *            the variable name
	 * @param value
	 ...
	 * @AppException AppException
	 */
	private void skipSpaces() throws AppException {
		try {
