...
public class TeamFoundationServerScm extends SCM {
    ...
    public static final String VERSION_SPEC = "Version Specification";
    ...
    public boolean checkout(AbstractBuild build, Launcher launcher, FilePath workspaceFilePath, BuildListener listener, File changelogFile) throws IOException, InterruptedException {
        ...
        try {
            final String previousVersionSpec;
            if (previousBuild instanceof AbstractBuild) {
                previousVersionSpec = (String) ((AbstractBuild) previousBuild).getBuildVariables().get(VERSION_SPEC);
            } else {
                previousVersionSpec = null;
            }
            List<ChangeSet> list = action.checkout(server, workspaceFilePath, (previousBuild != null ? previousBuild.getTimestamp() : null), build.getTimestamp(), previousVersionSpec, (String) build.getBuildVariables().get(VERSION_SPEC));
