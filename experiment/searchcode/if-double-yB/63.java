...
//	-------------------------------------------------------------------------
	boolean SetHoldForceoffPoint()
	{
		...
		for (p = 0; p < 8; p++) {
			...
			if ((movA.firstElement() == p || movB.firstElement() == p) && (movA.lastElement() == p || movB.lastElement() == p))
				looparound = true;

			if (looparound) { // Strategy for looparound phases ...................................
				// find forceoff by forward search
				k = 0;
				forceoffpoint[p] = 0;
				stime = 0;
				while ((k < numstages) && (movA.get(k) == p || movB.get(k) == p)) {
				...
				etime = cyclelength;
				while ((k >= 0) && (movA.get(k) == p || movB.get(k) == p)) {
					holdpoint[p] = etime - stagelength[k];
					etime -= stagelength[k];
					k--;
				}
			}
			else { // Strategy for non-looparound phases .................................
				stime = 0;
				for (k = 0; k < numstages; k++) {
		...
		}*/ // FIXME: AK something is fishy here
		return true;
	}
...
//	-------------------------------------------------------------------------
	...
	public boolean validate(){
		...
		} */ //FIXME: AK temporary - just want to make it work
