...
public class Camera implements Savable, Externalizable, Cloneable {
    ...
    /**
     * <code>lookAt</code> is a convienence method for auto-setting the frame based on a world position the user desires
    ...
    public void lookAt(final ReadOnlyVector3 pos, final ReadOnlyVector3 worldUpVector) {
        ...
        if (_left.equals(Vector3.ZERO)) {
            if (Double.compare(_direction.getX(), 0.0) != 0) {
    ...
    /**
     * <code>update</code> updates the camera parameters by calling <code>onFrustumChange</code>,
     * <code>onViewPortChange</code> and <code>onFrameChange</code>.
     * 
     * @see Camera#update()
     */
    public void update() {
    ...
    public Vector3 getWorldCoordinates(final Vector2 screenPos, final double zPos) {
        return getWorldCoordinates(screenPos, zPos, null);
    }

    public Vector3 getWorldCoordinates(final Vector2 screenPosition, final double zPos, Vector3 store) {
        if (store == null) {
            store = new Vector3();
        }
    ...
    public Vector3 getScreenCoordinates(final Vector3 worldPos) {
     ...
     * 
     * @see Camera#getScreenCoordinates(Vector3, Vector3)
     */
    public Vector3 getScreenCoordinates(final Vector3 worldPosition, Vector3 store) {
        if (store == null) {
            store = new Vector3();
        }
        checkModelViewProjection();
        ...
        position.multiplyLocal(1.0 / position.getW());
        store.setX(((position.getX() + 1) * (_viewPortRight - _viewPortLeft) / 2 + _viewPortLeft) * getWidth());
        store.setY(((position.getY() + 1) * (_viewPortTop - _viewPortBottom) / 2 + _viewPortBottom) * getHeight());
