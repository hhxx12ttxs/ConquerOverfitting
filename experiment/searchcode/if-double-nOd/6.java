...
//CHECKSTYLE:OFF
@Deprecated
public class LsaGradientMobileSource extends JPanel implements OutputMonitor<Double, Double, List<? extends ILsaMolecule>> {

	/**
	 * 
	 */
	private static final long serialVersionUID = 2692815628459176626L;
	...
	private LsaMolecule sensorTarget = new LsaMolecule("sensor");
	private final ILsaMolecule isField = new LsaMolecule("field,source,Value,TimeStamp");

	// private final ILsaMolecule isField = new
	// LsaMolecule("field,dest,Value,TimeStamp");
	...
	public LsaGradientMobileSource(final ISimulation<Double, Double, List<? extends ILsaMolecule>> s) {
		super();
		this.sim = s;
	}
					...
					for (int i = 0; i < num; i++) {
						...
						if (!nodes.isEmpty()) {
							...
							if (n.getConcentration(isField).size() != 0) {
								...
								s = n.getConcentration(isField).get(0).getArg(2).toString();

								if (n.getConcentration(new LsaMolecule("pump,Tstamp")).size() != 0) {
