 ...
 * 
 * <p>
 * Bohm, Walter and Andreas Geyer-Schulz. 1996.
 * "Exact Uniform Initialization for Genetic Programming". In <i>Foundations of
 * Genetic Algorithms IV,</i> Richard Belew and Michael Vose, eds. Morgan
 * Kaufmann. 379-407. (ISBN 1-55860-460-X)
 * 
 * <p>
 * The user-provided requested tree size is either provided directly to the
 * Uniform algorithm, or if the size is NOSIZEGIVEN, then Uniform will pick one
 * at random from the GPNodeBuilder probability distribution system (using
 * either max-depth and min-depth, or using num-sizes).
 * 
 * <p>
 * Further, if the user sets the <tt>true-dist</tt> parameter, the Uniform will
 * ignore the user's specified probability distribution and instead pick from a
 * distribution between the minimum size and the maximum size the user
 * specified, where the sizes are distributed according to the <i>actual</i>
 * number of trees that can be created with that size. Since many more trees of
 * size 10 than size 3 can be created, for example, size 10 will be picked that
 * much more often.
 * 
 * <p>
 * Uniform also prints out the actual number of trees that exist for a given
 * size, return type, and function set. As if this were useful to you. :-)
 * 
 * <p>
 * The algorithm, which is quite complex, is described in pseudocode below.
 * Basically what the algorithm does is this:
 * 
 * <ol>
 * <li>For each function set and return type, determine the number of trees of
 * each size which exist for that function set and tree type. Also determine all
 * the permutations of tree sizes among children of a given node. All this can
 * be done with dynamic programming. Do this just once offline, after the
 * function sets are loaded.
 * <li>Using these tables, construct distributions of choices of tree size,
 * child tree size permutations, etc.
 * <li>When you need to create a tree, pick a size, then use the distriutions to
 * recursively create the tree (top-down).
 * </ol>
 * 
 * <p>
 * <b>Dealing with Zero Distributions</b>
 * <p>
 * Some domains have NO tree of a certain size. For example, Artificial Ant's
 * function set can make NO trees of size 2. What happens when we're asked to
 * make a tree of (invalid) size 2 in Artificial Ant then? Uniform presently
 * handles it as follows:
 * <ol>
 * <li>If the system specifically requests a given size that's invalid, Uniform
 * will look for the next larger size which is valid. If it can't find any, it
 * will then look for the next smaller size which is valid.
 * <li>If a random choice yields a given size that's invalid, Uniform will pick
 * again.
 * <li>If there is *no* valid size for a given return type, which probably
 * indicates an error, Uniform will halt and complain.
 * </ol>
 * 
 * <h3>Pseudocode:</h3>
 * 
 ...
 * </pre>
 * 
 * Func CreateTreeOfType(type,size) Choose node from ROOT_D[type,size] If size >
 * 1 FillNodeWithChildren(node,0,size-1) return node
 * 
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>true-dist</tt><br>
 * <font size=-1>bool= true or false (default)</font></td>
 * <td valign=top>(should we use the true numbers of trees for each size as the
 * distribution for picking trees, as opposed to the user-specified
 * distribution?)</td>
 * </tr>
 * </table>
 * 
...
public class Uniform extends GPNodeBuilder {
    ...
    // [/*the nodes*/] is an array of <node,probability> pairs for all possible
    // nodes rooting
    // trees of the desired size and compatible with the given return type. It
    // says that if you
    // were to pick a tree, this would be the probability that this node would
    // be the root of it.
    public UniformGPNodeStorage ROOT_D[/* FunctionSet */][/* type */][/* size */][/*
                                                                                   * the
                                                                                   * nodes
                                                                                   */];
    ...
    public double CHILD_D[/* FunctionSet */][/* type */][/* outof */][/* pickchild */][/*
                                                                                        * the
                                                                                        * nodes
                                                                                        */];
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        useTrueDistribution = state.parameters.getBoolean(base.push(P_TRUEDISTRIBUTION), def.push(P_TRUEDISTRIBUTION),
                false);
        ...
        if (minSize > 0) // we're using maxSize and minSize
            ...
            state.output.fatal("Uniform is used for the GP node builder, but no distribution was specified."
                    + "  You must specify either a min/max size, or a full size distribution.", base.push(P_MINSIZE),
                    def.push(P_MINSIZE));
        // preprocess offline
        preprocess(state, maxtreesize);
    }

    public int pickSize(final EvolutionState state, final int thread, final int functionset, final int type) {
    ...
    public void preprocess(final EvolutionState state, final int _maxtreesize) {
        ...
        for (int x = 0; x < functionsets.length; x++) {
            ...
            while (e.hasMoreElements()) {
                ...
                if (!funcnodes.containsKey(tmpn)) // don't remap the node; it'd
                                                  // make holes
            ...
            for (int y = 0; y < numAtomicTypes + numSetTypes; y++)
                for (int z = 1; z <= maxtreesize; z++)
                    state.output.message("FunctionSet: " + functionsets[x].name + ", Type: " + types[y].name
                            + ", Size: " + z + " num: " + (_truesizes[x][y][z] = numTreesOfType(initializer, x, y, z)));
        ...
        for (int x = 0; x < functionsets.length; x++)
            for (int y = 0; y < numAtomicTypes + numSetTypes; y++) {
                ...
                // and if this is all zero (a possibility) we should be
                // forgiving (hence the 'true') -- I *think*
    ...
    public BigInteger numTreesOfType(final GPInitializer initializer, final int functionset, final int type,
            final int size) {
    ...
    public BigInteger numTreesRootedByNode(final GPInitializer initializer, final int functionset, final GPNode node,
            final int size) {
    ...
    public BigInteger numChildPermutations(final GPInitializer initializer, final int functionset, final GPNode parent,
            final int size, final int outof, final int pickchild) {
        if (NUMCHILDPERMUTATIONS[functionset][intForNode(parent)][size][outof][pickchild] == null) {
            ...
            if (pickchild == parent.children.length - 1 && size == outof)
                count = numTreesOfType(initializer, functionset,
                        parent.constraints(initializer).childtypes[pickchild].type, size);
            else if (pickchild < parent.children.length - 1 && outof - size >= (parent.children.length - pickchild - 1)) {
                BigInteger cval = numTreesOfType(initializer, functionset,
                        parent.constraints(initializer).childtypes[pickchild].type, size);
                BigInteger tot = BigInteger.valueOf(0);
                for (int s = 1; s <= outof - size; s++)
                    tot = tot
                            .add(numChildPermutations(initializer, functionset, parent, s, outof - size, pickchild + 1));
        ...
        for (int f = 0; f < NUMTREESOFTYPE.length; f++)
            for (int t = 0; t < NUMTREESOFTYPE[f].length; t++)
                for (int s = 0; s < NUMTREESOFTYPE[f][t].length; s++) {
                    ...
                    // System.out.println("Organizing " + f + " " + t + " " +
                    // s);
            ...
            for (int p = 0; p < NUMCHILDPERMUTATIONS[f].length; p++)
                for (int o = 0; o < maxtreesize + 1; o++)
                    for (int c = 0; c < maxarity; c++) {
                        ...
                        // organize the distribution
                        // System.out.println("Organizing " + f + " " + p + " "
                        // + o + " " + c);
                        // check to see if it's all zeros
                        for (int x = 0; x < CHILD_D[f][p][o][c].length; x++)
                            if (CHILD_D[f][p][o][c][x] != 0.0) {
                                // don't need to check for negatives here I
                                // believe
...

    {
        ...
        int choice = RandomChoice.pickFromDistribution(ROOT_D[functionset][type][size],
                ROOT_D[functionset][type][size][0], mt.nextDouble());
        ...
        if (node.children.length == 0 && size != 1) // uh oh
        {
            ...
            for (int x = 0; x < ROOT_D[functionset][type][size].length; x++)
                System.out.println("" + x + (GPNode) (ROOT_D[functionset][type][size][x].node) + " "
                        + ROOT_D[functionset][type][size][x].prob);
        }
        if (size > 1) // nonterminal
            fillNodeWithChildren(state, thread, initializer, functionset, node,
                    ROOT_D[functionset][type][size][choice].node, 0, size - 1, mt);
        return node;
    }

    void fillNodeWithChildren(final EvolutionState state, final int thread, final GPInitializer initializer,
            final int functionset, final GPNode parent, final GPNode parentc, final int pickchild, final int outof,
            final MersenneTwisterFast mt)

    {
        if (pickchild == parent.children.length - 1) {
            ...
            int size = RandomChoice.pickFromDistribution(CHILD_D[functionset][intForNode(parentc)][outof][pickchild],
                    mt.nextDouble());
            parent.children[pickchild] = createTreeOfType(state, thread, initializer, functionset,
                    parent.constraints(initializer).childtypes[pickchild].type, size, mt);
            fillNodeWithChildren(state, thread, initializer, functionset, parent, parentc, pickchild + 1, outof - size,
                    mt);
        }
    ...
    public GPNode newRootedTree(final EvolutionState state, final GPType type, final int thread,
        ...
        {
            final int BOUNDARY = 20; // if we try 20 times and fail, check to
                                     // see if it's possible to succeed
            int bound = 0;
            ...
            // if the number of trees of our arbitrarily-picked size is zero, we
            // try BOUNDARY
            // number of times to find a tree which will work, picking new sizes
            // each
            // time. If we still haven't found anything, we will continue to
            // search
            // for a working tree only if we know for sure that one exists in
            // the distribution.
            ...
            while (ROOT_D_ZERO[fset][typ][siz]) {
                if (++bound == BOUNDARY) {
                    check: if (!checked) {
                        ...
                        state.output
                                .fatal("ec.gp.build.Uniform was asked to build a tree with functionset "
                                        + set
                                        + " rooted with type "
                                        + type
                                        + ", but cannot because for some reason there are no trees of any valid size (within the specified size range) which exist for this function set and type.");
                    }
                }
            ...
            state.output
                    .fatal("ec.gp.build.Uniform requested to build a tree, but a requested size was given that is < 1.");
            ...
            // if the number of trees of the requested size is zero, we first
            // march up until we
            // find a tree size with non-zero numbers of trees. Failing that, we
            // march down to
            // find one. If that still fails, we issue an error. Otherwise we
            // use the size
            // we discovered.

            determineSize: if (ROOT_D_ZERO[fset][typ][siz]) {
                // march up
