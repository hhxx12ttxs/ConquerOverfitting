...
public class AdjacencyListGraph extends AbstractElement implements Graph {
	 ...
	 * @see #setAutoCreate(boolean)
	 */
	public AdjacencyListGraph(String id, boolean strictChecking,
			boolean autoCreate) {
		super(id);
		...
		edgeFactory = new EdgeFactory<AdjacencyListEdge>() {
			public AdjacencyListEdge newInstance(String id, Node src, Node trg,
					boolean directed) {
	...
	protected void classCastException(ClassCastException e, String message) {
		...
		System.err.printf("%s:%s:%d %s%n", ste.getFileName(),
	...
	public void setEdgeFactory(EdgeFactory<? extends Edge> ef) {
		try {
			this.edgeFactory = (EdgeFactory<? extends AdjacencyListEdge>) ef;
		} catch (ClassCastException e) {
			classCastException(e,
					"need an EdgeFactory<? extends AdjacencyListEdge>");
		}
	}
	...
	public void setNodeFactory(NodeFactory<? extends Node> nf) {
		try {
			this.nodeFactory = (NodeFactory<? extends AdjacencyListNode>) nf;
		} catch (ClassCastException e) {
			classCastException(e,
					"need an NodeFactory<? extends AdjacencyListNode>");
		}
	}
			...
			if (old != null) {
				...
				if (src.hasEdgeToward(trg) != null) {
					...
					try {
						...
						src.edges.add(edge);
						trg.edges.add(edge);
						listeners.sendEdgeAdded(sourceId, timeId, edgeId, from,
								to, directed);
					} catch (ClassCastException e) {
	...
	public <T extends Node> Collection<T> getNodeSet() {
		try {
			return Collections.unmodifiableCollection((Collection<T>) nodes
					.values());
		} catch (ClassCastException e) {
			classCastException(e, "invalid node type required");
		}
