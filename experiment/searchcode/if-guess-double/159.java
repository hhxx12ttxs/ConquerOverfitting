...
public class ResultsWriter{
	...
	public void writeHeader(TextPanel textPanel,ImageAndAnalysisDetails imageAndAnalysisDetails){
		...
		if(imageAndAnalysisDetails.stOn){
			String[] coHeadings = {"MuD [mg/cmł]","MuA [cm˛]","LeanMuD [mg/cmł]","LeanMuA [cm˛]","IntraFatD [mg/cmł]","IntraFatA [cm˛]","FatD [mg/cmł]","FatA [cm˛]","SubCutFatD [mg/cmł]","SubCutFatA [cm˛]","LimbD [mg/cmł]","LimbA [cm˛]","Density weighted fat percentage [%]"};
		...
		if(imageAndAnalysisDetails.cOn){
			String[] coHeadings = {"MaMassD [g/cmł]","StratecMaMassD [g/cmł]","MaD [mg/cmł]","MaA [mm˛]","CoD [mg/cmł]","CoA [mm˛]","Stratec CoD [mg/cmł]","Stratec CoA [mm˛]","SSI [mmł]","ToD [mg/cmł]","ToA[mm˛]","MeA [mm˛]","BSId[g˛/cm4]"};
			for (int i = 0;i<coHeadings.length;++i){
				headings+=coHeadings[i]+"\t";
			}
		}
		...
		if(imageAndAnalysisDetails.conOn){
			...
			for (int j = 0;j<imageAndAnalysisDetails.concentricDivisions;++j){
				for (int i = 0;i<((int) 360/imageAndAnalysisDetails.concentricSector);++i){
					headings+="Division "+(j+1)+" sector "+i*imageAndAnalysisDetails.concentricSector+"° - "+((i+1)*imageAndAnalysisDetails.concentricSector)+"° vBMD [mg/cmł]\t";
				}
			}
		}
		
		if(imageAndAnalysisDetails.dOn){
			headings+="Peeled mean vBMD [mg/cmł]\t";
			//Radial distribution
			for (int i =0; i < (int) imageAndAnalysisDetails.divisions; ++i){
				headings+= "Radial division "+i+" vBMD [mg/cmł]\t";
			}
			//Polar distribution
			for (int i = 0;i<((int) 360/imageAndAnalysisDetails.sectorWidth);++i){
				headings+= "Polar sector "+i+" vBMD [mg/cmł]\t";
			}
			...
			//Cortex BMD values			
			for (int i = 0;i<((int) 360/imageAndAnalysisDetails.sectorWidth);++i){
				headings+=i*imageAndAnalysisDetails.sectorWidth+"° - "+((i+1)*imageAndAnalysisDetails.sectorWidth)+"° endocortical vBMD [mg/cmł]\t";
			}
			for (int i = 0;i<((int) 360/imageAndAnalysisDetails.sectorWidth);++i){
				headings+=i*imageAndAnalysisDetails.sectorWidth+"° - "+((i+1)*imageAndAnalysisDetails.sectorWidth)+"° midcortical vBMD [mg/cmł]\t";
			}
			for (int i = 0;i<((int) 360/imageAndAnalysisDetails.sectorWidth);++i){
				headings+=i*imageAndAnalysisDetails.sectorWidth+"° - "+((i+1)*imageAndAnalysisDetails.sectorWidth)+"° pericortical vBMD [mg/cmł]\t";
			}
