...
//         final PruneCtrl pruneControl = new PruneCtrl();
    ...
    private Line2D.Float mLine2 = new Line2D.Float();
    ...
    */
    ...
    public static final Key KEY_LinkMidPoint = new Key<LWLink,Point2D>("link.mid.location") {
    	@Override public void setValue(LWLink l, Point2D val) { l.setMidPoint(val); }
    	@Override public Point2D getValue(LWLink l) { return l.getMidPoint(); }
    };
...
//     }
    ...
    public LWComponent getMid() {
    	if(mid.node == null || mid.node.isHidden(HideCause.PRUNE))
    		return null;
    	else
    		return tail.node;
    }
    ...
    public LWComponent getPersistMid() {
    	return mid.node;
    }
    /** persistance only */
    ...
    public void setPersistMid(LWComponent c) {
    	mid.node = c;
    	if( c != null)
    		c.addLinkRef(this);
    }
    ...
    public void setMid(LWComponent c)
    {
    	if (c == mid.node)
    		return;
    	if (mid.hasNode())
    		mid.node.removeLinkRef(this);
    	final LWComponent oldMid = mid.node;
    	setPersistMid(c);
    	mRecompute = true;
    	addCleanupTask(this);
    	if (alive()) notify("link.mid.connect", new Undoable(oldMid) { void undo() { setMid(oldMid); }} );
    }
    ...
    public void setMidPoint(float x, float y) {
    	mid.setPoint(this, x, y, KEY_LinkMidPoint);
    }
...
//     }
    ...
    public void draw(DrawContext dc) {
        ...
        if (dc.focal == this) {
            ...
            if (mid.hasNode()) {
                if (mid.node.getParent() == getParent() ||
                    (atTopLevel && mid.node.getParent().isTopLevel()))
                    // no need to transform: same as above case
                    mid.node.draw(dc.push()); dc.pop();
            }
        } else
            super.draw(dc);
    }

    boolean midControlLastMoved = false;
    ...
    public void controlPointMoved(int index, MapMouseEvent e) {
    	if(index == CMid)
    		midControlLastMoved = true;
    	else
    		midControlLastMoved = false;
    ...
    private void setControllerLocation(int index, float mapX, float mapY, MapMouseEvent e)
    {
        ...
        } else if (index == CMid && !mid.isPruning()) {
        	setMid(null);
        	setMidPoint(local.x, local.y);
        	if (e != null)
        		LinkTool.setMapIndicationIfOverValidTarget(mid.node, this, e);
    ...
    public boolean canLinkToImpl(LWComponent target) {
    	...
    	if(midControlLastMoved)
    	{
    		if(head.node == target || tail.node == target)
    			return false;
    	}
    	
    	return super.canLinkToImpl(target);
    }
    ...
    public void controlPointDropped(int index, MapMouseEvent e)
    {
        ...
        if (dropTarget != null && !e.isShiftDown()) {
            if (index == CHead && head.node == null )//&& tail.node != dropTarget)
                ...
                setTail(dropTarget);
            else if (index == CMid && mid.node == null && mid.node != dropTarget)
            	setMid(dropTarget);
    ...
    private static final int CMid = 4;
...
//     }
         ...
         boolean prunes)
    {
        ...
        final Point2D.Float mapMid  = mid.getMapPoint();
        
        //if(mid.hasNode())
        	//mapMid = 
        /*else
        {
        	mapMid = new Point2D.Float(mCenterX, mCenterY);
        */
        final AffineTransform mapTx = getZeroTransform();
        
        mapTx.transform(head.getPoint(), mapHead);
        mapTx.transform(tail.getPoint(), mapTail);
        mapTx.transform(mid.getPoint(), mapMid);
        ...
        //if(excludeConnected && mid.isConnected())
        	//mControlPoints[CMid] = null;
        //else //if (endpointDrags && !mid.hasPrunedNode())
        	mControlPoints[CMid] = new ConnectCtrl(mapMid, mid, false);
        //else
        	//mControlPoints[CMid] = null;
...
//             setFlag(Flag.DATA_LINK);
    ...
    protected void removeFromModel()
    {
        ...
        if (mid.hasNode()) mid.node.removeLinkRef(this);
    }
    ...
    protected void restoreToModel()
    {
        ...
        if (mid.hasNode()) mid.node.addLinkRef(this);
...
// //         }
        ...
        if (mCurve != null) {
            ...
            if (localRect.intersectsLine(mLine) || (mid.node != null && localRect.intersectsLine(mLine2)))
                return true;
        }
    ...
    public float distanceToEdgeSq(float x, float y) {
        ...
        if (mCurve != null) {
            ...
            if(mid.node != null)
            	minDistSq = Math.min(minDistSq, mLine2.ptSegDistSq(x, y));
        }
...
//             return false;
    ...
    private float pickLineDistance(float x, float y, PickContext pc)
    {
        ...
        if (mCurve != null) {
            ...
            final float distSq = (float)Math.min(mLine.ptSegDistSq(x, y), mLine2.ptSegDistSq(x, y));
            if (distSq <= hitDistSq)
                return 0;
            else
                minDistSq = distSq;
...
//     }
        ...
        if (mCurveControls == 2) {
            ...
            if (mQuad.ctrlx == NEEDS_DEFAULT) {
                ...
                if(mid.node != null)
                {
                	...
                	/*if(mid.node instanceof LWLink)
                	{
                		float dx = ((LWLink)mid.node).tail.x - ((LWLink)mid.node).head.x;
                		float dy = ((LWLink)mid.node).tail.y - ((LWLink)mid.node).head.y;
                	...
                	//mQuad.ctrlx = mid.x;//4 * mid.x - 4 * 0.75f * head.x - 2 * tail.x;
                	//mQuad.ctrly = mid.y;//4 * mid.y - 4 * 0.75f * head.y - 2 * tail.y;
                	
                	mQuad.ctrlx = (mid.x - (1 - t)*(1 - t) * head.x - t * t * tail.x) / (2 * (1 - t) * t);
                	mQuad.ctrly = (mid.y - (1 - t)*(1 - t) * head.y - t * t * tail.y) / (2 * (1 - t) * t);
                }
            }
        }
    }
    
    // Get true control points for quadratic curve with midpoint constraint
    private Point2D.Float [] getControlPoints()
    {
    	Point2D.Float [] cp = new Point2D.Float[2];
    	cp[0] = new Point2D.Float();
    	cp[1] = new Point2D.Float();
    	...
    	if(mid.node != null)
    	{
			...
			if(mid.node instanceof LWLink)
			{
				...
				float n = (float) Math.sqrt(dx * dx + dy * dy);
				float d = (float) Math.sqrt((mQuad.ctrlx - mid.x) * (mQuad.ctrlx - mid.x) + (mQuad.ctrly - mid.y) * (mQuad.ctrly - mid.y));
    ...
    public boolean isSelfLoop() {
    	return head.node == tail.node && head.node != null;
    }
    ...
    public Shape getZeroShape() {
        ...
        if (mCurveControls > 0)
        {
        	// Two-part quadratic curve to make midpoint tangent to a component
        	if(mCurveControls == 1 && mid.node != null)
        	{
        		mPath.reset();
        		
        		Point2D.Float [] cp = getControlPoints();
        		Point2D.Float [] ends = new Point2D.Float[3];
        		if(mid.node != null)
        		{
        			ends[0] = new Point2D.Float(head.x, head.y);
        			ends[1] = new Point2D.Float(mid.x, mid.y);
        			ends[2] = new Point2D.Float(tail.x, tail.y);
        		}
        		
        		mPath.moveTo(ends[0].x, ends[0].y);
        		
    			mPath.curveTo(ends[0].x / 3 + 2 * (cp[0].x) / 3, ends[0].y / 3 + 2 * (cp[0].y) / 3, ends[1].x / 3 + 2 * (cp[0].x) / 3, ends[1].y / 3 + 2 * (cp[0].y) / 3, ends[1].x, ends[1].y);
    			mPath.curveTo(ends[1].x / 3 + 2 * (cp[1].x) / 3, ends[1].y / 3 + 2 * (cp[1].y) / 3, ends[2].x / 3 + 2 * (cp[1].x) / 3, ends[2].y / 3 + 2 * (cp[1].y) / 3, ends[2].x, ends[2].y);
        	...
        	// draw a nice self-loop with rounded ends
        	else if (mCurveControls == 1 && isSelfLoop())
        	{
        		mPath.reset();
        		float d = Math.max(head.node.width, head.node.height) * 0.7f;
        		float r = d / 2;
        		float straightHeight = 10; // length of segment before rounded end, looks better when not scaled with node size
        		
        		// initial straight segment
        		mPath.moveTo(head.x, head.y);
        		mPath.lineTo(head.x, head.y - straightHeight);
        		
        		// top arc
        		mPath.append(new Arc2D.Float(head.x - d, head.y - r - straightHeight, d, d, 0f, 180, Arc2D.OPEN), false);
        		
        		// left straight segment
        		mPath.lineTo(tail.x - d, tail.y + straightHeight);
        		
        		// bottom arc
        		mPath.append(new Arc2D.Float(tail.x - d, tail.y + r + straightHeight - d, d, d, 180, 180, Arc2D.OPEN), false);
        		
        		// final straight segment
        		mPath.lineTo(tail.x, tail.y);

        		return mPath;
        	}
        	else if (mCurveControls == 2 && isSelfLoop() && mid.node != null)
        	{
        		mPath.reset();
        		
        		float dx = mid.x - head.x;
        		float dy = mid.y - head.y;
        		float theta1 = (float)Math.atan2(-dy, dx);
        		dx = tail.x - mid.x;
        		dy = tail.y - mid.y;
        		float theta2 = (float)Math.atan2(-dy, dx);
        		
        		dx = mid.node.getMapCenterX() - head.node.getMapCenterX();
        		dy = mid.node.getMapCenterY() - head.node.getMapCenterY();
        		float r = (float)Math.sqrt(dx * dx + dy * dy) / 3;
        		
        		float midTangent = mid.node.getTangentAngle(mid.x, mid.y);
        		
        		dx = tail.x - head.x;
        		dy = tail.y - head.y;
        		
        		double dot = dx * Math.cos(midTangent) + dy * -Math.sin(midTangent);
        		if(dot < 0)
        			midTangent += Math.PI;
        		//float tangentR = 
        		
        		mPath.moveTo(head.x, head.y);
        		mPath.curveTo(head.x + Math.cos(theta1) * r, head.y - Math.sin(theta1) * r, mid.x + r * Math.cos(midTangent+Math.PI) / 2, mid.y - r * Math.sin(midTangent+Math.PI) / 2, mid.x, mid.y);
        		mPath.curveTo(mid.x + Math.cos(midTangent) * r / 2, mid.y - Math.sin(midTangent) * r / 2, tail.x + Math.cos(theta2+Math.PI) * r, tail.y - Math.sin(theta2+Math.PI) * r, tail.x, tail.y);
        		return mPath;
        	}
            return mCurve;
        }
        else
        {
        	...
        	{
        		mPath.reset();
        		mPath.moveTo(mLine.x1, mLine.y1);
        		mPath.lineTo(mLine.x2, mLine.y2);
        		mPath.lineTo(mLine2.x2, mLine2.y2);
...
//     }
    ...
    public float getTangentAngleImpl(float x, float y) {
    	return (float)Math.atan2(-(mLine.getY2() - mLine.getY1()), mLine.getX2() - mLine.getX1());
    }
    ...
    private void computeLink()
    {
        ...
        // Modified for UCLA CNP (3/8/10): if head/tail node is hidden, draw link to/from its parent
        LWComponent redirectHead = head.node;
        
		// find first non-hidden ancestor to redirect links to
		while(redirectHead != null && redirectHead.isHidden() && redirectHead.parent != null)
			redirectHead = redirectHead.parent;
		
		LWComponent redirectTail = tail.node;
		
		// find first non-hidden ancestor to redirect links to
		while(redirectTail != null && redirectTail.isHidden() && redirectTail.parent != null)
			redirectTail = redirectTail.parent;
		
		LWComponent redirectMid = mid.node;
		
		// find first non-hidden ancestor to redirect links to
		while(redirectMid != null && redirectMid.isHidden() && redirectMid.parent != null)
			redirectMid = redirectMid.parent;
        ...
        if (redirectHead != null) {
            ...
            if (!mXMLRestoreUnderway && redirectHead instanceof LWLink && ((LWLink)redirectHead).mRecompute)
                ((LWLink)redirectHead).computeLink();
            
            // This will store the parent-local center x/y in head:
            redirectHead.getLinkConnectionCenterRelativeTo(head.getPoint(), parent);
        }
        if (redirectTail != null) {
            
            // see above comment
            if (!mXMLRestoreUnderway && redirectTail instanceof LWLink && ((LWLink)redirectTail).mRecompute)
                ((LWLink)redirectTail).computeLink();
            
            // This will store the parent-local center x/y in tail:
            redirectTail.getLinkConnectionCenterRelativeTo(tail.getPoint(), parent);
        }
        
        if(redirectMid != null) {
        	if (!mXMLRestoreUnderway && redirectMid instanceof LWLink && ((LWLink)redirectMid).mRecompute)
                ((LWLink)redirectMid).computeLink();
            
            // This will store the parent-local center x/y in mid:
            redirectMid.getLinkConnectionCenterRelativeTo(mid.getPoint(), parent);
        }
        ...
        if (redirectHead == null || redirectHead instanceof LWLink) {
            headShape = null;
            headTransform = null;
        } else {
            
                headShape = redirectHead.getZeroShape(); 
            	headTransform = redirectHead.getRelativeTransform(parent);
        ...
        if (headShape != null) {
            ...
            if (mCurveControls == 1) {
            	...
            	{
            		srcX = (float)head.node.getBounds().getCenterX();
            		srcY = (float)head.node.getBounds().getCenterY() - head.node.getHeight() * 2;
            	}
            	else
            	{
                srcX = mQuad.ctrlx;
      ...
      //-----------------------------------------------------------------------------
        ...
        final Shape midShape;
        final AffineTransform midTransform;
        if (redirectMid == null || redirectMid instanceof LWLink) {
            midShape = null;
            midTransform = null;
        } else {
	            // use zero/raw shape because we use the relative transform in computeIntersection
	            midShape = redirectMid.getZeroShape(); 
	            midTransform = redirectMid.getRelativeTransform(parent);
        }
        ...
        if (midShape != null) {
            ...
            final float[] result =
                VueUtil.computeIntersection(srcX, srcY, mid.x, mid.y, midShape, midTransform, intersection, 1);
            // If intersection fails for any reason, leave endpoint as center of object at tail.
            if (result != VueUtil.NoIntersection) {
                 mid.x = intersection[0];
                 mid.y = intersection[1];
            }
        }
        ...
        if (redirectTail == null || redirectTail instanceof LWLink) {
            tailShape = null;
            tailTransform = null;
        } else {
	            // use zero/raw shape because we use the relative transform in computeIntersection
	            tailShape = redirectTail.getZeroShape(); 
	            tailTransform = redirectTail.getRelativeTransform(parent);
        }
        ...
        float x1, x2, y1, y2;
        x1 = Math.min(tail.x, head.x);
        x2 = Math.max(tail.x, head.x);
        y1 = Math.min(tail.y, head.y);
        y2 = Math.max(tail.y, head.y);
        
        if(redirectMid != null)
        {
        	x1 = Math.min(x1, mid.x);
        	x2 = Math.max(x2, mid.x);
        	y1 = Math.min(y1, mid.y);
        	y2 = Math.max(y2, mid.y);
        }
        ...
        if(redirectMid == null)
        {
        	mid.setLocation(mCenterX, mCenterY);
        }
        ...
        {
        	mLine.setLine(head.x, head.y, mid.x, mid.y);
        	mLine2.setLine(mid.x, mid.y, tail.x, tail.y);
        }
        ...
        if (mCurveControls == 1) {
        	...
        	{
        		Point2D.Float [] cp = getControlPoints();
            head.rotation = computeVerticalRotation(head.x, head.y, cp[0].x, cp[0].y);
            tail.rotation = computeVerticalRotation(tail.x, tail.y, cp[1].x, cp[1].y);
        	}
        	else if(head.node == tail.node && head.node != null)
        	{
        	...
        	{
        		head.rotation = computeVerticalRotation(mid.x, mid.y, head.x, head.y);
                tail.rotation = computeVerticalRotation(tail.x, tail.y, mid.x, mid.y);
        	}
        } else if (mCurveControls == 2) {
        	if(redirectMid != null)
        	{
            ...
            if(redirectMid == null)
            	...
            	tail.rotation = computeVerticalRotation(mLine2.x1, mLine2.y1, mLine2.x2, mLine2.y2) + Math.PI;
        }
...
//         tail.pruneCtrlOffset = controlOffset;
        ...
        if (mCurveControls > 0) {
            ...
            bounds.x = x1;
            bounds.y = y1;
            //bounds.width = Math.max(Math.max(Math.abs(head.x - tail.x), Math.abs(head.x - mid.x)), Math.abs(tail.x - mid.x));
            //bounds.height = Math.max(Math.max(Math.abs(head.y - tail.y), Math.abs(head.y - mid.y)), Math.abs(tail.y - mid.y));
    ...
    private Rectangle2D.Float computeCurvedLink()
    {
        ...
        //if(mCurveControls == 1 && (mid.node != null || head.node == tail.node))
        	i = new FlatteningPathIterator(getZeroShape().getPathIterator(null), 1f);
        //else
        	//i = new FlatteningPathIterator(mCurve.getPathIterator(null), 1f);
...
//     }
    ...
    public void setMidPoint(Point2D p) {
    	if (mXMLRestoreUnderway) {
    		mid.x = (float) p.getX();
    		mid.y = (float) p.getY();
    	} else {
    		setMidPoint((float)p.getX(), (float)p.getY());
    	}
    }
    ...
    public Point2D.Float getMidPoint() {
    	return new Point2D.Float(mid.x, mid.y);
    }
