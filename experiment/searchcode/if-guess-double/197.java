...
// Additional modifications 2010

public abstract class AdaptiveRejectionSampleCR implements Serializable {
	...
	abstract double logp(double x, double[] dh);
	...
	// call this the first time before using the object; after this the variable
	// seg can be reused
	
	// scl is the guess for scale of variation in x
	public AdaptiveRejectionSampleCR(double scl) {
		int i = 0;
	...
	public double ars(double init)
	{
		...
		x = init; f = logp(x, dfe);       /* find point to the left of mode */
		df = dfe[0];
		max = f;
		...
		x = init; f = logp(x, dfe);      /* find point to the right of mode */
		df = dfe[0];
		while (df>-TINY || f>max-MIN_DROP) {
			x += scale+(x-init); 
		    f = logp(x, dfe);
