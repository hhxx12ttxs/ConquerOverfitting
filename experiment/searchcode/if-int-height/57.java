...
public class SquareBoard extends Object {
	...
	private class SquareBoardComponent extends Component {
		 ...
		 */
		private Map<Color, Color> lighterColors = new HashMap<>();
		 ...
		 * each and every square drawn.
		 */
		private Map<Color, Color> darkerColors = new HashMap<>();
		...
		public synchronized void paint(Graphics g) {
			...
			if (size == null || !size.equals(getSize())) {
				...
				bufferImage = createImage(width * squareSize.width, height
						* squareSize.height);
			}
			...
			// Paint image buffer
			g.drawImage(bufferImage, insets.left, insets.top, getBackground(),
					null);
		}
