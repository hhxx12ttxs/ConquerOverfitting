...
public class JoglRenderer extends Renderer {
    ...
    private final IntBuffer idBuff = BufferUtils.createIntBuffer(16);
     ...
     * 
     *            TODO Replace all of these fields with one surface reference? TODO If the capabilities are really
     *            context specific the field should be dropped.
     */
    public JoglRenderer(final int width, final int height) {
        if (width <= 0 || height <= 0) {
            logger.warning("Invalid width and/or height values.");
            throw new Ardor3dException("Invalid width and/or height values.");
        }
        this.width = width;
        this.height = height;

        logger.fine("JoglRenderer created. W:  " + width + "H: " + height);
    ...
    public void setSize(final int width, final int height) {
        if (width <= 0 || height <= 0) {
            ...
            throw new Ardor3dException("Invalid width and/or height values:" + width + "x" + height);
        }
        this.width = width;
        this.height = height;
    }
    ...
    public boolean takeScreenShot(final String filename) {
        if (null == filename) {
            throw new Ardor3dException("Screenshot filename cannot be null");
        }
        final File out = new File(filename + ".png");
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                final int index = 3 * ((height - y - 1) * width + x);
                final int argb = (((buff.get(index + 0)) & 0xFF) << 16) // r
    ...
    @Override
    public void setPolygonOffset(final float factor, final float offset) {
        final GL gl = GLU.getCurrentGL();
    ...
    public void setupVertexData(final FloatBuffer vertexBuffer, final VBOInfo vbo) {
        ...
        if ((caps.isVBOSupported() && vbo != null && vbo.getVBOVertexID() > 0)) {
            ...
            JoglRendererUtil.setBoundElementVBO(rendRecord, vbo.getVBOVertexID());
    ...
    public void setupTextureData(final List<TexCoords> textureCoords, final VBOInfo vbo) {
        ...
        if (ts != null) {
            ...
            prevTextureNumber = ts.getNumberOfSetTextures() < caps.getNumberOfFixedTextureUnits() ? ts
    ...
    public boolean doTransforms(final Transform transform) {
        ...
        if (!transform.isIdentity()) {
            synchronized (_transformMatrix) {
                ...
                transform.getHomogeneousMatrix(_transformMatrix).toDoubleBuffer(_transformBuffer);
                _transformBuffer.rewind();
