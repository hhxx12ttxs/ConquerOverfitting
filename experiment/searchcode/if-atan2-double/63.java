 ...
 * when looking at names; "?" may be the Greek Chi, rather than Roman.
 *
	...
	implements AstroConstants
{
	...
	void calcEqPosition() throws AstroError {
		...
		double ? = get(Field.EC_LONGITUDE);
		double ? = get(Field.EC_LATITUDE);
		
		// Now convert to apparent equatorial co-ordinates -- in other
		// words, taking nutation into account.
		double[] pos = new double[2];
		observation.eclipticToApparentEquatorial(?, ?, pos);
		double ? = pos[0];
		double ? = pos[1];
	
		put(Field.RIGHT_ASCENSION_AP, ?);
		put(Field.DECLINATION_AP, ?);
	}
	...
	void calcParallax() throws AstroError {
		...
		double ? = get(Field.RIGHT_ASCENSION_AP);
		double ? = get(Field.DECLINATION_AP);
		double ? = get(Field.EARTH_DISTANCE);
		double H = get(Field.LOCAL_HOUR_ANGLE);
	
		double ?sin?1 = observation.get(OField.RHO_SIN_PHI1);
		double ?cos?1 = observation.get(OField.RHO_COS_PHI1);
		
		// Calculate the equatorial horizontal parallax in radians.
		double sin? = 0.0000426345 / ?;
		double ? = asin(sin?);
		
		// Calculate the adjusted right ascension.
		double y1 = -?cos?1 * sin? * sin(H);
		double x = cos(?) - ?cos?1 * sin? * cos(H);
		double ?? = atan2(y1, x);
		double ?1 = ? + ??;
		
		// Calculate the adjusted declination.
		double y2 = (sin(?) - ?sin?1 * sin?) * cos(??);
		double ?1 = atan2(y2, x);
		
		put(Field.HORIZ_PARALLAX, ?);
		put(Field.RIGHT_ASCENSION_TOPO, ?1);
		put(Field.DECLINATION_TOPO, ?1);
	}
	...
	void calcAltAzimuth() throws AstroError {
		double ? = get(Field.DECLINATION_AP);
		double H = get(Field.LOCAL_HOUR_ANGLE);
		double ? = get(Field.HORIZ_PARALLAX);
		
		Position pos = observation.getObserverPosition();
		double ? = pos.getLatRads();

		double Ay = sin(H);
		double Ax = cos(H) * sin(?) - tan(?) * cos(?);
		...
		double sinh = sin(?) * sin(?) + cos(?) * cos(?) * cos(H);
		double h = asin(sinh);
		
		// Calculate the parallax in altitude (the parallax of azimuth is
		// very small).
		double ? = pos.getCentreDistance();
		double p = asin(? * sin(?) * cos(h));
	...
	void calcRiseSet() throws AstroError {
		...
		// Note that ?_o is the full diameter.
		double h_o;
		if (whichBody == Name.MOON) {
			double ? = get(Field.HORIZ_PARALLAX);
			h_o = 0.7275 * ? - REFRACTION;
		} else
			h_o = -(REFRACTION + whichBody.?_o / 2);
		...
		double ?0 = observation.get(Observation.OField.GAST_MIDNIGHT) * 15.0;
		?0 = toRadians(?0);
		...
		double ?1 = b1.get(Field.RIGHT_ASCENSION_AP);
		double ?1 = b1.get(Field.DECLINATION_AP);
		double ?2 = b2.get(Field.RIGHT_ASCENSION_AP);
		double ?2 = b2.get(Field.DECLINATION_AP);
		double ?3 = b3.get(Field.RIGHT_ASCENSION_AP);
		double ?3 = b3.get(Field.DECLINATION_AP);
		double[] ?n = { ?1, ?2, ?3 };
		double[] ?n = { ?1, ?2, ?3 };
		
		// OK!  First, calculate approximate time.
		double cosH0 = (sin(h_o) - sin(?) * sin(?2)) / (cos(?) * cos(?2));
		if (cosH0  < -1 || cosH0 > 1)
		...
		double transit = ((?2 - L - ?0) / TWOPI) % 1.0;
		if (transit < 0)
			transit += 1.0;
		...
		transit = refineRiseSet(h_o, ?0, ?, L, ?T, ?n, null, transit) * 24.0;
		rise = refineRiseSet(h_o, ?0, ?, L, ?T, ?n, ?n, rise) * 24.0;
		set = refineRiseSet(h_o, ?0, ?, L, ?T, ?n, ?n, set) * 24.0;
		...
		if (whichBody == Name.SUN) {
			...
			double t = calculateTwilight(?0, ?, L, ?T, ?n, ?n,
	...
	private double refineRiseSet(double h_o, double ?0, double ?, double L,
								 double ?T, double[] ?n, double[] ?n, double m)
	{
		final double[] args = { -1, 0, 1 };
		
		double ?0_now = modTwoPi(?0 + TWOPI * SIDEREAL_RATIO * m);
		double n = m + ?T / 86400;
		
		// Interpolate for ?, and calculate the local hour angle.  Note
		// the reversed sign of L.
		double ? = Util.interpolate(args, ?n, n);
		double H = ?0_now + L - ?;
		
		// Interpolate for ?, if required.
		double ?m;
		if (?n != null) {
			double ? = Util.interpolate(args, ?n, n);
			double sinh = sin(?) * sin(?) + cos(?) * cos(?) * cos(H);
			double h = asin(sinh);
			?m = (h - h_o) / (TWOPI * cos(?) * cos(?) * sin(H));
		} else
			?m = -H / TWOPI;
	...
	private double calculateTwilight(double ?0, double ?, double L, double ?T,
			 					     double[] ?n, double[] ?n, double m, double z)
	{
		double ?0_now = modTwoPi(?0 + TWOPI * SIDEREAL_RATIO * m);
		double n = m + ?T / 86400;
		...
		double cosH1 = (cos(z) - sin(?) * sin(?)) / (cos(?) * cos(?));
		if (cosH1  < -1 || cosH1 > 1)
	...
	void calcBrightLimb() throws AstroError {
		...
		double ?o = sun.get(Field.RIGHT_ASCENSION_AP);
		double ?o = sun.get(Field.DECLINATION_AP);
		
		// Calculate the angle from north of the bright limb.
		double ?? = ?o - ?;
		double y = cos(?o) * sin(??);
		double x = sin(?o) * cos(?) - cos(?o) * sin(?) * cos(??);
		double ? = modTwoPi(Math.atan2(y, x));
		
		// Calculate the position as seen by the observer.
		double obs = modTwoPi(? - q);

		put(Field.ABS_BRIGHT_LIMB, ?);
	...
	void calcMagnitude() throws AstroError {
		...
		double mag = whichBody.V_o + (r == 0.0 ? 0.0 : 5 * log10(r * ?));
		switch (whichBody) {
		case MOON:
			break;
		case MERCURY:
	 ...
	 * @param	?			The ecliptic longitude, in radians.
	 * @param	?			The ecliptic latitude, in radians.
	 * @param	Lsun		The true (geometric) longitude of the Sun.
	 * @param	pos			An array { ??, ?? } in which the adjustments
	 * 						for aberration in longitude and latitude in
	 * 						radians will be placed.
	 */
	static void aberrationEc(double td, double ?, double ?, double Lsun, double[] pos) {
		...
		double ? = 102.93735 + 1.71946 * T + 0.00046 * T2;
		? = toRadians(?);
		
		double ?? = (-ABERRATION * cos(Lsun - ?) + e * ABERRATION * cos(? - ?)) / cos(?);
		double ?? = -ABERRATION * sin(?) * (sin(Lsun - ?) - e * sin(? - ?));
