...
public class Parser implements java.io.Serializable {
   ...
   public ExpressionProgram parse(String str) {
      ...
      if (str == null)
         throw new ParseException("Can't parse a null string.", context);
      if (context.look() == ParserContext.END_OF_STRING)
         throw new ParseException("Can't parse an empty string.", context);
      boolean isBool;
      ...
      if (context.look() != ParserContext.END_OF_STRING)
         throw new ParseException("Extra data found after the end of a complete legal expression.",context);
      if (isBool)
         throw new ParseException("Found a logical-valued expression instead of a numeric expression.",context);
      context.prog.trim();
   ...
   public ExpressionProgram parseLogical(String str) {
      ...
      if (context.look() != ParserContext.END_OF_STRING)
         throw new ParseException("Extra data found after the end of a complete legal expression.",context);
      if (!isBool)
         throw new ParseException("Found a numeric-valued expression instead of a logical expression.",context);
      context.prog.trim();
      return context.prog;
   }   
   ...
   public boolean parseLogicalExpression(ParserContext context) {
      ...
      if (tok == ParserContext.OPCHARS && context.tokenString.equals("&") && !isBool)
             throw new ParseException("The AND operator can only be used with logical expressions.",context);
      ...
      if (tok == ParserContext.OPCHARS && context.tokenString.equals("?")) {
         if (!isBool)
            throw new ParseException("The conditional operator, ?, can only be applied to a logical-valued expression.",context);
         ...
         if (parseLogicalExpression(context))
            throw new ParseException("The cases in a conditional expression cannot be logical-valued expressions.",context);
         tok = context.look();
         if (tok == ParserContext.OPCHARS && context.tokenString.equals(":")) {
            context.next();
   ...
   public boolean parseLogicalTerm(ParserContext context) {
      ...
      int tok = context.look();
      if (tok == ParserContext.OPCHARS && context.tokenString.equals("|") && !isBool)
             throw new ParseException("The OR operator can only be used with logical expressions.",context);
      while (tok == ParserContext.OPCHARS && context.tokenString.equals("|")) {
         context.next();
   ...
   public boolean parseLogicalFactor(ParserContext context) {
       ...
       if (notCt > 0 && !isBool)
          throw new ParseException("The NOT operator can only be used with logical expressions.",context);
       if (notCt % 2 == 1)
   ...
   public boolean parseRelation(ParserContext context) {
       ...
       if (rel == 0)
          return isBool;
       if (isBool)
             throw new ParseException("A relational operator can only be used with numerical expressions.",context);
       context.next();
       ...
       if (tok == ParserContext.OPCHARS && 
             ( context.tokenString.equals("=") || context.tokenString.equals("<") || context.tokenString.equals(">") || 
               context.tokenString.equals("<=") || context.tokenString.equals(">=") || context.tokenString.equals("<>")) )
          throw new ParseException("It is illegal to string together relations operators; use \"AND\" instead.",context);
   ...
   public boolean parseExpression(ParserContext context) {
      ...
      if (neg) {
         if (isBool)
            throw new ParseException("A unary + or - cannot be applied to a logical expression.",context);
      ...
      if (tok == ParserContext.OPCHARS && (context.tokenString.equals("+") || context.tokenString.equals("-")) && isBool)
         throw new ParseException("A + or - operator cannot be applied to logical operands.",context);
   ...
   public boolean parsePrimary(ParserContext context) {
      ...
      if (tok == ParserContext.OPCHARS && context.tokenString.equals("^")) {
         if (isBool)
            throw new ParseException("The exponentiation operator cannot be applied to logical operands.",context);
         context.next();
         if (parsePrimary(context))
            throw new ParseException("The exponentiation operator cannot be applied to logical operands.",context);
   ...
   public boolean parseFactor(ParserContext context) {
      ...
      if (tok == ParserContext.NUMBER)
         ...
         throw new ParseException("Data ended in the middle of an incomplete expression.",context);
      else if (tok != ParserContext.OPCHARS)
         throw new ParseException("Internal error:  Unknown token type.",context);
      else if (context.tokenString.equals("("))
         ...
         throw new ParseException("Misplaced right brace with no matching left brace.",context);
      else if (context.tokenString.equals("]") && ((options & Parser.BRACKETS) != 0))
         throw new ParseException("Misplaced right bracket with no matching left bracket.",context);
      else if (context.tokenString.equals(")"))
         throw new ParseException("Misplaced right parenthesis with no matching left parenthesis.",context);
      else
         throw new ParseException("Illegal or misplaced character \"" + context.tokenString.charAt(0) + "\"",context);
      if ( (options & FACTORIAL) != 0 ) {
         tok = context.look();
         while (tok == ParserContext.OPCHARS && context.tokenString.equals("!")) {
             if (isBool)
                throw new ParseException("The factorial operator cannot be applied to a logical value.",context);
             context.next();
   ...
   private boolean parseGroup(char open, char close, ParserContext context) {
      ...
      if (tok == ParserContext.OPCHARS && context.tokenString.equals("" + close))
         ...
         throw new ParseException("Missing \"" + close + "\" to match a previous \"" + open + "\".",context);
      return isBool;
   }
   ...
   private void parseWord(ParserContext context) {
      if (context.tokenObject == null)
         throw new ParseException("Unknown word \"" + context.tokenString + "\" encountered in an expression.", context);
      if (context.tokenObject instanceof Variable || context.tokenObject instanceof Constant)
         ...
         if (tok == ParserContext.OPCHARS &&
             ...
             if (context.tokenString.equals("("))
                ...
                isBool = parseGroup('{','}',context);
             if (isBool)
                throw new ParseException("The argument of a function must be a numerical expression.", context);
         }
         else {
            if ( (options & OPTIONAL_PARENS) == 0 ) 
                throw new ParseException("Parentheses required around argument of standard function \"" + f.getName() + "\".", context);
            if (parseTerm(context))
                throw new ParseException("The argument of a function must be a numerical expression.", context);
         }
         ...
         throw new ParseException("Unexpected word \"" + context.tokenObject.getName() + "\" encountered in an expression.", context);
      else
         throw new ParseException("Unimplemented word \"" + context.tokenObject.getName() + "\" encountered in an expression.", context);
   }   
