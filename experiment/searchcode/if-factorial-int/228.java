...
import static cc.kevinlee.functional.recursion.TailCalls.*;
...
public class TailCallsTest {
  ...
  @Test
  public final void testTrampoline1() {
    /* given */
  ...
  private static TailCallable<Integer> howManyEven(final int acc, final int n) {
    if (n == 0) {
      return done(acc);
    }
    return () -> howManyEven(isEven(n) ? acc + 1 : acc, n - 1);
  }
  ...
  public final void testTrampoline3() {
    ...
    /* when */
    final long start = System.currentTimeMillis();
    final int actual = trampoline(howManyEven(0, 12345));
    System.out.println("It took " + (System.currentTimeMillis() - start) + "ms.");
