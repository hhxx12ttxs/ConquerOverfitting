...
public final class StatsMatcher
{
    ...
    {
        addPattern(new Pattern(wSP, PatternElements.TERM, predicate, PatternElements.ANY)) ;     // S, P, ? : approx weight
        addPattern(new Pattern(wPO,  PatternElements.ANY, predicate, PatternElements.TERM)) ;    // ?, P, O : approx weight
        addPattern(new Pattern(wP,   PatternElements.ANY, predicate, PatternElements.ANY)) ;     // ?, P, ?
    }
...
//    }
    ...
    {
        if ( item.sameSymbol(PatternElements.ANY.getSymbol()) )         return PatternElements.ANY ;
        if ( item.sameSymbol(PatternElements.VAR.getSymbol()) )         return PatternElements.VAR ;
        if ( item.sameSymbol(PatternElements.TERM.getSymbol()) )        return PatternElements.TERM ;
        if ( item.sameSymbol(PatternElements.URI.getSymbol()) )         return PatternElements.URI ;
        if ( item.sameSymbol(PatternElements.LITERAL.getSymbol()) )     return PatternElements.LITERAL ;
        if ( item.sameSymbol(PatternElements.BNODE.getSymbol()) )       return PatternElements.BNODE ;
        return item ;
    }
    ...
    private double matchWorker(Item subj, Item pred, Item obj)
    {
        ...
        if ( pred.isNodeURI() )
        {
            ...
            w = search(PatternElements.TERM, subj, pred, obj, w) ;  //?? 
            w = search(PatternElements.ANY, subj, pred, obj, w) ;   //??
            return w ;
        }
        ...
        {
            double w = NOMATCH ;
            w = search(PatternElements.VAR, subj, pred, obj, w) ;
            w = search(PatternElements.ANY, subj, pred, obj, w) ;
            return w ;
        }
...
//            return w ;
    ...
    private static boolean matchNode(Item node, Item item, MatchInfo details)
    {
