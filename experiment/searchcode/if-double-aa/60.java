...
public class HDP implements Serializable {
	...
	// allocated size of the topic array
	public int allocTopics = 5;
	// amount to grow cluster allocation by
	public int growTopicAlloc = 10;
	...
	// the number of topics across all samples
	public ArrayList<Integer> numTopicsSamples = new ArrayList<Integer>();
	public boolean collectNumTopics = true;
	
	public HDP() {
	}
	
	public void init(int nc,ArrayList<DirichletProcess> dd, HDPConcParam[] cp, UpDownConcParam[] udcp, String[] gn) {
		priorPos = 0.001;
		numTopics = nc;
		concParams = cp;
		UDconcParams = udcp;
	...
	public void initAllocTopics() {
		int totalAllocate = allocTopics;
		if (allocTopics < numTopics+2) {
			totalAllocate = numTopics+growTopicAlloc;
		}
		...
		for (i=0;i<totalAllocate;i++) {
			topics.add(new Topic(totalGenes));
		}
		
		allocTopics = totalAllocate;
	}
	...
	public double[] growDoubleArray(double[] oldArray) {
		if (oldArray.length >= numTopics + 5)
			return oldArray;
		
		double[] newArray = new double[numTopics + growTopicAlloc];
	...
	public int[] growIntArray(int[] oldArray) {
		...
		int[] newArray = new int[numTopics + growTopicAlloc];
		System.arraycopy(oldArray,0,newArray,0,oldArray.length);
		return newArray;
	}
	...
	public boolean[] growBooleanArray(boolean[] oldArray) {
		...
		boolean[] newArray = new boolean[numTopics + growTopicAlloc];
		System.arraycopy(oldArray,0,newArray,0,oldArray.length);
		return newArray;
	}
	...
	public void addTopic() {
		...
		if (topics.size() < numTopics+3) {
			for (jj=allocTopics;jj<allocTopics+growTopicAlloc;jj++) {
				topics.add(new Topic(totalGenes));
			}
			allocTopics = topics.size();
		}
		
		topics.get(numTopics+1).zeroCounts();
		
		// init appropriate vectors in each DP
		int dd = 0;
		for (jj=0;jj<DP.length;jj++) {
			DP[jj].addTopic(numTopics);
		}
		...
		for (jj=0;jj<DP.length;jj++) {
			if (DP[jj].state != DirichletProcess.HELDOUT) {
				DP[jj].sampleNewParameters(numTopics);
			}
		}
	...
	public void resampleConcParams(int numiter) {
  		...
  		if (UDconcParams != null) {
  			for (cp=0;cp<UDconcParams.length;cp++) {
  				UDconcParams[cp].resample(numiter,numTopics);
  			}
  		}
  	}
  		...
  		for (iters=0;iters<numIters;iters++) {
  			for (jj=DP.length-1;jj>=0;jj--) {
  				if (DP[jj].getClass() == TissueDP.class & DP[jj].state == DirichletProcess.ACTIVE) {
  					tissue = (TissueDP) DP[jj];
  					tissue.sampleDataTopicAssigns();
  				}
  				if (DP[jj].state == DirichletProcess.ACTIVE) {
  					DP[jj].sampleClusterNumTables(numTopics);
  				}
  			}
  			
  			for (cp=0;cp<concParams.length;cp++) {
  				concParams[cp].calcTotals(numTopics);
  			}
  			
  			for (jj=0;jj<DP.length;jj++) {
  				if (DP[jj].state == DirichletProcess.ACTIVE) {
  					DP[jj].resampleParameters(numTopics,condLike);
  				}
  			} 
  			...
  			if (!DPGroups.isEmpty() & iters > groupStartIter) {
  				for (cc=0;cc<bicluster;cc++) {
	  				for (jj=0;jj<DPGroups.size();jj++) {
	  					DPGroups.get(jj).sampleDPAssignments(numTopics);
  			...
  			System.out.println(iters + " " + numTopics);
  			
  			if (collectNumTopics) {
  				numTopicsSamples.add(numTopics);
  			}
	...
	public double calcLogLike() {
  		...
  		for (jj=0;jj<DP.length;jj++) {
  			if (DP[jj].getClass() == TissueDP.class & DP[jj].state == DirichletProcess.ACTIVE) {
  				tissue = (TissueDP) DP[jj];
		  		genes = tissue.genes;
		  		geneClusterAssigns = tissue.geneTopicAssigns;
  		...
  		for (jj=0;jj<DP.length;jj++) {
  			if (DP[jj].getClass() == TissueDP.class & DP[jj].state == DirichletProcess.ACTIVE) {
		  		...
		  		for (gg=0;gg<genes.length;gg++) {
		  			...
		  			lik = lik + Math.log((((double) myTopic.posCounts[gene]) + priorPos)/(((double) myTopic.totalCount) + priorPos*totalGenes));
		  		}
		  	}
	  	}
	...
	public void persistSelf(String fName) {
  	...
  	public void outputNumTopicsSamples(String fName) throws IOException {
  		FileWriter outFile = new FileWriter(fName);
  		int i = 0;
  		for (i=0;i<numTopicsSamples.size();i++) {
  			outFile.write(numTopicsSamples.get(i).toString());
  			outFile.write("\n");
  		}
  		outFile.close();
  	}
