...
//	  double LOGPI  = 1.14472988584940017414;
	...
	/**
	 * Incomplete Gamma inverse function, auch <tt>igami</tt> genannt.
	 * Gibt den Wert x zurück, für den die Fläche unter der Gamma-Dichte-Funktion
	 * dem Wert y entspricht.<br>
	 * @param a Parameter der Gamma-Verteilung
	 * @param y0 Fläche unter der Funktion (Wert zwischen 0 und 1)
	 */
	/* Cephes Math Library Release 2.3:  March, 1995
	 * Copyright 1984, 1987, 1995 by Stephen L. Moshier
	 * von C nach Java umgeschrieben durch Andrea Baldas*/
	@SuppressWarnings("unused")
	public static double incompleteGammaInverse(double a, double y0){
		...
		d = 1.0/(9.0*a);
		NormalDtr ndtr = new NormalDtr(0,1);
		y = ( 1.0 - d - ndtr.invDistFunction(y0) * Math.sqrt(d) );
		x = a * y * y * y;
		...
		for( i=0; i<10; i++ ){
			if( x > x0 || x < x1 ){
				//goto ihalve;
				x = ihalve(x, a, x0, x1, y0, yl, yh);
				return x;
			}
				
			y = incompleteGammaComplement(a,x);
			if( y < yl || y > yh ){
				//goto ihalve;
				x = ihalve(x, a, x0, x1, y0, yl, yh);
				return x;
	...
	/* Bedeutung: diese Funktion ersetzt ein "goto" im originalen Quellcode
	 * Cephes Math Library Release 2.3:  March, 1995
	 * Copyright 1984, 1987, 1995 by Stephen L. Moshier
	 * verändert durch Andrea Baldas
	 *
	 ...
	 */
	static double ihalve(double x, double a, double x0, double x1,  double y0, double yl, double yh){
		...
		if( x == 0.0 )throw new IllegalArgumentException("incompleteGammaInverse: underflow error");
			// underflow error
