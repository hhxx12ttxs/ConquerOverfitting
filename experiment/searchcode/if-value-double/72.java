...
//	@Override
	public List<byte[]> blpop(String key, long timeout) throws RedisException {
		byte[] keybytes = null;
		if((keybytes = getKeyBytes(key)) == null) 
			throw new IllegalArgumentException ("invalid key => ["+key+"]");
		byte[] timeoutbyte = Convert.toBytes(timeout);
		List<byte[]> multiBulkData= null;
		try {
			MultiBulkResponse response = (MultiBulkResponse) this.serviceRequest(Command.BLPOP, keybytes, timeoutbyte);
