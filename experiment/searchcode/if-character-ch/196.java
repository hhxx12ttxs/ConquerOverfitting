...
public final class TokenizerBytes implements Tokenizer
{
    ...
    public static final int CTRL_CHAR = Chars.B_STAR ;
    ...
    private void skip()
    {
        ...
        for ( ;; )
        {
            ...
            if ( ch == Chars.B_HASH )
            {
                ...
                {
                    ch = inputStream.peekByte() ;
                    if ( ch == Chars.EOF || RiotChars.isNewlineChar(ch) )
                        break ;
                    inputStream.readByte() ;
                }
            }
    ...
    private Token parseToken()
    {
        ...
        // ---- IRI
        if ( chByte == Chars.B_LT )
        {
            inputStream.readByte() ;
            token.setImage(allBetween(Chars.B_LT, Chars.B_GT, false, false)) ;
        ...
        // ---- Literal
        if ( chByte == Chars.B_QUOTE1 || chByte == Chars.B_QUOTE2 )
        {
            inputStream.readByte() ;
            ...
            if (ch2 == chByte )
            {
                ...
                if ( ch3 == chByte )
                {
                    ...
                    TokenType tt = (chByte == Chars.B_QUOTE1) ? TokenType.LONG_STRING1 : TokenType.LONG_STRING2 ;
                    token.setType(tt) ;
                }
                else
                {
                    ...
                    token.setImage("") ;
                    token.setType( (chByte == Chars.B_QUOTE1) ? TokenType.STRING1 : TokenType.STRING2 ) ;
                }
            }
            else
            {
            ...
            // Literal.  Is it @ or ^^
            if ( inputStream.peekByte() == Chars.B_AT )
            {
                inputStream.readByte() ;
        ...
        if ( chByte == Chars.B_UNDERSCORE )        // Blank node :label must be at least one char
        {
            expect("_:") ;
        ...
        { 
            ...
            case Chars.B_DOT:
                ...
                {
                    // Not a DOT after all.
                    inputStream.pushbackByte(Chars.B_DOT) ;
                    readNumber() ;
                    return token ;
                }
            ...
            case Chars.B_SEMICOLON:  inputStream.readByte() ; token.setType(TokenType.SEMICOLON) ; token.setImage(";") ; return token ;
            case Chars.B_COMMA:      inputStream.readByte() ; token.setType(TokenType.COMMA) ;     token.setImage(",") ; return token ;
            case Chars.B_LBRACE:     inputStream.readByte() ; token.setType(TokenType.LBRACE) ;    token.setImage("{") ; return token ;
            case Chars.B_RBRACE:     inputStream.readByte() ; token.setType(TokenType.RBRACE) ;    token.setImage("}") ; return token ;
            case Chars.B_LPAREN:     inputStream.readByte() ; token.setType(TokenType.LPAREN) ;    token.setImage("(") ; return token ;
            case Chars.B_RPAREN:     inputStream.readByte() ; token.setType(TokenType.RPAREN) ;    token.setImage(")") ; return token ;
            case Chars.B_LBRACKET:   inputStream.readByte() ; token.setType(TokenType.LBRACKET) ;  token.setImage("[") ; return token ;
            case Chars.B_RBRACKET:   inputStream.readByte() ; token.setType(TokenType.RBRACKET) ;  token.setImage("]") ; return token ;

            // Specials (if processing off) -- FIX ME
            //case B_COLON:      inputStream.readByte() ; token.setType(TokenType.COLON) ; return token ;
            case Chars.B_UNDERSCORE: inputStream.readByte() ; token.setType(TokenType.UNDERSCORE) ; token.setImage("_") ; return token ;
            case Chars.B_LT:         inputStream.readByte() ; token.setType(TokenType.LT) ; token.setImage("<") ; return token ;
            case Chars.B_GT:         inputStream.readByte() ; token.setType(TokenType.GT) ; token.setImage(">") ; return token ;
            // GE, LE
...
//            case B_RSLASH:
        ...
        if ( chByte == Chars.B_PLUS || chByte == Chars.B_MINUS || RiotChars.range(chByte, '0', '9'))
        {
            readNumber() ;
    ...
    private String readWordSub(boolean leadingDigitAllowed, boolean leadingSignAllowed)
    {
        ...
        // Check is ends in "."
        while ( idx > 0 && stringBuilder.charAt(idx-1) == Chars.B_DOT )
        {
            // Push back the dot.
            inputStream.pushbackByte(Chars.B_DOT) ;
...
//        }
        ...
        {
            inputStream.readByte() ;
            stringBuilder.append(Chars.B_DOT) ;
    ...
    private static void readHex(PeekInputStream inputStream, StringBuilder sb)
    {
        ...
        for(;;)
        {
            ...
            if ( ! RiotChars.range(ch, '0', '9') && ! RiotChars.range(ch, 'a', 'f') && ! RiotChars.range(ch, 'A', 'F') )
                break ;
            inputStream.readByte() ;
    ...
    private String readBlankNodeLabel()
    {
        ...
        // First character.
        {
            ...
            //if ( ! isAlpha(ch) && ch != '_' )
            // Not strict
            if ( ! RiotChars.isAlphaNumeric(ch) && ch != '_' )
        ...
        for(;;)
        {
            ...
            if ( ch == Chars.EOF )
                break ;
            if ( ! RiotChars.isAlphaNumeric(ch) && ch != '-' && ch != '_' )
                break ;
            inputStream.readByte() ;
            stringBuilder.append((char)ch) ;
        }
...
//            exception("Broken parser", y, x) ;
    ...
    int readLiteralEscape()
    {
        ...
        switch (c)
        {
            case 'n':   return Chars.NL ; 
            case 'r':   return Chars.CR ;
            case 't':   return '\t' ;
            case '"':   return '"' ;
            case '\'':  return '\'' ;
            case '\\':  return '\\' ;
