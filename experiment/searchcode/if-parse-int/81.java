...
//	}
	...
	public int[][] serializeNonCanonicalToArray(int[][] graphConnectivity, int[] graphEdgeLabel){
		boolean exception = this.dealWrongInput(graphConnectivity, graphEdgeLabel);
		if(exception == false){
	...
	public int[][] serializeToArray(Graph g){
		boolean exception = this.dealWrongInput(g);
		if(exception ==false)
			return this.minSequence;
		else{
			initialize(g);
	...
	public int[][] serializeToArray(int[][] graphConnectivity, int[] graphEdgeLabel){
		boolean exception = this.dealWrongInput(graphConnectivity, graphEdgeLabel);
		if(exception == false)
			return this.minSequence;
		else{
	...
	private boolean dealWrongInput(Graph g){
	...
	private boolean dealWrongInput(int[][] graphConnectivity, int[] graphEdgeLabel){
		// Dealing with the situation when g has no edge
	...
	private boolean findSerialization(){
		...
		for(int i = 0; i< startEdges.length; i++){
			...
			nodeDepth[startEdges[i][0]]=depth;// depth 0
			nodeDepth[startEdges[i][1]]=++depth;// depth 1
			parent[startEdges[i][0]]=HEAD;// No parent for first visited node
			parent[startEdges[i][1]]=startEdges[i][0];
			edgeDiscovery[connectivity[startEdges[i][0]][startEdges[i][1]]]=true;
			branches.push(startEdges[i][0]);

			// Keep on chaining edges starting from the second node
			//System.out.println("YDY: before the begining"
	...
	private void fillDFSEntry(int iNode, int jNode, int depthJNode){
		this.sequence[pos][0]=nodeDepth[iNode];
	...
	private boolean initialize(Graph g){
		...
		this.nodeDepth = new int[this.vertices.length];
		this.edgeDiscovery = new boolean[g.getEdgeCount()];
	...
	private boolean initialize(int[][] graphConnectivity, int[] edgeLabel){
		...
		// is postpone to initializeTree()
		this.nodeDepth = new int[this.vertices.length];
		this.edgeDiscovery = new boolean[edgeLabel.length];
		this.parent = new int[this.vertices.length];
	...
	private boolean initializeTree(){
		...
		for(int i = 0; i< parent.length; i++){
			this.nodeDepth[i]=NOTYET;
			this.parent[i]=NOTYET;
		}
		for(int i = 0; i< edgeDiscovery.length; i++){
			this.edgeDiscovery[i]=false;
		}
		return true;
	}
	/**
	...
	private boolean depthFirstSearch(int startNode, List<Integer> growedBackEdgeRes){
		//System.out.println("YDY: at the begining"
				//+ this.minSequenceValidIndex + " " + pos);
		...
		for(Iterator<Integer> it = backWardEdgeDest.iterator(); it.hasNext(); ){
			...
			fillDFSEntry(startNode, theOtherNode,nodeDepth[theOtherNode]);
			...
			if(growIsGoodIdea)
			{
				edgeDiscovery[connectivity[startNode][theOtherNode]]=true;
		...
		for(Iterator <Integer> it = nextForwardNode.iterator(); it.hasNext();i++){
			...
			if(this.minSequenceValidIndex == pos-1)
				...
				growIsGoodIdea = true;
			if(growIsGoodIdea){
				nodeDepth[theOtherNode]=++depth;
				edgeDiscovery[edge]=true;
			...
			nodeDepth[theOtherNode]=-1;
			edgeDiscovery[edge] = false;
			...
			for(Iterator<Integer> iit = theOtherBackDest.iterator(); iit.hasNext(); ){
				...
				edgeDiscovery[connectivity[theOtherNode][theOtherNodeBackDest]]=false;
			}
		}
	...
	private int[][] findMinimumEdge(){
		...
		int[][] edges = new int[2*edgeDiscovery.length][2];
		int index = 0;
	...
	private boolean findNextNodes(int startPoint, List<Integer> backWardEdgeDest, List<Integer> nextNodes){
		...
		for(int i = 0; i< vertices[startPoint].length; i++){
			...
			if(edgeDiscovery[edge]==true)
				continue; // this edge has already been used
		
			if(this.nodeDepth[anotherNode] != NOTYET)// backWardEdge
