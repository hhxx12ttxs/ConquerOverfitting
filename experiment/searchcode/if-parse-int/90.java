...
public class PatchReader {
	...
	public void parse(BufferedReader reader) throws IOException {
		...
		Map<String, DiffProject> diffProjects= new HashMap<String, DiffProject>(4);
		String line= null;
		...
		// TODO check, commented from original
...
//			lr.ignoreSingleCR(); // Don't treat single CRs as line feeds to be consistent with command line patch
		...
		this.fDiffs = (FilePatch[]) diffs.toArray(new FilePatch[diffs.size()]);
	}

	protected FilePatch createFileDiff(IPath oldPath, long oldDate,
	...
	private String readUnifiedDiff(List diffs, LineReader lr, String line, String diffArgs, String fileName, DiffProject diffProject) throws IOException {
		List<FilePatch> newDiffs= new ArrayList<FilePatch>();
		String nextLine= readUnifiedDiff(newDiffs, lr, line, diffArgs, fileName);
		for (FilePatch diff : newDiffs) {
		...
		while (true) {
			...
			if (line.startsWith("Index: ")) { //$NON-NLS-1$
				...
				if (!headerLines.isEmpty())
					setHeader((FilePatch)diffs.get(diffs.size() - 1), headerLines);
				diffArgs= fileName= null;
				reread= true;
			} else if (line.startsWith("*** ")) { //$NON-NLS-1$
	...
	private void setHeader(FilePatch diff, List headerLines) {
	...
	protected String readUnifiedDiff(List diffs, LineReader reader, String line, String args, String fileName) throws IOException {
		...
		FilePatch diff = createFileDiff(extractPath(oldArgs, 0, fileName),
	...
	public FilePatch[] getAdjustedDiffs() {
		if (!isWorkspacePatch() || this.fDiffs.length == 0)
			return this.fDiffs;
		List result = new ArrayList();
		for (int i = 0; i < this.fDiffs.length; i++) {
			FilePatch diff = this.fDiffs[i];
			result.add(diff.asRelativeDiff());
		}
		return (FilePatch[]) result.toArray(new FilePatch[result.size()]);
	}
