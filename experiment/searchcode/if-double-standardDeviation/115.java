...
class PdfRuntimeDependenciesReport {
	...
	/**
	 * Retourne les dépendances entre classes selon les requętes de counter,
	 ...
	 * Une classe ne peut pas avoir de dépendance avec elle-męme.<br/>
	 * Exemple:<br/><pre>
	 * ex :
	 * public class A {
	 *    @Inject
		...
		for (final Map.Entry<String, Set<CounterRequest>> entry : methodsCalledByCallerBeans
			...
			for (final CounterRequest childRequest : childRequests) {
				...
				if (callerBean.equals(calledBean)) {
					// si le bean est lui męme, il peut s'appeler autant qu'il veut
			...
			// methodsCalled peut ętre vide soit parce que childRequestsByBeans est vide
			// (il n'y a pas d'appels de méthodes pour ce bean, que des requętes sql par exemple),
			// soit parce qu'il n'y a que des appels de méthodes vers le bean lui-męme
	...
	private Map<String, Set<CounterRequest>> getMethodsCalledByCallerBeans() {
		...
		for (final CounterRequest request : requests) {
			...
			// męme si elle est appelée deux fois depuis la męme classe
			// ou męme ŕ un seul endroit mais avec des exécutions dans une boucle
			...
			for (final String childRequestId : childRequestIds) {
				// on ne regarde que les requętes du męme counter
				...
				if (counter.isRequestIdFromThisCounter(childRequestId)) {
					...
					// il peut y avoir une non synchronisation temporaire entre la liste des requętes
					// et les requętes filles
	...
	private void writeHeader() throws DocumentException {
		...
		for (final String header : headers) {
			...
			// pas la premičre entęte de colonne
