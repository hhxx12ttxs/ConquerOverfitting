...
class SourceConfigParser {
    ...
    private void initFilters(Node n) throws ConfigParserException {
        ...
        for (Object o : tcodes.keySet()) {
            ...
            if (_codes.containsKey(id))
                throw new ConfigParserException("Filter '" + id + "' already defined");
    ...
    private void inheritCommonAttrs(Element elem, HashSet<String> inherited) throws ConfigParserException {
        ...
        if (classId.length() > 0) {
            if (_commonAttrs.containsKey(classId)) {
                // Check cycle of reqursion
                if (inherited.contains(classId)) {
                    throw(new ConfigParserException("Class " + classId
                            + " inherited recursively: " + XmlUtils.toString(elem)));
                } else {
                    inherited.add(classId);
                }
                Element parent = _commonAttrs.get(classId);
                inheritCommonAttrs(parent, inherited); //reqursion inherit
    ...
    private void inheritCommonObjects(Element elem, HashSet<String> inherited) throws ConfigParserException {
        ...
        if (classId.length() == 0) {
            ...
            if (_commonObjects.containsKey(classId)) {
                ...
                // Check that we are not inherit object from <base>
                ...
                inheritCommonObjects(parent, inherited); //reqursion inherit
                inheritAllAttributes(elem, parent);
                inheritAttrsForObject(elem);
            } else
                throw(new ConfigParserException("Class not found: " + XmlUtils.toString(elem)));
    ...
    private void checkAttribute(Element elem) throws ConfigParserException {
        ...
        if (elem.hasAttribute("input-filter")) {
            ...
            if (!_codes.containsKey(filter) && !_colls.containsKey(filter))
                throw new ConfigParserException("Filter '" + filter + "' not found: " + XmlUtils.toString(elem));
        }
        ...
        if (elem.hasAttribute("sequence")) {
            ...
            if (!seq.equals(ObjectAttr.SEQUENCE_NONE) && !seq.equals(ObjectAttr.SEQUENCE_DEFAULT)) {
                if (!_sequences.containsKey(seq))
                    throw new ConfigParserException("Sequence '" + seq + "' not found: " + XmlUtils.toString(elem));
            }
        }
    }
    ...
    private void inheritAttrsForObject(Element elem) throws ConfigParserException {
        ...
        for (Element attr : list) {
            ...
            if (classId.length() > 0) {
                if (_commonAttrs.containsKey(classId))
                    inheritAllAttributes(attr, _commonAttrs.get(classId));
                else
                    throw new ConfigParserException("Class '" + classId + "' not found: " + XmlUtils.toString(attr));
            }
        }
        // get innherit attrs for object
        String classId = elem.getAttribute("class");
        if (classId.length() > 0) {
            if (_commonObjects.containsKey(classId)) {
                Element parent = _commonObjects.get(classId);
//                logger.debug("inheritAttrsForObject(): elem="+DOM.toString(elem));
//                logger.debug("inheritAttrsForObject(): parent="+DOM.toString(parent));
                // for parent attrs
                list = getAttributes(parent);
                for (Element parentAttr : list) {
                    //                    logger.debug("inheritAttrsForObject(): parentAttr="+parentAttr.getAttribute("id") +"="+parentAttr.getAttribute("class"));
                    Element attr = getObjectAttribute(elem, parentAttr.getAttribute("id"));
                    if (attr == null) {
                        // self attr not present -> import attr
                        Element newChild = (Element) parentAttr.cloneNode(false);
                        addObjectAttribute(elem, newChild);
                    } else {
                        // self attr present -> copy values of attributes
                        inheritAllAttributes(attr, parentAttr);
                    }
                }
            } else {
                throw(new ConfigParserException("Class '" + classId
                        + "' not found: " + XmlUtils.toString(elem)));
            }
        }
    }
    ...
    private void hasIdAttribute(Element elem) throws ConfigParserException {
        if (!elem.hasAttribute("id"))
            throw(new ConfigParserException("Missing 'id' attribute: " + XmlUtils.toString(elem)));
    }
...
//        insertFirstCommonObjectAttr(object, attrId);
//        addCommonObjectAttrMap(object.getAttribute("id"), attrId);
    }
    ...
    private void checkId(Element elem) throws ConfigParserException {
        if (getObjectAttribute(elem, "id") == null)
            throw new ConfigParserException("Missing 'id' object attribute: " + XmlUtils.toString(elem));
    }
    ...
    private void createIndexElements() throws ConfigParserException {
        ...
        for (Element object : _baseObjects.values()) {
            ...
            List<String> attrNames = createAttrOrder(object); // ?????? List c ??????? ????????? ??????? ???????
            setAttrOrder(object, attrNames); // ???????????? ???????? ????????? ? ??????? attrNames
        }
    }
    ...
    private List<String> createAttrOrder(Element objectElem) throws ConfigParserException {
        ...
        do {
            ...
            if (parentId.length() > 0) {
                ...
                List<String> parentAttrNames = getAttrIds(objectElem); // ?????? ???? ????????? ?? NodeList
                // ????????? ?? ????????? ??????? ? ????? ? ????????? ? ?????? ??????
    ...
    private void createIndexForObject(Element object) throws ConfigParserException {
        ...
        while (group != null) {
            createIndexForGroup(object, group); // ??????? ?????????????? group ?? object
    ...
    private List<Element> getAttributes(Element object) throws ConfigParserException {
            ...
            try {
                XmlUtils.findAllElements(object, (String) null /*"attr"*/, visitor);
            } catch(ErrorCodeException e) {
                throw new ConfigParserException(e);
            }
    ...
    private class GroupChildrenVisitor implements XMLVisitor {
        ...
        public void visit(Element element, int level) throws ErrorCodeException {
            if (!"attrref".equals(element.getTagName()))
                _parent.insertBefore(element, _group);
        }
    }
    ...
    private void createIndexForGroup(Element object, Element group) throws ConfigParserException {
        ...
        for (Element attrref : attrs) {
            if (!ids.contains(attrref.getAttribute("id")))
                throw new ConfigParserException("Unknown attrref in object '" + object.getAttribute("id") + "': " + XmlUtils.elementToString(attrref));
        }
        ...
        for (Element attr : attrs) {
            ...
            createIndexForAttr(object, attr); // ???????? ??? ????????? self ???????
            // ??????? ??????? ? ??????
