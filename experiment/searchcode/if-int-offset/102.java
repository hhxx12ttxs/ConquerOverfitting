package cc.creativecomputing.gpu.particles.util;
...
public class CCGPUNoise {
	 ...
	 */
	private void initSimplexTexture(GL gl, int[] texID) {
		gl.glGenTextures(1, texID, 0); // Generate a unique texture ID
		gl.glBindTexture(GL.GL_TEXTURE_1D, texID[0]); // Bind the texture to texture unit 1
		...
		// GLFW texture loading functions won't work here - we need
		// GL.GL_NEAREST lookup.
		gl.glTexImage1D(GL.GL_TEXTURE_1D, 0, GL.GL_RGBA, 64, 0, GL.GL_RGBA, GL.GL_UNSIGNED_BYTE, simplexBuffer);
