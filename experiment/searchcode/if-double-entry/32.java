...
public class Counter <E> implements Serializable {
  ...
  public static void main(String[] args) {
    ...
    E maxKey = null;
    for (Map.Entry<E, Double> entry : entries.entrySet()) {
      if (entry.getValue() < minCount || maxKey == null) {
        maxKey = entry.getKey();
	...
	public void scaleAddAll(double multiplier, Counter<E> c) {
		if(multiplier == 0) return;
	  for(Map.Entry<E, Double> e: c.entrySet()) {
	  	incrementCount(e.getKey(), e.getValue() * multiplier);
	  }
  }

	public void decrementCount(E e, double i) {
		incrementCount(e,-i);
  }
	
	public Counter<E> positivePart() {
		Counter<E> result = new Counter<E>();
		for(Map.Entry<E, Double> e: getEntrySet()) {
			if(e.getValue() > 0) {
				result.setCount(e.getKey(), e.getValue());
			}
		}
