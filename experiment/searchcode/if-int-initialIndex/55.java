...
public class ConversationUtils {
    ...
    public static abstract class Converser {
        ...
        public abstract boolean converse(Player player, ConversationMessage message);
        ...
        public String getMessage(String type, Object value) {
            return ChatColor.GREEN + "Set " + type + " to " + StringUtils.wrap(value) + ".";
        }
        ...
        protected static final String endMessage = ChatColor.GREEN + "Type in " + StringUtils.wrap("finish")
                + " to end or " + StringUtils.wrap("restart") + " to begin again.";
    }
    ...
    public static void onChat(AsyncPlayerChatEvent event) {
        ...
        if (getConverser(name) != null) {
            ...
            if (ChatType.get(event.getMessage()) != null) {
                finished = getConverser(name).special(event.getPlayer(), ChatType.get(event.getMessage()));
            } else {
                try {
                    finished = getConverser(name).converse(event.getPlayer(),
                            new ConversationMessage(event.getMessage()));
                } catch (NumberFormatException ex) {
                    event.getPlayer().sendMessage(ChatColor.RED + "That is not a valid number.");
                    finished = false;
                }
            }
            if (finished)
