...
public class BlockMod extends Mod implements EventListener, CommandListener {
	...
	public void onEvent(Event event) {
		if(event instanceof RenderEvent) {
									...
									if(wandType == WAND_SELECT
										...
										synchronized(lock) {
											...
											if(world instanceof MultiplayerWorld) {
												PlayerController controller = minecraft
														.getPlayerController();
			...
			if(((RenderEvent) event).getStatus() != RenderEvent.RENDER_ENTITIES_END)
				return;
			...
			double y = getMinY();
			double z = getMinZ();
			double length = getMaxX() - x + 1;
			double width = getMaxZ() - z + 1;
			double height = getMaxY() - y + 1;
			double dX = (mX - x);
			double dY = (mY - y);
			double dZ = (mZ - z);
			...
			double x1 = -(mX - location1.getX()), y1 = -(mY - location1.getY()) - 1, z1 = -(mZ - location1
					.getZ());
			...
			GL11.glVertex3d(x1 + length, y1, z1);
			GL11.glVertex3d(x1 + length, y1, z1 + width);
			GL11.glVertex3d(x1, y1, z1 + width);

			GL11.glVertex3d(x1, y1 + height, z1);
			GL11.glVertex3d(x1 + length, y1 + height, z1);
			GL11.glVertex3d(x1 + length, y1 + height, z1 + width);
			GL11.glVertex3d(x1, y1 + height, z1 + width);
			...
			if(((PacketEvent) event).getPacket() instanceof Packet15BlockPlace) {
				Packet15BlockPlace packet = (Packet15BlockPlace) ((PacketEvent) event)
						.getPacket();
			...
			} else if(((PacketEvent) event).getPacket() instanceof Packet14BlockDig) {
				Packet14BlockDig packet = (Packet14BlockDig) ((PacketEvent) event)
						.getPacket();
