 ...
 *
 * @author martin
 * @version $Id: GnomonicProjection.java,v 1.9 2006/07/07 07:47:18 martin Exp $
 */
public class GnomonicProjection extends AbstractProjector implements Projector
{
    /** Our logging object */
    private static Log log = LogFactory.getLog(GnomonicProjection.class);

    /** Creates a new instance of GnomonicProjection */
     ...
     * Creates a new instance of GnomonicProjection
     *
     * @param params The parameters to use when projecting
     */
    public GnomonicProjection(AllPlanetParameters params)
    {
        super(params);
    }
            ...
            for (theta1 = theta1; theta1 < 90.0; theta1 += hgrid)
            {
                ...
                if ((ymin <= y) && (y <= ymax))
                {
                            ...
                            theta2 += (0.5 / width / scale))
                    {
                        ...
                        if (z1 != 0.0)
                        {
                            i = (int) (0.5 * (((height * scale * x1) / z1) +
                                    width));
                            j = (int) (0.5 * (((height * scale * y1) / z1) +
                                    height));
