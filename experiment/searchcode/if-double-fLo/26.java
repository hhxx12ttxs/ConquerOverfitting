 ...
 *
 */
abstract public class GfrRndSurfacePolylineTloAbs extends GfrRndSurfacePolylineAbs
{
    // ----
    // begin: instantiate logger for this class
    final private static Logger _LOGGER_ = Logger.getLogger(GfrRndSurfacePolylineTloAbs.class.getName());

    static
    {
        GfrRndSurfacePolylineTloAbs._LOGGER_.addHandler(FileHandlerLogger.s_getInstance());
    }

    // end: instantiate logger for this class
    // ----
    
    protected GfrRndSurfacePolylineTloAbs(
   ...
   @Override
    protected void _setAttributes() throws Exception
    {
       ...
       double dblWidthOut = (double) GfrWrpRenderGlobeDimOneLine.getInstance().getThickness(_strId);
       bsa.setOutlineWidth(dblWidthOut);
      
       Color colRgbOut = GfrWrpRenderGlobeDimOneLine.getInstance().getColor(super._strId);  
       bsa.setOutlineMaterial(new Material(colRgbOut));
     
       float fltAlphaOut = GfrWrpRenderGlobeDimOneLine.getInstance().getTransparency(super._strId);
   ...
   @Override
    ...
    {
       boolean blnIsToolTip = GfrWrpRenderGlobeDimOneLine.getInstance().isTooltip(super._strId);
