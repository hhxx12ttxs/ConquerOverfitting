...
public class LDAFilter extends SimpleBatchFilter implements ILoggableProcessor {
	...
	private String attributeNamePrefix; // use only attributes with the given
										// name prefix for the LDA
	...
	protected Instances determineOutputFormat(Instances inputFormat) throws Exception {
		...
		if (dimReduction && outDimensions > 0) {
			...
			if (inDimensions > outDimensions) {
				...
				throw new IllegalArgumentException("output dimensions have to be lower than input dimensions (" + outDimensions + " !< "
						+ inDimensions + ")");
			}
		}
	...
	public Capabilities getCapabilities() {
		...
		result.enable(Capability.DATE_ATTRIBUTES); // date attribute is
													// accepted, but will only
													// be passed through
		result.enable(Capability.RELATIONAL_ATTRIBUTES); // relational
															// attributes are
															// accepted, but
															// will only be
															// passed through
		result.enable(Capability.NUMERIC_ATTRIBUTES); // only numeric attributes
														// are proccessed
		result.enable(Capability.NOMINAL_CLASS); // filter needs a nominal or
													// string class to be set
		result.enable(Capability.STRING_CLASS); // filter needs a nominal or
												// string class to be set
	...
	/**
	 * perfoms a Eigenvalue decomposition to determine the optimal projection
	 * matrix for the given Instances
	 * 
	 * @param inst
	 *            the instances to calculate the LDA for
	 * @throws Exception
	 *             throws an Exception if one of the covariance matrixes gets
	 *             singular
	 */
	public void train(Instances inst) throws Exception {
		...
		// This cast is save -> capabilities allow only String or Norminal
		// classes
		...
		for (int i = 0; i < subset.size(); i++) {
			for (int j = 0; j < inDimensions; j++) {
				for (int k = 0; k < inDimensions; k++) {
					// sb[j][k] += subset[i].size() *
					// (groupMean[i][j]-totalMean[j])*(groupMean[i][k]-totalMean[k]);
	...
	/**
	 * sort the Eigenvalues in descendig order and rearrange the Eigenvektor
	 * matrix accordingly
	 */
	...
	// TODO: find a way to set the ouput format after proccessing the input
	// (after determineProjectionMatrix)

	/*
	 * not used at the moment because the output format is determined before
	 * processing and a dynamicadaptation of the output format isn't allowed
	 */
	 ...
	 * 
	 * @param column
	 *            index of the column to calculate the mean for
	 * @param data
	 *            feature vectors to calculate the mean for
	 * @return mean value
	 */
	...
	/**
	 * calculates the mean value for the attribute at the given index over all
	 * instances
	 * 
	 * @param attIndex
	 *            index of the attribute to calculate the mean for
	 * @param inst
	 *            instances to calculate the mean for
	 * @return mean value
	 */
	 ...
	 * 
	 * @param fvs
	 *            the the feature vectors to calculate the covariances for
	 * @param means
	 *            array with mean values for each feature in the feature vector
	...
	/**
	 * Enables/disables the dimension reduction. Dimension reduction only works
	 * if a valid value for the amout of output dimensions is set.
	 * 
	 * @param b
	 *            true/false
	 */
	...
	/**
	 * returns the amount of attributes to use in the ouput if dimension
	...
	/**
	 * Sets an prefix for the attribute name. Only those attributes whose names
	 * start with the prefix are considered for the LDA
	 * 
	 * @param attributeNamePrefix
	 *            the attributeNamePrefix to set
	 */
