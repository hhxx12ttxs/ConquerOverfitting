...
public class MDSInterface {
	...
	public static String[] savedProcedureProjection = new String[] {
		...
		SavedProcedureSQLFormat.OBSERVER,
		SavedProcedureSQLFormat.SUBJECT
		};
	...
	private static String getMDSUrl(SharedPreferences preferences){
		String host = preferences.getString(Constants.PREFERENCE_MDS_URL,
											"localhost");
		String path = preferences.getString(Constants.PREFERENCE_MDS_PATH,
				Constants.PATH_MDS);
	...
	protected static MDSResult doExecute(Context ctx, HttpMethod method){
		...
		try {
			...
			Log.e(TAG, "postResponses(): IOEXCeption:" + e1.toString());
			e1.printStackTrace();
		} catch (JsonParseException e) {
			Log.e(TAG, "postResponses(): JsonParseException response: " 
					+ e.getMessage());
		} finally {
	...
	private static boolean postResponses(Context c, String savedProcedureGuid, 
			String jsonResponses, String subject)
	{
		SharedPreferences preferences = PreferenceManager
				.getDefaultSharedPreferences(c);
		String mdsURL = getMDSUrl(preferences);
		...
		postData.add(new NameValuePair("password", password));
		postData.add(new NameValuePair("responses", jsonResponses));
		postData.add(new NameValuePair("subject", subject));
		MDSResult postResponse = MDSInterface.doPost(c, mUrl, postData);
		return (postResponse != null)? postResponse.succeeded(): false;
			...
			int start, int end, byte byte_data[]) 
	{
		...
		//execute
		Log.d(TAG+".postBinaryAs File()", "File part: (" + type.getFilename() + ", " + byte_data.length + " )");
		MDSResult postResponse = MDSInterface.doPost(c, mUrl, parts);
		Log.d(TAG+".postBinaryAs File()", "Response: " + postResponse.toString());
		return (postResponse != null)? postResponse.succeeded(): false;
	}
	...
	public static boolean isProcedureAlreadyUploaded(Uri uri, Context context) {
		...
		Log.d(TAG, "Getting procedure " + procedureUri.toString());
		...
		try {
			...
			while(it.hasNext()) {
				...
				Log.d(TAG, "isProcedureAlreadyUploaded() : answer '" + key + "' : '" 
							+ answersDict.getString(key) +"'");
			}
			Log.d(TAG, "isProcedureAlreadyUploaded() : restoreAnswers");
			p.restoreAnswers(answersMap);
			elementMap = p.toElementMap();

		} catch (Exception e) {
			Log.e(TAG, "isProcedureAlreadyUploaded():" + e.toString());
			throw new NullPointerException(TAG + ":"+e.getMessage());
		}

		if(elementMap == null) {
			Log.e(TAG, "Empty answers from " + uri + ". Not uploading.");
			throw new NullPointerException("Trying to upload procedure with no data");
		}
			...
			for(String binaryId : ids) {
				...
				try {
					...
					case PICTURE:
						...
						if (!alreadyUploaded) result = false;
						break;
					case SOUND:
						cur = context.getContentResolver().query(binUri, 
					...
					Log.i(TAG, "Error checking if the binary files have been "
							+"uploaded: " + x.toString());
					result = false;
					throw new NullPointerException(TAG + ":"+x.getMessage());
				} finally {
				    if(cur != null)
					cur.close();
				}
	...
	{
		Log.d(TAG, "In Post procedure to Django server for background uploading service.");
		...
		//TODO Replace with getColumnIndex(name)
		int savedProcedureId = cursor.getInt(0);
		
		// Gets the procedure id
		int procedureId = cursor.getInt(1);
		
		//TODO Get Subject Data
		// Store as local uri/uuid in the table
		
		//TODO query obs table for matching encounter 
		String answersJson = cursor.getString(2);
		...
		String subject = cursor.getString(
				cursor.getColumnIndex(SavedProcedureSQLFormat.SUBJECT));

		String observer = cursor.getString(
				cursor.getColumnIndex(SavedProcedureSQLFormat.OBSERVER));
		
		
		cursor.close();
		Log.d(TAG, "posting " + subject + " " + observer);
		// Gets the procedure instance 
		// TODO Should really just store a uuid for the procedure
		Uri procedureUri = ContentUris.withAppendedId(
				ProcedureSQLFormat.CONTENT_URI, procedureId);
		...
		// query(Enounters 
		//TODO Replace section between ## markers with obs table
		//###
		...
		if(!finished) {
			Log.e(TAG, "postProcedureToDjangoServer() Encounter Incomplete" + uri);
			return false;
		}
		...
		try {
			...
			p.restoreAnswers(answersMap);
			elementMap = p.observationMap();

		} catch (Exception e) {
			Log.e(TAG, "isProcedureAlreadyUploaded():json:" + answersJson);
			Log.e(TAG, "isProcedureAlreadyUploaded():" + e.toString());
			e.printStackTrace();
			return false;
		}
		...
		// store observer
		// Add in procedureTitle as a fake answer
		String[] obsNames = observer.split(",");
		if(obsNames.length > 0){
		Map<String,String> observerMap = new HashMap<String,String>();
		observerMap.put("answer", obsNames[0]);
		observerMap.put("id", "observer_group");
		observerMap.put("concept", "OBSERVER_GROUP");
		observerMap.put("type", "ENTRY");
		elementMap.put("observer_group", observerMap);
		}
		if(obsNames.length > 1){
			Map<String,String> observerMap2 = new HashMap<String,String>();
			observerMap2.put("answer", obsNames[1]);
			observerMap2.put("id", "observer_name");
			observerMap2.put("concept", "OBSERVER_NAME");
			observerMap2.put("type", "ENTRY");
			elementMap.put("observer_name", observerMap2);
			}
		// store subject
		...
		for(Entry<String,Map<String,String>> e : elementMap.entrySet()) {
			try {
				...
				Log.e(TAG, "postProcedureToDjangoServer():" + e1.toString());
				e1.printStackTrace();
				return false;
			}
		...
		if(savedProcedureUploaded) {
			...
			while(tries < MAX_TRIES) {
				if (postResponses(context, savedProcedureGUID, json, subject)) {
		...
		for(ElementAnswer e : binaries) {
			...
			for(String binaryId : ids) {
				...
				try {
					...
					Log.e(TAG, "postProcedureToDjangoServer():" + ex.toString());
					ex.printStackTrace();
				}
			...
			Uri binaryUri, int startPacketSize) throws Exception 
	{
		...
		if(alreadyUploaded) {
			Log.i(TAG+".transmitBinary()", binaryUri + " was already uploaded. Skipping.");
			return startPacketSize;
		}
		// Ope the input stream
		InputStream is = c.getContentResolver().openInputStream(binaryUri);
		fileSize = is.available();
		Log.d(TAG+".transmitBinary()","Available: " + fileSize);
		...
		Log.i(TAG+".transmitBinary()", "transmitBinary uploading " + binaryUri + " " 
		...
		while(bytesRemaining > 0) {
			...
			while(!success) {
				Log.d(TAG+".transmitBinary()", "Bytes " + chunk.length);
				Log.d(TAG+".transmitBinary()", "Trying to upload " + bytesRead + " bytes for " 
				...
				if(success) {
					...
					Log.i(TAG+".transmitBinary()", "packet rate = (current) " + currPacketRate 
								+ ", (base) " + basePacketRate);
				...
				if(efficient) {
					packetSize *= 2;
					Log.i(TAG+".transmitBinary()", "Shifting packet size *2 =" + packetSize);
				} else {
					packetSize /= 2;
					Log.i(TAG+".transmitBinary()", "Shifting packet size /2 =" + packetSize);
				}
	...
	public static boolean validateCredentials(Context c) throws IOException {
		...
		Log.i(TAG, "validateCredentials(): auth:" + (result ? "success" : "failure"));
		return result;
	}
