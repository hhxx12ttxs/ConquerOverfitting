...
public class DefinitionManagerImpl extends AbstractDefinitionController
	...
	/**
	 * Inner class qu?n lý thông tin c?a m?t entity ch? ??nh, ch?a ??ng thông
	 * tin v? b?n thân entity vŕ ??i t??ng cha c?a nó ?ang ch?a ??ng thông tin
	 * c?a chính entity.
	 * 
	...
	/**
	 * Ki?m tra loop validate thông qua h? th?ng cây annotation.
	 * 
	 * @param object
	 *            ??i t??ng c?n ki?m tra loop validate annotation.
	 * @return Tr? v? <b>true</b> n?u object có ch?a annotation ?ính kčm, ng??c
	 *         l?i tr? v? <b>false</b>.
	 */
	private boolean checkLoopValidation(Object object) {

		// validate d? li?u thông tin annotation tr??c khi di?n d?ch thŕnh
		// definition.
			...
			try {
				for (Entry<ObjectAnno, Annotation[]> entry : temp.entrySet()) {
					...
					for (Annotation anno : list) {
						// N?u anno ?ă ???c ??ng kí trong AnnotationRegister
						if (this.annotationRegister.isRegisteredAnnotation(anno
								.annotationType())) {
							// ki?m tra thông tin annotation
							this.validateAnnnotation(anno, list, obj.object,
									obj.rawObject);
							// n?u anno hi?n hŕnh c?ng có ?ính kčm thông tin
							// annotation khác.
	...
	public synchronized void loadCustomizedDefinition(Class<?> clazz,
		...
		// Ki?m tra ?i?u ki?n các annotation
		if (annotation != null) {
	...
	private synchronized Definition loadDefinition(Object object) {
		...
		// validate thông tin annotation tr??c khi
		// di?n d?ch thŕnh definition.
		checkLoopValidation(object);
		Definition result = buildDefinition(object);
		// N?u ??i t??ng truy?n vŕo lŕ m?t method.
		if (ReflectUtils.isMethod(object)) {
			Method method = (Method) object;
			/*
			 * Ki?m tra thông tin parameter n?u có
			 */
		...
		// N?u ??i t??ng truy?n vŕo lŕ m?t Class
		else if (ReflectUtils.isClass(object)) {
			Class<?> clazz = (Class<?>) object;
			// Kh?i t?o definition cho field c?a class n?u có
									...
									thisValidator = (AnnotationValidator<Annotation>) this.annotationRegister
											.getValidatorlist().get(
													((Annotation) object)
															.annotationType());
									/*
									 * Ki?m tra xem validator hi?n hŕnh c?a
									 * object g?c có ??ng kí catch exception cho
									 * annotation hi?n hŕnh hay không
									 */
