...
public interface ObjectAttach<U> extends IPresentLoadingClass {
	...
	/**
	 * Ch? ??nh thu?c tính lazy-init cho ánh x? hi?n hŕnh. Thu?c tính
	 * <b>lazy_init</b> ch? ??nh cách th?c kh?i t?o c?a bean ???c ??nh ngh?a b?i
	 * <code>configuration</code>. M?c ??nh <code>Context</code> s? t? ??ng kh?i
	 * t?o các instance bean khi vŕ ch? khi instance bean ???c get ra t? Context
	 * , t??ng ???ng thu?c tính <b>lazy_init</b> lŕ <b>true</b>. Ng??c l?i n?u
	 * thu?c tính <b>lazy-init</b> lŕ <b>false</b>, instance bean c?a
	 * <code>definition</code> ?ang xét s? t? ??ng ???c kh?i t?o ngay khi
	 * definition ???c n?p vŕo Context (JGentle container).<br>
	 * <br>
	 * <b>Chú ý:</b> Giá tr? m?c ??nh khi không ch? ??nh t??ng minh c?a thu?c
	 * tính lŕ <b>true</b> vŕ thu?c tính th?c s? không có hi?u l?c v?i các bean
	 * ???c ch? ??nh <b>scope</b> lŕ <code>Prototype</code>.
	 * 
	 * @param lazyInit
	 *            giá tr? boolean c?n ch? ??nh.
