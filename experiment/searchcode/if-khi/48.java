...
public abstract class JGentle {
	...
	private static Context buildContext(boolean serviceProvider,
		...
		/*
		 * Th?c thi các ti?n x? lý tr??c khi th?c thi c?u hěnh (invoke configure
		 * methods).
		 */
		...
		for (Class<? extends Configurable> targetClass : absCfgList) {
			Configurable result = ConfigurationProxy.createProxy(targetClass,
					argsType, args);
			result.configure();
			// Těm các imported configurable class n?u có
			...
			for (Configurable objResult : allResults) {
				// N?u context kh?i t?o lŕ m?t Services Context
				if (serviceProvider) {
					// Th?c thi ??ng kí annotation n?u trong config instance có
					// t?n t?i m?t ho?c nhi?u binding CSC
	...
	protected static Context buildContext(ServiceHandler serviceHandler,
		...
		for (Configurable config : configurations) {
			try {
				...
				// Th?c thi các x? lý ???c ch? ??nh b?i ATC
	...
	public static ServiceProvider buildServiceProvider(Class<?>[] argsType,
		...
		/*
		 * Kh??i ta?o services context
		 */
		ServiceProvider result = (ServiceProvider) buildContext(true, argsType,
				args, configClasses);
		/*
		 * Th?c thi các x? lý tr??c khi kh?i t?o CSC
		 */
		...
		// Th?c thi init tręn csc
		// Th?c thi init method tręn m?i CSC c?a services context v?a kh?i t?o.
		...
		/*
		 * Th?c thi các x? lý kh?i t?o CSC
		 */
		...
		// Duy?t qua t?ng danh sách Object Class c?a danh sách các CSC class.
		for (Class<? extends ComponentServiceContextType<?>> clazz : cscClassList) {
			...
			/*
			 * X? lý thông tin ch? ??nh trong @ComponentServiceContext n?u có
			 */
			ComponentServiceContext anno = null;
			...
			if (anno != null) {
				// Th?c thi ??ng kí annotation n?u có.
				if (!anno.beforeConfigure()) {
					JGentle.registerAnnotationInCSC(result.getServiceHandler(),
							anno, true, result);
				}
				// Kh?i t?o vŕ ??ng kí service Class
			...
			// Th?c thi add CSC vŕo ServiceProvider hi?n hŕnh.
			result.addCSContext(
					anno == null
		...
		/*
		 * Xóa b? toŕn b? các beforeInitContext object
		 */
	 ...
	 * 
	 * @param provider
	 *            the current {@link ServiceProvider} ??i t??ng
	 * @param csc
	...
	private static void registerAnnotations(ServiceHandler serviceHandler) {
		...
		// ??ng kí thông tin các annotation c?a core JGentle vŕo danh sách
		// annotation registered
		serviceHandler.registerAnnotations(RegisterAnnotationInjecting.class);
		// ??ng kí thông tin t?t c? các annotation c?a AOP System vŕo danh sách
		// annotation registered.
		serviceHandler.registerAnnotations(RegisterAnnotationAOP.class);
		// ??ng kí thông tin t?t c? các annotation c?a JGentle Context vŕo danh
