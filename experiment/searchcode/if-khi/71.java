...
public final class ReflectUtils {
	...
	public static AnnotationMetadata buildAnnoMeta(Annotation annos,
		...
		if (methods.length > 0) {
			for (Method obj : methods) {
				...
				/*
				 * ??a list sub annotationMetadata v?a ???c t?o vŕo trong
				 * annotationMetadata hi?n hŕnh
				 */
				thisMeta.putMetaData(part);
			}
		}
		/*
		 * add annotationMetadata vŕo container (annotationMetadata cha ch? nó)
		 */
	 ...
	 *            bean. instance.
	 * @return tr? v? ??i t??ng Object v?a ???c kh?i t?o.
	 */
	...
	/**
	 * Th?c thi tr? v? các {@link Field} t??ng ?ng v?i <code>object class</code>
	 * cung c?p t??ng ?ng.
	 * <p>
	 * <i>Method nŕy th?c thi t??ng ???ng
	 * {@link ReflectUtils#fields(String, Class, boolean)} v?i</i> giá tr?
	 * boolean (<code>'superClass'</code>) <i>ch? ??nh lŕ <b>false</b></i>. ?i?u nŕy có ngh?a r?ng
	 * danh sách těm ki?m s? ch? bao g?m các <code>fields</code> ???c khai báo
	 * trong ph?m vi <code>Class</code> t??ng ?ng, không bao g?m
	 * <code>superClass</code> c?a Class ch? ??nh .
	 * <p>
	 * L?u ý r?ng ph??ng th?c nŕy s? ch? tr? v? các
	 * <code>non-static fields</code>.
	 * <p>
	 * - Có th? s? d?ng <code>Regular Expression</code> ?? ch? ??nh tęn
	 * {@link Field} tr? v?.
	 * <p>
	 * <b>EX:</b>
	 * <p>
	 * <code>field("str*")</code> s? ch? ??nh tr? v? t?t c? các {@link Field}
	 * b?t ??u v?i chu?i "str" ho?c
	 * <p>
	 * <code>field("?name")</code> s? ch? ??nh tr? v? t?t c? các {@link Field}
	 * b?t ??u v?i 1 ch? b?t kě vŕ theo sau lŕ chu?i 'name'.
	 * 
	 * @param name
	 *            tęn ??nh danh c?a field
	 * @param clazz
	 *            object class ch? ??nh t??ng ?ng.
	 * @return tr? v? m?t m?ng các {@link Field} t??ng ?ng těm ???c n?u có, n?u
	 *         không tr? v? m?t danh sách r?ng.
	 */
	 ...
	 *            tęn ??nh danh c?a field
	 * @param clazz
	 *            object class t??ng ?ng
	 * @param superClass
	 *            n?u ch? ??nh lŕ <b>true</b>, danh sách các <code>fields</code>
	 *            těm ki?m s? bao g?m c? các <code>fields</code> ???c khai báo
	 *            b?i <code>superClass</code> c?a Class hi?n hŕnh. Ng??c l?i,
	 *            n?u ch? ??nh lŕ <b>false</b>, danh sách těm ki?m ch? n?m trong
	 *            ph?m vi các <code>fields</code> ???c khai báo trong
	 *            <code>Class</code> t??ng ?ng.
	 * @return tr? v? m?t m?ng các {@link Field} t??ng ?ng těm ???c n?u có, n?u
	 *         không tr? v? m?t danh sách r?ng.
	 */
	...
	public static List<Type> getAllGenericInterfaces(Class<?> clazz,
		...
		// Těm tręn t?t c? các interfaces c?a clazz hi?n hŕnh
		...
		// Těm tręn t?t c? các superClass c?a clazz hi?n hŕnh
	...
	/**
	 * Těm ki?m vŕ tr? v? field t??ng ?ng trong m?t class ch? ??nh. <br>
	 * <br>
	 * <b>L?u ý:</b> Hŕm s? t? ??ng těm trong <code>object class</code> ch?
	 * ??nh, n?u không có s? těm těm ti?p trong các <code>SuperClass</code> c?a
	 * nó.
	 * 
	 * @param cls
	 *            ??i t??ng <code>object class</code> truy v?n
	 *            <code>field</code>.
	 * @param name
	 *            tęn c?a field c?n těm.
	 * @return tr? v? field těm ???c.
	 * @throws NoSuchFieldException
	 *             ném ra ngo?i l? n?u nh? không có field nŕo trong object class
	 *             ch? ??nh ?ng v?i tęn name cung c?p.
	 */
	...
	/**
	 * Tr? v? <code>declared method</code> có support trong <code>clazz</code>
	 * ???c ch? ??nh d?a tręn tęn <code>name</code> vŕ <code>paramTypes</code>.
	 * N?u <code>class</code> hi?n t?i không support Method c?n těm thě s? těm
	 * trong <code>SuperClass</code> c?a <code>class</code> ???c ch? ??nh, n?u
	 * nh? không t?n t?i b?t kě <code>Method</code> nŕo t??ng ?ng, m?t ngo?i l?
	 * {@link NoSuchMethodException} s? ???c ném ra.
	 * 
	 * @param clazz
	 *            Class Object c?n truy v?n.
	 * @param name
	 *            tęn method c?n truy v?n.
	 * @param paramTypes
	 *            m?ng array các Class Object Type tham s? truy?n c?a method.
	 * @return tr? v? Method mu?n truy v?n
	 * @throws NoSuchMethodException
	 *             ném ra ngo?i l? nŕy n?u nh? không těm th?y b?t kě method nŕo
	 *             t??ng ?ng có trong clazz ho?c trong các SuperClass c?a nó.
	 */
	 ...
	 * @param superBool
	 *            n?u lŕ <b>true</b> s? truy těm ph??ng th?c tręn
	 *            <code>Object Class</code> vŕ c? <code>SuperClass</code> c?a
	 *            <code>object</code> ch? ??nh t??ng ?ng , ng??c l?i n?u lŕ
	 *            <b>false</b> s? ch? truy v?n tręn <code>Object Class</code>
	 *            ch? ??nh.
	 * @param noThrows
	 *            n?u lŕ <b>true</b> s? không ném ra <code>exception</code> n?u
	 *            nh? g?p l?i trong lúc thi hŕnh, ng??c l?i n?u lŕ <b>false</b>
	 *            s? ném ra ngo?i l? n?u g?p l?i trong lúc thi hŕnh. L?u ý r?ng
	 *            tham s? <code>noThrows</code> ch? có hi?u l?c tręn các ti?n
	 *            trěnh <code>reflect</code> lúc <code>invoke method</code>, n?u
	 *            b?n thân <code>method</code> ???c <code>invoked</code> ném ra
	 *            <code>exception</code> thě có ch? ??nh t??ng minh
	 *            <code>noThrows</code> hay không ??u không có tác d?ng.
	 * @return tr? v? <code>Object</code> tr? v? sau khi invoke method, n?u
	 *         <code>method</code> c?n tri?u g?i không <code>return</code> thě
	 *         k?t qu? tr? v? s? lŕ <b>NULL</b>.
	 * @throws NoSuchMethodException
	...
	public static Object invokeMethod(Object obj, String methodName,
		...
		clazz = obj.getClass();
		/*
		 * Ki?m tra thông tin h?p l? c?a parameters
		 */
	...
	/**
	 * Ph??ng th?c s? ki?m tra xem <code>annotation</code> ch? ??nh có
	 * {@link Target} t??ng ?ng h?p l? v?i ??i t??ng <code>object</code> hay
	 * không (??i t??ng <code>object</code> có th? lŕ b?t kě th?c th? ch? ??nh
	 * nŕo cho phép annotate annotation nh?
	 * <code>Class, Interface, Annotation, Enum, Method, Field, Constructor, ...</code>
	 * )
	 * 
	 * @param annotation
	 *            ??i t??ng <code>annotation</code> c?n ki?m tra.
	 * @param obj
	 *            ??i t??ng <code>object</code> c?n ki?m tra
	 * @return tr? v? <b>true</b> n?u có, n?u không tr? v? <b>false</b>.
	 */
