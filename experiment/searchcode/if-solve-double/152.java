...
public class HntmAnalyzer {
            ...
            {  
                ...
                if (analysisParams.harmonicModel == HntmAnalyzerParams.HARMONICS_PLUS_TRANSIENTS_PLUS_NOISE && labels!=null)
                {
                    ...
                    if (!isInTransientSegment) //Perhaps start of a new transient segment
                    {
                        for (j=0; j<transientPhonemesList.length; j++)
                        {
                            if (labels.items[currentLabInd].phn.compareTo(transientPhonemesList[j])==0)
                            {
                                ...
                                ((HntmPlusTransientsSpeechSignal)output.hnmSignal).transients.segments[transientSegmentInd].startTime = Math.max(0.0f, (((float)pm.pitchMarks[i])/fs)-analysisParams.overlapBetweenTransientAndNontransientSectionsInSeconds);
                                break;
                            }
                        }
                    }
                    else //Perhaps end of an existing transient segment
                    {
                        ...
                        {
                            float endTime = Math.min((((float)pm.pitchMarks[i]+0.5f*ws)/fs)+analysisParams.overlapBetweenTransientAndNontransientSectionsInSeconds, output.hnmSignal.originalDurationInSeconds);
                ...
                maxVoicingIndex = SignalProcUtils.time2frameIndex(output.hnmSignal.frames[i].tAnalysisInSeconds, analysisParams.hnmPitchVoicingAnalyzerParams.mvfAnalysisWindowSizeInSeconds, analysisParams.hnmPitchVoicingAnalyzerParams.mvfAnalysisSkipSizeInSeconds);
                ...
                else
                {
                    ...
                    if (i==0)
                        ...
                        frmStartIndex = SignalProcUtils.time2sample(output.hnmSignal.frames[i].tAnalysisInSeconds-0.5*numPeriods/f0InHz, fs);
                    ...
                    if (numHarmonics>0)
                    {
                        if (isVoiced)
                        {
                            ...
                            double[] envelope = new double[SignalProcUtils.halfSpectrumSize(fftSize)];
                            for (int ff=0; ff<envelope.length; ff++)
                            {
                                envelope[ff] = LpcAnalyser.calcSpecValLinear(hnmSignal.frames[i].lpCoeffs, hnmSignal.frames[i].lpGain, SignalProcUtils.index2freq(ff, fs, envelope.length-1), fs); 
                            }
                            MaryUtils.plot(MathUtils.linear2db(envelope));
                            SignalProcUtils.displayDFTSpectrumInDB(frm, fftSize);
                            MaryUtils.plot(MathUtils.linear2db(linearAmps));
                            //
                            */
                            ...
                            if(i==10)
                            {
                                ...
                                double[] vocalTractDB = RegularizedPreWarpedCepstrumEstimator.cepstrum2dbSpectrumValues(output.hnmSignal.frames[i].h.getCeps(output.hnmSignal.frames[i].f0InHz, fs, analysisParams), SignalProcUtils.halfSpectrumSize(fftSize)-1, fs);
                                 ...
                                 boolean[] isInTransientSegments)
    {
        ...
        // and makes this estimation independent of actual harmonic part amplitude representation used in synthesis and modifications
        ...
        if (hnmSignal!=null)
        {
            ...
            {  
                ...
                if (!isInTransientSegments[i])
                {
                    ...
                    noiseFrmStartInd = Math.max(0, SignalProcUtils.time2sample(hnmSignal.frames[i].tAnalysisInSeconds-0.5f*analysisParams.noiseAnalysisWindowDurationInSeconds, hnmSignal.samplingRateInHz));
                    ...
                    if (isNoised)
                    {
                        ...
                        if (hnmSignal.frames[i].maximumFrequencyOfVoicingInHz-analysisParams.overlapBetweenHarmonicAndNoiseRegionsInHz>0.0f)
                            y = SignalProcUtils.fdFilter(frmNoise, hnmSignal.frames[i].maximumFrequencyOfVoicingInHz-analysisParams.overlapBetweenHarmonicAndNoiseRegionsInHz, 0.5f*hnmSignal.samplingRateInHz, hnmSignal.samplingRateInHz, fftSizeNoise);
                        ...
                        {    
                            ...
                            //hnmSignal.frames[i].setLpCoeffs(lpcs.getLPRefc(), 1.0f); //Reflection coefficients (Lattice filter required for synthesis!
                        ...
                        else if (analysisParams.noiseModel==HntmAnalyzerParams.PSEUDO_HARMONIC)
                        {
                            ...
                            noiseHarmonics = estimateComplexAmplitudes(frmNoise, wgtNoise, analysisParams.noiseF0InHz, numNoiseHarmonics, hnmSignal.samplingRateInHz, analysisParams.hnmPitchVoicingAnalyzerParams.lastCorrelatedHarmonicNeighbour);
                            ...
                            //noiseHarmonics= estimateComplexAmplitudesUncorrelated2(frmNoise, wgtSquared, numNoiseHarmonics, NOISE_F0_IN_HZ, fs);
                            ...
                            if (!analysisParams.useNoiseAmplitudesDirectly)
                            {
                                ...
                                ((FrameNoisePartPseudoHarmonic)hnmSignal.frames[i].n).ceps = ArrayUtils.subarrayDouble2Float(linearAmpsNoise, 0, linearAmpsNoise.length); //Use amplitudes directly
                        
                            /*
                            //The following is only for visualization
                            //int fftSize = 4096;
                            //double[] vocalTractDB = RegularizedPreWarpedCepstrumEstimator.cepstrum2logAmpHalfSpectrum(((FrameNoisePartPseudoHarmonic)hnmSignal.frames[i].n).ceps, fftSize, hnmSignal.samplingRateInHz);
                            double[] vocalTractDB = new double[numNoiseHarmonics];
                            for (j=0; j<numNoiseHarmonics; j++)
                                vocalTractDB[j] = RegularizedPreWarpedCepstrumEstimator.cepstrum2linearSpectrumValue(((FrameNoisePartPseudoHarmonic)hnmSignal.frames[i].n).ceps, (j+1)*HnmAnalyzer.NOISE_F0_IN_HZ, hnmSignal.samplingRateInHz);
                        ...
                        {
                            if (i<totalFrm-1)
                                waveformNoiseEndInd = Math.max(0, SignalProcUtils.time2sample(hnmSignal.frames[i+1].tAnalysisInSeconds, hnmSignal.samplingRateInHz));
                            else
    ...
    public ComplexNumber[] estimateComplexAmplitudesUncorrelated(double[] frm, double[] wgtSquared, int L, double f0InHz, double samplingRateInHz)
    {
        ...
        for (k=1; k<=L; k++)
        {
            ...
            for (t=0; t<M; t++)
            {
                ...
                tmp = new ComplexNumber((float)(wgtSquared[t]*frm[t]*Math.cos(omega)), (float)(wgtSquared[t]*frm[t]*Math.sin(omega)));
