                             ...
                             Scriptable thisObj, Object[] args)
    {
        ...
        switch (methodId) {
            ...
            case Id_asin:
                x = ScriptRuntime.toNumber(args, 0);
                if (x == x && -1.0 <= x && x <= 1.0) {
                    x = (methodId == Id_acos) ? HelpMath.acos(x) : HelpMath.asin(x);
                } else {
                    x = Double.NaN;
                }
                break;
            ...
            case Id_atan2:
                x = ScriptRuntime.toNumber(args, 0);
                x = HelpMath.atan2(x, ScriptRuntime.toNumber(args, 1));
                break;
...
//                    : Math.exp(x);
//                break;
            throw new UnsupportedOperationException("NativeMath");
            case Id_floor:
                x = ScriptRuntime.toNumber(args, 0);
                x = Math.floor(x);
                break;
...
//                break;
            	throw new UnsupportedOperationException("NativeMath.pow");
