...
public class BasicString extends MaverickString {      
    ...
    private final static char[] DELIMITERS = { 0, Delimiter.AM, Delimiter.VM, Delimiter.SM, 0, 0 };

    private final static char[][] REPLACE_DELIMITERS = {
        null,
        { Delimiter.AM },
        { Delimiter.AM, Delimiter.VM },
        { Delimiter.AM, Delimiter.VM, Delimiter.SM },
        null,
        null,
    };
    ...
    public MaverickString append(char c) {
        switch (type) {
    ...
    public MaverickString append(String s) {        
        int len = s.length();
        switch (type) {
            case TYPE_BASIC_STRING:
                char[] buffer = (char[])data;
    ...
    public MaverickString append(ConstantString mvs) {
        int len = mvs.length();
        switch (type) {
            case TYPE_BASIC_STRING:
                char[] buffer = (char[])data;
    ...
    public MaverickString append(ConstantString mvs, int start, int len) {
        switch (type) {
            case TYPE_BASIC_STRING:
                char[] buffer = (char[])data;
                if (len > buffer.length - value) {
    ...
    */
        ...
        switch (type) {
            case TYPE_BASIC_STRING:
                if (value >= 0) {
                    char[] buffer = (char[])data;
                    for (int i = 0; i < value; i++) {                
                        if (buffer[i] >= Delimiter.TM && buffer[i] <= Delimiter.RM) return true;
                    }
                }       
                return false;   
            case TYPE_JAVA_DOUBLE:
            ...
            case TYPE_JAVA_STRING:
                if (value >= 0) {
                    ...
                    for (int i = 0; i < value; i++) {
                        char c = s.charAt(i);
                        if (c >= Delimiter.TM && c <= Delimiter.RM) return true;
                    }
                }       
                return false;                
            case TYPE_INDEX_AM_STRING:
    ...
    public MaverickString DCOUNT(MaverickString result, ConstantString substr, int offset) { 
        switch (type) {
            case TYPE_INDEX_AM_STRING:
            case TYPE_INDEX_VM_STRING: 
            case TYPE_INDEX_SM_STRING:
                {
                    ...
                    if (sublen > 0) {
                        ...
                        if (c == DELIMITERS[type] && offset == 1)  {
                            result.set(value);
                        } else {
                            //XXX if substr.length > 1 this will break
                            BasicString[] array = (BasicString[])data;
                            int len = (value > 0) ? 1 : 0;
                            for (int i = 0; i < value; i++) {
                                array[i].DCOUNT(result, substr, offset);
                        ...
                        int len = length();
                        result.set((len > 0) ? len + 1 : 0);
                    }
                    return result;
                }
            default:
                return super.DCOUNT(result, substr, offset);
        }
    }

    /**
    * Deletes an attribute, value or subvalue from a MaverickString, the corresponding
    ...
    * @return the MaverickString
    */        
    public MaverickString DELETE(ConstantString attrib, ConstantString val, ConstantString subval) {        
    ...
    */   
        ...
        switch (type) {
            case TYPE_BASIC_STRING:
                if (o instanceof ConstantString) {
                    ConstantString mvcs = (ConstantString)o;
                    if (mvcs.isIntegral() && isIntegral()) {
                        return (mvcs.longValue() == longValue());
                    } else if (value == ((ConstantString)o).length()) {
                        char[] buffer = ((char[])data);
                        for (int i = 0; i < value; i++) {
                ...
                return false;
            case TYPE_JAVA_DOUBLE:
                if (o instanceof ConstantString && ((ConstantString)o).isNumeric()) {
                    double d = ((ConstantString)o).getDouble();
                    return (value == doubleToLongBits(d));
                }
                return false;
            case TYPE_JAVA_LONG:
                if (o instanceof ConstantString && ((ConstantString)o).isIntegral()) {
                    return (value == ((ConstantString)o).longValue());
                }
                return false;
            case TYPE_JAVA_STRING:
                if (o instanceof ConstantString) {
            ...
            case TYPE_INDEX_SM_STRING:
                if (o instanceof ConstantString) {
                    ...
                    ConstantString o2 = (ConstantString)o;
    ...
    public ConstantString EXTRACT(ConstantString attrib, ConstantString val, ConstantString subval) {        
        if (type != TYPE_INDEX_AM_STRING) {
            convertToMultivalue(TYPE_INDEX_AM_STRING);
        }
        int a = attrib.intValue();
    ...
    public int findLength(int start, int len, char delim) {
        ...
        char[] buffer = (char[])data;
        int end = start;
        while (end < len && buffer[end] != delim) end++;
        return end - start;
    }
    ...
    public MaverickString INSERT(ConstantString attrib, ConstantString val, ConstantString subval, ConstantString insert) {
        if (type != TYPE_INDEX_AM_STRING) {
            convertToMultivalue(TYPE_INDEX_AM_STRING);
        }
        int a = attrib.intValue();
        ...
        if (v != 0) {
            if (a < 0 || a > value) {
                ...
                //reset array as data may have been reallocated
            ...
            if (s != 0) {
                ...
                if (v < 0 || v > array[a].value) {
                    ...
                    //reassign as data may have been reallocated
                    array2 = (BasicString[])array[a].data;
                } else {
                    v--;
                }
    ...
    */    
    ...
    public Program getProgram() throws MaverickException {
        switch (type) {
            ...
            default:
                if (length() > 0) {
                    ...
                    //XXX runtime error!
                    throw new MaverickException(0, "Cannot resolve program!");
                }
        }
    }
    ...
    public int intValue() {
        //isIntegral forces a conversion to type = long if possible
        if (isIntegral() && type == TYPE_JAVA_LONG) {
            return (int)value;
        } else if (isNumeric() && type == TYPE_JAVA_DOUBLE) {
    ...
    public boolean isIntegral() {
        switch(type) {
            case TYPE_BASIC_STRING:
				...
				if (hasMinus) {
                    for (int i = 1; i < value; i++) {
                        ...
                        if (!hasDecimal && c == '.') {
                            ...
                            return false;
                        } else if (hasDecimal && Character.digit(c, 10) != 0) {
                            return false;
                        } else {
                            val *= 10;
				...
				if ((hasDecimal || hasMinus) && value == 1) return false;
				if (hasDecimal && hasMinus && value == 2) return false;
                type = TYPE_JAVA_LONG;
                value = val;
                return true;
                }
            ...
            case TYPE_JAVA_STRING:
                {
                    ...
                    if ((hasDecimal || hasMinus) && s.length() == 1) {
                        return false;
                    }
                    if (hasDecimal && hasMinus && s.length() == 2) {
                        return false;
                    }
                    type = TYPE_JAVA_LONG;
                    value = val;
            ...
            case TYPE_INDEX_SM_STRING:
                if (value == 1) {
                    BasicString bs = ((BasicString[])data)[0];
                    if (bs.isIntegral()) {
                        type = TYPE_JAVA_LONG;
                        value = bs.longValue();
                        return true;
    ...
    public boolean isNumeric() {
        switch(type) {
            case TYPE_BASIC_STRING:
                char[] buffer = (char[])data;
                boolean hasMinus = (value > 0 && buffer[0] == '-');
                boolean hasDecimal = false;
            ...
            case TYPE_JAVA_STRING:
                String s = (String)data;
                boolean hasMinus2 = (value > 0 && s.charAt(0) == '-');
                boolean hasDecimal2 = false;
                int index2 = (hasMinus2) ? 1 : 0;
            ...
            case TYPE_INDEX_SM_STRING:
                if (value == 1) {
                    ...
                    if (bs.isNumeric()) {
                        type = TYPE_JAVA_DOUBLE;
                        value = doubleToLongBits(bs.getDouble());
                        return true;
                    }
                }
                return false;
    ...
    */     
    ...
    public ConstantString REMOVE(MaverickString colpos, MaverickString delim) {
    ...
    public void replace(ConstantString mvs, int start, int offset, int len) {
        switch (type) {
            case TYPE_JAVA_DOUBLE:                          
                data = doubleToString().toCharArray();            
                value = ((char[])data).length;
    ...
    * @param replace MaverickString to be replaced
    * @return result string containing the replaced entry
    */
    public MaverickString REPLACE(ConstantString attrib, ConstantString val, ConstantString subval, ConstantString replace) {                
        ...
        //now need to check if replace contains delimiters
        char[] delims = REPLACE_DELIMITERS[item.type];
        int rstart = 0;
        int rlen = replace.length();
        int rindex = rstart + findLength(replace, rstart, rlen, delims);

        array[index].set(replace, rstart, rindex - rstart);

        while (rindex < rlen) {
            char delim = replace.charAt(rindex);
            rstart = rindex + 1;
            rindex = rstart + findLength(replace, rstart, rlen, delims);
            BasicString rec = (BasicString)factory.getString();
            rec.set(replace, rstart, rindex - rstart);
            switch (delim) {
                case Delimiter.AM:
                    {
                        switch (item.type) {
                            ...
                            case TYPE_INDEX_VM_STRING:
                                {
                                    a++;
                                    insert(a + 1, rec);
                                    BasicString orig = item;
                                    item = ((BasicString[])data)[a];
                                    v = moveTree(orig, v + 1, item);
                                }
                                break;
                            case TYPE_INDEX_SM_STRING:
                                {
                                    BasicString ip = ((BasicString[])data)[a];
                                    a++;
                                    insert(a + 1, rec);
                                    BasicString dest = ((BasicString[])data)[a];
                                    moveTree(ip, v + 1, dest);
                                    v = 0;
                                    BasicString orig = item;
                                    item = ((BasicString[])dest.data)[v];
                                    s = moveTree(orig, s + 1, item);
                                }
                                break;
                        }
                    }
                ...
                case Delimiter.VM:
                    {
                        switch (item.type) {
                            ...
                            case TYPE_INDEX_SM_STRING:
                                {
                                    BasicString dest = ((BasicString[])data)[a];
                                    if (dest.type != TYPE_INDEX_VM_STRING) {
                                        dest.convertToMultivalue(TYPE_INDEX_VM_STRING);
                                    }
                                    v++;
                                    dest.insert(v + 1, rec);
                                    BasicString orig = item;
    ...
    private int findLength(ConstantString s, int start, int len, char[] delims) {
        //XXX this should be merged with findLength() into a single routine
        int end = start;
        while (end < len) {
            char c = s.charAt(end);
    ...
    private int moveTree(BasicString src, int srcoffset, BasicString dest) {
        
        if (src.type != dest.type) {
            //assuming some type of index string
            dest.convertToMultivalue(src.type);
        }

        BasicString[] sarr = (BasicString[])src.data;
        int insertpoint = (int)dest.value;
        while (src.value > srcoffset) {
            BasicString s2 = sarr[(int)(src.value - 1)];
            sarr[(int)(src.value - 1)] = null;
            dest.insert(insertpoint + 1, s2);
            src.value--;
        }
        return insertpoint;
    }
