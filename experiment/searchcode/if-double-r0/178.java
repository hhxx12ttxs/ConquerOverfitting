...
/**
 * @author tag
 * @version $Id: ImageUtil.java 13200 2010-03-11 19:27:48Z dcollins $
 */
public class ImageUtil
{
     ...
     * @throws IOException if there is a problem opening the file.
     */
    private static AVList handleGeotiff(BufferedImage image, GeotiffReader reader, int imageIndex, int interpolation_mode)
        throws IOException
    {
    ...
    public static Sector calcBoundingBoxForUTM(AVList params) throws IOException
    {
        ...
        if( !params.hasKey(WorldFile.WORLD_FILE_ZONE) )
        {
        ...
        if( !params.hasKey(WorldFile.WORLD_FILE_HEMISPHERE) )
        {
            String message = Logging.getMessage("WorldFile.InvalidUTMHemisphere" );
            Logging.logger().severe(message);
            throw new IOException( message );
        }
        ...
        int zone = Integer.parseInt((String) params.getValue(WorldFile.WORLD_FILE_ZONE));
