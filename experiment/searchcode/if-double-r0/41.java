...
// import edu.cmu.sphinx.linguist.language.ngram.large4.BigramBuffer;
// import edu.cmu.sphinx.linguist.language.ngram.large4.BigramProbability;
// import edu.cmu.sphinx.linguist.language.ngram.large4.TrigramBuffer;
 ...
 */
public class LargeQuadrigramModel implements LanguageModel {
    /**
 ...
 /**
     ...
     * cached
     */
    public static final String PROP_QUADRIGRAM_CACHE_SIZE = "quadrigramCacheSize";

    /**
     * The default value for the PROP_TRIGRAM_CACHE_SIZE property
     */
    public static final int PROP_QUADRIGRAM_CACHE_SIZE_DEFAULT = 100000;
     ...
     * CMU-Cambridge Statistical Language Modelling Toolkit.
     */
    public static final int ID_FIELDS_PER_TRIGRAM = 4;
    public static final int ID_FIELDS_PER_QUADRIGRAM = 2;
    ...
    private int maxQuadrigramCacheSize;
    ...
    private int quadrigramMisses;
    private int quadrigramHit;
    ...
    private LRUCache quadrigramCache;
    private Map loadedQuadrigramBuffer;
    ...
    private int[] quadrigramSegmentTable;
    private float[] bigramProbTable;
    private float[] trigramProbTable;
    private float[] trigramBackoffTable;
    private float[] quadrigramProbTable;
    private float[] quadrigramBackoffTable;//c 'est plutot tribackof mais bon
	...
	registry.register(PROP_QUADRIGRAM_CACHE_SIZE, PropertyType.INT);
    ...
    public void newProperties(PropertySheet ps) throws PropertyException {
        ...
        maxQuadrigramCacheSize = ps.getInt(PROP_QUADRIGRAM_CACHE_SIZE,
                PROP_QUADRIGRAM_CACHE_SIZE_DEFAULT);
    ...
    public void allocate() throws IOException {
        ...
        loadedQuadrigramBuffer = new HashMap();
        quadrigramCache = new LRUCache(maxQuadrigramCacheSize);
	...
	quadrigramProbTable = loader.getQuadrigramProbabilities();
        quadrigramBackoffTable = loader.getQuadrigramBackoffWeights();
        quadrigramSegmentTable = loader.getQuadrigramSegments();
        ...
        logger.info("Quadrigrams: " + loader.getNumberQuadrigrams());
    ...
    private void clearCache() {
        ...
        loadedTrigramBuffer = new HashMap();
        loadedQuadrigramBuffer = new HashMap();
        logger.info("LM Cache: 4-g "+ quadrigramCache.size() +" 3-g " + trigramCache.size() + " 2-g "
                + bigramCache.size());
    ...
    public float getProbability(WordSequence wordSequence) {
	 ...
	 if (numberWords <= maxDepth) {
	     if (numberWords == 4) {
		 ret= getQuadrigramProbability(wordSequence);}
	     else
            if (numberWords == 3) {
        ...
        if (numberWords <= maxDepth) {
	    if (numberWords == 4) {
                return  getQuadrigramProbability(wordSequence);}
            if (numberWords == 3) {
	...
	if (bigramCache.containsKey(ws)) 
	    return (BigramProbability) bigramCache.get(ws);
        BigramProbability bigramProbability = null; // (BigramProbability) bigramCache.get(ws);
	    // je garde les bigrammes non present
    ...
    private BigramBuffer loadBigramBuffer(int firstWordID, int numberFollowers) {
        ...
        int size = (numberFollowers + 1) * ID_FIELDS_PER_BIGRAM * loader.getBytesPerIDField();
        long position = (long) (loader.getBigramOffset() + (firstBigramEntry * ID_FIELDS_PER_BIGRAM * loader.getBytesPerIDField()));
        try {
            byte[] buffer = loader.loadBuffer(position, size);
            followers = new BigramBuffer(buffer, numberFollowers + 1, loader.getBigEndian(), loader.getBytesPerIDField());
    ...
    private float getTrigramProbability(WordSequence wordSequence) {
        ...
        TrigramProbability trigramProbability = findTrigram(wordSequence);
	float score =0.0f;
	
	if (trigramProbability != null) {
	    trigramHit++;
	     score= trigramProbTable[trigramProbability.getProbabilityID()];
	} else {
	    trigramMisses++;
	...
	if (trigramCache.containsKey(wordSequence))
	    return (TrigramProbability) trigramCache.get(wordSequence);
    ...
    TrigramBuffer  getTrigramBuffer(WordSequence ws) {
        WordSequence oldest= ws.getOldest();
	TrigramBuffer trigramBuffer = (TrigramBuffer) loadedTrigramBuffer
	    .get(oldest);
        ...
        if (bigramBuffer != null) {
            ...
            if (bigram != null) {
                ...
                int numberTrigrams = getFirstTrigramEntry(nextBigram,
                        firstBigramEntry)
                        - firstTrigramEntry;
		numberTrigrams++; //pour la sentinelle 
                int size = numberTrigrams * ID_FIELDS_PER_TRIGRAM * loader.getBytesPerIDField();
                long position = (loader.getTrigramOffset() + (long) (firstTrigramEntry * ID_FIELDS_PER_TRIGRAM * loader.getBytesPerIDField()));

                try {
                    // System.out.println("Loading TrigramBuffer from disk");
                    byte[] buffer = loader.loadBuffer(position, size);
                    trigramBuffer = new TrigramBuffer(buffer, numberTrigrams, loader.getBigEndian(), loader.getBytesPerIDField());
                } catch (IOException ioe) {
                    ioe.printStackTrace();
 ...
 */
   ...
   /**
     * Returns the language probability of the given quadrigram.
     * 
     * @param wordSequence
     *                the quadrigram word sequence
     * 
     * @return the quadrigram probability
     */
    private float getQuadrigramProbability(WordSequence wordSequence) {
        Word firstWord = wordSequence.getWord(0);

        if (loader.getNumberQuadrigrams() == 0 || !hasUnigram(firstWord)) {
            return getTrigramProbability(wordSequence.getNewest());
        }

        Float probability = (Float) quadrigramCache.get(wordSequence);

        if (probability == null) {
            float score = 0.0f;

            int quadrigramProbID = findQuadrigram(wordSequence);

            if (quadrigramProbID != -1) {
                quadrigramHit++;
                score = quadrigramProbTable[quadrigramProbID];
            } else {
                quadrigramMisses++;
                TrigramProbability trigram = findTrigram(wordSequence.getOldest());
                if (trigram != null) {
                    score = quadrigramBackoffTable[trigram.getBackoffID()]
                            + getTrigramProbability(wordSequence.getNewest());
                } else {
                    score = getTrigramProbability(wordSequence.getNewest());
                }
            }
            probability = new Float(score);
            quadrigramCache.put(wordSequence, probability);
        }
    ...
    /**
     * Finds or loads the quadrigram probability of the given quadrigram.
     * 
     * @param wordSequence
     *                the quadrigram to load
     * 
     * @return a QuadrigramProbability of the given quadrigram
     */
    private int findQuadrigram(WordSequence wordSequence) {

        int quadrigram = -1;

        WordSequence oldest = wordSequence.getOldest();
        QuadrigramBuffer quadrigramBuffer = (QuadrigramBuffer) loadedQuadrigramBuffer
                .get(oldest);

        if (quadrigramBuffer == null) {

//             int firstWordID = getWordID(wordSequence.getWord(0));
//             int secondWordID = getWordID(wordSequence.getWord(1));

            quadrigramBuffer = loadQuadrigramBuffer(oldest);

            if (quadrigramBuffer != null) {
                loadedQuadrigramBuffer.put(oldest, quadrigramBuffer);
            }
        }

        if (quadrigramBuffer != null) {
            int forthWordID = getWordID(wordSequence.getWord(3));
            quadrigram = quadrigramBuffer.findProbabilityID(forthWordID);
        }
    ...
    /**
     * Loads into a buffer all the quadrigram followers of the given bigram.
     * 
     * @param firstWordID
     *                the ID of the first word
     * @param secondWordID
     *                the ID of the second word
     * 
     * @return a QuadrigramBuffer of all the quadrigram followers of the given two
     *         words
     */
    private QuadrigramBuffer loadQuadrigramBuffer(WordSequence ws) {
        QuadrigramBuffer quadrigramBuffer = null;

        TrigramBuffer TrigramBuffer = getTrigramBuffer(ws);

        if (TrigramBuffer != null) {
            TrigramProbability trigram = TrigramBuffer.findTrigram(getWordID(ws.getWord(2)));
		...
		int firstTrigramEntry = getFirstTrigramEntry(bigram,
							     firstBigramEntry);
                int ceTri= firstTrigramEntry+trigram.getWhichFollower();
		int debQuadri=quadrigramSegmentTable[ceTri >> loader.getLogTrigramSegmentSize()]
		    + trigram.getFirstQuadrigramEntry();
                

                TrigramProbability nextTrigram = TrigramBuffer
		    .getTrigramProbability(trigram.getWhichFollower() + 1);
                int numberQuadrigrams = quadrigramSegmentTable[ceTri+1 >> loader.getLogTrigramSegmentSize()]+ nextTrigram.getFirstQuadrigramEntry()
		    - debQuadri;
		/*

		j'en suis la mais a priori il faut un xgram de plus par buffer pour etre sur d'avoir la fin pour les tris et c'est dumper
		*/
                int size = numberQuadrigrams * ID_FIELDS_PER_QUADRIGRAM * loader.getBytesPerIDField();
                long position = (loader.getQuadrigramOffset() + (long) (debQuadri * ID_FIELDS_PER_QUADRIGRAM * loader.getBytesPerIDField()));

                try {
                    // System.out.println("Loading QuadrigramBuffer from disk");
                    byte[] buffer = loader.loadBuffer(position, size);
                    quadrigramBuffer = new QuadrigramBuffer(buffer, numberQuadrigrams, loader.getBigEndian(), loader.getBytesPerIDField());
                } catch (IOException ioe) {
                    ioe.printStackTrace();
                    throw new Error("Error loading quadrigrams.");
                }
            }
        }
        return quadrigramBuffer;
    }

    /**
     * Returns the index of the first quadrigram entry of the given bigram
     * 
     * @param bigram
     *                the bigram which first quadrigram entry we're looking for
     * @param firstBigramEntry
     *                the index of the first bigram entry of the bigram in
     *                question
     * 
     * @return the index of the first quadrigram entry of the given bigram
     
    private int getFirstQuadrigramEntry(BigramProbability bigram,
            int firstBigramEntry) {
        int firstQuadrigramEntry = quadrigramSegmentTable[(firstBigramEntry + bigram
                .getWhichFollower()) >> loader.getLogBigramSegmentSize()]
                + bigram.getFirstQuadrigramEntry();
 ...
 */
 ...
 public int getQuadrigramMisses() {
        return quadrigramMisses;
    }

    /**
     * Returns the number of quadrigram hits.
     * 
     * @return the number of quadrigram hits
     */
    public int getQuadrigramHits() {
        return quadrigramHit;
    }
            ...
            for (int j = 0; j < bigram.getNumberNGrams(); j++) {
                ...
                if (trigram == null) {
                    ...
                    for (int l = 0; l < trigram.getNumberNGrams(); l++) {
                        ...
                        float logtgprob 
                            = trigramProbTable[trigram.getTrigramProbability(l).getProbabilityID()];
