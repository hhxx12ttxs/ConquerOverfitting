...
public class JobGraph extends AbstractGraph implements XulEventHandler, Redrawable, TabItemInterface, LogParentProvidedInterface, MouseListener, MouseMoveListener, MouseTrackListener, MouseWheelListener, KeyListener
{
							...
							if (jge != null) // Create duplicate of existing entry 
							{
								...
								if (jge.isDrawn()) {
									...
									if (newjge != null) {
										...
										if (log.isDebug())
											log.logDebug("entry aft = " + ((Object) jge.getEntry()).toString()); //$NON-NLS-1$
										...
										if (log.isDebug())
											log.logDebug("jge is not cloned!"); //$NON-NLS-1$
									}
								}
								else {
									if (log.isDebug())
										log.logDebug(jge.toString() + " is not drawn"); //$NON-NLS-1$
									jge_changed = true;
								}
	...
	public void editEntryParallel()
	{
		...
		if (getJobEntry().isLaunchingInParallel()) {
			...
			{
				MessageDialogWithToggle md = new MessageDialogWithToggle(shell, BaseMessages.getString(PKG, "JobGraph.ParallelJobEntriesWarning.DialogTitle"), //$NON-NLS-1$
	...
	protected synchronized void setMenu(int x, int y)
	{
		...
		if (jobEntry != null) // We clicked on a Job Entry!
		{
			...
			if (menu != null) {
				...
				item.setLabel(BaseMessages.getString(PKG, "JobGraph.PopupMenu.JobEntry.AllignDistribute.SnapToGrid") + ConstUI.GRID_SIZE + ")\tALT-HOME");
	...
	public void launchStuff(JobEntryCopy jobEntryCopy)
	{
		if (jobEntryCopy.isJob()) {
			...
			if ((entry != null && entry.getJobObjectId() == null && !Const.isEmpty(entry.getFilename()) && spoon.rep == null) || (entry != null && !Const.isEmpty(entry.getName()) && spoon.rep != null)
			...
			if ((entry != null && entry.getTransObjectId() == null && !Const.isEmpty(entry.getFilename()) && spoon.rep == null) || (entry != null && entry.getName() != null && spoon.rep != null)
	...
	public Image getJobImage(Device device, int x, int y, float magnificationFactor)
	{
		...
		JobPainter jobPainter = new JobPainter(gc, jobMeta, new Point(x, y), new SwtScrollBar(hori), new SwtScrollBar(vert), hop_candidate, drop_candidate, selectionRegion, areaOwners, mouseOverEntries, PropsUI.getInstance().getIconSize(),
	...
	public static boolean editProperties(JobMeta jobMeta, Spoon spoon, Repository rep, boolean allowDirectoryChange)
	{
		...
		if (jd.isSharedObjectsFileChanged()) {
			try {
				...
				new ErrorDialog(spoon.getShell(), BaseMessages.getString(PKG, "Spoon.Dialog.ErrorReadingSharedObjects.Title"), BaseMessages.getString(PKG, "Spoon.Dialog.ErrorReadingSharedObjects.Message", spoon.delegates.tabs.makeTabName(jobMeta, true)),
					e);
			}
		}
