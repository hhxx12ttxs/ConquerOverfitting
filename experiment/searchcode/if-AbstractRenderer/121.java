		...
		if (plot == null) {
			...
			if (chartType == ChartConstants.OVERLAY_XY || chartType == ChartConstants.COMBINED_XY) {
				switch (rendererIndex) {
					...
					case PlotConstants.SCATTER :
					case PlotConstants.STEP :
					case PlotConstants.SPLINE :
					case PlotConstants.STACKED_XY_AREA :
					case PlotConstants.STACKED_XY_AREA2 :
						check(data, XYDataset.class, rendererIndex);
						plot = new XYPlot((XYDataset) data, null, null, (XYItemRenderer) rend);
						break;
					case PlotConstants.XY_VERTICAL_BAR :
						check(data, IntervalXYDataset.class, rendererIndex);
						plot = new XYPlot((IntervalXYDataset) data, null, null, (XYItemRenderer) rend);
						break;
					case PlotConstants.CANDLESTICK :
					case PlotConstants.HIGH_LOW :
				...
				switch (rendererIndex) {
					case PlotConstants.AREA :
					case PlotConstants.VERTICAL_BAR :
					case PlotConstants.LINE :
					case PlotConstants.SHAPES_AND_LINES :
	...
	public void check(Dataset data, Class clazz, int plotType) throws IncompatibleDatasetException {
		if (!clazz.isInstance(data)) {
			throw new IncompatibleDatasetException(
				"Plots of type " + PlotTypes.typeList.get(plotType) + " need a dataset of type " + clazz.getName());
		}
	}
