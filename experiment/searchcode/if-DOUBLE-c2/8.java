 ...
 * Open Source Physics Project. The original package contains code for other
 * transformations and other data types.
 * 
 ...
 * <P>
 * This algorithm appears to be faster than the Radix2 method, when both methods
 * apply, but requires extra storage (which FFTComplex manages itself).
 * <P>
...
public class FFT {
	 ...
	 * @return
	 */
	public static int[] factor(final int n, final int fromfactors[]) {
	 ...
	 * 
	 * @param n
	 *            the number of complex data points
	 */
	...
	/**
	 * Computes the back Fast Fourier Transform of data leaving the result in
	 * data. The given array is returned after it has been transformed.
	 * 
	 * @param data
	 *            double[] the data to be transformed
	 * @return double[] the data after the FFT
	 */
	public double[] backtransform(final double data[]) {
		if (data.length != 2 * n) {
			if (data.length % 2 != 0) {
				throw new IllegalArgumentException(
						"Number of points in array is not even");
			}
			setN(data.length / 2);
		}
	...
	/**
	 * Gets an array containing the frequencies in natural order. The first data
	 * point is at xmin (tmin) and the last data point is at xmax (tmax).
	 * 
	 * @param xmin
	 * @param xmax
	 * @return the array of frequencies
	 */
	public double[] getNaturalFreq(final double xmin, final double xmax) {
	 ...
	 * @return the array of frequencies
	 */
	public double[] getNaturalOmega(final double delta) {
	 ...
	 * @return the array of frequencies
	 */
	public double[] getNaturalOmega(final double xmin, final double xmax) {
	...
	/**
	 * Gets an array containing the frequencies in wrap-around order. Samples in
	 * the data are separated by delta.
	 * 
	 * @param delta
	 * @return the array of frequencies
	 */
	public double[] getWrappedFreq(final double delta) {
		double[] freq = new double[n];
		double f = -0.5 / delta, df = -2 * f / (n - n % 2);
		for (int i = 0; i < n; i++) {
	...
	/**
	 * Gets an array containing the frequencies in wrap-around order. The first
	 * data point is at xmin (tmin) and the last data point is at xmax (tmax).
	 * 
	 * @param xmin
	 * @param xmax
	 * @return the array of frequencies
	 */
	public double[] getWrappedFreq(final double xmin, final double xmax) {
		return getNaturalFreq((xmax - xmin) / (n - n % 2));
	}
	...
	/**
	 * Gets an array containing the angular frequencies (wavenumber) in
	 * wrap-around order. Samples in the orginal data are separated by delta.
	 * 
	 * @param delta
	 * @return the array of frequencies
	 */
	public double[] getWrappedOmega(final double delta) {
		return getWrappedFreq(delta / PI2);
	}

	/**
	 * Gets an array containing the angular frequencies (wavenumber) in
	 * wrap-around order. The first data point is at xmin (tmin) and the last
	 * data point is at xmax (tmax).
	 * 
	 * @param xmin
	 * @param xmax
	 * @return the array of frequencies
	 */
	public double[] getWrappedOmega(final double xmin, final double xmax) {
		return getWrappedFreq((xmax - xmin) / (n - n % 2) / PI2);
	}

	/**
	 * Computes the (nomalized) inverse FFT of data, leaving it in place. The
	 ...
	 * @return double[] the data after the FFT
	 */
	public double[] inverse(final double data[]) {
		backtransform(data);
	 ...
	 * @param product
	 */
	void pass_2(final int fi, final double in[], final int in0, final int istride, final double out[],
			final int out0, final int ostride, final int sign, final int product) {
		int k, k1;
		int factor = 2;
		int m = n / factor;
		int q = n / product;
	 ...
	 * @param product
	 */
	void pass_3(final int fi, final double in[], final int in0, final int istride, final double out[],
			final int out0, final int ostride, final int sign, final int product) {
		int k, k1;
		int factor = 3;
		int m = n / factor;
	 ...
	 * @param product
	 */
	void pass_4(final int fi, final double in[], final int in0, final int istride, final double out[],
			final int out0, final int ostride, final int sign, final int product) {
		int k, k1;
		int factor = 4;
		int m = n / factor;
	 ...
	 * @param product
	 */
	void pass_5(final int fi, final double in[], final int in0, final int istride, final double out[],
			final int out0, final int ostride, final int sign, final int product) {
		int k, k1;
		int factor = 5;
		int m = n / factor;
	 ...
	 * @param product
	 */
	void pass_6(final int fi, final double in[], final int in0, final int istride, final double out[],
			final int out0, final int ostride, final int sign, final int product) {
		int k, k1;
		int factor = 6;
		int m = n / factor;
	 ...
	 * @param product
	 */
	void pass_7(final int fi, final double in[], final int in0, final int istride, final double out[],
			final int out0, final int ostride, final int sign, final int product) {
		int k, k1;
		int factor = 7;
		int m = n / factor;
	 ...
	 * @param product
	 */
	void pass_n(final int fi, final double in[], final int in0, final int istride, final double out[],
			final int out0, final int ostride, final int sign, final int factor, final int product) {
		int i = 0, j = 0;
		int k, k1;
		int m = n / factor;
		int q = n / product;
	 ...
	 *            double
	 */
	public void setNormalization(final double norm) {
		this.norm = norm;
	}

	/* ______________________________________________________________________ */

	private void setup_wavetable(final int n) {
		if (n <= 0) {
	 ...
	 * @return double[] the data after the FFT
	 */
	public double[] toNaturalOrder(final double data[]) {
		System.arraycopy(data, 0, scratch, 0, 2 * n); // save the data
		System.arraycopy(scratch, n + n % 2, data, 0, n - n % 2); // copy the
		// second
		// half of
		// the data
		// into the
		// first
		// half
		System.arraycopy(scratch, 0, data, n - n % 2, n + n % 2); // copy the
		// first
		// half of
		// the data
		// into the
	 ...
	 * @return double[] the data after the FFT
	 */
	public double[] toWrapAroundOrder(final double data[]) {
		System.arraycopy(data, 0, scratch, 0, 2 * n); // save the data
		System.arraycopy(scratch, n + n % 2, data, 0, n - n % 2); // copy the
		// second
		// half of
	 ...
	 *            FORWARD or BACKWARD
	 */
	void transform_internal(final double data[], final int i0, final int stride, final int sign) {
		if (n == 1) {
