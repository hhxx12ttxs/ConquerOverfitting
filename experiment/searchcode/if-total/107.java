...
/**
 * MultiBreedingPipeline is a BreedingPipeline stores some <i>n</i> child
 * sources; each time it must produce an individual or two, it picks one of
 * these sources at random and has it do the production.
 * 
 * <p>
 * <b>Typical Number of Individuals Produced Per <tt>produce(...)</tt> call</b><br>
 * If by <i>base</i>.<tt>generate-max</tt> is <tt>true</tt>, then always the
 * maximum number of the typical numbers of any child source. If
 * <tt>false</itt>, then varies
 ...
 * <td valign=top>(Each time produce(...) is called, should the
 * MultiBreedingPipeline force all its sources to produce exactly the same
 * number of individuals as the largest typical number of individuals produced
 * by any source in the group?)</td></tr> </table>
 * 
 * <p>
 * <b>Default Base</b><br>
 * breed.multibreed
...
public class MultiBreedingPipeline extends BreedingPipeline {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        for (int x = 0; x < sources.length; x++) {
            ...
            else if (sources[x].probability < 0.0) // null checked from
                                                   // state.output.error above
                state.output.error("Pipe #" + x + " must have a probability >= 0.0", base); // convenient
                                                                                            // that
                                                                                            // NO_PROBABILITY
                                                                                            // is
                                                                                            // -1...
    ...
    {
        BreedingSource s = sources[BreedingSource.pickRandom(sources, state.random[thread].nextFloat())];
        int total;
