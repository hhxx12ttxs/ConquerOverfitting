...
public class CSharpCCGlobals {
	 ...
	 */
	static public boolean cstreeGenerated;

	/**
	 * Set to true if this file has been processed by CSCov.
	 */
	static public boolean mcccovGenerated;
	...
	static public void bannerLine(String fullName, String ver) {
		...
		System.out.println(")");
		//System.out.println("Copyright Â 2002 Sun Microsystems, Inc. All rights reserved. Use is subject to license terms.");
	}
	...
	static protected int mcc2index = 0;
   ...
   */
	public static Vector getToolNames(String fileName) {
		...
		try {
			...
			try { stream.close(); } catch (Exception e3) { }
		}
	...
	public static void setOutputDir() {
		...
		if (!tmp.exists()) {
			if (outputDirNameLoc == null)
				CSharpCCErrors.warning("Output directory \"" + outputDirName + "\" does not exist. Creating the directory.");
			else
				CSharpCCErrors.warning(outputDirNameLoc, "Output directory \"" + outputDirName + "\" does not exist. Creating the directory.");

			if (!tmp.mkdir()) {
				if (outputDirNameLoc == null)
					CSharpCCErrors.semantic_error("Cannot create the output directory : " + outputDirName);
				else
					CSharpCCErrors.semantic_error(outputDirNameLoc, "Cannot create the output directory : " + outputDirName);
		...
		if (!tmp.isDirectory()) {
			if (outputDirNameLoc == null)
				CSharpCCErrors.semantic_error("\"" + outputDirName + " is not a valid output directory.");
			else
				CSharpCCErrors.semantic_error(outputDirNameLoc, "\"" + outputDirName + " is not a valid output directory.");
		...
		if (!tmp.canWrite()) {
			if (outputDirNameLoc == null)
				CSharpCCErrors.semantic_error("Cannot write to the output output directory : \"" + outputDirName + "\"");
			else
				CSharpCCErrors.semantic_error(outputDirNameLoc, "Cannot write to the output output directory : \"" + outputDirName + "\"");
    ...
    static public String visibilityOpt() {
