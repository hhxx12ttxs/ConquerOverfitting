...
public class loan implements Runnable {
    ...
    public loan(double amt, Player play, int numLoans) {
        ...
        color.sendColouredMessage(player, "&2You will be charged &f" + DynamicEconomy.currencySymbol
                + (interest + amount) + "&2 in &f" + DynamicEconomy.paybackTime + "&2 minutes.");
        Utility.writeToLog(player.getName() + " has taken a loan for " + DynamicEconomy.currencySymbol
                + (amount + interest));
        ...
        if (args.length != 1) {
            ...
            if (hasOutstandingDebt(player.getName())) {
                color.sendColouredMessage(
                        player,
                        DynamicEconomy.prefix
                                + "&2You have an outstanding debt and therefore &fcannot &2take out any loans until it is payed.");
                return false;
            }
            ...
            try {
                ...
                color.sendColouredMessage(player, DynamicEconomy.prefix
                        + "&2Wrong Command Usage. &f/loan [Amount], Amount must be a valid quantity.");
                Utility.writeToLog(stringPlay + " incorrectly called /loan");
                return false;
            }
            ...
            if (amount > DynamicEconomy.maxLoanAmount) {
                color.sendColouredMessage(player, "&2The requested loan is too large; Max loan amount is &f"
                        + DynamicEconomy.currencySymbol + DynamicEconomy.maxLoanAmount);
                return false;
            }
            if (amount < DynamicEconomy.minLoanAmount) {
                color.sendColouredMessage(player, "&2The requested loan is too small; Min loan amount is &f"
            ...
            if (DynamicEconomy.useLoanAccount) {
                if (!DynamicEconomy.loanAccountName.equals("")) {
                    ...
                    if (amount > bal) {
                        color.sendColouredMessage(player, "&2The Bank does not have enough money to loan you &f"
                                + DynamicEconomy.currencySymbol + amount);
    ...
    public static void getLoans(Player player, String[] args) {
        ...
        if (args.length == 0) {
            ...
            for (int x = 0; x < loansSet.size(); x++) {
                ...
                int seconds = (int) (remainingTime % 60); // Find seconds
                                                          // remaining
                ...
                if (debtStatus) {
                    ...
                    color.sendColouredMessage(player, "&2Total due: &f" + DynamicEconomy.currencySymbol
                            + (interest + amount));
                }

                color.sendColouredMessage(player, "&2Due in approximately &f" + minutes + "&2 minutes and &f" + seconds
                        + "&2 seconds");
                color.sendColouredMessage(player, "&2---------&fLoan &f" + (x + 1) + "&2 of &f" + loansSet.size()
                        + "&2---------");
            }
        } else {
    ...
    private static void updateDebtStatus(String playerName) {
        ...
        if (loansFileConfig.contains("loans." + playerName) == false) return;
            ...
            if (loansSet != null) {
                ...
                for (int i = 0; i < loans.length; i++) {
                    ...
                    for (int x = 0; x < userLoansSet.size(); x++) {
                        ...
                        // indivLoanSet =
                        // loansFileConfig.getConfigurationSection(node).getKeys(false);
                                ...
                                if (player != null) {
                                    color.sendColouredMessage(player,
                                            DynamicEconomy.prefix + "&2You have been charged &f"
                                                    + DynamicEconomy.currencySymbol + decFormat.format(bal)
                                                    + "&2 for your loan.");
                                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2You have  &f"
                                            + DynamicEconomy.currencySymbol + decFormat.format(debt)
                                            + "&2 remaining to pay.");
                                    color.sendColouredMessage(player, DynamicEconomy.prefix
                ...
                /*
                 * node = "loans." + loans[i] + ".loan" + (x+1); time =
                 * loansFileConfig.getLong(node + ".time"); interest =
                 * loansFileConfig.getLong(node + ".interest"); amount =
                 * loansFileConfig.getLong(node + ".amount"); long curTime =
                 * Calendar.getInstance().getTimeInMillis();
                 * color.sendColouredMessage(player, "loan" + (x+1) + " time: "
                 * + time + " interest: " + interest + " amount: " + amount); if
                 * (curTime > time) {
                 * DynamicEconomy.economy.withdrawPlayer(loans[i], (amount +
                 * interest)); //OfflinePlayer offlinePlayer =
                 * DE.getServer().getOfflinePlayer(loans[i]); Player player =
                 * Bukkit.getServer().getPlayer(loans[i]);
                 * color.sendColouredMessage(player, DynamicEconomy.prefix +
                 * "&2You have been charged &f$" + (amount + interest) +
                 * "&2 for your loan."); color.sendColouredMessage(player,
                 * "loan" + (x+1) + " time: " + time + " interest: " + interest
                 * + " amount: " + amount); Utility.writeToLog(loans[i] +
                 * " has been charged " + (amount + interest) +
                 * " for their loan."); loansFileConfig.set(node, null); try {
                 * loansFileConfig.save(loansFile); } catch (Exception e) {
                 * e.printStackTrace(); } if
                 * (!(DynamicEconomy.loanAccountName.equals(""))) {
                 * DynamicEconomy
                 * .economy.depositPlayer(DynamicEconomy.loanAccountName,
                 * (amount + interest)); } }
                 */}
        }
