...
public class Triangulate {
	 ...
	 * circumcircle
	 */
	static boolean CircumCircle(double xp, double yp, double x1, double y1,
	...
	/*
	 * Triangulation subroutine Takes as input NV vertices in array pxyz
	 * Returned is a list of ntri triangular faces in the array v These
	 * triangles are arranged in a consistent clockwise order. The triangle
	 * array 'v' should be malloced to 3 * nv The vertex array pxyz must be big
	 * enough to hold 3 more points The vertex array must be sorted in
	 * increasing x values say
	 * 
	...
	static public int Triangulate(int nv, XYZ pxyz[], ITRIANGLE v[]) {
		 ...
		 * which have a vertex number greater than nv
		 */
	...
	public static void main(String[] args) {
		...
		for (int tt = 0; tt < points.length; tt++) {
			System.out.println("rect(" + points[tt].x + "," + points[tt].y
					+ ", 3, 3);");
		}

		for (int tt = 0; tt < ntri; tt++) {
			System.out.println("beginShape(TRIANGLES);");
			System.out.println("vertex( " + points[triangles[tt].p1].x + ","
					+ points[triangles[tt].p1].y + ");");
			System.out.println("vertex( " + points[triangles[tt].p2].x + ","
					+ points[triangles[tt].p2].y + ");");
			System.out.println("vertex( " + points[triangles[tt].p3].x + ","
					+ points[triangles[tt].p3].y + ");");
