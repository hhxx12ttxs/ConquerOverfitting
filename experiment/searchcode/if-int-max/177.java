...
/**
 * IntegerVectorIndividual is a VectorIndividual whose genome is an array of
 * ints. Gene values may range from species.mingene(x) to species.maxgene(x),
        ...
        if (s.mutationProbability > 0.0)
            for (int x = 0; x < genome.length; x++)
                if (state.random[thread].nextBoolean(s.mutationProbability))
                    genome[x] = randomValueFromClosedInterval((int) s.minGene(x), (int) s.maxGene(x),
                            state.random[thread]);
    }
