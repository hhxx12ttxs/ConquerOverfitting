 ...
 * There are two categories of parameters associated with this class. First,
 * there are parameters which guide the initial number of rules to be created
 * when a ruleset is initialized for the first time, or totally reset. Second,
 * there are parameters which indicate how rulesets are to be mutated under the
 * "default" rule mutation operator.
 * 
 * <p>
 * First the initialization parameters. You need to specify a distribution from
 * which the system will pick random integer values X. When a ruleset is to be
 * initialized, a random value X is picked from this distribution, and the
 * ruleset will be created with X initial rules. You can specify the
 * distribution in one of two ways. First, you can specify a minimum and maximum
 * number of rules; the system will then pick an X uniformly from between the
 * min and the max. Second, you can specify a full distribution of size
 * probabilities for more control. For example, to specify that the system
 * should make individuals with 0 rules 0.1 of the time, 1 rule 0.2 of the time,
 * and 2 rules 0.7 of the time, you set <i>reset-num-sizes</i> to 3 (for rule
 * sizes up to but not including 3), and then set reset-size.0 to 0.1,
 * reset-size.1 to 0.2, and reset-size.2 to 0.7.
 * 
 * <p>
 * Next the mutation parameters. The default mutation procedure works as
 * follows. First, every rule in the ruleset is mutated. It is up to the rule to
 * determine by how much or whether it will be mutated (perhaps by flipping a
 * coin) when its mutate function is called. Second, the system repeatedly flips
 * a coin with "p-del" probability of being true, until it comes up false. The
 * number of times it came up true is the number of rules to remove from the
 * ruleset; rules to be removed are chosen at random. Third, the system
 * repeatedly flips a coin with "p-add" probability of being true, until it
 * comes up false. The number of times it came up true is the number of new
 * randomly-generated rules to add to the ruleset; rules are added to the end of
 * the array. Fourth, with "p-rand-order" probability, the order of rules in the
 * ruleset array is randomized; this last item might or might not matter to you
 * depending on whether or not your rule interpreter differs depending on rule
 * order.
 * 
 ...
 *          <td valign=top>(number of rule set constraints)</td>
 *          </tr>
 * 
 *          <tr>
 *          <td valign=top><i>base.n</i>.<tt>name</tt><br>
 *          <font size=-1>String</font></td>
 *          <td valign=top>(name of rule set constraint <i>n</i>)</td>
 *          </tr>
 * 
 *          <tr>
 *          <td valign=top><i>base.n</i>.<tt>reset-min-size</tt><br>
 *          <font size=-1>int >= 0 (default=0)</font></td>
 *          <td valign=top>(for rule set constraint <i>n</i>, the minimum number
 *          of rules that rulesets may contain upon initialization (resetting),
 *          see discussion above)</td>
 *          </tr>
 * 
 *          <tr>
 *          <td valign=top><i>base.n</i>.<tt>reset-max-size</tt><br>
 *          <font size=-1>int >= <i>base.n</i>.<tt>reset-min-size</tt>
 *          (default=0)</font></td>
 *          <td valign=top>(for rule set constraint <i>n</i>, the maximum number
 *          of rules that rulesets may contain upon initialization (resetting),
 *          see discussion above)</td>
 *          </tr>
 * 
 *          <tr>
 *          <td valign=top><i>base.n</i>.<tt>reset-num-sizes</tt><br>
 *          <font size=-1>int >= 0</font> (default=unset)</td>
 *          <td valign=top>(for rule set constraint <i>n</i>, the number of
 *          sizes in the size distribution for initializtion, see discussion
 *          above)</td>
 *          </tr>
 * 
 *          <tr>
 *          <td valign=top><i>base.n</i>.<tt>reset-size</tt>.<i>i</i><br>
 *          <font size=-1>0.0 <= float <= 1.0</font></td>
 *          <td valign=top>(for rule set constraint <i>n</i>, the probability
 *          that <i>i</i> will be chosen as the number of rules upon
 *          initialization, see discussion above)</td>
 *          </tr>
 * 
 *          <tr>
 *          <td valign=top><i>base.n</i>.<tt>p-add</tt><br>
 *          <font size=-1>0.0 <= float <= 1.0</font></td>
 *          <td valign=top>(the probability that a new rule will be added, see
 *          discussion)</td>
 *          </tr>
 * 
 *          <tr>
 *          <td valign=top><i>base.n</i>.<tt>p-del</tt><br>
 *          <font size=-1>0.0 <= float <= 1.0</font></td>
 *          <td valign=top>(the probability that a rule will be deleted, see
 *          discussion)</td>
 *          </tr>
 * 
 *          <tr>
 *          <td valign=top><i>base.n</i>.<tt>p-rand-order</tt><br>
 *          <font size=-1>0.0 <= float <= 1.0</font></td>
 *          <td valign=top>(the probability that the rules' order will be
 *          randomized, see discussion)</td>
 *          </tr>
 *          </table>
 */
