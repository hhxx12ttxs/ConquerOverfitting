 ...
 * length may be lengthened or shortened. There are two crossover options
 * available: one-point and two-point. One-point crossover picks a crossover
 * point for each of the vectors (the crossover point can be different), and
 * then does one-point crossover using those points. Two-point crossover picks
 * TWO crossover points for each of the vectors (again, the points can be
 * different among the vectors), and swaps the middle regions between the
 * respective crossover points.
 * 
 * <p>
 * ListCrossoverPipeline will try tries times to meet certain constraints:
 * first, the resulting children must be no smaller than min-child-size. Second,
 * the amount of material removed from a parent must be no less than
 * mix-crossover-percent and no more than max-crossover-percent.
 * 
 * <p>
 * If toss is true, then only one child is generated, else at most two are
 * generated.
 * 
 * <p>
 ...
 * toss is set, in which case it's simply the minimum typical number.
 * 
 * <p>
 ...
 * <td valign=top>(number of times to try finding valid crossover points)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>min-child-size</tt><br>
 * <font size=-1>int &gt;= 0 (default)</font></td>
 * <td valign=top>(the minimum allowed size of a child)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>min-crossover-percent</tt><br>
 * <font size=-1>0 (default) &lt;= float &lt;= 1</font></td>
 * <td valign=top>(the minimum percentage of an individual that may be removed
 * during crossover)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>max-crossover-percent</tt><br>
 * <font size=-1>0 &lt;= float &lt;= 1 (default)</font></td>
 * <td valign=top>(the maximum percentage of an individual that may be removed
 * during crossover)</td>
 * </tr>
 * 
 * </table>
...
public class ListCrossoverPipeline extends BreedingPipeline {
    ...
    public ListCrossoverPipeline() {
        parents = new VectorIndividual[2];
    }
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        minChildSize = state.parameters.getIntWithDefault(base.push(P_MIN_CHILD_SIZE), def.push(P_MIN_CHILD_SIZE), 0);

        numTries = state.parameters.getIntWithDefault(base.push(P_NUM_TRIES), def.push(P_NUM_TRIES), 1);
        ...
        String crossoverTypeString = state.parameters.getStringWithDefault(base.push(VectorSpecies.P_CROSSOVERTYPE),
                def.push(VectorSpecies.P_CROSSOVERTYPE), VectorSpecies.V_TWO_POINT);

        // determine the crossover method to use (only 1-point & 2-point
        // currently supported)
        ...
        if (maxCrossoverPercentage < 0.0 || maxCrossoverPercentage > 1.0) {
            state.output
                    .error("ListCrossoverPipeline:\n"
                            ...
                            + Float.toString(maxCrossoverPercentage)
                            + "\n"
...

    {
        ...
        if (!state.random[thread].nextBoolean(likelihood))
            return reproduce(n, start, subpopulation, inds, state, thread, true); // DO
                                                                                  // produce
                                                                                  // children
                                                                                  // from
                                                                                  // source
        ...
        for (int q = start; q < n + start; /* no increment */) // keep on going
                                                               // until we're
                                                               // filled up
        {
            ...
            if (sources[0] == sources[1]) // grab from the same source
            {
                ...
                if (!(sources[0] instanceof BreedingPipeline)) // it's a
                                                               // selection
                                                               // method
                                                               // probably
                {
            ...
            } else // grab from different sources
            {
                ...
                if (!(sources[0] instanceof BreedingPipeline)) // it's a
                                                               ...
                                                               // probably
                    parents[0] = (VectorIndividual) (parents[0].clone());
                if (!(sources[1] instanceof BreedingPipeline)) // it's a
                                                               // selection
                                                               // method
                                                               // probably
                    parents[1] = (VectorIndividual) (parents[1].clone());
            ...
            // determine min and max crossover segment lengths, in terms of
            // chunks
            for (int i = 0; i < 2; i++) {
            ...
            // attempt 'num-tries' times to produce valid children (which are
            // bigger than min-child-size)
            boolean valid_children = false;
            int attempts = 0;
            while (valid_children == false && attempts < numTries) {
                // generate split indices for one-point (tail end used as end of
                // segment)
                if (crossoverType == VectorSpecies.C_ONE_POINT) {
                    for (int i = 0; i < 2; i++) {
                        // select first index at most 'max_chunks' away from
                        // tail end of vector
                        split[i][0] = size_in_chunks[i] - max_chunks[i];
                        // shift back towards tail end with random value based
                        // on min/max parameters
                ...
                // generate split indices for two-point (both indicies have
                // randomized positions)
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        // second index must be at least 'min_chunks' after the
                        // first index
                        split[i][1] = split[i][0] + min_chunks[i];
                        // add a random value up to max crossover size, without
                        // exceeding size of the parent
                        split[i][1] += state.random[thread].nextInt(Math.min(max_chunks[i] - min_chunks[i],
                                size_in_chunks[i] - split[i][0]));
                        // convert split from chunk numbers to array indices
                        split[i][0] *= chunk_size;
                ...
                // use the split indices generated above to split the parents
                // into pieces
                parents[0].split(split[0], pieces[0]);
                parents[1].split(split[1], pieces[1]);

                // create copies of the parents, swap the middle segment, and
                // then rejoin the pieces
                // - this is done to test whether or not the resulting children
                // are of a valid size,
                // - because we are using Object references to an undetermined
                // array type, there is no way
                // to cast it to the appropriate array type (i.e. short[] or
                // double[]) to figure out the
                // length of the pieces
                // - instead, we use the join method on copies, and let each
                // vector type figure out its own
