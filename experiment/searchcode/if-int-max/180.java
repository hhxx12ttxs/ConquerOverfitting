...
public abstract class KozaBuilder extends GPNodeBuilder {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        // load maxdepth and mindepth, check that maxdepth>0, mindepth>0,
        // maxdepth>=mindepth
        maxDepth = state.parameters.getInt(base.push(P_MAXDEPTH), def.push(P_MAXDEPTH), 1);
        if (maxDepth <= 0)
            state.output.fatal("The Max Depth for a KozaBuilder must be at least 1.", base.push(P_MAXDEPTH),
                    def.push(P_MAXDEPTH));

        minDepth = state.parameters.getInt(base.push(P_MINDEPTH), def.push(P_MINDEPTH), 1);
        if (minDepth <= 0)
            state.output.fatal("The Min Depth for a KozaBuilder must be at least 1.", base.push(P_MINDEPTH),
                    def.push(P_MINDEPTH));

        if (maxDepth < minDepth)
            state.output.fatal("Max Depth must be >= Min Depth for a KozaBuilder", base.push(P_MAXDEPTH),
                    def.push(P_MAXDEPTH));
    }

    /**
     * A private recursive method which builds a FULL-style tree for
     * newRootedTree(...)
     */
    protected GPNode fullNode(final EvolutionState state, final int current, final int max, final GPType type,
            final int thread, final GPNodeParent parent, final int argposition, final GPFunctionSet set) {
        // fullNode can mess up if there are no available terminals for a given
        // type. If this occurs,
        // and we find ourselves unable to pick a terminal when we want to do
        // so, we will issue a warning,
        // and pick a nonterminal, violating the "FULL" contract. This can lead
        ...
        boolean triedTerminals = false; // did we try -- and fail -- to fetch a
                                        // terminal?
        ...
        // pick a terminal when we're at max depth or if there are NO
        // nonterminals
        ...
        // else force a nonterminal unless we have no choice
            ...
            if (nodesToPick == null || nodesToPick.length == 0) // no
                                                                // nonterminals,
                                                                // hope the guy
                                                                // knows what
                                                                // he's doing!
                nodesToPick = set.terminals[type.type]; // this can only happen
                                                        // with the warning
                                                        // about nonterminals
                                                        // above
    ...
    /**
     * A private function which recursively returns a GROW tree to
     * newRootedTree(...)
     */
    protected GPNode growNode(final EvolutionState state, final int current, final int max, final GPType type,
            final int thread, final GPNodeParent parent, final int argposition, final GPFunctionSet set) {
        // growNode can mess up if there are no available terminals for a given
        // type. If this occurs,
        // and we find ourselves unable to pick a terminal when we want to do
        // so, we will issue a warning,
        // and pick a nonterminal, violating the maximum-depth contract. This
        // can lead to pathological situations
        // where the system will continue to go on and on unable to stop because
        // it can't pick a terminal,
        // resulting in running out of memory or some such. But there are cases
        // where we'd want to let
