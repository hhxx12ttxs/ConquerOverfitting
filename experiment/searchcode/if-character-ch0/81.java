...
//
// Contributions by Carlos Gรณmez of Asturias, Spain, in the area of unary operators
 ...
 *     '^'       80 / 81     NO_SIDE        Power
 *     'ฑ'       60 / 60     RIGHT_SIDE     Unary negation (internal, substituted for '-')
 *     '*'       40 / 40     NO_SIDE        Multiple (conventional computer notation)
 *     'ื'       40 / 40     NO_SIDE        Multiple (because it's a Unicode world out there)
 *     'ท'       40 / 40     NO_SIDE        Multiple (because it's a Unicode world out there)
 *     '('       40 / 40     NO_SIDE        Multiply (implicit due to brackets, e.g "(a)(b)")
 *     '/'       40 / 40     NO_SIDE        Divide (conventional computer notation)
 *     '๗'       40 / 40     NO_SIDE        Divide (because it's a Unicode world out there)
    ...
    for(ofs=beg; (ofs=skipWhitespace(expression,ofs,end))<=end; ofs++) {
        ...
        EvaluateOperand: {
            ...
            if(cur.unary!=LEFT_SIDE) {
                ...
                if(ch0=='-') { nxt=getOperator('ฑ'); }                                              // unary '-': right-binding, high precedence operation (different from subtract)
                }
...
// *************************************************************************************************
    ...
    public double evaluateOperator(double lft, char opr, double rgt) {
        switch(opr) {
            ...
            case 'ฑ' : return -rgt;                                                                 // unary negation
            case '*' : return lft*rgt;                                                              // multiply (classical)
            case 'ื' : return lft*rgt;                                                              // multiply (because it's a Unicode world out there)
            case 'ท' : return lft*rgt;                                                              // multiply (because it's a Unicode world out there)
            case '(' : return lft*rgt;                                                              // multiply implicitly due to brackets (e.g (a)(b)
            case '/' : return lft/rgt;                                                              // divide (classical computing)
            case '๗' : return lft/rgt;                                                              // divide (because it's a Unicode world out there)
...
//              if(fncnam.equalsIgnoreCase("nextUp"        )) { return Math.nextUp     (fncargs.next());                } needs Java 6
    ...
    static private final Operator       OPR_NEG =new Operator('ฑ',60,60,RIGHT_SIDE,true ,DefaultImpl.INSTANCE); // unary negation
    static private final Operator       OPR_MLT1=new Operator('*',40                    ,DefaultImpl.INSTANCE); // multiply (classical)
    static private final Operator       OPR_MLT2=new Operator('ื',40                    ,DefaultImpl.INSTANCE); // multiply (because it's a Unicode world out there)
    static private final Operator       OPR_MLT3=new Operator('ท',40                    ,DefaultImpl.INSTANCE); // multiply (because it's a Unicode world out there)
    ...
    static private final Operator       OPR_DIV2=new Operator('๗',40                    ,DefaultImpl.INSTANCE); // divide (because it's a Unicode world out there)
