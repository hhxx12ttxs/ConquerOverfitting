...
package terris.movingworld.renders.override;
...
import terris.movingworld.OLD.entities.EntityShip;
import net.minecraft.src.* ;


public class SubRenderGlobal_OLD extends RenderGlobal
    implements IWorldAccess
{
    ...
    public SubRenderGlobal_OLD(Minecraft minecraft, RenderEngine renderengine)
    {
...
/*        glSkyList = 1; 											//TODO check this					
    ...
    public void renderEntities(Vec3D vec3d, ICamera icamera, float f)
    {
        ...
        EntityLiving renderViewEntity = mc.renderViewEntity;
        RenderManager.renderPosX = ((Entity) (renderViewEntity)).lastTickPosX + (((Entity) (renderViewEntity)).posX - ((Entity) (renderViewEntity)).lastTickPosX) * (double)f;
        RenderManager.renderPosY = ((Entity) (renderViewEntity)).lastTickPosY + (((Entity) (renderViewEntity)).posY - ((Entity) (renderViewEntity)).lastTickPosY) * (double)f;
        RenderManager.renderPosZ = ((Entity) (renderViewEntity)).lastTickPosZ + (((Entity) (renderViewEntity)).posZ - ((Entity) (renderViewEntity)).lastTickPosZ) * (double)f;
        TileEntityRenderer.staticPlayerX = ((Entity) (renderViewEntity)).lastTickPosX + (((Entity) (renderViewEntity)).posX - ((Entity) (renderViewEntity)).lastTickPosX) * (double)f;
        TileEntityRenderer.staticPlayerY = ((Entity) (renderViewEntity)).lastTickPosY + (((Entity) (renderViewEntity)).posY - ((Entity) (renderViewEntity)).lastTickPosY) * (double)f;
        TileEntityRenderer.staticPlayerZ = ((Entity) (renderViewEntity)).lastTickPosZ + (((Entity) (renderViewEntity)).posZ - ((Entity) (renderViewEntity)).lastTickPosZ) * (double)f;
    ...
    public void loadRenderers()
    {
        ...
        for(int j1 = 0; j1 < renderChunksWide; j1++)
        {
            for(int k1 = 0; k1 < renderChunksTall; k1++)
            {
                ...
                {
                	int location = (l1 * renderChunksTall + k1) * renderChunksWide + j1 ; 
                	
                    worldRenderers[location] = new WorldRenderer(worldObj, tileEntities, j1 * 16, k1 * 16, l1 * 16, 16, glRenderListBase + k);
                    if(occlusionEnabled)
                    {
                        worldRenderers[location].glOcclusionQuery = glOcclusionQueryBase.get(l);
                    }
                    worldRenderers[location].isWaitingOnOcclusionQuery = false;
                    worldRenderers[location].isVisible = true;
                    worldRenderers[location].isInFrustum = true;
                    worldRenderers[location].chunkIndex = l++;
                    worldRenderers[location].markDirty();
                    sortedWorldRenderers[location] = worldRenderers[location];
                    worldRenderersToUpdate.add(worldRenderers[location]);
                    k += 3;
                }
    ...
    public boolean func_27307_a(double d, double d1, double d2, float f) { return false; }
