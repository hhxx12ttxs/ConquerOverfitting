...
public class NfaState {
	...
	void AddChar(char c) {
		...
		if (!unicodeWarningGiven && c > 0xff &&
			!Options.B("CSHARP_UNICODE_ESCAPE") &&
	...
	void MergeMoves(NfaState other) {
		...
		if (asciiMoves == other.asciiMoves) {
			CSharpCCErrors.semantic_error("Bug in CSharpCC : Please send " +
	...
	public static boolean CanStartNfaUsingAscii(char c) {
		if (c >= 128)
			throw new Error("CSharpCC Bug: Please send mail to sankar@cs.stanford.edu");
	...
	void GenerateNonAsciiMoves(java.io.PrintWriter ostr) {
		...
		for (i = 0; i <= 255; i++) {
			...
			if (common != null) {
				...
				// original JavaCC code:
				//tmp = "{\n   0x" + Long.toHexString(common[0]) + "L, " +
				//	  "0x" + Long.toHexString(common[1]) + "L, " +
				//	  "0x" + Long.toHexString(common[2]) + "L, " +
				//	  "0x" + Long.toHexString(common[3]) + "L\n};";
				// modified CSharpCC code:
				tmp = "{\n   " + Long.toString(common[0]) + ", " +
				Long.toString(common[1]) + "L, " +
				Long.toString(common[2]) + "L, " +
				Long.toString(common[3]) + "L\n};";

				if ((ind = (Integer)lohiByteTab.get(tmp)) == null) {
					allBitVectors.addElement(tmp);

					if (!AllBitsSet(tmp))
						ostr.println("static readonly long[] mccbitVec" + lohiByteCnt + " = " + tmp);
				...
				// modified CSharpCC code:
				tmp = "{\n   " + Long.toString(loBytes[i][0]) + "L, " +
				Long.toString(loBytes[i][1]) + "L, " +
				Long.toString(loBytes[i][2]) + "L, " +
				Long.toString(loBytes[i][3]) + "L\n};";
				if ((ind = (Integer)lohiByteTab.get(tmp)) == null) {
					allBitVectors.addElement(tmp);
	...
	private static int AddCompositeStateSet(String stateSetString, boolean starts) {
		...
		if (nameSet == null)
			throw new Error("CSharpCC Bug: Please send mail to sankar@cs.stanford.edu; nameSet null for : " + stateSetString);
	...
	public static void DumpStateSets(java.io.PrintWriter ostr) {
		...
		ostr.print("static readonly int[] mccnextStates = {");
	...
	private boolean FindCommonBlocks() {
		...
		for (i = 0; i < nameSet.length; i++) {
			if (live[i]) {
				if (((NfaState)indexedAllStates.elementAt(nameSet[i])).
					isComposite) {
					return false;
				}
	...
	private static void FixStateSets() {
		...
		for (i = 0; i < allStates.size(); i++) {
			...
			if ((newSet = (int[])fixedSets.get(tmpState.next.
	epsilonMovesString)) != null) {
	...
	private static void DumpHeadForCase(java.io.PrintWriter ostr, int byteNum) {
		if (byteNum == 0) {
			...
			ostr.println("         long l = 1L << (curChar & 63);");
		} else {
			if (Options.B("CSHARP_UNICODE_ESCAPE") || unicodeWarningGiven) {
			...
			ostr.println("         long l2 = 1L << (curChar & 63);");
		}

		ostr.println("         do");
		ostr.println("         {");

		ostr.println("            switch(mccstateSet[--i])");
		ostr.println("            {");
	}
	...
	private String PrintNoBreak(java.io.PrintWriter ostr, int byteNum,
				boolean[] dumped) {
		if (inNextOf != 1) {
			throw new Error(
				"CSharpCC Bug: Please send mail to sankar@cs.stanford.edu");
		}
	...
	private static void DumpCompositeStatesAsciiMoves(java.io.PrintWriter ostr,
		...
		if (neededStates == 1) {
			...
			if (!dumped[toBePrinted.stateName] && !stateBlock &&
			toBePrinted.inNextOf > 1) {
	...
	private void DumpAsciiMoveForCompositeState(java.io.PrintWriter ostr,
						int byteNum, boolean elseNeeded) {
		...
		if (asciiMoves[byteNum] != 0xffffffffffffffffL) {
			...
			if (oneBit != -1) {
				...
				// original JavaCC code:
				//ostr.println("                  " + (elseNeeded ? "else " : "") +
				//		 "if ((0x" + Long.toHexString(asciiMoves[byteNum]) +
				//		 "L & l) != 0L)");
				// modified CSharpCC code:
				ostr.println("                  " + (elseNeeded ? "else " : "") + "if ((" + Long.toString(asciiMoves[byteNum]) +
				 " & l) != 0L)");
			}
			prefix = "   ";
		}
		...
		if (next != null && next.usefulEpsilonMoves > 0) {
			...
			if (next.usefulEpsilonMoves == 1) {
				...
				if (nextIntersects) {
					ostr.println(prefix + "                  mccCheckNAdd(" +
						 name + ");");
				} else {
					ostr.println(prefix +
						"                  mccstateSet[mccnewStateCnt++] = " +
						 name + ";");
				}
			} else if (next.usefulEpsilonMoves == 2 && nextIntersects) {
				ostr.println(prefix +
						 "                  mccCheckNAddTwoStates(" +
						 stateNames[0] + ", " + stateNames[1] + ");");
			} else {
				int[] indices = GetStateSetIndicesForUse(next.
					epsilonMovesString);
				...
				if (nextIntersects) {
					...
					ostr.println(prefix + "                  mccAddStates(" +
						 indices[0] + ", " + indices[1] + ");");
				}
			}
		}
	...
	private void DumpAsciiMove(java.io.PrintWriter ostr, int byteNum,
				   boolean dumped[]) {
		...
		for (int j = 0; j < allStates.size(); j++) {
			...
			if (onlyState &&
			(asciiMoves[byteNum] & temp1.asciiMoves[byteNum]) != 0L) {
				onlyState = false;
			}
		...
		if (asciiMoves[byteNum] != 0xffffffffffffffffL) {
			...
			kindToPrint != Integer.MAX_VALUE) {
				...
				if (oneBit != -1) {
					...
					// modified CSharpCC code:
					ostr.println("                  if ((" +
						 Long.toString(asciiMoves[byteNum]) +
						 " & l) != 0L" + kindCheck + ")");
				}
		...
		if (kindToPrint != Integer.MAX_VALUE) {
			...
			if (oneBit != -1) {
				...
				//		 "L & l) == 0L)");
				// modified CSharpCC code:
				ostr.println("                  if ((" + Long.toString(asciiMoves[byteNum]) + " & l) == 0L)");
				ostr.println("                     break;");
			}
	...
	private static void DumpAsciiMoves(java.io.PrintWriter ostr, int byteNum) {
		boolean[] dumped = new boolean[Math.max(generatedStates,
							dummyStateIndex+1)];
		Enumeration e = compositeStateTable.keys();
		...
		while (e.hasMoreElements()) {
			DumpCompositeStatesAsciiMoves(ostr, (String)e.nextElement(),
						  byteNum, dumped);
		}
	...
	private static void DumpCompositeStatesNonAsciiMoves(java.io.PrintWriter
		ostr,
	...
	private final void DumpNonAsciiMoveForCompositeState(java.io.PrintWriter
		ostr) {
		boolean nextIntersects = selfLoop();
		for (int j = 0; j < allStates.size(); j++) {
			NfaState temp1 = (NfaState)allStates.elementAt(j);
		...
		if (!Options.B("CSHARP_UNICODE_ESCAPE") && !unicodeWarningGiven) {
			if (loByteVec != null && loByteVec.size() > 1) {
				ostr.println("                  if ((mccbitVec" +
						 ((Integer)loByteVec.elementAt(1)).intValue() +
						 "[i2" +
						 "] & l2) != 0L)");
			}
		} else {
			ostr.println("                  if (mccCanMove_" + nonAsciiMethod +
				 "(hiByte, i1, i2, l1, l2))");
		}
		...
		if (next != null && next.usefulEpsilonMoves > 0) {
			...
			if (next.usefulEpsilonMoves == 1) {
				int name = stateNames[0];
				if (nextIntersects) {
					ostr.println("                     mccCheckNAdd(" + name +
						 ");");
				} else {
					ostr.println(
						"                     mccstateSet[mccnewStateCnt++] = " +
				...
				ostr.println("                     mccCheckNAddTwoStates(" +
						 stateNames[0] + ", " + stateNames[1] + ");");
			} else {
				int[] indices = GetStateSetIndicesForUse(next.
					epsilonMovesString);
				...
				if (nextIntersects) {
					ostr.println("                     mccCheckNAddStates(" +
						 indices[0] +
						 (notTwo ? (", " + indices[1]) : "") + ");");
				} else {
					ostr.println("                     mccAddStates(" +
						 indices[0] + ", " + indices[1] + ");");
				}
			}
		}
	...
	private final void DumpNonAsciiMove(java.io.PrintWriter ostr,
					boolean dumped[]) {
		boolean nextIntersects = selfLoop() && isComposite;
		...
		if (kindToPrint != Integer.MAX_VALUE) {
			...
			ostr.println("                     kind = " + kindToPrint + ";");
			prefix = "";
		} else if (!Options.B("CSHARP_UNICODE_ESCAPE") && !unicodeWarningGiven) {
			if (loByteVec != null && loByteVec.size() > 1) {
				ostr.println("                  if ((mccbitVec" +
						 ((Integer)loByteVec.elementAt(1)).intValue() +
						 "[i2" +
	...
	public static void DumpCharAndRangeMoves(java.io.PrintWriter ostr) {
		...
		while (e.hasMoreElements()) {
			DumpCompositeStatesNonAsciiMoves(ostr, (String)e.nextElement(),
							 dumped);
		}
	...
	void DumpNonAsciiMoveMethod(java.io.PrintWriter ostr) {
		int j;
		ostr.println("private static bool mccCanMove_" + nonAsciiMethod +
		...
		if (loByteVec != null && loByteVec.size() > 0) {
			for (j = 0; j < loByteVec.size(); j += 2) {
				...
				if (!AllBitsSet((String)allBitVectors.elementAt(
					...
					ostr.println("         return ((mccbitVec" +
						 ((Integer)loByteVec.elementAt(j + 1)).
						 intValue() + "[i2" +
						 "] & l2) != 0L);");
				} else {
	...
	static void PrintBoilerPlate(java.io.PrintWriter ostr) {
		ostr.println((Options.B("STATIC") ? "static " : "") + "private void " +
				 "mccCheckNAdd(int state)");
		ostr.println("{");
		ostr.println("   if (mccrounds[state] != mccround)");
		ostr.println("   {");
		ostr.println("      mccstateSet[mccnewStateCnt++] = state;");
		ostr.println("      mccrounds[state] = mccround;");
		ostr.println("   }");
		ostr.println("}");

		ostr.println((Options.B("STATIC") ? "static " : "") + "private void " +
				 "mccAddStates(int start, int end)");
		ostr.println("{");
		ostr.println("   do {");
		ostr.println(
		...
		ostr.println((Options.B("STATIC") ? "static " : "") + "private void " +
				 "mccCheckNAddTwoStates(int state1, int state2)");
		ostr.println("{");
		ostr.println("   mccCheckNAdd(state1);");
		ostr.println("   mccCheckNAdd(state2);");
		ostr.println("}");

		ostr.println((Options.B("STATIC") ? "static " : "") + "private void " +
				 "mccCheckNAddStates(int start, int end)");
		ostr.println("{");
		ostr.println("   do {");
		ostr.println("      mccCheckNAdd(mccnextStates[start]);");
		ostr.println("   } while (start++ != end);");
		ostr.println("}");

		ostr.println((Options.B("STATIC") ? "static " : "") + "private void " +
				 "mccCheckNAddStates(int start)");
		ostr.println("{");
		ostr.println("   mccCheckNAdd(mccnextStates[start]);");
		ostr.println("   mccCheckNAdd(mccnextStates[start + 1]);");
		ostr.println("}");
	}
	...
	Outer:
		for (j = 0; j < allStates.size(); j++) {
			...
			if (tmpState.stateName == -1 || tmpState.dummy ||
			...
			for (i = 0; i < nexts.length; i++) {
				...
				if (!put[state] && tmp.inNextOf > 1 && !tmp.isComposite &&
					tmp.stateForCase == null) {
					cnt++;
					nexts[i] = -1;
					put[state] = true;
	...
	public static void DumpMoveNfa(java.io.PrintWriter ostr) {
		 ...
		 */
			...
			if (kindsForStates == null) {
				...
				statesForState[LexGen.lexStateIndex] = new int[Math.max(
					generatedStates,dummyStateIndex+1)][];
			}

			kindsForStates[temp.stateName] = temp.lookingFor;
			statesForState[LexGen.lexStateIndex][temp.stateName] = temp.
				compositeStates;
		...
		while (e.hasMoreElements()) {
			...
			if (state >= generatedStates) {
				statesForState[LexGen.lexStateIndex][state] = (int[])
					allNextStates.get(s);
			}
		}
	 ...
	 */
		...
		ostr.println((Options.B("STATIC") ? "static " : "") + "private int " +
				 "mccMoveNfa" + LexGen.lexStateSuffix +
		...
		if (LexGen.mixed[LexGen.lexStateIndex]) {
			ostr.println("   int strKind = mccmatchedKind;");
			ostr.println("   int strPos = mccmatchedPos;");
			ostr.println("   int seenUpto = curPos + 1;");
			ostr.println("   input_stream.Backup(seenUpto);");
		...
		ostr.println("   mccnewStateCnt = " + generatedStates + ";");
		ostr.println("   int i = 1;");
		ostr.println("   mccstateSet[0] = startState;");

		if (Options.B("DEBUG_TOKEN_MANAGER")) {
			ostr.println(
				"      debugStream.WriteLine(\"   Starting NFA to match one of : \" + " +
				"mccKindsForStateVector(curLexState, mccstateSet, 0, 1));");
		}

		if (Options.B("DEBUG_TOKEN_MANAGER")) {
			ostr.println("      debugStream.WriteLine(" +
				 (LexGen.maxLexStates > 1 ?
		...
		// original JavaCC code:
		//ostr.println("   int j, kind = 0x" +
		//		 Integer.toHexString(Integer.MAX_VALUE) + ";");
		// modified CSharpCC code:
		ostr.println("   int j, kind = Int32.MaxValue;");
		ostr.println("   for (;;)");
		ostr.println("   {");
		// original JavaCC code:
		// ostr.println("      if (++mccround == 0x" +
		//		 Integer.toHexString(Integer.MAX_VALUE) + ")");
		// modified CSharpCC code:
		ostr.println("      if (++mccround == Int32.MaxValue)");
		...
		// original JavaCC code:
		//ostr.println("      if (kind != 0x" +
		//		 Integer.toHexString(Integer.MAX_VALUE) + ")");
		// modified CSharpCC code:
		ostr.println("      if (kind != Int32.MaxValue)");
		ostr.println("      {");
		ostr.println("         mccmatchedKind = kind;");
		ostr.println("         mccmatchedPos = curPos;");
		// original JavaCC code:
		//ostr.println("         kind = 0x" +
		//		 Integer.toHexString(Integer.MAX_VALUE) + ";");
		// modified CSharpCC code:
		ostr.println("         kind = Int32.MaxValue;");
		ostr.println("      }");
		ostr.println("      ++curPos;");
		...
		if (Options.B("DEBUG_TOKEN_MANAGER")) {
			// original JavaCC code:
			// ostr.println(
			//	"      if (mccmatchedKind != 0 && mccmatchedKind != 0x" +
			//	Integer.toHexString(Integer.MAX_VALUE) + ")");
			// modified CSharpCC code:
			ostr.println("      if (mccmatchedKind != 0 && mccmatchedKind != Int32.MxValue)");
			ostr.println("         debugStream.WriteLine(\"   Currently matched the first \" + (mccmatchedPos + 1) + \" characters as a \" + tokenImage[mccmatchedKind] + \" token.\");");
		}

		ostr.println("      if ((i = mccnewStateCnt) == (startsAt = " +
				 generatedStates + " - (mccnewStateCnt = startsAt)))");
		if (LexGen.mixed[LexGen.lexStateIndex]) {
			ostr.println("         break;");
		} else {
			ostr.println("         return curPos;");
		...
		if (Options.B("DEBUG_TOKEN_MANAGER")) {
			ostr.println(
				"      debugStream.WriteLine(\"   Possible kinds of longer matches : \" + " +
				"mccKindsForStateVector(curLexState, mccstateSet, startsAt, i));");
		}

		ostr.println("      try { curChar = input_stream.ReadChar(); }");

		if (LexGen.mixed[LexGen.lexStateIndex]) {
			ostr.println("      catch(System.IO.IOException) { break; }");
		} else {
			ostr.println(
		...
		if (LexGen.mixed[LexGen.lexStateIndex]) {
			ostr.println("   if (mccmatchedPos > strPos)");
			ostr.println("      return curPos;");
			ostr.println("");
			...
			ostr.println("         try { curChar = input_stream.ReadChar(); }");
			ostr.println("         catch(System.IO.IOException) { throw new Exception(\"Internal Error : Please send a bug report.\"); }");
			ostr.println("");
			ostr.println("   if (mccmatchedPos < strPos)");
			ostr.println("   {");
			ostr.println("      mccmatchedKind = strKind;");
			ostr.println("      mccmatchedPos = strPos;");
			ostr.println("   }");
			ostr.println(
	...
	public static void DumpStatesForState(java.io.PrintWriter ostr) {
		ostr.print("protected static readonly int[][][] statesForState = ");
	...
	public static void DumpStatesForKind(java.io.PrintWriter ostr) {
		...
		ostr.print("protected static readonly int[][] kindForState = ");
