...
//        oldColor = 0;
    ...
    public void init(Tools tools1, paintchat.M.Info info1, Res res1, Res res2)
    {
        info = info1;
        mg = info1.m;
    ...
    public void paint2(Graphics g)
    {
        try
        {
            if(cls == null)
            {
                ...
                {
                    cls[i] = new Color(config.getP("color_" + (i + 2), i < clsDef.length ? clsDef[i] : 0xffffff));
                    cls[i + 1] = new Color(config.getP("color_" + (i + 1), i < clsDef.length ? clsDef[i] : 0xffffff));
                }
...
//        int _tmp = dimension.height - j;
    ...
    {
        int j = mg.isText() ? 255 : info.getPenMask()[mg.iPenM].length;
        int k = mg.iSize;
