	...
	public static void showBalanceSheet (Composite grandParent, String endDate, String tbType)
	{
		...
		try {
			...
			for(int rowcounter =grpcode2; rowcounter < grpcode9; rowcounter ++)
			{
				...
				Integer rows1 = 0;
				conventionalbalancesheet cnblsht10 = new conventionalbalancesheet("", "", "", "", "", "");
				
				if(convbaldata_asset.size()>convbaldata_liabilities.size())
				{
					rows1=convbaldata_asset.size()-convbaldata_liabilities.size();
					for (int i = 0; i < rows1; i++) {
						convbaldata_liabilities.add(convbaldata_liabilities.size()-2,cnblsht10);
					}
				}
				if(convbaldata_liabilities.size()>convbaldata_asset.size())
				{
					rows1 = convbaldata_liabilities.size()-convbaldata_asset.size();
					for (int i = 0; i < rows1; i++) {
						convbaldata_asset.add(convbaldata_liabilities.size()-2, cnblsht10);
					}
				}	
			...
			try {
				...
				Object[] result = (Object[]) globals.client.execute("reports.getProfitLoss" , serverParams);
					...
					pandlData.add(pnlr); 	
						...
						ArrayList<String[]> Bys = new ArrayList<String[]>();
						
						for (int i = 0; i < pandlData.size(); i++) {
							ProfitAndLossReport p = pandlData.get(i);
							if(p.getaccountname()!="")
							{
								if(p.getaccountname().trim().equalsIgnoreCase("Total Of Amounts"))
								{
									if(Tos.size()<Bys.size())
									{
										for (int j = Tos.size(); j < Bys.size() ; j++) {
											String[] row = new String[]{"","",""};
											Tos.add(row);
										}
									}
									String[] row = new String[]{p.getto(),p.getaccountname(),p.getamount()};
									Tos.add(row);
								}
								else
								{
									if(!p.getamount().trim().equals("0.00"))
									{
										String[] row = new String[]{p.getto(),p.getaccountname(),p.getamount()};
										Tos.add(row);
									}
							...
							if(p.getaccountname1()!="")
							{
								if(p.getaccountname1().trim().equalsIgnoreCase("Total Of Amounts"))
								{
									if(Tos.size()>Bys.size())
									{
										for (int j = Bys.size(); j < Tos.size()-1 ; j++) {
											String[] row = new String[]{"","",""};
											Bys.add(row);
										}
									}
									String[] row = new String[]{p.getby(),p.getaccountname1(),p.getamount1()};
									Bys.add(row);

								}
								else
								{
									if (!p.getamount1().trim().equals("0.00")) {
										String[] row = new String[] {
												p.getby(), p.getaccountname1(),
												p.getamount1() };
										Bys.add(row);
									}
								}
						...
						for (int i = 0; i < count; i++) 
						{
							...
							String[] ByRow = new String[3];
							
								ByRow = Bys.get(i);
							
							ProfitAndLossReport pnlr2 = new ProfitAndLossReport(ToRow[0],ToRow[1],ToRow[2],ByRow[0],ByRow[1],ByRow[2]);
							pandlData.add(pnlr2);
						}
							/*if (ToRow[1].trim().equals("Total Of Amounts"))
							{
								toindex=i;
										
							}
							if (ByRow[1].trim().equals("Total Of Amounts"))
							{
								 byindex= i;
							}
							if (ToRow[1] !="" && ToRow[2].trim().equals(""))
							{
								toindex2 = i;
							}
							if (ByRow[1] !="" && ByRow[2].trim().equals(""))
							{
								byindex2 = i;
							}
							
							if (toindex2>0 && byindex2>0)
							{
								String[] To_Row = new String[3];
								String[] By_Row = new String[3];
								To_Row=Tos.get(toindex2);
								By_Row =Bys.get(byindex2);
								ProfitAndLossReport pnlr2 = new ProfitAndLossReport(To_Row[0],To_Row[1],To_Row[2],By_Row[0],By_Row[1],By_Row[2]);
								pandlData.add(pnlr2);
							...
							if (toindex>0 && byindex>0)
									{
								System.out.println("1");
									String[] To_Row = new String[3];
									String[] By_Row = new String[3];
									To_Row=Tos.get(toindex);
									By_Row =Bys.get(byindex);
									ProfitAndLossReport pnlr2 = new ProfitAndLossReport(To_Row[0],To_Row[1],To_Row[2],By_Row[0],By_Row[1],By_Row[2]);
									pandlData.add(pnlr2);
							...
							{System.out.println("2");
								ProfitAndLossReport pnlr2 = new ProfitAndLossReport("","","",ByRow[0],ByRow[1],ByRow[2]);
								pandlData.add(pnlr2);
							}
							if (toindex==0 && byindex>0)
							{	System.out.println("3");
								ProfitAndLossReport pnlr2 = new ProfitAndLossReport(ToRow[0],ToRow[1],ToRow[2],"","","");
								pandlData.add(pnlr2);
							}
							if (toindex==0 && byindex==0)
							{	System.out.println("4");
								
								if ((!ToRow[1].trim().equals("Total Of Amounts")) &&(!ByRow[1].trim().equals("Total Of Amounts") )) 
								{
									ProfitAndLossReport pnlr2 = new ProfitAndLossReport(ToRow[0],ToRow[1],ToRow[2],ByRow[0],ByRow[1],ByRow[2]);
									pandlData.add(pnlr2);	
								}
								...
								/*if (ByRow[1].trim().equals("Total Of Amounts"))
								{
									System.out.println("YAHA GAYAAAAAAAAAAAAAAAA2");
									ProfitAndLossReport pnlr2 = new ProfitAndLossReport(to_row[0],to_row[1],to_row[2],ByRow[0],ByRow[1],ByRow[2]);
									pandlData.add(pnlr2);
									continue;
								}*/
								/*else
								{
									System.out.println("YAHA GAYAAAAAAAAAAAAAAAA3333");
									ProfitAndLossReport pnlr2 = new ProfitAndLossReport("","","",ByRow[0],ByRow[1],ByRow[2]);
									pandlData.add(pnlr2);
									continue;
								}
							}
							else if (ByRow[1].trim().equals("Total Of Amounts")) 
							{
								System.out.println("YAHA GAYAAAAAAAAAAAAAAAA44444");
								if (!ToRow[1].trim().equals("Total Of Amounts"))
								{
									System.out.println("YAHA GAYAAAAAAAAAAAAAAAA55555");
									ProfitAndLossReport pnlr2 = new ProfitAndLossReport(ToRow[0],ToRow[1],ToRow[2],"","","");
									pandlData.add(pnlr2);
									continue;
								}
							...
							{
								System.out.println("YAHA GAYAAAAAAAAAAAAAAAA6666666");
								ProfitAndLossReport pnlr2 = new ProfitAndLossReport(ToRow[0],ToRow[1],ToRow[2],ByRow[0],ByRow[1],ByRow[2]);
								pandlData.add(pnlr2);
						...
						/*for (int i = count - 4; i < count; i++) {
							...
							String[] ToRow;
							if (Tos.size()>Bys.size()) {
								diff = Tos.size() - Bys.size();
							}
							ToRow = Tos.get(i);
							String[] ByRow = Bys.get(i);
							ProfitAndLossReport pnlr2 = new ProfitAndLossReport(ToRow[0],ToRow[1],ToRow[2],ByRow[0],ByRow[1],ByRow[2]);
							pandlData.add(pnlr2);
						}*/
