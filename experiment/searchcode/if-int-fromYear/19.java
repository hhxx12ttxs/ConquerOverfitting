...
public class TransactionHistoryManagedBean
{
    ...
    public TransactionHistoryManagedBean()
    {
        ...
        //Set the from date to 01/01/1990
    ...
    {
        transactions = transactionManager.getTransactionsForAccountBetweenDates(accountId, fromDate, toDate);
        return transactions;
    }
    ...
    public String changePeriod()
    {
        ...
        //If the from date is after the to date, flag as invalid
        if(newFromDate.after(newToDate))
        {
            validRange = false;
            ManagedBeanUtil.displayMessage(new FacesMessage("Invalid 'From Date' Specified", "('From Date' must occur before the specified 'To Date')"));
        }
        //If the to date is before the from date, flag as invalid
        else if(newToDate.before(newFromDate))
        {
            validRange = false;
            ManagedBeanUtil.displayMessage(new FacesMessage("Invalid 'To Date' Specified", "('To Date' must occur after the specified 'From Date')"));
        }
        else
        {
    ...
    public ArrayList<SelectItem> getFromYears()
    {
        ...
        //Add eqch year from 1990 to the current year
