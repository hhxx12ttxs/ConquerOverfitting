...
public class ProjectHelper {
	 ...
	 * @return
	 */
	public static MidiEvent filterFirstMetaMessage(Track track, int type)
	...
	public static Vector<NoteEvent> allNotes(int trackIndex) {
		...
		MidiEvent e; MidiMessage m; ShortMessage sm;
		
		// Iterate over all events in the track
		for( int i = 0; i < track.size(); i++){
			
			e = track.get(i);
			m = e.getMessage();

			// Check that m is a ShortMessage
			if( m instanceof ShortMessage ){
				
				sm = (ShortMessage) m;
				if( sm.getData1() < 0 || sm.getData1() > 127 )
					// If note value not in [0..127], go to next iteration
					continue;
				else if( sm.getCommand() == ShortMessage.NOTE_ON && sm.getData2() > 0 ){
					// If NOTE_ON, store the index into the memory
					memory[ sm.getData1() ] = e;
				} else if (sm.getCommand() == ShortMessage.NOTE_OFF || sm.getCommand() == ShortMessage.NOTE_ON && sm.getData2() == 0){
					// If NOTE_OFF, lookup the corresponding NOTE_ON (if any), create a new 
					// NoteEvent object and put it into the list
	...
	public static Sequence cloneSequence(Sequence s) {
		...
		try {
			...
			for(Track t : tracks) {
				...
				//System.out.println("Copied " + t + " to " + t2);
				tc++;
			}
			
			//System.out.println("Tracks copied: " + tc);
			return s2;
    ...
    public static Vector<MidiEvent> allMidiEvents(Track t1, Track t2){
        ...
        if(t1.size() == 0)
            return allMidiEvents(t2);
        else if(t2.size() == 0)
            return allMidiEvents(t1);
        else {
            ...
            for( int imax = t1.size(), jmax = t2.size(), i = 0, j = 0;;){
                //System.out.printf("i:%d/%d j:%d/%d t1:%d t2:%d\n",i,imax,j,jmax,e1.getTick(),e2.getTick());
                if(e1.getTick() < e2.getTick()){
                    ...
                    else e1 = new MidiEvent(new ShortMessage(), Long.MAX_VALUE -1); // should be enough for anyone!
                    ...
                    if(i<imax)
                        ...
                        e2 = new MidiEvent(new ShortMessage(), Long.MAX_VALUE -1); // should be enough for anyone!
                    ...
                    if(j<jmax){
                        ...
                        else e2 = new MidiEvent(new ShortMessage(), Long.MAX_VALUE -1); // should be enough for anyone!
                    }
                }
                if( j >= jmax && i>=imax)
                    return events;
    ...
    public static String prettyPrint(MidiEvent e) {
        ...
        if (m instanceof ShortMessage) {
            ...
            s += "(SY) No. " + sm.getStatus() + " (data=" +
                Arrays.toString(sm.getData()) + ")";
    ...
    private static String prettyPrint(MetaMessage mm) {
        ...
        switch (mm.getType()) {
            ...
            case MetaMessageType.END_OF_TRACK:
                return "End of track";
            default:
                return "No. " + String.valueOf(mm.getType()) + 
                    " (data=" + Arrays.toString(mm.getData()) + ")";
        }
    }
    ...
    private static String prettyPrint(ShortMessage sm) {
        switch(sm.getCommand()) {
            ...
            default:
                return "Cmd: " + sm.getCommand() + " (ch=" + sm.getChannel() + ", data1=" 
                    + sm.getData1() + ", data2=" + sm.getData2()
                    + ")";
        }
