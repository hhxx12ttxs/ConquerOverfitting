...
public class Optimizer implements Comparator<double[]>{
	...
	// these variables don't change during the entire program
	 ...
	 // these variables will contain all state parameters
	    ...
	    // allow printing of all states to a file
	    private static Hashtable<String,ArrayList<int[]>> _htToPrint = new Hashtable<String,ArrayList<int[]>>();
	...
	public int[] runEvo(int iBudget, int iYear, int[] viCurLvls){
		...
		String sKey              = iBudget + "," + iYear;
		ArrayList<int[]> alValue = new ArrayList<int[]>();
		
		if (dBudget < dCurCost){
			System.out.println("Budget (" + dBudget + ") < dCurCost: " + dCurCost + "  ==> error ");
			Arrays.fill(viOptProtProf, -1);
			for (int i=0;i<_iPop; i++) alValue.add(viOptProtProf);
			...
			alValue = (ArrayList<int[]>) singletonEOA.getPopulation().clone();
			_3DPrvPop[iYear] = (int[][])singletonEOA.getPopulation().toArray(new int[_iPop][_iNrOfMGs]);
		}
		_htToPrint.put(sKey, (ArrayList<int[]>)(alValue));
	...
	private void fileOutput(int iMax){
		...
		try{
			...
			for (int iR = 0; iR < iMax; iR++ ){
				...
				for (int iS = 0; iS < _iNrOfYears; iS++){
					String sKey = iR + "," + iS;
					ArrayList<int[]> alValue = _htToPrint.get(sKey);
					for (int i=0;i<alValue.size();i++){
						int []     viOptProtProf = alValue.get(i);
