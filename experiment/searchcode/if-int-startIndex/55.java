...
public abstract class AbstractDataBaseHandler {
  ...
  abstract protected void createSuperLogTable(String pubName) throws
      SQLException, RepException;

  abstract protected void createRepTable(String pubName) throws
      SQLException, RepException;
  ...
  abstract public AbstractColumnObject getColumnObject(TypeInfo typeInfo) throws
      RepException;
  ...
  abstract public void createShadowTable(String pubsubName, String tableName,
                                         String allColSequence) throws
      RepException;
  ...
  abstract public void createShadowTableTriggers(String pubName,
                                                 ...
                                                 String[] primCols) throws
      RepException;

  abstract public boolean isPrimaryKeyException(SQLException ex) throws
      SQLException;
...
//        MetaDataInfo mdi = new MetaDataInfo(connection);
 >>>>>>> 1.67.6.4
         ...
         try {
           ...
           ArrayList alterTabelStatements = mdi.getForiegnKeyConstraints(sname.
              getSchemaName(), sname.getTableName());
...
//          ex.printStackTrace();
     ...
     for (int i = 0; i < pubTableQueries.length; i++) {
       if(((RepTable)repTables.get(i)).getCreateShadowTable().equalsIgnoreCase(RepConstants.NO))
         continue ;
       ...
       String allColSequence = getShadowTableColumnDataTypeSequence(colInfoList,pcols);
       createShadowTable(subName, sname.toString(), allColSequence);
   ...
   * @return String
   */
  public String getShadowTableColumnDataTypeSequence(ArrayList nameTypeList,
      String[] primCols) {
    StringBuffer toRet = new StringBuffer();
    toRet.append(getColumnDataTypeSequence(nameTypeList))
        .append(getPrimaryColumnDataTypeSequence(primCols));
    return toRet.toString();
  }

  private String getPrimaryColumnDataTypeSequence(String[] primCols) {
    StringBuffer toRet = new StringBuffer();
    for (int i = 0, size = primCols.length; i < size; i++) {
      toRet.append(" , rep_old_").append(primCols[i]).append(" ")
          .append(ColumnsInfo.getDataTypeDeclaration(primCols[i]));
    }
    return toRet.toString();
  }

  private String getColumnDataTypeSequence(ArrayList colTypeMap) {
    StringBuffer toRet = new StringBuffer();
  ...
  public String saveRepTableData(Connection connection, String pubsubName, RepTable repTable) throws SQLException,
      RepException {
    StringBuffer sb = new StringBuffer();
...
//System.out.println("AbstractDataBaseHandler.saveRepTableData(connection, pubsubName, repTable) hashCode ="+repTable.hashCode() +"  cyclic dependency ="+repTable.getCyclicDependency() );
  ...
  public void setIgnoredColumns(Connection connection,String pubsubName,RepTable repTable) throws RepException,SQLException{
    ...
    String query="SELECT "+RepConstants.ignoredColumnsTable_ignoredcolumnName2+" FROM "+getIgnoredColumns_Table()+ " WHERE " +RepConstants.ignoredColumnsTable_tableId1 +" = ?";
