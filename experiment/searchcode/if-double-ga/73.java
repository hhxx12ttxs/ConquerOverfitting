 ...
 * </pre>
 * <p/> In the above example changing the text color of the attributes set will affect all annotations refering it.
 * However, changing the text color of one of those annotations will also affect all others since it will in fact change
 * the common attributes set. <p> To use an attributes object only as default values for a serie of annotations use:
 * </p>
 * <pre>
 ...
 * </pre>
 * <p/> Finer control over attributes inheritence can be achieved using default or fallback attributes set. <p> Most
 * attributes can be set to a 'use default' value which is minus one for numeric values and <code>null</code> for
 * attributes refering objects (colors, dimensions, insets..). In such a case the value of an attribute will be that of
