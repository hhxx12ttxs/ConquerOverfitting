...
import org.abrantix.rockon.rockonnggl.prefs.PrefKeys;
...
public class RockOnCubeRenderer extends RockOnRenderer implements
	...
	private void initNonGlVars(Context context, int browseCat, boolean force) {
		...
		if (mCursor == null || force) {
			...
			if (browseCat == Constants.BROWSECAT_ARTIST) {
				 ...
				 */
				Cursor helperCursor = cursorUtils
						.getArtistListFromPlaylist(Constants.PLAYLIST_ALL);
				...
				Thread helperThread = new Thread() {
					public void run() {
						if (mCursor != null && mCursor.getCount() > 0) {
							mArtistAlbumHelper = new ArtistAlbumHelper[mCursor
									.getCount()];
							cursorUtils.fillArtistAlbumHelperArray(mCursor,
									mArtistAlbumHelper);
							// trigger some update
							if (!mStopThreads)
			...
			{
				mPreferArtistSorting = PreferenceManager
						.getDefaultSharedPreferences(mContext).getBoolean(
								PrefKeys.PREFER_ARTIST_SORTING, true);
				Cursor helperCursor = cursorUtils.getAlbumListFromPlaylist(
						PreferenceManager.getDefaultSharedPreferences(mContext)
								.getInt(PrefKeys.PLAYLIST_ID,
		...
		for (int i = 0; i < mCacheSize; i++) {
			...
			n.index = -999;
			n.cover = Bitmap.createBitmap(mBitmapWidth, mBitmapHeight,
					Bitmap.Config.RGB_565);
			n.label = Bitmap.createBitmap(mLabelBitmapWidth,
					mLabelBitmapHeight, Bitmap.Config.ARGB_8888);
			mNavItem[i] = n;
		...
		for (int i = 0; i < mCacheSize; i++) {
			...
			mAlphabetNavItem[i].letterBitmap = Bitmap.createBitmap(
					mBitmapWidth, mBitmapHeight, Bitmap.Config.ARGB_8888);
		}
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		// gl.glLightfv(GL10.GL_LIGHT2, GL10.GL_AMBIENT,
		// FloatBuffer.wrap(lightAmbient));
		...
		// gl.glLightfv(GL10.GL_LIGHT2, GL10.GL_DIFFUSE,
		// FloatBuffer.wrap(lightDiffuse));
		// gl.glLightfv(GL10.GL_LIGHT3, GL10.GL_DIFFUSE,
		// FloatBuffer.wrap(lightDiffuse));
		// gl.glLightfv(GL10.GL_LIGHT4, GL10.GL_DIFFUSE,
		// FloatBuffer.wrap(lightDiffuse));
		// gl.glLightfv(GL10.GL_LIGHT5, GL10.GL_DIFFUSE,
		// FloatBuffer.wrap(lightDiffuse));
		// gl.glLightfv(GL10.GL_LIGHT6, GL10.GL_DIFFUSE,
		// FloatBuffer.wrap(lightDiffuse));
		...
		// FloatBuffer.wrap(lightPosition));
		// gl.glLightfv(GL10.GL_LIGHT3, GL10.GL_POSITION,
		// FloatBuffer.wrap(lightPosition));
		// gl.glLightfv(GL10.GL_LIGHT4, GL10.GL_POSITION,
		// FloatBuffer.wrap(lightPosition));
		// gl.glLightfv(GL10.GL_LIGHT5, GL10.GL_POSITION,
		// FloatBuffer.wrap(lightPosition));
		// gl.glLightfv(GL10.GL_LIGHT6, GL10.GL_POSITION,
		// FloatBuffer.wrap(lightPosition));
		// gl.glLightfv(GL10.GL_LIGHT7, GL10.GL_POSITION,
		// FloatBuffer.wrap(lightPosition));
		//
	...
	public void onDrawFrame(GL10 gl) {
		...
		if (mClickAnimation) {
			/* camera eye update */
			mEyeX += updateFraction * Constants.SCROLL_SPEED_SMOOTHNESS
					* (mEyeTargetX - mEyeX);
			mEyeY += updateFraction * Constants.SCROLL_SPEED_SMOOTHNESS
					* (mEyeTargetY - mEyeY);
			mEyeZ += updateFraction * Constants.SCROLL_SPEED_SMOOTHNESS
					* (mEyeTargetZ - mEyeZ);
			/* minimum movements */
			if (Math.abs(mEyeTargetX - mEyeX)
					* Constants.SCROLL_SPEED_SMOOTHNESS * updateFraction < updateFraction
					* Constants.MIN_SCROLL)
				mEyeX += Math.signum(mEyeTargetX - mEyeX)
						* Math.min(Math.abs(mEyeTargetX - mEyeX),
								updateFraction * Constants.MIN_SCROLL);
			if (Math.abs(mEyeTargetY - mEyeY)
					* Constants.SCROLL_SPEED_SMOOTHNESS * updateFraction < updateFraction
					* Constants.MIN_SCROLL)
				mEyeY += Math.signum(mEyeTargetY - mEyeY)
						* Math.min(Math.abs(mEyeTargetY - mEyeY),
								updateFraction * Constants.MIN_SCROLL);
			if (Math.abs(mEyeTargetZ - mEyeZ)
					* Constants.SCROLL_SPEED_SMOOTHNESS * updateFraction < updateFraction
					* Constants.MIN_SCROLL)
				mEyeZ += Math.signum(mEyeTargetZ - mEyeZ)
						* Math.min(Math.abs(mEyeTargetZ - mEyeZ),
								updateFraction * Constants.MIN_SCROLL);
			/* end of animation */
			// Log.i(TAG, "X: "+mEyeX+" - "+mEyeTargetX);
			// Log.i(TAG, "Y: "+mEyeY+" - "+mEyeTargetY);
			// Log.i(TAG, "Z: "+mEyeZ+" - "+mEyeTargetZ);
			if (mEyeX == mEyeTargetX && mEyeY == mEyeTargetY
					&& mEyeZ == mEyeTargetZ && !isSpinning())
				mClickAnimation = false;
		} else {
			/* move camera when scrolling cube in Y axis */
			if (mPositionY != mTargetPositionY) {
				distanceToRotationLimits = Math.max(
						Math.min(.5f * (Math.min(Math.abs(mPositionY
								- mTargetPositionY), Math.abs(mPositionY
								- mRotationInitialPositionY)) - 1), 1.5f), 0.f);

				// adjust our 'eye'
				mEyeZ = -5.f - distanceToRotationLimits;
				// adjust the fog
				gl.glFogf(GL10.GL_FOG_START,
						4.5f + .5f * distanceToRotationLimits);
				gl.glFogf(GL10.GL_FOG_END, 5.5f + distanceToRotationLimits);
			}
		}
		...
		/* Calculate rotations */
		if (mPositionX != 0)
			rotationAngleX = ((flooredPositionX % mCacheSize) + positionOffsetX) / 4.f * 360.f;
		else
			rotationAngleX = 0;

		rotationAngleY = ((flooredPositionY % mCacheSize) + positionOffsetY)
				/ (float) mCacheSize * 360.f;
		...
		/* draw label */
		if (Math.abs(mPositionY - mTargetPositionY) < .5f && mPositionX > -1.f
				&& mPositionX < 1.f) {
			gl.glLoadIdentity();
			GLU.gluLookAt(gl, mEyeX, mEyeY, mEyeZ, 0f, 0f, 0f, 0f, -1.0f, 0.0f);
			...
			gl.glRotatef(rotationAngleX, 0.f, 1.f, 0.f);
			// Y rotation
			gl.glRotatef((.5f - Math.abs(positionOffsetY - 0.5f)) * 2 * 90.f,
					1.f, 0.f, 0.f);
			tmpTextureIdx = (int) Math.round(mPositionY % mTextureId.length);
			if (tmpTextureIdx >= mTextureId.length || tmpTextureIdx < 0)
		...
		if (mPositionX == 0 && positionOffsetY == 0 && positionOffsetX == 0
				&& mPositionX % 1 == 0 && mPositionY % 1 == 0
				&& !mClickAnimation && !texturesUpdated) {
			stopRender();
		} else {
		...
		// Log.i(TAG,
		// "mEyeTargetX: "+mEyeTargetX+" mEyeTargetY: "+mEyeTargetY+" mEyeTargetZ: "+mEyeTargetZ);
	...
	public void setCurrentTargetYByProgress(float progress) {
		mTargetPositionY = Math.min(Math.round(progress * mCursor.getCount()),
				mCursor.getCount() - 1);
	...
	public void onSurfaceChanged(GL10 gl, int w, int h) {
		...
		if (mWidth > 320 && mHeight > mWidth) {
			...
			gl.glViewport((int) (.20f * (mWidth - 320) / 2), // x
					(int) (mViewportTop), // y
	...
	private boolean updateTextures(GL10 gl) {
		...
		if (mCursor != null && !mCursor.isClosed()) {
			...
			for (int i = 0; i < mCacheSize; i++) {
				cursorIndexTmp = (int) (FloatMath.floor((float) flooredPositionY
						/ (float) mCacheSize)
						* mCacheSize + i);
				...
				if (setupAlbumTextures(gl, i, cursorIndexTmp,
						mForceTextureUpdate))
					changed = true;
			}
					...
					try {
						if (lastInitial == -1) {
							...
							if (flooredPositionY > mCursor.getCount() - 1) {
								mCursor.moveToLast();
								Log.i(TAG,
										"Renderer album cursor overflow XXX FIXME");
							...
							/*
							 * Get the first meaningful initial of the current
							 * position
							 */
							lastInitial = getFirstLetterLowerCaseAbstractFromCategory(
									mBrowseCat, mCursor, oCharArrayBuffer);
						}

						// Log.i(TAG, "flooredX: "+flooredPositionX);
						// Log.i(TAG, "lastInitial: "+(char)lastInitial);
						// Log.i(TAG,
						// "flooredPositionX: "+flooredPositionX+" (int)/mCacheSize: "+(int)((flooredPositionX)/mCacheSize));
						// Log.i(TAG,
						// "flooredPositionX: "+flooredPositionX+" %mCacheSize: "+(flooredPositionX)%mCacheSize);

						charTmp = lastInitial
								+ (int) ((flooredPositionX) / mCacheSize)
								* mCacheSize + i;
						...
						if (flooredPositionX % mCacheSize < 0 && i != 0) {
							charTmp -= mCacheSize;
						}

						if (setupAlphabetTextures(gl, i, charTmp,
								mForceTextureUpdate))
							changed = true;
	...
	// Bitmap.Config.RGB_565);
	//
	private boolean setupAlbumTextures(GL10 gl, int cacheIndex, int navIndex,
			boolean force) {
		/** texture needs update? */
		if (mNavItem[cacheIndex].index != navIndex || force) {

			// Log.i(TAG,
			// "albumIndexTmp: "+albumIndexTmp+" flooredPosition: "+flooredPosition+" mPosition: "+mPosition);
			...
			if (navIndex < 0 || navIndex >= mCursor.getCount()) {
				...
				try {
					if (!mNavItem[cacheIndex].cover.isRecycled()) {
						// mNavItem[cacheIndex].cover.eraseColor(Color.argb(127,
						// 122, 122, 0));
						NavItemUtils.fillLastCover(mNavItem[cacheIndex].cover,
								navIndex < 0);
					}
					...
					if (!mNavItem[cacheIndex].label.isRecycled())
						mNavItem[cacheIndex].label.eraseColor(Color.argb(0, 0,
								0, 0));
				} catch (NullPointerException e) {
				...
				case Constants.BROWSECAT_ALBUM:
					if (!mNavItemUtils.fillAlbumInfo(mCursor,
							mNavItem[cacheIndex], navIndex)) {
						mNavItem[cacheIndex].albumName = null;
						mNavItem[cacheIndex].artistName = null;
						mNavItem[cacheIndex].albumKey = null;
					}
					if (!mNavItemUtils.fillAlbumBitmap(mNavItem[cacheIndex],
							mBitmapWidth, mBitmapHeight, mColorComponentBuffer,
							mTheme)) {
						mNavItemUtils.fillAlbumUnknownBitmap(
								mNavItem[cacheIndex], mContext, mBitmapWidth,
								mBitmapHeight,
								// mNavItem[cacheIndex].cover.getWidth(),
								// mNavItem[cacheIndex].cover.getHeight(),
								mColorComponentBuffer, mTheme);
					}
					if (!mNavItemUtils.fillAlbumLabel(mNavItem[cacheIndex],
							mLabelBitmapWidth, mLabelBitmapHeight)) {
						if (!mNavItem[cacheIndex].label.isRecycled())
				...
				case Constants.BROWSECAT_ARTIST:
					if (navIndex < mArtistAlbumHelper.length) {
						if (!mNavItemUtils.fillArtistInfo(mCursor,
								mNavItem[cacheIndex],
								mArtistAlbumHelper[navIndex], navIndex)) {
						...
						if (!mNavItemUtils.fillArtistBitmap(
								mNavItem[cacheIndex],
								mArtistAlbumHelper[navIndex], mBitmapWidth,
								mBitmapHeight, mColorComponentBuffer, mTheme)) {
							mNavItemUtils.fillAlbumUnknownBitmap(
									mNavItem[cacheIndex], mContext,
						...
						if (!mNavItemUtils.fillArtistLabel(
								mNavItem[cacheIndex], mLabelBitmapWidth,
								mLabelBitmapHeight)) {
							if (!mNavItem[cacheIndex].label.isRecycled())
								mNavItem[cacheIndex].label.eraseColor(Color
										.argb(0, 0, 0, 0));
						}
					} else {
			...
			bindTexture(gl, mNavItem[cacheIndex].label,
					mTextureLabelId[cacheIndex]);
	...
	private boolean setupAlphabetTextures(GL10 gl, int cacheIndex, int letter,
			boolean force) {
		...
		if (mAlphabetNavItem[cacheIndex].letter != letter || force) {
			...
			if (letter < 'a' - 1 || letter > 'z') // 24?????
			{
				...
				mAlphabetNavItem[cacheIndex].letterBitmap.eraseColor(Color
						.argb(127, 122, 122, 0));
			} else {
				if (!mNavItemUtils.fillAlphabetBitmap(
						mAlphabetNavItem[cacheIndex], mBitmapWidth,
						mBitmapHeight)) {
					Log.i(TAG, " + letter failed to create bitmap: "
							+ (char) letter);
					if (!mAlphabetNavItem[cacheIndex].letterBitmap.isRecycled())
						mAlphabetNavItem[cacheIndex].letterBitmap
								.eraseColor(Color.argb(127, 122, 122, 0));
			...
			/** bind new texture */
			bindTexture(gl, mAlphabetNavItem[cacheIndex].letterBitmap,
					mTextureAlphabetId[cacheIndex]);
	...
	private boolean updatePosition(boolean force) {
		...
		/** time independence */
		itvlFromLastRender = Math.min(System.currentTimeMillis() - pTimestamp,
		...
		* .001;
		...
		if (updateFraction > 0 && updateFraction < .05f)
			updateFraction = Constants.CPU_SMOOTHNESS * itvlFromLastRender
		...
		if (mTargetPositionX > mPositionX)
			mPositionX += Math.min(
					Math.min(Math.max(updateFraction * CUBE_SMOOTH
							* (mTargetPositionX - mPositionX), updateFraction
							* CUBE_MIN_SCROLL), mTargetPositionX - mPositionX),
					updateFraction * CUBE_MAX_SCROLL);
		else if (mTargetPositionX < mPositionX)
			mPositionX += Math
					.max(Math.max(Math.min(updateFraction * CUBE_SMOOTH
							* (mTargetPositionX - mPositionX), updateFraction
							* -CUBE_MIN_SCROLL), mTargetPositionX - mPositionX),
							updateFraction * -CUBE_MAX_SCROLL);
		/**
		 ...
		 */
		if (mTargetPositionY != mPositionY && mTargetPositionX != 0) {
			lastInitial = -1;
		}
		...
		flooredPositionX = (int) FloatMath.floor(mPositionX);
		flooredPositionY = (int) FloatMath.floor(mPositionY);
		...
		if (mCursor != null
				&& mPositionY >= mCursor.getCount() - 1
						+ Constants.MAX_POSITION_OVERSHOOT + 2)
			mPositionY = mCursor.getCount() - 1;
		...
		if (mTargetPositionY > mPositionY)
			mPositionY += Math.min(
					Math.min(Math.max(updateFraction * CUBE_SMOOTH
							* (mTargetPositionY - mPositionY), updateFraction
							* CUBE_MIN_SCROLL), mTargetPositionY - mPositionY),
					updateFraction * CUBE_MAX_SCROLL);
		...
		if (mCursor != null) {
			...
			if (lastInitial != -1
				...
				// Log.i(TAG,
				// "lastInitial: "+(char)lastInitial+" mPositionX: "+mPositionX+" current: "+(char)(lastInitial
				// + mPositionX));
			...
			if (mPositionY <= -Constants.MAX_POSITION_OVERSHOOT)
				mTargetPositionY = 0;
					...
					+ Constants.MAX_POSITION_OVERSHOOT)
	...
	private char getFirstLetterLowerCaseAbstractFromCategory(int cat, Cursor c,
			CharArrayBuffer cBuf) {
		try {
			switch (mBrowseCat) {
			case Constants.BROWSECAT_ALBUM:
			...
			case Constants.BROWSECAT_ARTIST:
				c.copyStringToBuffer(
						c.getColumnIndex(MediaStore.Audio.Artists.ARTIST), cBuf);
				oTmpC = this.filterTheFunnyStuff(cBuf);
				break;
			}
			/*
			...
			/*
			 * if it is outside the alphabet make it the '0' of the alphabet
			 */
			if (oTmpC < 'a')
				oTmpC = 'a' - 1;
			if (oTmpC > 'z')
	...
	private int findAlbumPositionAfterAlphabeticalScroll() {
		if (mCursor != null && (int) mPositionY >= 0
			...
			try {
				 ...
				 */
				if (mPositionX > 0) {
					for (letterIdx = (int) mPositionY; letterIdx < mCursor
							.getCount(); letterIdx++) {
						mCursor.moveToPosition(letterIdx);
						if (getFirstLetterLowerCaseAbstractFromCategory(
								mBrowseCat, mCursor, oCharArrayBuffer) >= newLetter) {
							break;
						}
					}
				} else {
	...
	public void stopScrollOnTouch() {
		if (mTargetPositionY > mPositionY)
			mTargetPositionY = FloatMath.ceil(mPositionY);
		else
			mTargetPositionY = FloatMath.floor(mPositionY);
	}
	...
	public void scrollOnTouchMove(float px, int direction) {
		...
		case Constants.SCROLL_MODE_VERTICAL:
			...
			else if (mTargetPositionY >= mCursor.getCount() - 1
					+ Constants.MAX_POSITION_OVERSHOOT)
				mTargetPositionY = mCursor.getCount() - 1
						+ Constants.MAX_POSITION_OVERSHOOT;
			/* update position */
	...
	public void inertialScrollOnTouchEnd(float scrollSpeed, int direction) {
		...
		case Constants.SCROLL_MODE_VERTICAL:
			...
			if (Math.abs(scrollSpeed / (mHeight * .6f)) < Constants.MAX_LOW_SPEED) {
				mTargetPositionY = Math.round(mPositionY + 0.5f
						* Math.signum(scrollSpeed / (mHeight * .5f)) // needs to
																		// be
																		// .5f
																		// because
																		// of
			...
			/* full speed ahead */
				...
				mTargetPositionY = Math.round(mPositionY + 1.33f
						* Constants.SCROLL_SPEED_BOOST * scrollSpeed
						/ (mHeight * .5f));
			}

			/* limit move to X covers */
			if (Math.abs(mTargetPositionY - mPositionY) > 12)
				mTargetPositionY = Math.round(mPositionY)
			...
			return;
		case Constants.SCROLL_MODE_HORIZONTAL:
			mTargetPositionX = Math
					.round(mPositionX
							- (2.0f * Constants.SCROLL_SPEED_BOOST
									* scrollSpeed / (mWidth * .9f)));
			/* small optimization to avoid weird moves on the edges */
	...
	boolean isSpinning() {
		...
		if (mTargetPositionY != mPositionY || mTargetPositionX != mPositionX) {
			return true;
		} else {
			return false;
		}
	...
	int getShownElementId(float x, float y) {
		try {
			if (mTargetPositionY != mPositionY || mCursor == null
					|| mCursor.isClosed() ||
					/**
	...
	int getElementId(int position) {
		try {
			 ...
			 * FIXME: this is a quick cursor overflow bugfix, unverified
			 */
			position > mCursor.getCount() - 1 || position < 0) {
				// Log.i(TAG,
				// "Target was not reached yet: "+mTargetPosition+" - "+mPosition);
				return -1;
			} else {
			...
			if (mCursor != null) {
				for (int i = 0; i < mCursor.getCount(); i++) {
					try {
						...
						if (mCursor
								.getLong(mCursor
										.getColumnIndexOrThrow(MediaStore.Audio.Albums._ID)) == albumId) {
							mTargetPositionY = i;
							mPositionY = i
									- Math.signum(mTargetPositionY - mPositionY)
									* Math.min(
											Math.abs(mTargetPositionY
													- mPositionY), 5.5f);
			...
			if (mCursor != null) {
				for (int i = 0; i < mCursor.getCount(); i++) {
					try {
						...
						if (mCursor
								.getLong(mCursor
										.getColumnIndexOrThrow(MediaStore.Audio.Artists._ID)) == artistId) {
							mTargetPositionY = i;
							mPositionY = i
									- Math.signum(mTargetPositionY - mPositionY)
									* Math.min(
...
class RockOnCube {
	...
	public RockOnCube(int[] textureId, int[] textureAlphabetId) {
		...
		/**
		 * Our cube coordinate declaration we have 4 sets of 4 faces *only*
		 * 
		 * @ each instant only one of the sets is being drawn when
		 * transitioning, 2 sets may be drawn
		 */
		...
		/**
		 * Generate our openGL buffers with the vertice and texture coordinates
		 * and drawing indexes HORIZONTAL
		 */
		for (int l = 0; l < mCubeFaces; l++) {
			for (int k = 0; k < mCubeFaces; k++) {
				// Buffers to be passed to gl*Pointer() functions
				...
				for (int i = 0; i < VERTS; i++) {
					for (int j = 0; j < 3; j++) {
						...
						mFVertexBufferHorizontal[l][k].put(coordsAlphabet[l
								* VERTS * 3 * mCubeFaces + k * VERTS * 3 + i
								* 3 + j]);
					}
				}
	...
	public void draw(GL10 gl) {
		...
		if (horizontalIndexFloat == 0) {
			for (int i = 0; i < mTextureId.length; i++) {
				...
				// GL10.GL_TRIANGLE_STRIP,
						...
						// GL10.GL_LINE_LOOP,
						VERTS, GL10.GL_UNSIGNED_SHORT, mIndexBuffer[i]);
			}
		}
		/**
		...
		if (y < 0)
			y = 0;
		if (horizontalIndexFloat != 0) { // should be a floating point otherwise
											// we only get indexes after one
											// face has rotated
			for (int i = 0; i < 4; i++) {
				...
				gl.glVertexPointer(3, GL10.GL_FLOAT, 0,
						mFVertexBufferHorizontal[y][i]);
				gl.glEnable(GL10.GL_TEXTURE_2D);
				gl.glTexCoordPointer(2, GL10.GL_FLOAT, 0,
						mTexBufferHorizontal[y][i]);
				gl.glDrawElements(
						// GL10.GL_TRIANGLE_STRIP,
						GL10.GL_TRIANGLE_FAN,
						// GL10.GL_LINE_LOOP,
						VERTS, GL10.GL_UNSIGNED_SHORT,
...
class AlbumLabelGlText {
	...
	public AlbumLabelGlText(int textureId) {
		...
		float[] coords0 = {
				// X, Y, Z
				-1.f, .25f, 0.f, 1.f, .25f, 0.f, 1.f, -.25f, 0.f, -1.f, -.25f,
				0.f };
		faceCoordsArray.add(coords0);
