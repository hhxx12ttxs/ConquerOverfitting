...
/**
 * RehangPipeline picks a nonterminal node other than the root and "rehangs" it
 * as a new root. Imagine if the tree were nodes connected with string. Grab the
 * new node and pick it up, letting the other nodes hang underneath it as a new
 * "root". That's in effect what you're doing.
 * 
 * <p>
 * <b>Important Note</b>: Because it must be free of any constraints by nature,
 * RehangPipeline does not work with strong typing. You must not have more than
 * one type defined in order to use RehangPipeline.
 * 
 * <p>
 * RehangPipeline picks a random tree, then picks randomly from all the
 * nonterminals in the tree other than the root, and rehangs the chosen
 * nonterminal as the new root. If its chosen tree has no nonterminals, it
 * repeats the choose-tree process. If after <i>tries</i> times it has failed to
 * find a tree with nonterminals (other than the root), it gives up and simply
 * copies the individual. As you might guess, determining if a tree has
 * nonterminals is very fast, so <i>tries</i> can be pretty large with little to
 * no detriment to evolution speed.
 * 
 * <p>
 * "Rehanging" is complicated to describe. First, you pick a random child of
 * your chosen nonterminal <i>n</i>, and remove this subtree from the tree. Call
 * this subtree <i>T</i>. Next, you set the nonterminal as a new root; its
 ...
 * <i>n</i>. <i>q</i>'s former parent <i>r</i> now fills the slot left behind by
 * <i>p</i>, and so on. This proceeds all the way up to the old root, which will
 * be left with one empty slot (where its former child was that is now its new
...
public class RehangPipeline extends GPBreedingPipeline {
    ...
    /**
     * The number of times the pipeline tries to find a tree with a nonterminal
     * before giving up and just copying the individual.
     */
    int numTries;
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (((GPInitializer) state.initializer).numAtomicTypes + ((GPInitializer) state.initializer).numSetTypes > 1)
            state.output.fatal("RehangPipeline only works when there is only one type (the system is typeless", base,
                    def);
    ...
    private void rehang(final EvolutionState state, final int thread, GPNode pivot, final GPNode root) {
    ...
    public int produce(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
        ...
        for (int q = start; q < n + start; q++) {
            ...
            if (tree != TREE_UNFIXED && (tree < 0 || tree >= i.trees.length))
                // uh oh
                state.output
                        .fatal("RehangPipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
