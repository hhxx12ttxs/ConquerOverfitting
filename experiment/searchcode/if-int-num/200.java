...
public class FrequentQueryGenerater {
	...
	/**
	 * Given the whole set of frequent features
	 * filter features with edge < minSize, features with edge >= maxSize
	 * For each feature size, choose "num" as queries
	 * Output are written in the outputFolder, with id = edge count;
	 * @param queries
	 * @param minSize
	 * @param maxSize
	 * @param num
	 * @throws IOException
	 */
	public static void generateFrequentQueries(NoPostingFeatures<OneFeatureImpl> frequentFeatures, 
			int minSize, int maxSize, int num,
		...
		for(int i = 0; i< frequentFeatures.getfeatureNum() && currentSize < maxSize;i++){
	...
	private static void doSaving(String FrequentQueryFolder, 
		...
		List<OneFeatureImpl> selectedFeatures = new ArrayList<OneFeatureImpl>(num);
		...
		NoPostingFeatures<OneFeatureImpl> gFeatures =
			new NoPostingFeatures<OneFeatureImpl>(FrequentQueryFolder + File.separator+ id, 
