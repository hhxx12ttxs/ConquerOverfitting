...
public abstract class EntityPlayer extends EntityLiving
{
     ...
     * MODIF EntityPlayer: getCurrentPlayerStrVsBlock(Block par1Block)
     */
    public float getCurrentPlayerStrVsBlock(Block par1Block)
    {
        ...
        if(isInsideOfMaterial(Material.water) && isSkillActive(TKHSkill.exacavationWater))
        {
        	if(!onGround)		
        		return f1*0.788884F;
        	return f1*1.1555555F;
        }
     ...
     * MODIF EntityPlayer: attackTargetEntityWithCurrentItem(Entity par1Entity)
     */
    public void attackTargetEntityWithCurrentItem(Entity par1Entity)
    {
        ...
        if (isSkillActive(TKHSkill.stun) && isSkillActive(TKHSkill.invisibility)) 
        {
        	if(par1Entity instanceof EntityLiving && getLookVec().zCoord > par1Entity.getLookVec().zCoord)
        		((EntityLiving)par1Entity).addSkillEffect(new TKHSkillEffect(TKHSkill.isStun.skillID, 200, 0));
    		removeSkillEffect(TKHSkill.stun.skillID);
    		removeSkillEffect(TKHSkill.invisibility.skillID);
    	}
        
        if (isSkillActive(TKHSkill.knockBack)) {
        	par1Entity.addVelocity(-MathHelper.sin((rotationYaw*(float)Math.PI)/180F)*2 , 0.2, MathHelper.cos((rotationYaw*(float)Math.PI)/180F)*2);
    		removeSkillEffect(TKHSkill.knockBack.skillID);
    	}
        
    	if (isSkillActive(TKHSkill.poisoning)) {
    		((EntityLiving) par1Entity).addPotionEffect(new PotionEffect(
    				Potion.poison.id, 200, 0));
    		removeSkillEffect(TKHSkill.poisoning.skillID);
    	}
    	
    	if (isSkillActive(TKHSkill.carbonization)) {
    		par1Entity.setFire(10);
    		removeSkillEffect(TKHSkill.carbonization.skillID);
    	}    
    	
    	if (isSkillActive(TKHSkill.doubleDamage))
    		i*=2;
        ...
        if (i > 0 || k > 0)
        {
            ...
            if (flag)
            {
            	i += isSkillActive(TKHSkill.incCC)?(rand.nextInt(i / 2 + 2))*2:rand.nextInt(i / 2 + 2);
            }
     ...
     * MODIF EntityPlayer: addExperience(int par1)
     */
    public void addExperience(int par1)
    {
        ...
        if(isSkillActive(TKHSkill.doubleExp))
        	i*=2;
