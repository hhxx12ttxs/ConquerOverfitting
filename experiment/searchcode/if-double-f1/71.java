...
public class RenderGlobal implements IWorldAccess
{
    ...
    public int sortAndRender(EntityLiving par1EntityLiving, int par2, double par3)
    {
        ...
        if (d6 > 16D)
        {
            ...
            int i3 = (int)par1EntityLiving.posX;
            int k3 = (int)par1EntityLiving.posZ;
            char c = 2000;

            if (Math.abs(i3 - WorldRenderer.globalChunkOffsetX) > c || Math.abs(k3 - WorldRenderer.globalChunkOffsetZ) > c)
            {
                WorldRenderer.globalChunkOffsetX = i3;
                WorldRenderer.globalChunkOffsetZ = k3;
                loadRenderers();
            }
        }
        ...
        if (updatethread != null)
        {
            ...
            if (updatethread.hasWorkToDo())
            {
                ...
                k = Math.min(k, updatethread.getPendingUpdatesCount());
        ...
        if (occlusionEnabled && mc.gameSettings.advancedOpengl && !mc.gameSettings.anaglyph && par2 == 0)
        {
            int j1 = 0;
            byte byte1 = 20;
            checkOcclusionQueryResult(j1, byte1, par1EntityLiving.posX, par1EntityLiving.posY, par1EntityLiving.posZ);

            for (int k1 = j1; k1 < byte1; k1++)
            {
                sortedWorldRenderers[k1].isVisible = true;
            }

            Profiler.endStartSection("render");
            l += renderSortedRenderers(j1, byte1, par2, par3);
            int i2 = byte1;
            int k2 = 0;
            byte byte2 = 30;
            int j3 = renderChunksWide / 2;

            while (i2 < sortedWorldRenderers.length)
            {
                ...
                if (i2 > sortedWorldRenderers.length)
                {
                    i2 = sortedWorldRenderers.length;
                }
                ...
                checkOcclusionQueryResult(byte3, i2, par1EntityLiving.posX, par1EntityLiving.posY, par1EntityLiving.posZ);
                Profiler.endSection();
                GL11.glPushMatrix();
                float f4 = 0.0F;
                float f5 = 0.0F;
                ...
                for (int l3 = byte3; l3 < i2; l3++)
                {
                    WorldRenderer worldrenderer1 = sortedWorldRenderers[l3];
                    ...
                    {
                        float f7 = Math.abs((float)(worldrenderer1.visibleFromX - par1EntityLiving.posX));
                        float f9 = Math.abs((float)(worldrenderer1.visibleFromY - par1EntityLiving.posY));
                        float f11 = Math.abs((float)(worldrenderer1.visibleFromZ - par1EntityLiving.posZ));
                        float f13 = f7 + f9 + f11;

                        if ((double)f13 < 10D + (double)l3 / 1000D)
                        {
                            worldrenderer1.isVisible = true;
                            continue;
                        }
                    ...
                    float f8 = (float)((double)worldrenderer1.posXMinus - d);
                    float f10 = (float)((double)worldrenderer1.posYMinus - d1);
                    float f12 = (float)((double)worldrenderer1.posZMinus - d2);
                    float f14 = f8 - f4;
                    float f15 = f10 - f5;
                    float f16 = f12 - f6;
                ...
                Profiler.endStartSection("render");
                l += renderSortedRenderers(byte3, i2, par2, par3);
            }
        }
        else
        {
            Profiler.endStartSection("render");
            l += renderSortedRenderers(0, sortedWorldRenderers.length, par2, par3);
        }
        ...
        if (updatethread != null)
        {
            ...
            if (k > 0)
            {
                ...
                float f3 = timePerUpdateMs * (1.0F + (float)(k - 1) / 2.0F);
                float f1 = f3;
            ...
            if (k > 0)
            {
                int j2 = updatethread.resetUpdateCount();
    ...
    public boolean updateRenderers(EntityLiving par1EntityLiving, boolean par2)
    {
        ...
        for (int i1 = 0; i1 < worldRenderersToUpdate.size(); i1++)
        {
            WorldRenderer worldrenderer1 = (WorldRenderer)worldRenderersToUpdate.get(i1);
            ...
            float f1 = worldrenderer1.distanceToEntitySquared(par1EntityLiving);

            if (f1 < 355F)
            {
                if (f1 < 303F && isActing() && worldrenderer1.isInFrustum || firstUpdate)
                {
                    if (updatethread != null)
                    {
        ...
        int j1 = Config.getUpdatesPerFrame();
        boolean flag = false;
        ...
        if (updatethread != null)
        {
            j1 = updatethread.getUpdateCapacity();
        ...
        if (worldrenderer != null)
        {
            updateRenderer(worldrenderer);
            worldRenderersToUpdate.set(l, null);
            i++;
            float f2 = f / 5F;

            for (int l1 = 0; l1 < worldRenderersToUpdate.size() && i < j1; l1++)
            {
        ...
        if (worldRenderersToUpdate.size() > 100 && k < (worldRenderersToUpdate.size() * 4) / 5)
        {
            int k1 = 0;
