...
import com.csci3130.tileentity.TileEntityRefiner;
 ...
 *
 * Class for the magical furnace block.
 */
public class BlockRefiner extends BlockContainer {
	 ...
	 * @param isActive
	 */
	public BlockRefiner(boolean isActive) {
		super(Material.rock);
		isBurning2 = isActive;
		setHardness(5f);
		setResistance(7.0f);
	...
	/**
	 * Registers textures for the multiple sides of the refiner and sets the front
	 * depending on whether the block is active.
	 * @param iconregister - IIconRegister
	 */
	@SideOnly(Side.CLIENT)
	public void registerBlockIcons(IIconRegister iconregister) {
		this.blockIcon = iconregister.registerIcon(Reference.MODID + ":BlockRefinerSide");
		this.front = iconregister.registerIcon(this.isBurning2 ? Reference.MODID + ":BlockRefinerActive" : Reference.MODID + ":BlockRefinerInactive");
		this.top = iconregister.registerIcon(Reference.MODID + ":BlockRefinerTop");
	}

	/**
	 * Gets registered textures for refiner as an IIcon.
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9) {
		player.openGui(Factory.modInstance, 0, world, x, y, z);
		return true;
	}
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(FactoryBlocks.factoryRefiner);
	}
	 ...
	 * @param world (World) - current world
	 * @param par1 (int) - unknown
	 * @return TileEntity - TileEntityMagicalFurnace
	 */
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack itemstack) {
		...
		if (itemstack.hasDisplayName()) {
			((TileEntityRefiner) world.getTileEntity(x, y, z)).furnaceName(itemstack.getDisplayName());
		}
	}
	...
	public static void updateBlockState(boolean burning, World world, int x, int y, int z) {
		...
		if (burning) {
			world.setBlock(x, y, z, FactoryBlocks.factoryRefinerActive);
		} else {
			world.setBlock(x, y, z, FactoryBlocks.factoryRefiner);
		}
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
		if (!isBurning) {
			TileEntityRefiner tileentitymagicalfurnace = (TileEntityRefiner) world.getTileEntity(x, y, z);

			if (tileentitymagicalfurnace != null) {
				for (int i = 0; i < tileentitymagicalfurnace.getSizeInventory(); ++i) {
					ItemStack itemstack = tileentitymagicalfurnace.getStackInSlot(i);
