...
public class AgglomerativeClusterer
{
    ...
    public DirectedGraph cluster()
    {
        ...
        System.out.printf("Level %2d: %25s (%5d leaves, gi=%7.3f -->", 
                newFeatureList.length, featureDefinition.getFeatureName(iBestFeature), newLeaves.length, minFSGI);
    ...
    private double computeImpurity(LeafNode leaf) {
        /*impurities.remove(leaf);
        double i1 = computeMutualDistanceImpurity(leaf);
        impurities.remove(leaf);
        double i2 = computeVarianceImpurity(leaf);
        System.out.printf("mdi=%.3f, vi=%.3f\n", i1, i2);
        return i2;
        */
     ...
     */
    /*
    private double computeMutualDistanceImpurity(LeafNode leaf)
    {
        ...
        rmsDistance = Math.sqrt(rmsDistance);
        
        //System.out.println("len="+len+", baseI="+impurity+", rmsDist="+rmsDistance);
        impurity += rmsDistance;
