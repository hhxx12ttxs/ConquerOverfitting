...
/*        SignalGraph graph = new SignalGraph(signal, samplingFrequency);
        graph.showInJFrame("Test signal", true, true);
        SignalGraph graph2 = new SignalGraph(result, samplingFrequency);
        graph2.showInJFrame("Test result", true, true);
        try {Thread.sleep(100000);} catch(Exception e) {}
*/
        assertTrue("Result has length "+result.length+" instead of expected "+signal.length+" (impulse response length is "+filter.getImpulseResponseLength()+")", result.length == signal.length);
    }
