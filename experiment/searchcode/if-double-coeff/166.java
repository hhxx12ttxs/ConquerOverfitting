...
public class ReflectionCoefficients {
    ...
    public static double[] lpc2lprefc(double[] oneMinusA) 
    {
        ...
        for ( int i = lpcOrder; i > 0; i-- ) {
            ...
            for( int j = 1; j <= (i>>1); j++ ) {
                // Note: in the above, (i>>1) is to be understood as the integer division by 2
                a1 = a[j];
                a2 = a[i-j];
    ...
    public static double[] latticeSynthesisFilter(double[] k, double[] x)
    {
        ...
        SignalProcUtils.displayDFTSpectrumInDB(x, 1024, Window.HAMMING);
        SignalProcUtils.displayDFTSpectrumInDB(fg[0], 1024, Window.HAMMING);
        */
        ...
        int P = k.length;
        int N = x.length;
        double[][] e = new double[P+1][N];
        double[][] b = new double[P+1][N];
        double[] y = new double[N];
        
        int n, i;
        System.arraycopy(x, 0, e[0], 0, N);
        for (i=1; i<P; i++)
            Arrays.fill(e[i], 0.0);
        ...
        for (n=0; n<N; n++)
        {
            for (i=1; i<=P; i++)
            {
                e[i][n] = e[i-1][n] + k[P-i]*b[i][n];
                if (n>0)
                    b[i-1][n] = -1.0*k[P-i]*e[i][n]+b[i][n-1];
                else
                    b[i-1][n] = -1.0*k[P-i]*e[i][n];
            }
        ...
        */
        ...
        SignalProcUtils.displayDFTSpectrumInDB(x, 1024, Window.HAMMING);
        SignalProcUtils.displayDFTSpectrumInDB(y, 1024, Window.HAMMING);
        */
