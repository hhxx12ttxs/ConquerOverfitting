...
//import schemamatchings.meta.experiments.TopKPlot; //Tomer Sagi commented this out due to movement of this class to the ontobuilder research environment.
...
import com.modica.ontology.match.MatchTableModel;
...
/**
 * This class implements the Meta top k Wizard
 * @author haggai
 */
public class OntologyMetaTopKWizard 
{
	...
	private Vector availableLocalFAggs;
	private Vector availableGlobalFAggs;
	private Vector availableLocalHAggs;
	private Vector availableGlobalHAggs;
	private Vector availableMetaAlgsNames;
	...
	public OntologyMetaTopKWizard(JFrame parent,Vector ontologies,Ontology target,Vector algorithms)
	{
		this.parent=parent;
		this.ontologies=ontologies;
		this.target=target;
		this.availableAlgs=algorithms;
		this.selectedAlgs = new Vector();
		availableLocalFAggs = new Vector(MetaAlgorithmUtilities.loadAllLocalAggregators());
		availableGlobalFAggs = new Vector(MetaAlgorithmUtilities.loadAllGlobalAggregators());
		availableLocalHAggs = new Vector(MetaAlgorithmUtilities.loadAllLocalAggregators());
		availableGlobalHAggs = new Vector(MetaAlgorithmUtilities.loadAllGlobalAggregators());
		availableMetaAlgsNames = new Vector(MetaAlgorithmUtilities.getMetaAlgorithmNames());
	...
	{
		if(start().getNextAction()==WizardStatus.CANCEL_ACTION) return null;
		if(normalize)
		{
			target.normalize();
			candidate.normalize();
		...
		{
//			System.out.println("************************************************************************************************************************************************");
//			System.out.println("Running algorithm " + algorithm.getName() + " with threshold of " + algorithm.getThreshold()*100 + "% \n\n\n");
		}
		
		if (properties != null) {
		    HashMap dataMap = ((PropertiesTableModel)properties.getModel()).getDataMap();
		    algorithm.updateProperties(dataMap);
		}
		
		matchInfos = new HashMap();
		Iterator algItr = selectedAlgs.iterator();
		...
		while (algItr.hasNext()){
			...
			if(matchInformation==null) return null;
			else matchInfos.put(algorithm.getName(), matchInformation);
		...
		//TODO
		if(showMatchInformation().getNextAction()==WizardStatus.CANCEL_ACTION) return null;
	...
	private Vector runMetaAlgorithm(){
		if (metaAlgorithmName.equals(MetaAlgorithmNames.ALL[MetaAlgorithmNames.THERSHOLD_ALGORITHM])) return runTA();
		else if (metaAlgorithmName.equals(MetaAlgorithmNames.ALL[MetaAlgorithmNames.MATRIX_DIRECT_ALGORITHM])) return runMD();
		else if (metaAlgorithmName.equals(MetaAlgorithmNames.ALL[MetaAlgorithmNames.MATRIX_DIRECT_WITH_BOUNDING_ALGORITHM])) return runMDB();
		else return runCrossThreshold();
	}
	...
	private Vector runMD(){
		  Object[] params = new Object[4];
			...
			Iterator it = selectedAlgs.iterator();
			int i=0;
			while (it.hasNext()){
			...
			try {
				mda = (MatrixDirectAlgorithm)MetaAlgorithmsFactory.getInstance().buildMetaAlgorithm(MetaAlgorithmNames.MATRIX_DIRECT_ALGORITHM,params);
	...
	public WizardStatus start()
	{
		...
		final JDialog startDialog=new JDialog(parent,ApplicationUtilities.getResourceString("metaTopK.windowTitle"),true);
		JPanel panel=new JPanel();
		panel.setLayout(new BorderLayout());

		startDialog.setSize(new Dimension(ApplicationUtilities.getIntProperty("mergewizard.start.width"),ApplicationUtilities.getIntProperty("mergewizard.start.height")));
		...
		south.add(nextButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.next")));
		...
		south.add(cancelButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.cancel")));
		...
		txtOntologyName.addKeyListener(new KeyAdapter()
			{
				public void keyTyped(KeyEvent event)
				{
					SwingUtilities.invokeLater(new Runnable()
					{
						public void run()
						{
							if(!txtOntologyName.getText().trim().equals("") && target!=null && candidate!=null)
		...
		targetList.addListSelectionListener(new ListSelectionListener()
			{
				public void valueChanged(ListSelectionEvent e)
				{
					...
					txtOntologyName.setText((target!=null?target.getName():"") + (candidate!=null?"-"+candidate.getName():""));
					nextButton.setEnabled(!txtOntologyName.getText().trim().equals("") && target!=null && candidate!=null);
				}
			}
		);
		...
		JCheckBox chkNormalize=new JCheckBox(ApplicationUtilities.getResourceString("mergewizard.start.normalize"));
		...
		{// Title
			JLabel title=new JLabel(ApplicationUtilities.getResourceString("metaTopK.title"),JLabel.LEFT);
		...
		{//	Explanation
			...
			center.add(new MultilineLabel(ApplicationUtilities.getResourceString("metaTopK.explanation")),gbcl);
		}
		...
		{//	Ontology Name
			...
			JLabel ontologyLabel=new JLabel(ApplicationUtilities.getResourceString("mergewizard.start.ontologyName") + ":");
			ontologyLabel.setFont(new Font(ontologyLabel.getFont().getName(),Font.BOLD,ontologyLabel.getFont().getSize()));
		...
		{//	Targets
			...
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel targets=new JLabel(ApplicationUtilities.getResourceString("mergewizard.target") + ":");
		...
		{//	Candidates
			...
			gbcl.insets=new Insets(0,5,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel candidates=new JLabel(ApplicationUtilities.getResourceString("mergewizard.candidate") + ":");
			candidates.setFont(new Font(candidates.getFont().getName(),Font.BOLD,candidates.getFont().getSize()));
		...
		startDialog.addWindowListener(new WindowAdapter()
			{
				...
				{
					SwingUtilities.invokeLater(new Runnable() { public void run() { candidateList.requestFocus(); } } );
				}
		...
		{
			case WizardStatus.NEXT_ACTION: return useExactMapping();
	...
	public WizardStatus useExactMapping(){
		...
		final JDialog dialog=new JDialog(parent,ApplicationUtilities.getResourceString("mergewizard.windowTitle"),true);
		JPanel panel=new JPanel();
		panel.setLayout(new BorderLayout());

		dialog.setSize(new Dimension(ApplicationUtilities.getIntProperty("mergewizard.start.width"),ApplicationUtilities.getIntProperty("mergewizard.start.height")));
		...
		{// Title
			JLabel title=new JLabel(ApplicationUtilities.getResourceString("mergewizard.exactMapping.title"),JLabel.LEFT);
			title.setFont(new Font(title.getFont().getFontName(),Font.BOLD,title.getFont().getSize()+6));
			center.add(title, new AbsoluteConstraints(10, 10, -1, -1));
		}

		{//	Explanation
			center.add(new MultilineLabel(ApplicationUtilities.getResourceString("mergewizard.exactMapping.explanation")),
					new AbsoluteConstraints(10, 40, 400, -1));
		} 
		
		//Exact Mapping
			final JButton open = new JButton(ApplicationUtilities.getResourceString("mergewizard.exactMapping.openButton"));
		...
		south.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.black),BorderFactory.createEmptyBorder(0,10,10,10)));
		JButton backButton;
		south.add(backButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.back")));
				...
				public void actionPerformed(ActionEvent e)
				{
                    ...
                    try {
                    	if (name.getText() != null && name.getText().length() > 0){
							exactMapping = SchemaMatchingsUtilities.readXMLBestMatchingFile(name.getText());
                    	}
						status.setNextAction(WizardStatus.NEXT_ACTION);
                    } catch (Exception e1) {
                    	JOptionPane.showMessageDialog(null,"Faild to read exact maping file.",ApplicationUtilities.getResourceString("error"),JOptionPane.ERROR_MESSAGE);
					}
					dialog.dispose();
				}
			}
		...
		switch(status.getNextAction())
		{
			case WizardStatus.NEXT_ACTION: return selectAlgorithms();
			case WizardStatus.BACK_ACTION: return start();
			default: return status;
		}
	...
	public WizardStatus selectAlgorithms()
	{
		...
		properties = algorithm.getProperties();
		properties.getModel().addTableModelListener(new TableModelListener(){

			public void tableChanged(TableModelEvent event) {
				algorithm.updateProperties(((PropertiesTableModel)properties.getModel()).getDataMap());
			}
		...
		dialog.setSize(new Dimension(ApplicationUtilities.getIntProperty("metatopkwizard.algorithm.width"),ApplicationUtilities.getIntProperty("metatopkwizard.algorithm.height")));
		dialog.setLocationRelativeTo(parent);
		dialog.setResizable(false);
		...
		{// Title
			JLabel title=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.algorithm.title"),JLabel.LEFT);
			title.setFont(new Font(title.getFont().getFontName(),Font.BOLD,title.getFont().getSize()+6));
			GridBagConstraints gbcl=new GridBagConstraints();
			gbcl.weightx=1;
			gbcl.fill=GridBagConstraints.HORIZONTAL;
		...
		{//	Explanation
			...
			gbcl.insets=new Insets(0,0,10,0);
			gbcl.anchor=GridBagConstraints.WEST;
			center.add(new MultilineLabel(ApplicationUtilities.getResourceString("metatopkwizard.algorithm.explanation")),gbcl);
		}
		...
		{//	Algorithms
			...
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel algorithmsLabel=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.algorithms.available") + ":");
		    ...
		    remove.addActionListener(new ActionListener()
		    	...
		    	public void actionPerformed(ActionEvent e)
		    	{
		    		...
		    		if (selectedAlgs.contains(values[i])){
			    		...
			    		if (selectedAlgs.isEmpty()) nextButton.setEnabled(false);
		    		}
		    		}
		    		if (values.length > 0)
		    			algorithmsList2.setListData(selectedAlgs);
		...
		{ 
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel algorithmsLabel2=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.algorithms.selected") + ":");
		...
		{//	Properties
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel propertiesLabel=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.algorithms.properties") + ":");
		...
		{//	Description
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel description=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.algorithms.description") + ":");
		...
		dialog.addWindowListener(new WindowAdapter()
			{
				public void windowOpened(WindowEvent e)
				{
					SwingUtilities.invokeLater(new Runnable() { public void run() { algorithmsList.requestFocus(); } } );
				}
		...
		switch(status.getNextAction())
		{
			case WizardStatus.BACK_ACTION: return useExactMapping();
			case WizardStatus.NEXT_ACTION: return selectAggregators();
	...
	public WizardStatus selectMetaAlgorithms()
	{
		...
		{// Title
			JLabel title=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.metaalgorithm.title"),JLabel.LEFT);
			title.setFont(new Font(title.getFont().getFontName(),Font.BOLD,title.getFont().getSize()+6));
			GridBagConstraints gbcl=new GridBagConstraints();
			gbcl.weightx=1;
			gbcl.fill=GridBagConstraints.HORIZONTAL;
		...
		{//	Explanation
			...
			gbcl.insets=new Insets(0,0,10,0);
			gbcl.anchor=GridBagConstraints.WEST;
			center.add(new MultilineLabel(ApplicationUtilities.getResourceString("metatopkwizard.metaalgorithm.explanation")),gbcl);
		}
		...
		{//	Algorithms
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel algorithmsLabel=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.metaalgorithm.available") + ":");
			center.add(algorithmsLabel,gbcl);
		...
		{//	K
			...
			JLabel algorithmsLabel=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.metaalgorithm.k") + ":");
			gbcl.anchor=GridBagConstraints.WEST;
		...
		{//	Description
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel description=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.metaalgorithm.description") + ":");
			center.add(description,gbcl);
		...
		switch(status.getNextAction())
		{
			case WizardStatus.BACK_ACTION: return selectAggregators();
			case WizardStatus.NEXT_ACTION: k = Integer.parseInt(kParam.getText());
			case WizardStatus.CANCEL_ACTION:
			default: return status;
		}
	}
	...
	public WizardStatus selectAggregators()
	{
		...
		aggregatorList1.addListSelectionListener(new ListSelectionListener()
			{
				public void valueChanged(ListSelectionEvent e)
				{
					...
					if (localf == null) return;
					txtExplanation.setText("Local "+localf.toString()+" f aggregator");
					if (localf.getAggregatorType().indexOf("Average") != -1){
						ThresholdDialog td = new ThresholdDialog(dialog, ((AverageLocalAggregator)localf).getThreshold());
						td.show();
						((AverageLocalAggregator)localf).setThreshold(td.getThreshold());
					}
					
					//Adjust h local aggregator
					String orderKey = localf.getOrderKey();
					AbstractLocalAggregator hl;
					double th = -1;
					Vector aggrs = new Vector();
					Iterator it = availableLocalHAggs.iterator();
					while (it.hasNext()){
						hl = (AbstractLocalAggregator)it.next();
						
						if (DominantsMap.isDominated(hl.getOrderKey(),localf.getOrderKey())){
							aggrs.add(hl);
							if (localf.getOrderKey().equals("AVG") && 
									hl.getOrderKey().equals("AVG")){
		...
		aggregatorList2.addListSelectionListener(new ListSelectionListener()
				{
					public void valueChanged(ListSelectionEvent e)
					{
						...
						if (globalF == null) return;
						txtExplanation.setText("Global "+globalF.toString()+" F aggregator");
						if (globalF.getAggregatorType().indexOf("Average") != -1){
							ThresholdDialog td = new ThresholdDialog(dialog,((AverageGlobalAggregator) globalF).getThreshold());
							td.show();
							((AverageGlobalAggregator) globalF).setThreshold(td.getThreshold());
							
						}
						
						//Adjust F global aggregator
						String orderKey = globalF.getOrderKey();
						AbstractGlobalAggregator hg;
						double th = -1;
						Vector aggrs = new Vector();
						Iterator it = availableGlobalHAggs.iterator();
						while (it.hasNext()){
							hg = (AbstractGlobalAggregator)it.next();
							if (DominantsMap.isDominated(hg.getOrderKey(),globalF.getOrderKey())){
								aggrs.add(hg);
								if (globalF.getOrderKey().equals("AVG") && 
										hg.getOrderKey().equals("AVG")){
		...
		aggregatorList3.addListSelectionListener(new ListSelectionListener()
				{
					public void valueChanged(ListSelectionEvent e)
					{
						...
						if (localh.getAggregatorType().indexOf("Average") != -1){
							...
							ThresholdDialog td = new ThresholdDialog(dialog,((AverageLocalAggregator) localh).getThreshold());
							td.show();
							((AverageLocalAggregator) localh).setThreshold(Math.max(cachedThreshold,td.getThreshold()));
		...
		aggregatorList4.addListSelectionListener(new ListSelectionListener()
				{
					public void valueChanged(ListSelectionEvent e)
					{
						...
						if (globalH.getAggregatorType().indexOf("Average") != -1){
							...
							ThresholdDialog td = new ThresholdDialog(dialog,((AverageGlobalAggregator) globalH).getThreshold());
							td.show();
							((AverageGlobalAggregator) globalH).setThreshold(Math.max(cachedThreshold,td.getThreshold()));
						}
		...
		{// Title
			JLabel title=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.aggregators.title"),JLabel.LEFT);
			title.setFont(new Font(title.getFont().getFontName(),Font.BOLD,title.getFont().getSize()+6));
			GridBagConstraints gbcl=new GridBagConstraints();
			gbcl.weightx=1;
			gbcl.fill=GridBagConstraints.HORIZONTAL;
		...
		{//	Explanation
			...
			gbcl.insets=new Insets(0,0,10,0);
			gbcl.anchor=GridBagConstraints.WEST;
			center.add(new MultilineLabel(ApplicationUtilities.getResourceString("metatopkwizard.aggregators.explanation")),gbcl);
		}
		...
		{//	Algorithms
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel algorithmsLabel=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.aggregators.availablefl") + ":");
			center.add(algorithmsLabel,gbcl);
		...
		{ 
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel algorithmsLabel2=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.aggregators.availablefg") + ":");
			center.add(algorithmsLabel2,gbcl);
		...
		{ 
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel algorithmsLabel3=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.aggregators.availablehl") + ":");
		...
		{ 
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel algorithmsLabel4=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.aggregators.availablehg") + ":");
		...
		{//	Description
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel description=new JLabel(ApplicationUtilities.getResourceString("metatopkwizard.aggregators.description") + ":");
			center.add(description,gbcl);
		...
		dialog.addWindowListener(new WindowAdapter()
			{
				public void windowOpened(WindowEvent e)
				{
					SwingUtilities.invokeLater(new Runnable() { public void run() { aggregatorList2.requestFocus(); } } );
				}
		...
		switch(status.getNextAction())
		{
			case WizardStatus.BACK_ACTION: return selectAlgorithms();
			case WizardStatus.NEXT_ACTION: return selectMetaAlgorithms();
			case WizardStatus.CANCEL_ACTION:
			default: return status;
		}
	}
...
//			System.out.println("Algorithm Results:\n\n" + matchInformation.getReport() + "\n\n\n");
		...
		dialog.setSize(new Dimension(ApplicationUtilities.getIntProperty("mergewizard.matchInformation.width"),ApplicationUtilities.getIntProperty("mergewizard.matchInformation.height")));
		dialog.setLocationRelativeTo(parent);
		//dialog.setResizable(false);
		...
		south.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.black),BorderFactory.createEmptyBorder(0,10,10,10)));
//		JButton saveButton;
//		south.add(saveButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.save")));
...
//						catch(Exception ex)
//						{
//							JOptionPane.showMessageDialog(null,ApplicationUtilities.getResourceString("error") + ": " + ex.getMessage(),ApplicationUtilities.getResourceString("error"),JOptionPane.ERROR_MESSAGE);
//						}
//					}
//				}
//			}
//		);
		...
		south.add(finishButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.finish")));
		...
		{// Title
			JLabel title=new JLabel(ApplicationUtilities.getResourceString("mergewizard.matchInformation.title"),JLabel.LEFT);
			title.setFont(new Font(title.getFont().getFontName(),Font.BOLD,title.getFont().getSize()+6));
			GridBagConstraints gbcl=new GridBagConstraints();
			gbcl.weightx=1;
			gbcl.gridwidth=2;
		...
		{//	Explanation
			...
			gbcl.insets=new Insets(0,0,10,0);
			gbcl.anchor=GridBagConstraints.WEST;
			center.add(new MultilineLabel(ApplicationUtilities.getResourceString("mergewizard.matchInformation.explanation")),gbcl);
		}
		...
		{//	Target Ontology
			...
			gbcl.anchor=GridBagConstraints.EAST;
			JLabel targetOntologyLabel=new JLabel(ApplicationUtilities.getResourceString("mergewizard.matchInformation.target") + ":");
		...
		{//	Candidate Ontology
			...
			gbcl.anchor=GridBagConstraints.EAST;
			gbcl.insets=new Insets(0,5,0,5);
			JLabel candidateOntologyLabel=new JLabel(ApplicationUtilities.getResourceString("mergewizard.matchInformation.candidate") + ":");
		...
		{//	Information
			...
			informationPanel.addTab(metaAlgorithmName, generateTabbedPaneForMetaMatcher(topKMatchings));
			
			Iterator matchersInfo = matchInfos.keySet().iterator();
			String algName;
			int i=1;
			while (matchersInfo.hasNext()){
				algName = (String)matchersInfo.next();
				informationPanel.addTab(algName, generateTabbedPaneForMatcher((MatchInformation)matchInfos.get(algName)));
			}
	...
	private JTabbedPane generateTabbedPaneForMetaMatcher(final Vector matches){
		...
		JPanel statisticsPanel=new JPanel();
			...
			statisticsPanel.add(new JScrollPane(matchInformation.getStatistics(exactMapping)));
			
		if (exactMapping != null){
			XYSeries xy = new XYSeries(metaAlgorithmName);
			/*TopKPlot kplot = metaAlg.getStatistics().getMappingsPlot();
			for (int i=0;i<k;i++){
				XYDataPair pair = new XYDataPair(new Integer(i+1), new Integer((int)kplot.getYPlot(i)));
				try {
					xy.add(pair);
				} catch (SeriesException e1) {
					
				}	
			}*/ //Tomer Sagi commented this out due to TopKPlot belonging to a package that should not be part of the Ontobuilder project
			...
			if (exactMapping != null){
				...
				for (int i=0;i<numMatches;i++){
					...
					try {
						pAtK.add(new XYDataPair(new Integer(i+1), new Double(SchemaMatchingsUtilities.calculatePrecision(exactMapping,st,mmatrix))));
						rAtK.add(new XYDataPair(new Integer(i+1), new Double(SchemaMatchingsUtilities.calculateRecall(exactMapping,st,mmatrix))));
					} catch (SeriesException e1) {
		...
		informationPanel.addTab(ApplicationUtilities.getResourceString("mergewizard.matchInformation.statistics"),statisticsPanel);

		// Buttons
		final JButton removeMatchButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.removeMatch"));
		final JButton matchButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.match"));
		final JLabel mismatchMatchLabel=new JLabel("",null,JLabel.CENTER);
		JCheckBox updateThesaurusCheckBox=new JCheckBox(ApplicationUtilities.getResourceString("mergewizard.checkbox.updateThesaurus"));
...
//		// Mismatch Lists
//		final JList targetMismatchesList=new JList(matchInformation.getMismatchesTargetOntology().toArray());
//		final JList candidateMismatchesList=new JList(matchInformation.getMismatchesCandidateOntology().toArray());
//
//		targetMismatchesList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
//		targetMismatchesList.getSelectionModel().addListSelectionListener(new ListSelectionListener()
//			{
//				public void valueChanged(ListSelectionEvent e)
//				{
...
//					mismatchMatchLabel.setText((targetObject!=null?targetObject:"") + (targetObject!=null && candidateObject!=null?" <--> ":"") + (candidateObject!=null?candidateObject:""));
//				}
//			}
//		);
//		candidateMismatchesList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
//		candidateMismatchesList.getSelectionModel().addListSelectionListener(new ListSelectionListener()
//			{
//				public void valueChanged(ListSelectionEvent e)
//				{
//					ListSelectionModel model=(ListSelectionModel)e.getSource();
...
//		Object targetMismatchColumns[]={matchInformation.getTargetOntology().getName(),ApplicationUtilities.getResourceString("mergewizard.matchInformation.include")};
...
//		}
//		final JTable mismatchesTargetTable=new JTable(new DefaultTableModel(targetMismatchData,targetMismatchColumns)
//			{
...
//		Object candidateMismatchColumns[]={matchInformation.getCandidateOntology().getName(),ApplicationUtilities.getResourceString("mergewizard.matchInformation.include")};
...
//		}
//		final JTable mismatchesCandidateTable=new JTable(new DefaultTableModel(candidateMismatchData,candidateMismatchColumns)
//			{
//				public boolean isCellEditable(int row,int col)
//				{
//					return col==1;
...
//					if(matchInformation.getMismatchesTargetOntology().size()>targetMismatches) // the mismatches were updated
//					{
//						Mismatch mismatchTarget=(Mismatch)matchInformation.getMismatchesTargetOntology().get(matchInformation.getMismatchesTargetOntology().size()-1);
//						DefaultTableModel targetModel=(DefaultTableModel)mismatchesTargetTable.getModel();
//						targetModel.addRow(new Object[] {mismatchTarget,new Boolean(mismatchTarget.isSelected())});
//						targetMismatchesList.setListData(matchInformation.getMismatchesTargetOntology().toArray());
//					}
//					if(matchInformation.getMismatchesCandidateOntology().size()>candidateMismatches) // the mismatches were updated
//					{
//						Mismatch mismatchCandidate=(Mismatch)matchInformation.getMismatchesCandidateOntology().get(matchInformation.getMismatchesCandidateOntology().size()-1);
//						DefaultTableModel candidateModel=(DefaultTableModel)mismatchesCandidateTable.getModel();
//						candidateModel.addRow(new Object[] {mismatchCandidate,new Boolean(mismatchCandidate.isSelected())});
...
//		gbc.weightx=1;
//		mismatchesInformationPanel.add(new JLabel(matchInformation.getTargetOntology().getName()),gbc);
//		gbc.gridx=1;
//		mismatchesInformationPanel.add(new JLabel(matchInformation.getCandidateOntology().getName()),gbc);
//		gbc.weightx=0;
//		gbc.weighty=1;
...
//				public void actionPerformed(ActionEvent e)
//				{
//					Mismatch targetMismatch=(Mismatch)matchInformation.getMismatchesTargetOntology().get(targetMismatchIndex);
//					Mismatch candidateMismatch=(Mismatch)matchInformation.getMismatchesCandidateOntology().get(candidateMismatchIndex);
...
//					matchModel.addRow(new Object[] {targetTerm.getName() + " (" + targetTerm.getAttributeValue("name") + ")",candidateTerm.getName() + " (" + candidateTerm.getAttributeValue("name") + ")","100%"});
//
//					DefaultTableModel targetMismatchModel=(DefaultTableModel)mismatchesTargetTable.getModel();
//					targetMismatchModel.removeRow(targetMismatchIndex);
//
//					DefaultTableModel candidateMismatchModel=(DefaultTableModel)mismatchesCandidateTable.getModel();
...
//		final JButton targetCheckAllButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.checkAll"));
...
//		final JButton targetUncheckAllButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.uncheckAll"));
...
//		final JButton candidateCheckAllButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.checkAll"));
...
//		final JButton candidateUncheckAllButton=new JButton(ApplicationUtilities.getResourceString("mergewizard.button.uncheckAll"));
...
//		mismatchesCandidatePanel.add(BorderLayout.CENTER,new JScrollPane(mismatchesCandidateTable));
//		informationPanel.addTab(ApplicationUtilities.getResourceString("mergewizard.matchInformation.mismatchesCandidate"),mismatchesCandidatePanel);
		...
		final JGraph graph=matchInformation.getGraph(exactMapping);
		...
		informationPanel.addTab(ApplicationUtilities.getResourceString("mergewizard.matchInformation.graph"),graphPanel);
		// Graph Toolbox
		ToolBar graphToolbar=new ToolBar(null);
		// Zoom In
		Action action = new AbstractAction(ApplicationUtilities.getResourceString("action.zoomIn"),ApplicationUtilities.getImage("zoomin.gif"))
		{
			public void actionPerformed(ActionEvent e)
			{
			    if(mgs.getDisplayedGraph()==null) return;
				double scale=mgs.getScale();
				mgs.getDisplayedGraph().setScale(scale*1.5);
				mgs.setScale(scale*1.5);
			}
		};
		action.putValue(Action.LONG_DESCRIPTION,ApplicationUtilities.getResourceString("action.zoomIn.longDescription"));
		action.putValue(Action.SHORT_DESCRIPTION,ApplicationUtilities.getResourceString("action.zoomIn.shortDescription"));
		action.putValue(Action.MNEMONIC_KEY,new Integer(KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.zoomIn.mnemonic")).getKeyCode()));
		action.putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.zoomIn.accelerator")));
		graphToolbar.addButton(action);

		// Zoom Out
		action = new AbstractAction(ApplicationUtilities.getResourceString("action.zoomOut"),ApplicationUtilities.getImage("zoomout.gif"))
		{
			public void actionPerformed(ActionEvent e)
			{
				if(mgs.getDisplayedGraph()==null) return;
		...
		action.putValue(Action.LONG_DESCRIPTION,ApplicationUtilities.getResourceString("action.zoomOut.longDescription"));
		action.putValue(Action.SHORT_DESCRIPTION,ApplicationUtilities.getResourceString("action.zoomOut.shortDescription"));
		action.putValue(Action.MNEMONIC_KEY,new Integer(KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.zoomOut.mnemonic")).getKeyCode()));
		action.putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.zoomOut.accelerator")));
		graphToolbar.addButton(action);

		graphToolbar.addSeparator();

		// Save image
		action = new AbstractAction(ApplicationUtilities.getResourceString("action.saveImage"),ApplicationUtilities.getImage("saveimage.gif"))
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				if(file!=null)
				{
					try
					{
						...
						ImageUtilities.saveImageToFile(GraphUtilities.toImage(mgs.getDisplayedGraph()),format,theFile);
					}
		...
		action.putValue(Action.LONG_DESCRIPTION,ApplicationUtilities.getResourceString("action.saveImage.longDescription"));
		action.putValue(Action.SHORT_DESCRIPTION,ApplicationUtilities.getResourceString("action.saveImage.shortDescription"));
		action.putValue(Action.MNEMONIC_KEY,new Integer(KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.saveImage.mnemonic")).getKeyCode()));
		action.putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.saveImage.accelerator")));
		graphToolbar.addButton(action);
		...
		//Best 1:1 mapping
		action = new AbstractAction(ApplicationUtilities.getResourceString("action.bestOneToOneMatching"),ApplicationUtilities.getImage("topk.gif"))
		{
			public void actionPerformed(ActionEvent e)
			{
				    ...
				    try {
				      ...
				      ArrayList matchs = st.toOntoBuilderMatchList((MatchMatrix)(metaAlg.getMatchMatrix(0) != null ? metaAlg.getMatchMatrix(0) : metaAlg.getCombinedMatrix()));
				      ////////////
				      matchInformation.setMatches(matchs);
				      JGraph graph=matchInformation.getGraph(exactMapping);
...
//							precisionLabel.setText("N/A");
				      ...
				      ((MatchTableModel)matchTable.getModel()).refreshData(matchInformation.getTableModelData(), matchs.size());
				    }catch(Exception sme) {
				        //ignore
				    }
				}
		...
		action.putValue(Action.LONG_DESCRIPTION,ApplicationUtilities.getResourceString("action.bestOneToOneMatching.longDescription"));
		action.putValue(Action.SHORT_DESCRIPTION,ApplicationUtilities.getResourceString("action.bestOneToOneMatching.shortDescription"));
		action.putValue(Action.MNEMONIC_KEY,new Integer(KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.bestOneToOneMatching.mnemonic")).getKeyCode()));
		action.putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.bestOneToOneMatching.accelerator")));
		topKToolbar.addButton(action);
		
		// Previous match
		action = new AbstractAction(ApplicationUtilities.getResourceString("action.previousBestMatching"),ApplicationUtilities.getImage("back.gif"))
		{
			public void actionPerformed(ActionEvent e)
			{

				    try {
				       if (mgs.getCurrentK() == 1) return;//do nothing
...
//							precisionLabel.setText("N/A");
		...
		action.putValue(Action.LONG_DESCRIPTION,ApplicationUtilities.getResourceString("action.previousBestMatching.longDescription"));
		action.putValue(Action.SHORT_DESCRIPTION,ApplicationUtilities.getResourceString("action.previousBestMatching.shortDescription"));
		action.putValue(Action.MNEMONIC_KEY,new Integer(KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.previousBestMatching.mnemonic")).getKeyCode()));
		action.putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.previousBestMatching.accelerator")));
		topKToolbar.addButton(action);

		// Next Match
		action = new AbstractAction(ApplicationUtilities.getResourceString("action.nextBestMatching"),ApplicationUtilities.getImage("forward.gif"))
		{
			public void actionPerformed(ActionEvent e)
			{
...
//							precisionLabel.setText("N/A");
		...
		action.putValue(Action.LONG_DESCRIPTION,ApplicationUtilities.getResourceString("action.nextBestMatching.longDescription"));
		action.putValue(Action.SHORT_DESCRIPTION,ApplicationUtilities.getResourceString("action.nextBestMatching.shortDescription"));
		action.putValue(Action.MNEMONIC_KEY,new Integer(KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.nextBestMatching.mnemonic")).getKeyCode()));
		action.putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.nextBestMatching.accelerator")));
		topKToolbar.addButton(action);
		
		//Save Match
		action = new AbstractAction(ApplicationUtilities.getResourceString("action.saveTopKMatching"),ApplicationUtilities.getImage("saveontology.gif"))
		{
			public void actionPerformed(ActionEvent e)
			{
				    ...
				    try {
				        ...
				        if (st == null) {//1:N matching
//				            JOptionPane.showMessageDialog(null,ApplicationUtilities.getResourceString("error") + ":" + "No Top K matching generated",ApplicationUtilities.getResourceString("error"),JOptionPane.ERROR_MESSAGE);
//				            return;
...
//						FileUtilities.fileViewer.removeAllViewers();
//						FileUtilities.fileViewer.addViewer(XMLUtilities.xmlFileViewer);
//						File file=FileUtilities.saveFileDialog(null,new File(matchInformation.getTargetOntology().getName()+"-"+matchInformation.getCandidateOntology().getName() +"-"+mgs.getTopkIndex()+"-th_top_match.xml"));
						...
						for (int i=0;i<metadata.length;i++){
							if (metadata[i].getExClass().equalsIgnoreCase(ExporterMetadata.MATCHING)){
								filters.add(new GeneralFileFilter(metadata[i].getExtension(), "Save matching as "+metadata[i].getType()+" file"));
							}
						}
						FileUtilities.configureFileDialogFilters(filters);
						// Viewers
...
//						Previewers
						...
						for (int i=0;i<metadata.length;i++){
							if (metadata[i].getExClass().equalsIgnoreCase(ExporterMetadata.MATCHING)){
								filePrefviewer.addPreviewer(new GeneralFilePreviewer(metadata[i].getExtension()));
							}
						}
				        ...
				        if(file!=null)
						{
							try
							{
								Exporter exporter = ExportUtilities.getExporterPlugin(FileUtilities.getFileExtension(file));
							    HashMap params = new HashMap();
							    params.put("Matching", st);
							    params.put("index", new Integer(mgs.getTopkIndex()));
							    params.put("candName", matchInformation.getCandidateOntology().getName());
							    params.put("targetName", matchInformation.getTargetOntology().getName());
		...
		action.putValue(Action.LONG_DESCRIPTION,ApplicationUtilities.getResourceString("action.saveTopKMatching.longDescription"));
		action.putValue(Action.SHORT_DESCRIPTION,ApplicationUtilities.getResourceString("action.saveTopKMatching.shortDescription"));
		action.putValue(Action.MNEMONIC_KEY,new Integer(KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.saveTopKMatching.mnemonic")).getKeyCode()));
		action.putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.saveTopKMatching.accelerator")));
		topKToolbar.addButton(action);
...
////			pie.setValue(" "+ApplicationUtilities.getResourceString("mergewizard.matchInformation.mismatches"),new Double(nf.format(1-precision)));
////			pie.setValue(ApplicationUtilities.getResourceString("mergewizard.matchInformation.matches"),new Double(nf.format(precision)));
...
//		informationPanel.addTab(ApplicationUtilities.getResourceString("mergewizard.matchInformation.mismatchesCandidate"),mismatchesCandidatePanel);
		...
		//TODO: 6/5/2007 here there may some bug - fix it
	      ArrayList matchs = st.toOntoBuilderMatchList(mgs.getTopk());
	      ////////////
	      matchInformation.setMatches(matchs);
	      statisticsPanel.add(new JScrollPane(matchInformation.getStatistics(exactMapping)));
	      if (chart != null)
...
//							precisionLabel.setText("N/A");
		...
		action = new AbstractAction(ApplicationUtilities.getResourceString("action.previousBestMatching"),ApplicationUtilities.getImage("back.gif"))
		{
			public void actionPerformed(ActionEvent e)
			{
				    ...
				    try {
				       if (mgs.getTopkIndex() == 1) return;//do nothing
...
//	private double getRecall(SchemaTranslator exact, MatchInformation matchInfo){
//		SchemaTranslator st1_ = new SchemaTranslator(matchInfo, true, false);
//		SchemaTranslator st2_ = new SchemaTranslator(matchInfo, true, true);
//		return Math.max(SchemaMatchingsUtilities.calculateRecall(exact, st1_), SchemaMatchingsUtilities.calculateRecall(exact, st2_));
//	}
//
//	private double getPrecision(SchemaTranslator exact, MatchInformation matchInfo){
//		SchemaTranslator st1_ = new SchemaTranslator(this, true, false);
//		SchemaTranslator st2_ = new SchemaTranslator(this, true, true);
//		return Math.max(SchemaMatchingsUtilities.calculatePrecision(st, st1_),SchemaMatchingsUtilities.calculatePrecision(st, st2_)) ;
//	}
