...
public class OntologyMergeWizard
{
	...
	public OntologyMergeWizard(JFrame parent,Vector ontologies,Ontology target,Vector algorithms)
	{
		this.parent=parent;
	...
	public Ontology startOntologyMerge()
	{
		...
		if(showMatchInformation(matchInformation).getNextAction()==WizardStatus.CANCEL_ACTION) return null;
	...
	public WizardStatus start()
	{
		...
		final JDialog startDialog=new JDialog(parent,ApplicationUtilities.getResourceString("mergewizard.windowTitle"),true);
		...
		chkNormalize.addItemListener(new ItemListener()
			{
				...
				{
					setNormalizeBeforeMatch(e.getStateChange()==e.SELECTED);
				}
			}
		);
		...
		{// Title
			JLabel title=new JLabel(ApplicationUtilities.getResourceString("mergewizard.title"),JLabel.LEFT);
		...
		{//	Explanation
			...
			center.add(new MultilineLabel(ApplicationUtilities.getResourceString("mergewizard.explanation")),gbcl);
		}
	...
	public WizardStatus selectAlgorithms()
	{
		...
		dialog.setSize(new Dimension(ApplicationUtilities.getIntProperty("mergewizard.algorithm.width"),ApplicationUtilities.getIntProperty("mergewizard.algorithm.height")));
		dialog.setLocationRelativeTo(parent);
		dialog.setResizable(false);
		...
		{// Title
			JLabel title=new JLabel(ApplicationUtilities.getResourceString("mergewizard.algorithm.title"),JLabel.LEFT);
			title.setFont(new Font(title.getFont().getFontName(),Font.BOLD,title.getFont().getSize()+6));
			GridBagConstraints gbcl=new GridBagConstraints();
			gbcl.weightx=1;
			gbcl.fill=GridBagConstraints.HORIZONTAL;
		...
		{//	Explanation
			...
			gbcl.insets=new Insets(0,0,10,0);
			gbcl.anchor=GridBagConstraints.WEST;
			center.add(new MultilineLabel(ApplicationUtilities.getResourceString("mergewizard.algorithm.explanation")),gbcl);
		}
		...
		{//	Algorithms
			...
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel algorithmsLabel=new JLabel(ApplicationUtilities.getResourceString("mergewizard.algorithms") + ":");
		...
		{//	Properties
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel propertiesLabel=new JLabel(ApplicationUtilities.getResourceString("mergewizard.algorithms.properties") + ":");
		...
		{//	Description
			...
			gbcl.insets=new Insets(0,10,0,0);
			gbcl.anchor=GridBagConstraints.WEST;
			JLabel description=new JLabel(ApplicationUtilities.getResourceString("mergewizard.algorithms.description") + ":");
	...
	public WizardStatus showMatchInformation(final MatchInformation matchInformation)
	{
				...
				public void actionPerformed(ActionEvent e)
				{
					...
					if(file!=null)
					{
						try
						{
							...
							matchInformation.save(new File(extFile),format);
						}
		...
		{//	Algorithm
			...
			gbcl.anchor=GridBagConstraints.EAST;
			gbcl.insets=new Insets(0,5,0,5);
			JLabel algorithmLabel=new JLabel(ApplicationUtilities.getResourceString("mergewizard.matchInformation.algorithm") + ":");
		...
		{//	Threshold
			...
			gbcl.anchor=GridBagConstraints.EAST;
			gbcl.insets=new Insets(0,5,0,5);
			JLabel thresholdLabel=new JLabel(ApplicationUtilities.getResourceString("mergewizard.matchInformation.threshold") + ":");
        ...
        final JLabel recallLabel = new JLabel(recall != -1 ? ""+nf.format(recall*100)+"%" : "N/A");
        ...
        final JLabel precisionLabel = new JLabel(precision != -1 ? ""+nf.format(precision*100)+"%" : "N/A");
		...
		{//	Information
			...
			if (exactMapping != null){
				...
				JFreeChart chart=ChartFactory.createPieChart(ApplicationUtilities.getResourceString("mergewizard.matchInformation.statistics"),pie,true);
			...
			//Best 1:N mapping
			action = new AbstractAction(ApplicationUtilities.getResourceString("action.bestOneToManyMatching"),ApplicationUtilities.getImage("rsource.gif"))
			{
				public void actionPerformed(ActionEvent e)
				{
					      ...
					      ((MatchTableModel)matchTable.getModel()).refreshData(mgs.getMatchInformation().getTableModelData(), mgs.getMatchInformation().getTotalMatches() );
					
				}
			};
			action.putValue(Action.LONG_DESCRIPTION,ApplicationUtilities.getResourceString("action.bestOneToManyMatching.longDescription"));
			action.putValue(Action.SHORT_DESCRIPTION,ApplicationUtilities.getResourceString("action.bestOneToManyMatching.shortDescription"));
			action.putValue(Action.MNEMONIC_KEY,new Integer(KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.bestOneToManyMatching.mnemonic")).getKeyCode()));
			action.putValue(Action.ACCELERATOR_KEY,KeyStroke.getKeyStroke(ApplicationUtilities.getResourceString("action.bestOneToManyMatching.accelerator")));
