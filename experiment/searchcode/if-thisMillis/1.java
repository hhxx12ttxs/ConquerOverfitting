...
public class HDateTime extends HVal {
    ...
    public static HDateTime make(long millis) {
        return make(millis, HTimeZone.getDefault());
    }
    ...
    public long millis() {
        if (millis <= 0) {
            GregorianCalendar c = new GregorianCalendar(date.year, date.month - 1, date.day, time.hour, time.min,
                    time.sec);
            c.setTimeZone(utc);
    ...
    protected List<ValidationMessage> validate(List<ValidationMessage> messages) {
        ...
        if (date == null)
            messages = addValidationMessage(messages, new ValidationMessage("date", "haystack.required"));
        if (time == null)
            messages = addValidationMessage(messages, new ValidationMessage("time", "haystack.required"));
        if (tz == null)
            messages = addValidationMessage(messages, new ValidationMessage("tz", "haystack.required"));
    ...
    public boolean equals(Object that) {
        if (!(that instanceof HDateTime))
            return false;
        HDateTime x = (HDateTime) that;
        return date.equals(x.date) && time.equals(x.time) && tzOffset == x.tzOffset && tz.equals(x.tz);
    }
