		...
		try {
			...
			_setRegressors.add (new UnitRegressionExecutor ("CreateSNAC", _strRegressionScenario) {
				...
				@Override public boolean preRegression() {
					...
					if (null == (_dc = DiscountCurve.CreateFromFlatRate (_dtStart, _strCurrency, 0.04)))
						return false;
					...
					for (int i = 0; i < 5; ++i) {
						...
						if (null == (_aCDS[i] = CreditDefaultSwap.CreateSNAC (_dtStart, (i + 1) + "Y", 0.01, "CORP")))
							return false;
			 		}
				...
				@Override public boolean execRegression() {
					if (null == (_cc = CreditCurveScenarioContainer.CreateCreditCurve ("CORP", _dtStart,
			...
			_setRegressors.add (new UnitRegressionExecutor ("FromFlatHazard", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					if (null == (_ccFromFlatHazard = CreditCurve.FromFlatHazard (_dtStart.getJulian(),
						"CORP", 0.02, 0.4)))
						return false;
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							...
							rnvd.set ("HazardRateFromHazardCurve[" + adt[i] + "]", FIGen.FormatSpreadSimple
								(adblHazard[i], 1, 4, 1));

							if (!RegressionUtil.WithinTolerance (adblHazard[i], 0.02)) return false;
			...
			_setRegressors.add (new UnitRegressionExecutor ("FromSurvival", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					if (null == (_ccFromSurvival = CreditCurve.FromSurvival (_dtStart.getJulian(), "CORP",
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							...
							rnvd.set ("SurvivalFromOriginal[" + dt + "]", FIGen.FormatSpreadSimple
								(_adblSurvival[i], 1, 4, 1));

							rnvd.set ("SurvivalFromSurvival[" + dt + "]", FIGen.FormatSpreadSimple
								(adblSurvivalCalc[i], 1, 4, 1));

							if (!RegressionUtil.WithinTolerance (adblSurvivalCalc[i], _adblSurvival[i]))
								return false;
						} catch (Exception e) {
							e.printStackTrace();
			...
			_setRegressors.add (new UnitRegressionExecutor ("FromHazard", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					if (null == (_ccFromHazard = CreditCurve.CreateCreditCurve (_dtStart, "CORP", _adblDate,
						_adblHazard, 0.4)))
						return false;
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							...
							rnvd.set ("HazardFromOriginal[" + dt1 + "-" + dt2 + "]", FIGen.FormatSpreadSimple
								(_adblHazard[i], 1, 4, 1));

							rnvd.set ("HazardFromHazard[" + dt1 + "-" + dt2 + "]", FIGen.FormatSpreadSimple
								(adblHazardCalc[i], 1, 4, 1));

							if (!RegressionUtil.WithinTolerance (adblHazardCalc[i], _adblHazard[i]))
								return false;
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					for (int i = 0; i < _adblQuotes.length; ++i) {
						...
						try {
							...
							rnvd.set ("CompQuote" + "_" + strCode + "[" + dt + "]", FIGen.FormatSpreadSimple
								(dblCompQuote, 1, 4, 1));

							rnvd.set ("NodeQuote" + "_" + strCode + "[" + dt + "]", FIGen.FormatSpreadSimple
								(_adblQuotes[i], 1, 4, 1));

							if (!RegressionUtil.WithinTolerance (dblCompQuote, _adblQuotes[i])) return false;
						} catch (Exception e) {
							e.printStackTrace();
			...
			_setRegressors.add (new UnitRegressionExecutor ("ParallelHazardShiftedCurve",
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < adblQuotes.length; ++i) {
						...
						rnvd.set ("BaseCurveHazard[" + dt1 + "-" + dt + "]", FIGen.FormatSpreadSimple
							(dblBaseHazard, 1, 4, 1));

						rnvd.set ("ParallelShiftedCurveHazard[" + dt1 + "-" + dt + "]",
							FIGen.FormatSpreadSimple (dblShiftedHazard, 1, 4, 1));

						dt = dt1;

						if (!RegressionUtil.WithinTolerance (dblBaseHazard + 0.0005, dblShiftedHazard))
							return false;
					}
			...
			_setRegressors.add (new UnitRegressionExecutor ("ParallelQuoteShiftedCurve",
				...
				@Override public boolean execRegression() {
					if (null == (_ccParallelShifted = _cc.createParallelShiftedCurve (5.))) return false;
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < adblQuotes.length; ++i) {
						...
						rnvd.set ("BaseCurveQuote[" + dt + "]", FIGen.FormatSpreadSimple (adblQuotes[i], 1,
							5, 1));

						rnvd.set ("ParallelShiftedCurveQuote[" + dt + "]", FIGen.FormatSpreadSimple
							(adblQuotesShifted[i], 1, 5, 1));

						dt = dt1;

						if (!RegressionUtil.WithinTolerance (adblQuotes[i] + 5., adblQuotesShifted[i]))
							return false;
					}
			...
			_setRegressors.add (new UnitRegressionExecutor ("NodeTweakedCurve", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					if (null == (_ccTweakedCurve = _cc.createTweakedCurve (_cntp))) return false;
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < adblQuotes.length; ++i) {
						...
						rnvd.set ("UntweakedHazard[" + dt + "]", FIGen.FormatSpreadSimple (dblBaseHazard, 1,
							5, 1));

						rnvd.set ("TweakedHazard[" + dt + "]", FIGen.FormatSpreadSimple (dblShiftedHazard, 1,
							5, 1));
			...
			_setRegressors.add (new UnitRegressionExecutor ("FlatCurve", _strRegressionScenario) {
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						...
						rnvd.set ("FlatHazard[" + dt + "]", FIGen.FormatSpreadSimple (dblHazard, 1, 5, 1));
					}
			...
			_setRegressors.add (new UnitRegressionExecutor ("EffectiveSurvival", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							if (Double.isNaN (_adblSurvival[i] = _cc.getEffectiveSurvival ((i + 1) + "Y", (i
								+ 2) + "Y")))
								return false;
						} catch (Exception e) {
							e.printStackTrace();
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							rnvd.set ("EffectiveSurvival[" + new JulianDate (_adblDate[i]) + "]",
								FIGen.FormatSpreadSimple (_adblSurvival[i], 1, 4, 1));
						} catch (Exception e) {
							e.printStackTrace();
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							rnvd.set ("EffectiveRecovery[" + (i + 1) + "Y-" + (i + 2) + "Y]",
								FIGen.FormatSpreadSimple (_adblEffectiveRecovery[i], 1, 4, 1));

							rnvd.set ("CurveRecovery[" + (i + 1) + "Y-" + (i + 2) + "Y]",
								FIGen.FormatSpreadSimple (_cc.getRecovery ((i + 1) + "Y"), 1, 4, 1) + "-" +
									FIGen.FormatSpreadSimple (_cc.getRecovery ((i + 2) + "Y"), 1, 4, 1));
						} catch (Exception e) {
							e.printStackTrace();
