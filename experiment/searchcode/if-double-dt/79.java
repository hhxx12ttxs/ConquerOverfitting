	...
	@Override public boolean setupRegressors() {
		...
		try {
			_setRegressors.add (new UnitRegressionExecutor ("CreateFromRatesInstruments",
				...
				@Override public boolean preRegression() {
					...
					for (int i = 0; i < 7; ++i) {
						...
						try {
							_aCompCalib[i] = Cash.CreateCash (_dtStart.addDays (2), new JulianDate
					...
					EDFuture[] aEDF = EDFuture.GenerateEDPack (_dtStart, 8, _strCurrency);
					...
					for (int i = 0; i < 15; ++i) {
						...
						try {
							_aCompCalib[i + 15] = InterestRateSwap.CreateIRS (_dtStart.addDays (2), new
				...
				@Override public boolean execRegression() {
					if (null == (_dc = IRCurveScenarioContainer.CreateDiscountCurve (_dtStart, _strCurrency,
			...
			_setRegressors.add (new UnitRegressionExecutor ("CreateFromFlatRate", _strRegressionScenario) {
				@Override public boolean execRegression() {
					if (null == (_dcFromFlatRate = DiscountCurve.CreateFromFlatRate (_dtStart, _strCurrency,
						0.04)))
						return false;
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							if (Double.isNaN (adblDiscountFactorFlatRate[i] = _dcFromFlatRate.getDF
			...
			_setRegressors.add (new UnitRegressionExecutor ("BuildFromDF", _strRegressionScenario) {
				...
				@Override public boolean preRegression() {
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							if (Double.isNaN (_adblDiscountFactorFlatRate[i] = _dc.getDF (_adblDate[i] =
				...
				@Override public boolean execRegression() {
					if (null == (_dcFromDF = DiscountCurve.BuildFromDF (_dtStart, _strCurrency, _adblDate,
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							...
							rnvd.set ("DiscountFactorFlatRate[" + new JulianDate (_adblDate[i]) + "]",
								FIGen.FormatSpreadSimple (_adblDiscountFactorFlatRate[i], 1, 3, 1));

							rnvd.set ("DiscountFactorDFCurve[" + new JulianDate (_adblDate[i]) + "]",
								FIGen.FormatSpreadSimple (dblDiscountFactorDFCurve, 1, 3, 1));

							if (!RegressionUtil.WithinTolerance (dblDiscountFactorDFCurve,
			...
			_setRegressors.add (new UnitRegressionExecutor ("CreateDC", _strRegressionScenario) {
				...
				@Override public boolean preRegression() {
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							if (Double.isNaN (_adblRate[i] = _dc.calcImpliedRate (_adblDate[i] =
								_dtStart.addYears (i + 1).getJulian())))
								return false;
						} catch (Exception e) {
							e.printStackTrace();
				...
				@Override public boolean execRegression() {
					if (null == (_dcFromRates = DiscountCurve.CreateDC (_dtStart, _strCurrency, _adblDate,
						_adblRate)))
						return false;
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							...
							rnvd.set ("DiscountFactorOriginalDC[" + new JulianDate (_adblDate[i]) + "]",
								FIGen.FormatSpreadSimple (_adblRate[i], 1, 3, 1));

							rnvd.set ("DiscountFactoRateImplied[" + new JulianDate (_adblDate[i]) + "]",
								FIGen.FormatSpreadSimple (dblRate, 1, 3, 1));

							if (!RegressionUtil.WithinTolerance (dblRate, _adblRate[i])) return false;
						} catch (Exception e) {
							e.printStackTrace();
					...
					for (int i = 0; i < _aCalibComp.length; ++i) {
						...
						try {
							...
							rnvd.set ("CompQuote" + "_" + strCalibCompCode + "{" + dt + "}",
								FIGen.FormatSpreadSimple (dblQuote, 1, 4, 1));

							rnvd.set ("NodeQuote" + "_" + strCalibCompCode + "{" + dt + "}",
								FIGen.FormatSpreadSimple (_adblQuote[i], 1, 4, 1));
						} catch (Exception e) {
							e.printStackTrace();
			...
			_setRegressors.add (new UnitRegressionExecutor ("ParallelShiftedCurve", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					if (null == (_dcShifted = _dc.createParallelShiftedCurve (0.0004))) return false;
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < aCalibComp.length; ++i) {
						...
						try {
							...
							rnvd.set ("BaseCurve" + "_" + strCalibCompCode, FIGen.FormatSpreadSimple
								(dblQuote, 1, 4, 1));

							rnvd.set ("ParallelShiftedCurve" + "_" + strCalibCompCode,
								FIGen.FormatSpreadSimple (dblQuoteShifted, 1, 4, 1));
						} catch (Exception e) {
							e.printStackTrace();
			...
			_setRegressors.add (new UnitRegressionExecutor ("RateShiftedCurve", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					if (null == (_dcShifted = _dcFromFlatRate.createParallelShiftedCurve (0.0004)))
						return false;
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							if (Double.isNaN (adblRate[i] = _dcFromFlatRate.calcImpliedRate (adblDate[i] =
								_dtStart.addYears (i + 1).getJulian())))
								return false;

							if (Double.isNaN (adblRateShifted[i] = _dcShifted.calcImpliedRate (adblDate[i] =
								_dtStart.addYears (i + 1).getJulian())))
								return false;

							JulianDate dt = new JulianDate (adblDate[i]);

							rnvd.set ("RateBase[" + dt + "]", FIGen.FormatSpreadSimple (adblRate[i], 1, 4,
								1));

							rnvd.set ("RateShifted[" + dt + "]", FIGen.FormatSpreadSimple
								(adblRateShifted[i], 1, 4, 1));

							if (!RegressionUtil.WithinTolerance (adblRate[i] + 0.0004, adblRateShifted[i]))
								return false;
						} catch (Exception e) {
							e.printStackTrace();
			...
			_setRegressors.add (new UnitRegressionExecutor ("BasisRateShiftedCurve", _strRegressionScenario)
			{
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							if (Double.isNaN (adblRate[i] = _dcFromFlatRate.calcImpliedRate (_adblDate[i])))
								return false;

							if (Double.isNaN (adblRateShifted[i] = _dcBasisShifted.calcImpliedRate
								(_adblDate[i])))
								return false;

							JulianDate dt = new JulianDate (_adblDate[i]);

							rnvd.set ("RateBasisEmpty[" + dt + "]", FIGen.FormatSpreadSimple (adblRate[i], 1,
								4, 1));

							rnvd.set ("RateBasisShifted[" + dt + "]", FIGen.FormatSpreadSimple
								(adblRateShifted[i], 1, 4, 1));
						} catch (Exception e) {
							e.printStackTrace();
			...
			_setRegressors.add (new UnitRegressionExecutor ("CreateTweakedCurve", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					if (null == (_dcNTP = _dcFromFlatRate.createTweakedCurve (_ntp))) return false;
				...
				@Override public boolean postRegression (final RegressionRunDetail rnvd) {
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							...
							if (Double.isNaN (adblRateNTP[i] = _dcNTP.calcImpliedRate (_adblDate[i])))
								return false;

							JulianDate dt = new JulianDate (_adblDate[i]);

							rnvd.set ("RateUnTweaked[" + dt + "]", FIGen.FormatSpreadSimple (adblRate[i], 1,
								4, 1));

							rnvd.set ("RateTweaked[" + dt + "]", FIGen.FormatSpreadSimple (adblRateNTP[i], 1,
								4, 1));

							if (!RegressionUtil.WithinTolerance (adblRate[i] + 0.0005, adblRateNTP[i]))
								return false;
						} catch (Exception e) {
							e.printStackTrace();
			...
			_setRegressors.add (new UnitRegressionExecutor ("EffectiveDF", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							if (Double.isNaN (_adblDiscountFactor[i] = _dc.getEffectiveDF ((i + 1) + "Y", (i
								+ 2) + "Y")))
								return false;
						} catch (Exception e) {
							e.printStackTrace();
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							rnvd.set ("Effective[" + new JulianDate (_adblDate[i]) + "]",
								FIGen.FormatSpreadSimple (_adblDiscountFactor[i], 1, 4, 1));
						} catch (Exception e) {
							e.printStackTrace();
			...
			_setRegressors.add (new UnitRegressionExecutor ("CalcImpliedRate", _strRegressionScenario) {
				...
				@Override public boolean execRegression() {
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							if (Double.isNaN (_adblImpliedRate[i] = _dc.calcImpliedRate ((i + 1) + "Y", (i +
								2) + "Y")))
								return false;
						} catch (Exception e) {
							e.printStackTrace();
					...
					for (int i = 0; i < NUM_DC_INSTRUMENTS; ++i) {
						try {
							rnvd.set ("ImpliedRate[" + new JulianDate (_adblDate[i]) + "]",
								FIGen.FormatSpreadSimple (_adblImpliedRate[i], 1, 4, 1));
						} catch (Exception e) {
							e.printStackTrace();
