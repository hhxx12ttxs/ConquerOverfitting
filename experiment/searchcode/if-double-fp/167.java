...
import org.jikesrvm.dsu.DsuOnStackReplacementEvent;
import org.jikesrvm.dsu.DsuThread;
import org.jikesrvm.dsu.GCSafeBridge;
...
public class RVMThread extends ThreadContext {
  ...
  static final BlockAdapter[] blockAdapters = new BlockAdapter[] {
      suspendBlockAdapter, handshakeBlockAdapter };
  ...
  public static final int DSUOSR = 6;
  ...
  private DsuOnStackReplacementEvent dsuOnStackReplacementEvent = null;
   ...
   */
  public static class OSRSwitchInfo {
    public final GCSafeBridge bridge;
    /** Foo frame pointer offset */
    public final Offset fooFPOffset;
    /** Thread switch frame pointer offset */
    public final Offset tsFPOffset;

    public OSRSwitchInfo(GCSafeBridge bridge, Offset fooFPOffset, Offset tsFPOffset) {
      this.bridge = bridge;
      this.fooFPOffset = fooFPOffset;
      this.tsFPOffset = tsFPOffset;
    }
  }

  public OSRSwitchInfo[] osrSwitchInfo;
  public int osrSwitchInfoIndex;

  /**
   * Thread local slot to store return value of a callee when performing OSR.
   * @see BaselineExecutionStateExtractor.doesCalleeReturnAValue
   */
  @SuppressWarnings("unused")
  private Object refTypeReturnValue;
  @SuppressWarnings("unused")
  private long primitiveReturnValue;
  ...
  /**
   * Flag set by external signal to request dsu activation at next thread
   * switch. See also: RunBootImage.C
   * Functionality is an exact copy of @see debugRequested
   */
  public static volatile boolean dsuRequested;

  /**
   * Flag indicating that DSU is currently in progress.
   */
  public static boolean dsuInProgress;
  ...
  /**
   * Lock (mutex) used for servicing dsu requests.
   */
  public static NoYieldpointsMonitor dsuLock;
  ...
  public static void boot() {
    ...
    dsuLock = new NoYieldpointsMonitor();
  ...
  @NoCheckStore
  public static final void checkDsuRequest() {
    if (dsuRequested && !dsuInProgress) {
      dsuLock.lockNoHandshake();
      if (dsuRequested && !dsuInProgress) {
        dsuInProgress = true;
        DsuThread.wakeupDsuThread();
      }
      dsuLock.unlock();
    }
  }
  ...
  void timerTick() {
    ...
    doAsynchronousRuntimeProfiling();
  }
    ...
    for (;;) {
      ...
      for (int i=0;i<numToHandshake;++i) {
        ...
        if (t.blockedFor(handshakeBlockAdapter) ||
            notRunning(t.asyncBlock(handshakeBlockAdapter))) {
      ...
      for (int i=0;i<numToHandshake;++i) {
        RVMThread t=handshakeThreads[i];
        observeExecStatusAtSTW(t.block(handshakeBlockAdapter));
        handshakeThreads[i]=null; // help GC
      }
    }
    handshakeLock.unlock();
  ...
  /** Is this an application thread? */
  public final boolean isApplicationThread() {
    return !isSystemThread() && isAlive();
  }
  ...
  /**
   * @return the dsuOnStackReplacementEvent
   */
  public DsuOnStackReplacementEvent getDsuOnStackReplacementEvent() {
    return dsuOnStackReplacementEvent;
  }

  /**
   * @param dsuOnStackReplacementEvent the dsuOnStackReplacementEvent to set
   */
  public void setDsuOnStackReplacementEvent(DsuOnStackReplacementEvent dsuOnStackReplacementEvent) {
    this.dsuOnStackReplacementEvent = dsuOnStackReplacementEvent;
  }
  ...
  //////////////////////// Asynchronous Runtime Profiling /////////////////////////
  
  int inLockingPath;
  
  static int samplesInLockingPath;
  static int samplesNotInLockingPath;
  
  public static void reportRuntimeProfile() {
    VM.sysWriteln("Runtime Profile:");
    VM.sysWriteln("  Locking path: ",samplesInLockingPath,"/",
                  samplesInLockingPath+samplesNotInLockingPath," (",
                  (double)samplesInLockingPath/((double)samplesInLockingPath+
                                                samplesNotInLockingPath),
                  ")");
  }
  
  void doAsynchronousRuntimeProfiling() {
    if (shouldBeSampled()) {
      if (inLockingPath>0) {
        samplesInLockingPath++;
      } else {
        samplesNotInLockingPath++;
      }
    }
  ...
  static void enterLockingPath() {
    RVMThread t=getCurrentThread();
    t.inLockingPath++;
  }
  
  static void leaveLockingPath() {
    RVMThread t=getCurrentThread();
    t.inLockingPath--;
    if (VM.VerifyAssertions) VM._assert(t.inLockingPath>=0);
  }
}
