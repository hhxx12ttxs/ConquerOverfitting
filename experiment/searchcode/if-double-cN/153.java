 ...
 * 
 * <p>
 * This is an old file that respect a large part of the GML syntax but not all.
 * It also adds extensions to allow to include other files and to read and store
...
public class FileSourceGML extends FileSourceBase {
	...
	/**
	 * Gives access to the currently parsed element (a graph, an edge or a
	 * node).
	 * 
		...
		do {
			...
			if (w.equals("directed")) {
				...
				if (inInclude > 0) {
					...
					if (!finished)
						parseError("expecting `]' here, got EOF inlude="
								+ inInclude);
					return false;
				}
			} else {
	...
	protected String parseId() throws IOException {
		return getStringOrWordOrNumber(); // Allows the GML to parse single
											// unquoted words.
	}
	...
	protected boolean parseBooleanProperty(String property) throws IOException {
		...
		if ((!isTrue(s)) && (!isFalse(s)))
			parseError("expecting boolean value (0/1,\"true\"/\"false\",\"yes\"/\"no\",\"on\"/\"off\"), got `"
					+ s + "'");
	...
	protected void nodeEvent(Mode mode) throws IOException {
		...
		if (mode == Mode.ADD) {
			...
			if (CN.attrs != null) {
				for (String key : CN.attrs.keySet()) {
					Object value = CN.attrs.get(key);
					sendNodeAttributeChanged(graphName, CN.tag, key, null,
							value);
				}
			}
		}
		...
		if (CE.src == null || CE.trg == null) {
			if (CE.src == null && CE.trg == null) {
				...
				if (CE.attrs != null) {
					for (String key : CE.attrs.keySet()) {
						Object value = CE.attrs.get(key);
						sendEdgeAttributeChanged(graphName, CE.tag, key, null,
								value);
					}
				}
	...
	protected void parseEdgeContent() throws IOException {
		String w;
		boolean loop = true;
		...
		do {
			...
			if (key.equals("x") || key.equals("y") || key.equals("z")) {
				...
				if (type.equals("arc")) {
					...
					// parseError(
					// "expecting 'arc', 'bitmap', 'image', 'line', 'oval', 'polygon', 'rectangle' or 'text', got '"+type+"'"
					// );
				}
			} else if (key.equals("]")) {
				loop = false;
	 ...
	 */
	protected ArrayList<Point3> readPoints() throws IOException {
		boolean loop = true;
		String key = null;
	 ...
	 */
	protected Point3 readPoint() throws IOException {
		boolean loop = true;
		String key = null;
	 ...
	 */
	protected void parseUnknown(String unknown) throws IOException {
		...
		if (tok == '[') {
			...
			parseError("waiting a '[', a string constant, a word or a number, "
					+ gotWhat(tok));
		}
	}
	 ...
	 *            The name of the unknown attribute.
	 */
	protected void parseUnknownAndForget(String unknown) throws IOException {
		int tok = st.nextToken();
	 ...
	 * attribute and first '[' have been eaten already). The attribute must have
	 * a description to be parsed. Use {@link #addAttributeClass(String,String)}
	 * . If the parser as no information on what class to instantiate when this
	 * attribute is read, the attribute is only skipped.
	 * 
	 * @param unknown
	 *            The attribute name.
	 ...
	 */
	protected Object parseUnknownAttribute(String unknown) throws IOException {
		String w, s, c;
		boolean loop = true;
		Class<?> cls;
		Object obj;
		...
		if (c == null) {
			...
			System.err
					.printf("GraphReaderGML: ignoring attribute '%s', not class map.%n",
							unknown);
			...
			// System.err.printf(
			// "GraphReaderGML: mapping attribute '%s' to class '%s'.%n",
			// unknown, cls.getName() );
				...
				if (w.equals("]")) {
					...
					if (s.equals("[")) {
						try {
							...
							parseError("cannot modify field '" + w
									+ "' of class '" + cls.getName() + "': "
									+ e.getMessage());
						} catch (NoSuchFieldException e) {
							parseError("cannot find field '" + w
									+ "' of class '" + cls.getName() + "': "
									+ e.getMessage());
						}
					} else {
	...
	/**
	 * Recursive method to read the contents of a field that is in fact an
	 * object. If this object is itself composed of fields, they are read
	 * recursively.
	 * 
	 * @param fieldName
			...
			if (key.equals("]")) {
				...
				if (s.equals("[")) {
					try {
						...
						parseError("cannot modify field '" + key
								+ "' of class '" + cls.getName() + "': "
								+ e.getMessage());
					} catch (NoSuchFieldException e) {
						parseError("cannot find field '" + key + "' of class '"
	 ...
	 */
	protected Class<?> findClass(String cls) throws IOException {
		Class<?> c = null;

		try {
			c = Class.forName(cls);
		} catch (ExceptionInInitializerError e) {
			parseError("cannot initialize class `" + cls + "': "
					+ e.getMessage());
	 ...
	 */
	protected Object instanciateClass(Class<?> cls) throws IOException {
		Object obj = null;

		try {
			obj = cls.newInstance();
		} catch (ExceptionInInitializerError e) {
			parseError("cannot instanciate class `" + cls.getName() + "': "
					+ e.getMessage());
	 ...
	 * 
	 * @param cls
	 *            The class of the object where the value will be stored.
	 * @param obj
	 *            The object where the value will be stored.
	 * @param field
	 *            The field of the object where the value will be stored.
	 * @param value
	 *            The value to store.
	 */
	protected void storeFieldValue(Class<?> cls, Object obj, String field,
			...
			if (fields[i].getName().equals(field)) {
				...
				try {
					if (c.equals(Boolean.TYPE)) {
						...
						parseError("unknown field type `" + c.getName()
								+ "', cannot set its value to `" + value + "'");
					}
				} catch (IllegalAccessException e) {
					parseError("cannot access field `" + fields[i].getName()
							+ "' of class `" + c.getName() + "'");
				} catch (NumberFormatException e) {
					parseError("cannot transform value `" + value
							+ "' into a number (boolean, integer or real)");
				}
		...
		parseError("cannot find field `" + field + "' in class `"
				+ cls.getName() + "'");
	}
	 ...
	 */
	protected static class CurrentGraph extends CurrentThing {
		public CurrentGraph() {
		}
	}
	 ...
	 */
	protected static class CurrentEdge extends CurrentThing {
		protected static enum Direction {
			DIRECTED, UNDIRECTED, NOT_SPECIFIED
		}
	 ...
	 */
	protected static class CurrentNode extends CurrentThing {
		public String tag = null;
	}
}
