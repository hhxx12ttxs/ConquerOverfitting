...
public abstract class Z80 {
    ...
    /**
     * Reset all registers to power on state (simulate reset-pin)
     */
    public void reset() {
        PC(0);
        SP(0xffff);
    ...
    /**
     * process maskable interrupt (INT), if it is not blocked
     */
    private void execMaskableInterrupt() {
        externIntSignal = false;

        if (IFF1() == false) {
            // external interrupt are currently blocked - ignore
            OZvm.displayRtmMessage("INT blocked");
            return;
        } else {
            IFF1(false);
            IFF2(false);
