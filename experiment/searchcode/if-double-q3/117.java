...
public class test {
  public static void main(String[] args) {
    try {
      ...
      // REngine is the backend-agnostic API -- using eng instead of c makes
      // sure that we don't use Rserve extensions inadvertently
      REngine eng = c;
      ...
      }
      {
        ...
        if (q1[0] != true || q1[1] != false || q1[2] != false || q2[0] != false || q2[1] != true || q2[2] != false || q3[0] != false
          || q3[1] != false || q3[2] != true)
      ...
      }
      {
        ...
        c.parseAndEval("crossprod(m,m)").asDoubleMatrix();
        System.out.println("PASSED");
      }
      ...
      { /*
         ...
         * and vice versa. Only Rserve 0.6-2 and later support NAa in character
         * vectors.
         */
        /*
         * repression test: assigning empty string '' has failed in previous
         * versions of RserveEngine
         */
        ...
        if (nas.length != 4 || nas[0] != REXPLogical.FALSE || nas[1] != REXPLogical.FALSE || nas[2] != REXPLogical.TRUE
          || nas[3] != REXPLogical.FALSE)
      ...
      { // test control commands (works only when enabled and in Rserve 0.6-0
        // and higher only) - must be the last test since it closes the
        // connection and shuts down the server
        System.out.println("* Test control commands (this will fail if control commands are disabled) ...");
        System.out.println("  server eval");
        String key = "rn" + Math.random(); // generate a random number to
