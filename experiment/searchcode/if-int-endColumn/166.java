...
public final class ClassOrInterfaceDeclaration extends TypeDeclaration {
    ...
    public void removeMethod(String method) {
		...
		for (BodyDeclaration type : this.getMembers()) {
			if(type instanceof MethodDeclaration){
				MethodDeclaration me = (MethodDeclaration)type;
				if(me.description().equals(method)){
					remove = type;
					break;
				}
			}
		}
		if(remove!=null)this.getMembers().remove(remove);
	}
    
    public void removeMethod(MethodDeclaration me){
    	BodyDeclaration remove = null;
    	for (BodyDeclaration type : this.getMembers()) {
			if(type instanceof MethodDeclaration && type.equals(me)){
				remove = type;
				break;
			}
		}
