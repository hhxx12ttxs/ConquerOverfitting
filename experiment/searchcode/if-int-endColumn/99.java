package il.ac.technion.cs.ssdl.parsing; import static il.ac.technion.cs.ssdl.utils.DBC.require;
...
public class Location implements Serializable, Comparable<Location> {
	 ...
	 */
	public Location(InputSource inputResource, TokenInfo t) {
		this(inputResource, t.beginLine, t.beginColumn, t.endLine, t.endColumn, false);
	}

	public Location(InputSource ir, int beginLine, int beginColumn, int endLine, int endColumn) {
	 ...
	 */
	public Location(InputSource inputResource, int beginLine, int beginColumn, int endLine, int endColumn, boolean synthetic) {
	 ...
	 */
	public Location(InputSource inputResource, Position begin, Position end) {
	...
	private String computeFragment(InputSource ir) {
		try {
			final List<String> lines = ir.lines();
			require(end.line < lines.size(), "end=" + end + " lines.size=" + lines.size() + " lines=\n" + IO.concatLines(lines));
	...
	public int compareTo(Location other) {
		final int diff = sourceFileId - other.sourceFileId;
		if (diff != 0)
			return diff;
