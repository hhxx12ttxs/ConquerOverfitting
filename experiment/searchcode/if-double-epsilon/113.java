...
/**
 * This deterministic quantile estimator cuts a stream into chunks of equal size. If a chunks becomes active,
 * i.e. the predecessor of that chunk reached its maximum size, an instance of {@link GKQuantiles} is started.
 * Every time a chunk reached it's maximum size, a sequence of quantile queries creates a summary of this chunk.
 * Nothing but this summary will be stored for quantile queries.<br>
 * This implementation is an ensemble ansatz, because not all chunks will be stored during the whole time. There
 * is a parameter describing the maximum number of chunks. If the ensemble exceeds this number an the update mode 
 * is called. There are seven different kinds of update modes. Please see {@link #setUpdateMode(String)} for 
 * further informations.<br>
 * To compute an answer to a quantile query, the chunks get concatenated to a small summary representing the whole
 * stream.
 * 
...
public class EnsembleQuantiles extends DeterministicQuantileEstimator {
	 ...
	 */
	/**
	 * Because this class extends {@link DeterministicQuantileEstimator} this constructor
