...
public class STMatching {
	...
	public static void assignTransmissionProbability(){
		if(previousNodesList.size()>1){
			...
			//âtrueâ path from GPS_FIX_i-1 to GPS_FIX_i follows the shortest path from candidate node e to candidate node f
			...
			for(CandidateNode f:previousNodesList.get(previousNodesList.size()-1)){
				for(CandidateNode e:previousNodesList.get(previousNodesList.size()-2)){
					...
					if(e.bestMatch == true){
						...
						double distanceBetweenRawPoints = e.getParentFix().distanceTo(f.getParentFix());
						...
						locationOfTheNextPoint.setLatitude(f.getLatitude());
						locationOfTheNextPoint.setLongitude(f.getLongitude());
