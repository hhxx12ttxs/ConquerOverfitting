...
/**
 * Butcher5 implements a fifth order Runge-Kutta ODE solver using an algorithm
...
public class Butcher5 extends AbstractODESolver {
	...
	/**
	 * Constructs the Butcher5 ODESolver for a system of ordinary differential
	 * equations.
	 * 
	 * @param ode
	 *            the system of differential equations.
	 */
	public Butcher5(final ODE ode) {
		super(ode);
	}
	...
	public double step() {
		...
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i] + stepSize * (rate1[i] + rate2[i])
					/ 8;
		}
		...
		for (int i = 0; i < numEqn; i++) {
			state[i] = state[i]
					+ stepSize
					* (7 * rate1[i] + 32 * rate3[i] + 12 * rate4[i] + 32
							* rate5[i] + 7 * rate6[i]) / 90.0;
		}
		return stepSize;
	}
}
