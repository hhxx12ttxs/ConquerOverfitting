...
/**
 * ODEBisectionEventSolver is an ODEEventSolver that uses the bisection method
 * for root finding.
 * 
 * @author Francisco Esquembre (March 2004)
 */
public class ODEBisectionEventSolver implements ODEEventSolver,
		ODEAdaptiveSolver {
	...
	/**
	 * Creates a new solver that uses the bisection method for finding the
	 * events. Example of use: solver = new BisectionEventSolver
	 * (anOde,math.RK4.class); solver.addEvent(aStateEvent); // for the rest it
	 * works as any other ODESolver. Tested status: Tested with fixed step
	 * solvers. Tested with adaptive algorithms. Tested with interpolation
	 * algorithms. (See the checks for ODEInterpolationSolver!) Thought it could
	 * probably be improved a bit by changing initialize() to a
	 * (non-yet-existing) synchronize()... Performance is quite good in
	 * examples, though.
	 * 
	...
	public ODEBisectionEventSolver(final ODE ode, final Class<?> solverClass) {
		...
		try { // Create the solver by reflection
			...
			System.err.println("BisectionEventSolver: Solver class "
	...
	/**
	 * Gets the eventHappend flag. The falg is true if an event occured during
	 * the last step.
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Advances the ODE as usual, except if an event takes place. Then it finds
	 * the event point and applies the actions
	 * 
	...
	public double step() { // Step from t=a to t=b(=a+dt)
		...
		do {
			...
			happened.clear();
			for (Enumeration<StateEvent> e = eventList.elements(); e
					.hasMoreElements();) {
			...
			if (eventFound == null) { // Now find by subdivision
				...
				for (int i = 0; i < MAX; i++) { // Start the subdivision
					...
					if (previousFound != null) {
						 ...
						 */
						for (StateEvent evt : happened) {
							if (evt != previousFound
									&& (evt.evaluate(state) > -evt
											.getTolerance())) {
								happened.remove(evt);
							}
						}
				...
				if (eventFound == null) { // If this happens, the event is most
					...
					System.err
							.println("BisectionEventSolver Warning : Event not found after "
									+ MAX + " subdivisions!!!");
					System.err.println("  Event = " + eventFound);
					System.err
							.println("  Please check your event algorithm or decrease the initial stepTime.");
