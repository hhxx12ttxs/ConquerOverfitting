...
implements KeyListener, ControllerListener, MainWindowAPI {
    ...
    private void initComponents() {
        ...
        rangeCheckButton = new javax.swing.JButton();
        ...
        rangeCheckButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                rangeCheckButtonActionPerformed(evt);
            }
        });
    ...
    private void rangeCheckButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rangeCheckButtonActionPerformed
        try {
            ...
            for (String s : Files.readAllLines(this.gcodeFile.toPath(), cs)) {
                String[] splited = s.split("\\s+");
                for (int i=0; i<splited.length-1; ++i) {
                    try {
                        double n = Double.parseDouble(splited[i+1]);
                        String cmd = splited[i].toUpperCase();
                        if (!mins.containsKey(cmd) || mins.get(cmd) > n) {
                            mins.put(cmd, n);
                        }
                        if (!maxes.containsKey(cmd) || maxes.get(cmd) < n) {
                            maxes.put(cmd, n);
                        }
            ...
            for (Map.Entry<String, Double> e : mins.entrySet()) {
                System.out.println("min "+ e.getKey() +" = "+ e.getValue());            
            }
            for (Map.Entry<String, Double> e : maxes.entrySet()) {
                System.out.println("max "+ e.getKey() +" = "+ e.getValue());            
            }
...
//            timer.stop();
    ...
    }//GEN-LAST:event_rangeCheckButtonActionPerformed
    ...
    private void updateControlsForState(ControlState state) {
        ...
        switch (state) {
            case FILE_SELECTED:
                ...
                this.rangeCheckButton.setEnabled(true);
    ...
    private javax.swing.JButton rangeCheckButton;
