...
public class Fehlberg8 extends AbstractODESolver {
	private double[] rate1, rate2, rate3, rate4, rate5, rate6, rate7, rate8,
			rate9, rate10, rate11, rate12, rate13, estimated_state;
	 ...
	 */
	public Fehlberg8(final ODE ode) {
		super(ode);
	}
	...
	public double step() {
		...
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i] + stepSize
					* (rate1[i] + 3 * rate2[i]) / 36;
		}
		ode.getRate(estimated_state, rate3);
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i] + stepSize
					* (rate1[i] + 3 * rate3[i]) / 24;
		}
		ode.getRate(estimated_state, rate4);
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i]
					+ stepSize
					* (5. / 12. * rate1[i] - 25. / 16. * rate3[i] + 25. / 16. * rate4[i]);
		}
		ode.getRate(estimated_state, rate5);
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i] + stepSize
					* (rate1[i] + 5 * rate4[i] + 4 * rate5[i]) / 20;
		}
		ode.getRate(estimated_state, rate6);
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i]
					+ stepSize
					* (-25 * rate1[i] + 125 * rate4[i] - 260 * rate5[i] + 250 * rate6[i])
					/ 108;
		}
		...
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i]
					+ stepSize
					* (31. / 300. * rate1[i] + 61. / 225. * rate5[i] - 2. / 9.
							* rate6[i] + 13. / 900. * rate7[i]);
		}
		ode.getRate(estimated_state, rate8);
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i]
					+ stepSize
					* (2 * rate1[i] - 53. / 6. * rate4[i] + 704. / 45.
							* rate5[i] - 107. / 9. * rate6[i] + 67. / 90.
							* rate7[i] + 3 * rate8[i]);
		}
		...
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i]
					+ stepSize
					* (-91. / 108. * rate1[i] + 23. / 108. * rate4[i] - 976.
							/ 135. * rate5[i] + 311. / 54. * rate6[i] - 19.
							/ 60. * rate7[i] + 17. / 6. * rate8[i] - rate9[i] / 12.);
		}
		ode.getRate(estimated_state, rate10);
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i]
					+ stepSize
					* (2383. / 4100. * rate1[i] - 341. / 164. * rate4[i]
							+ 4496. / 1025. * rate5[i] - 301. / 82. * rate6[i]
							+ 2133. / 4100. * rate7[i] + 45. / 82. * rate8[i]
		...
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i]
					+ stepSize
					* (3. / 205. * rate1[i] - 6. / 41. * rate6[i] - 3. / 205.
							* rate7[i] - 3. / 41. * rate8[i] + 3. / 41.
							* rate9[i] + 6. / 41. * rate10[i]);
		}
		ode.getRate(estimated_state, rate12);
		for (int i = 0; i < numEqn; i++) {
			estimated_state[i] = state[i]
					+ stepSize
					* (-1777. / 4100. * rate1[i] - 341. / 164. * rate4[i]
							+ 4496. / 1025. * rate5[i] - 289. / 82. * rate6[i]
							+ 2193. / 4100. * rate7[i] + 51. / 82. * rate8[i]
		...
		for (int i = 0; i < numEqn; i++) {
			state[i] = state[i]
					+ stepSize
					* (34. / 105. * rate6[i] + 9. / 35. * rate7[i] + 9. / 35.
							* rate8[i] + 9. / 280. * rate9[i] + 9. / 280.
							* rate10[i] + 41. / 840. * rate12[i] + 41. / 840. * rate13[i]);
		}
		return stepSize;
	}
}
