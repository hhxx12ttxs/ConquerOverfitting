 ...
 *******************************************************************************/
package org.eclipse.core.patch.core.internal;
...
import org.eclipse.core.patch.core.IFilePatch;
import org.eclipse.core.patch.core.IFilePatchResult;
import org.eclipse.core.patch.core.IHunk;
import org.eclipse.core.patch.core.PatchConfiguration;
import org.eclipse.core.patch.core.ReaderCreator;
...
public class FileDiffResult implements IFilePatchResult {
	...
	public FileDiffResult(FilePatch diff, PatchConfiguration configuration) {
		super();
		this.fDiff = diff;
	 ...
	 */
	 public void refresh(ReaderCreator content) {
		this.fMatches= false;
		...
		if (this.fDiff.getDiffType(getConfiguration().isReversed()) == FilePatch.ADDITION) {
			if ((!exists || isEmpty(content)) && canCreateTarget(content)) {
				this.fMatches= true;
			} else {
				// file already exists
				this.fDiffProblem= true;
				this.fErrorMessage= "File already exists";
			}
			create= true;
			...
			if (exists) {
				...
				// file doesn't exist
				this.fDiffProblem= true;
				this.fErrorMessage= "File doesn't exists";
			}
		}
		...
		if (this.fDiffProblem) {
			...
			// results for display
			this.fBeforeLines = new ArrayList<String>(getLines(content, false));
			this.fAfterLines = this.fMatches ? new ArrayList<String>() : this.fBeforeLines;
	...
	public void patch(List<String> lines) {
		...
		if (getConfiguration().getFuzz() != 0) {
			calculateFuzz(this.fBeforeLines);
		}
		int shift = 0;
		IHunk[] hunks = this.fDiff.getHunks();
		for (int i = 0; i < hunks.length; i++) {
	 ...
	 */
	public int calculateFuzz(List<String> lines) {
		...
		for (int j = 0; j < hunks.length; j++) {
			...
			result.setShift(shift);
			int fuzz = result.calculateFuzz(lines);
			shift = result.getShift();
			if (fuzz > highestFuzz)
				highestFuzz = fuzz;
		}
	...
	public List<IHunk> getFailedHunks() {
		List<IHunk> failedHunks = new ArrayList<IHunk>();
		IHunk[] hunks = this.fDiff.getHunks();
		for (int i = 0; i < hunks.length; i++) {
	...
	public IHunk[] getRejects() {
		List<IHunk> failedHunks = getFailedHunks();
