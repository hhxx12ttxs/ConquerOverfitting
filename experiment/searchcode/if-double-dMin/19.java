		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				if (!isCustomType) {
					...
					String clazz = ((JavaTypeEnum) cbxJavatype.getSelectedItem()).getClazz();
				...
				enableMinMaxValue(((JavaTypeEnum) cbxJavatype.getSelectedItem()).getClazz());
				if (!isCustomType) {
					setInputValidation(selectedType.getJavaType(), model.getAttribute().getInputValidation());
				} else {
		...
		if(model.getAttribute().getDatatyp().getJavaType().equals("java.util.Date")) {
			...
			if(!(dMin != null && dMax != null)) {
				throw new CommonValidationException(SpringLocaleDelegate.getInstance().getMessage(
						"wizard.step.attributeproperties.24", "Mindest- und Maximalwert m?ssen gesetzt sein."));
			}
