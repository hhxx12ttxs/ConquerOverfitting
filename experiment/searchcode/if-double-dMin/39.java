...
public final class StockDataSetProcessorImpl extends DataSetAdapter
{
	 ...
	 * (creating one if necessary). For the StockDataElement, the data should be
	 * provided in the form: 'H <high value> L <low value> O <open value> C
	 * <close value>, H <next high value> L...' i.e. 'H', 'L', 'O' and 'C' are
	 * used to designate a value as either the high, low, open or close
			...
			throws ChartException
	{
		...
		while ( strTokDataElement.hasMoreTokens( ) )
		{
			...
			if ( strTokComponents.countTokens( ) == 1 )
			{
				...
				vData.add( new StockEntry( dComponent,
						dComponent - 2,
						dComponent * 2,
						dComponent + 2 ) );
				continue;
			...
			StockEntry entry = new StockEntry( Double.NaN,
					Double.NaN,
					Double.NaN,
					Double.NaN );
	...
	private StringBuffer toStockString( Object[] stockArray )
			throws ChartException
	{
