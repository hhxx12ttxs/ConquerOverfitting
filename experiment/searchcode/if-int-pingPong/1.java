...
void R.AliasProjectFinalVert (finalvert_t *fv, auxvert_t *av);
void R.Alias_clip_top (finalvert_t *pfv0, finalvert_t *pfv1,
	finalvert_t *out);
void R.Alias_clip_bottom (finalvert_t *pfv0, finalvert_t *pfv1,
	finalvert_t *out);
void R.Alias_clip_left (finalvert_t *pfv0, finalvert_t *pfv1,
	finalvert_t *out);
void R.Alias_clip_right (finalvert_t *pfv0, finalvert_t *pfv1,
	finalvert_t *out);
...
================
*/
void R.Alias_clip_z (finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out)
{
	float		scale;
	...
	R.AliasProjectFinalVert (out, &avout);
...
void R.Alias_clip_left (finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out)
{
	float		scale;
	int			i;
...
void R.Alias_clip_top (finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out)
{
	float		scale;
	int			i;
...
int R.AliasClip (finalvert_t *in, finalvert_t *out, int flag, int count,
...
================
*/
void R.AliasClipTriangle (mtriangle_t *ptri)
{
	int				i, k, pingpong;
	mtriangle_t		mtri;
	unsigned		clipflags;
...
// clip
	...
	if (clipflags & ALIAS_Z_CLIP)
	{
		...
		k = R.AliasClip (fv[0], fv[1], ALIAS_Z_CLIP, 3, R.Alias_clip_z);
		if (k == 0)
			return;
	...
	{
		k = R.AliasClip (fv[pingpong], fv[pingpong ^ 1],
							ALIAS_LEFT_CLIP, k, R.Alias_clip_left);
		if (k == 0)
			return;
	...
	{
		k = R.AliasClip (fv[pingpong], fv[pingpong ^ 1],
							ALIAS_RIGHT_CLIP, k, R.Alias_clip_right);
		if (k == 0)
			return;
	...
	{
		k = R.AliasClip (fv[pingpong], fv[pingpong ^ 1],
							ALIAS_BOTTOM_CLIP, k, R.Alias_clip_bottom);
		if (k == 0)
			return;
	...
	{
		k = R.AliasClip (fv[pingpong], fv[pingpong ^ 1],
							ALIAS_TOP_CLIP, k, R.Alias_clip_top);
		if (k == 0)
			return;
