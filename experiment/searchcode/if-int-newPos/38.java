...
public class Bot extends LocalPlayerObject {
	...
	*/
	...
	public boolean nextStep(BotClient client, World world, long ticks, Level level, ClientObjectManager com, LocalBombManager lbm, List<SimpleVector> playerPos, PlayerDummy dummy) {
		...
		if (!isDead() && this.getAnimation()!=Animations.DIE && this.getAnimation()!=Animations.DEAD) {
			...
			if (path==null || pathIndex==path.length || targetScheduled>NO_NEW_TARGET) {
				...
				GridPosition newTarget=finder.findTarget(this, new AStar(mask, true), decisionMask, level, lbm, playerPos2, targetScheduled);
				if (newTarget!=null) {
