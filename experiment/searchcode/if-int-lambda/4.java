...
/**
 * MuCommaLambdaBreeder is a Breeder which, together with ESSelection,
 * implements the (mu,lambda) breeding strategy and gathers the comparison data
 * you can use to implement a 1/5-rule mutation mechanism.
 * 
 * <p>
 * Evolution strategies breeders require a "mu" parameter and a "lambda"
 * parameter for each subpopulation. "mu" refers to the number of parents from
 * which the new population will be built. "lambda" refers to the number of
 * children generated by the mu parents. Subpopulation sizes will change as
 * necessary to accommodate this fact in later generations. The only rule for
 * initial subpopulation sizes is that they must be greater than or equal to the
 * mu parameter for that subpopulation.
 * 
 * <p>
 * You can now set your initial subpopulation size to whatever you like, totally
 * independent of lambda and mu, as long as it is &gt;= mu.
 * 
 * <p>
 * MuCommaLambdaBreeder stores mu and lambda values for each subpopulation in
 * the population, as well as comparisons. A comparison tells you if &gt;1/5,
 * &lt;1/5 or =1/5 of the new population was better than its parents (the
 * so-called evolution strategies "one-fifth rule". Although the comparisons are
 * gathered, no mutation objects are provided which actually <i>use</i> them --
 * you're free to use them in any mutation objects you care to devise which
 * requires them.
 * 
 * <p>
 ...
 * the same parent. They only increment to the next parent in the pool of mu
 ...
 * <table>
 * <tr>
 * <td valign=top>es.lambda.<i>subpop-num</i><br>
 * <font size=-1>int >= 0</font></td>
 * <td>Specifies the 'lambda' parameter for the subpopulation.</td>
 * </tr>
 * <tr>
 * <td valign=top>es.mu.<i>subpop-num</i><br>
 * <font size=-1>int: a multiple of "lambda"</font></td>
 * <td>Specifies the 'mu' parameter for the subpopulation.</td>
 * </tr>
 * </table>
 * 
 * @author Sean Luke
...
public class MuCommaLambdaBreeder extends Breeder {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        for (int x = 0; x < size; x++) {
            ...
            else if ((lambda[x] / mu[x]) * mu[x] != lambda[x]) // note integer
                                                               // division
    ...
    /**
     * Sets all subpopulations in pop to the expected lambda size. Does not fill
     * new slots with individuals.
     */
    ...
    public Population breedPopulation(EvolutionState state) {
        ...
        if (parentPopulation != null) {
            // Only go from 0 to lambda-1, as the remaining individuals may be
            // parents.
            // A child C's parent's index I is equal to C / mu[subpopulation].
            for (int x = 0; x < state.population.subpops.length; x++) {
                int numChildrenBetter = 0;
                for (int i = 0; i < lambda[x]; i++) {
                    int parent = i / (lambda[x] / mu[x]); // note integer
                                                          // division
                    if (state.population.subpops[x].individuals[i].fitness
                            .betterThan(parentPopulation.subpops[x].individuals[parent].fitness))
                        numChildrenBetter++;
                }
                if (numChildrenBetter > lambda[x] / 5.0) // note float division
                    comparison[x] = C_OVER_ONE_FIFTH_BETTER;
                else if (numChildrenBetter < lambda[x] / 5.0) // note float
                                                              // division
        ...
        if (mu.length != state.population.subpops.length) // uh oh
            state.output
                    .fatal("For some reason the number of subpops is different than was specified in the file (conflicting with Mu and Lambda storage).",
                            null);

        // next, load our population, make sure there are no subpopulations
        // smaller than the mu's
        for (int x = 0; x < state.population.subpops.length; x++) {
        ...
        for (int x = 0; x < state.population.subpops.length; x++) {
            ...
            java.util.Arrays.sort(i, new java.util.Comparator() {
                public int compare(Object o1, Object o2) {
                    ...
                    if (b.fitness.betterThan(a.fitness))
                        return 1;
                    // return -1 if a should appear before object b in the
                    // array.
        ...
        for (int y = 0; y < state.breedthreads; y++)
            for (int x = 0; x < state.population.subpops.length; x++) {
                ...
                if (y < state.breedthreads - 1) // not last one
                    ...
                    numinds[y][x] = lambda[x] / state.breedthreads + (lambda[x] - (lambda[x] / state.breedthreads) // note
                                                                                                                   // integer
                                                                                                                   // division
                            * state.breedthreads);
    ...
    public void breedPopChunk(Population newpop, EvolutionState state, int[] numinds, int[] from, int threadnum) {
        for (int subpop = 0; subpop < newpop.subpops.length; subpop++) {
            // reset the appropriate count slot -- this used to be outside the
            // for-loop, a bug
            // I believe
            count[threadnum] = 0;
            ...
            if (count[threadnum] == 0) // the ESSelection didn't set it to
                                       // nonzero to inform us of his existence
                state.output
                        .warning("Whoa!  Breeding Pipeline for subpop "
                                + subpop
                                + " doesn't have an ESSelection, but is being used by MuCommaLambdaBreeder or MuPlusLambdaBreeder.  That's probably not right.");
            // reset again
            count[threadnum] = 0;
