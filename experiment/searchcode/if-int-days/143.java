 ...
 *
 */
public class UPRScheduleSTAT extends UPRSchedule {
	 ...
	 * 
	 */
	public UPRScheduleSTAT(Money upfrontPremium, double cedingRate, DebtServiceSchedule dss, PremiumSchedule ps) {
		computeSTATSchedule(upfrontPremium, cedingRate, dss, ps);
	}
	
	public void computeSTATSchedule(Money upfrontPremium, double cedingRate, DebtServiceSchedule dss, PremiumSchedule ps) {
		...
		for (Iterator<DebtService> ids = dss.iterator(); ids.hasNext(); ) {
			...
			_schedule.add(new UPRRowSTAT(ds.getDate(), ds.getParBefore(), ds.getParAfter(), 
		...
		for (Iterator<PremiumPayment> ips = ps.iterator(); ips.hasNext(); ) {
			...
			UPRRowSTAT row = (UPRRowSTAT)getUPRRowOn(pp.getDate());
			if (row == null) {
				//
				// This is our first row, before debt service, when we pay our upfront premium and 
				// anchor the table.
				//
				System.out.println("Getting prin reminaing on " + pp.getDate());
				_schedule.add(0, new UPRRowSTAT(pp.getDate(), dss.getParBefore(pp.getDate()), dss.getParAfter(pp.getDate()), 
						pp.getUpfrontAmount(), pp.getInstallmentAmount(), 
		...
		double factor = upfrontPremium.doubleValue() / totalDSDays;
		BAMDate prevDate = null;
		if (debug || BobUtil.masterDebug) {
			System.out.println("STAT UPR Total DS days = " + BobUtil.nf.format(totalDSDays) + ", upfrontPremium = " 
					+ upfrontPremium + ", factor = " + factor);
		}
		for (Iterator<UPRRow> iupr = _schedule.iterator(); iupr.hasNext(); ) {
			UPRRowSTAT row = (UPRRowSTAT)iupr.next();
			if (row.getUpfrontPremium().notEquals(0)) {				// WAS > 0.0
				...
				Money cededPremEarned = new Money(earnedPremium.doubleValue() * cedingRate);
		...
		for (int i = 1; i <= ps.numInstallmentPremiums(); i++) {
			...
			for (; iupr.hasNext(); ) {
				...
				Money earnedPremium = new Money((days / totalInstallDays) * installPrem.doubleValue());
		...
		//
		// 5.  Compute the UPR on all rows.  Add in any premiums, subtract any earned premiums.
		//
		Money UPR = Money.Zero;
		for (Iterator<UPRRow> iupr = _schedule.iterator(); iupr.hasNext(); ) {
			UPRRow row = iupr.next();
			//System.out.println("Setting UPR on " + row.getDate() + ", UPR from prior row was " + Policy.nf.format(UPR)
			//		+ ", installPrem = " + Policy.nf.format(row.getInstallPremium())
			//		+ ", prem earned = " + Policy.nf.format(row.getPremiumEarned()));
			UPR = UPR.add(row.getInstallPremium()).add(row.getUpfrontPremium()).minus(row.getPremiumEarned());
			//System.out.println("\tComputed UPR = " + Policy.nf.format(UPR));
			row.setUPR(UPR);
	 ...
	 * @param matID
	 */
	public UPRScheduleSTAT(Connection c, String policyID) throws SQLException {
		// NOT IMPLEMENTED YET
	}
