 ...
 * 
 * <p>
 * The class also contains utility methods or computing pareto dominance, Pareto
 * Fronts and Pareto Front Ranks, and distance in multiobjective space. The
 * default comparison operators use Pareto Dominance, though this is often
 * overridden by subclasses.
 * 
 * <p>
 * The fitness() method returns the maximum of the fitness values, which is
 * clearly nonsensical: you should not be using this method.
 * 
 * <p>
 * Subclasses of this class may add certain auxiliary fitness measures which are
 * printed out by MultiObjectiveStatistics along with the multiple objectives.
 * To have these values printed out, override the getAuxiliaryFitnessNames() and
 * getAuxiliaryFitnessValues() methods.
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
...
public class MultiObjectiveFitness extends Fitness {
    ...
    /**
     * Returns auxilliary fitness value names to be printed by the statistics
     * object. By default, an empty array is returned, but various algorithms
     * may override this to provide additional columns.
     */
     ...
     * By default, an empty array is returned, but various algorithms may
    ...
    public void setup(EvolutionState state, Parameter base) {
        ...
        for (int i = 0; i < numFitnesses; i++) {
            ...
            minObjective[i] = state.parameters.getFloatWithDefault(base.push(P_MINOBJECTIVES),
                    def.push(P_MINOBJECTIVES), 0.0f);
            maxObjective[i] = state.parameters.getFloatWithDefault(base.push(P_MAXOBJECTIVES),
                    def.push(P_MAXOBJECTIVES), 1.0f);

            // load specifics if any
            minObjective[i] = state.parameters.getFloatWithDefault(base.push(P_MINOBJECTIVES).push("" + i),
                    def.push(P_MINOBJECTIVES).push("" + i), minObjective[i]);
            maxObjective[i] = state.parameters.getFloatWithDefault(base.push(P_MAXOBJECTIVES).push("" + i),
     ...
     * equal in all criteria, then equivalentTo is true. Multiobjective
     * optimization algorithms may choose to override this to do something else.
     */
    ...
    /**
     * Returns true if I'm better than _fitness. The DEFAULT rule I'm using is
     * this: if I am better in one or more criteria, and we are equal in the
     * others, then betterThan is true, else it is false. Multiobjective
     * optimization algorithms may choose to override this to do something else.
     */
    ...
    /**
     * Divides an array of Individuals into the Pareto front and the "nonFront"
     * (everyone else). The Pareto front is returned. You may provide ArrayLists
     * for the front and a nonFront. If you provide null for the front, an
     * ArrayList will be created for you. If you provide null for the nonFront,
     * non-front individuals will not be added to it. This algorithm is O(n^2).
     */
    public static ArrayList partitionIntoParetoFront(Individual[] inds, ArrayList front, ArrayList nonFront) {
        ...
        for (int i = 1; i < inds.length; i++) {
            ...
            for (int j = 0; j < frontSize; j++) {
                ...
                if (((MultiObjectiveFitness) (frontmember.fitness))
                ...
                else if (((MultiObjectiveFitness) (ind.fitness))
                        .paretoDominates((MultiObjectiveFitness) (frontmember.fitness))) {
                    yank(j, front);
                    // a front member is dominated by the new individual.
                    // Replace him
                    frontSize--; // member got removed
                    j--; // because there's another guy we now need to consider
                         // in his place
                    if (nonFront != null)
    ...
    /**
     * Returns the sum of the squared difference between two Fitnesses in
     * Objective space.
     */
    ...
    /**
     * Returns the Manhattan difference between two Fitnesses in Objective
     * space.
     */
