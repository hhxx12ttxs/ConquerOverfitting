...
public class BaseTest {
	...
	public static void deleteAllEvaluations() {
		...
		while (htmldiv.findElements(By.className("t_eval_delete")).size() > 0) {
	 ...
	 * appearing in the table Page: Coordinator home 
	...
	public static void verifyAddedCourse(String courseId, String courseName) {
		...
		assertEquals(courseId,
				getElementText(By.id("courseID0")));

		// Check for course name
		assertEquals(courseName,
				getElementText(By.id("courseName0")));

		// Check for default number of teams - 0
		assertEquals("0",
				getElementText(By
						.cssSelector("#dataform td.t_course_teams")));
	}
