 ...
 * 
 * 
 * This class contains certain information associated with a slave: its name,
 * connection socket, input and output streams, and the job queue. Additionally,
 * the class sets up an auxillary thread which reads and writes to the streams
 * to talk to the slave in the background. This thread uses the SlaveMonitor as
 * its synchronization point (it sleeps with wait() and wakes up when notified()
 * to do some work).
 * 
 * <P>
 * Generally SlaveConnection is only seen by communicates only with
 * SlaveMonitor.
 * 
...
class SlaveConnection {
    ...
    // given that we expect the slave to return the evaluated individuals in the
    // exact same order,
     ...
     * thread that is communicating with the remote slave to read back the
     * results of the evaluations.
     */
    public SlaveConnection(EvolutionState state, String slaveName, Socket evalSocket, DataOutputStream dataOut,
            DataInputStream dataIn, SlaveMonitor slaveMonitor) {
    ...
    /**
     * This method is called whenever there are any communication problems with
     * the slave (indicating possibly that the slave might have crashed). In
     * this case, the jobs will be rescheduled for evaluation on other slaves.
     */
    ...
    protected void shutdown(final EvolutionState state) {
        ...
        } // exception, not IOException, because JZLib throws some array
          // exceptions
        try {
            dataOut.flush();
        } catch (Exception e) {
        ...
        slaveMonitor.unregisterSlave(this); // unregister me BEFORE I reschedule
                                            // my jobs
        rescheduleJobs(state);
        synchronized (jobs) {
            // notify my threads now that I've closed stuff in case they're
            // still waiting
            slaveMonitor.notifyMonitor(jobs);
            reader.interrupt(); // not important right now but...
            writer.interrupt(); // very important that we be INSIDE the jobs
                                // synchronization here so the writer doesn't
                                // try to wait on the monitor again.
        }
    ...
    Job oldestUnsentJob() {
        ...
        // This all could have been O(1) if we had used two queues, but we're
        // being
    ...
    boolean writeLoop() {
        ...
        try {
            synchronized (jobs) {
                ...
                if ((job = oldestUnsentJob()) == null) // automatically marks as
                                                       // sent
                {
    ...
    boolean readLoop() {
        ...
        try {
            ...
            // okay, we've got a job. Grab the earliest job, that's what's
            // coming in
            ...
            for (int i = 0; i < job.newinds.length; i++) {
                ...
                if (i > 0)
                    val = dataIn.readByte(); // otherwise we've got it already
            ...
            // /// NEXT STEP: COPY THE NEWLY-READ INDIVIDUALS BACK INTO THE
            // ORIGINAL
            // /// INDIVIDUALS. THIS IS QUITE A HACK, IF YOU READ JOB.JAVA

            // Now we have all the individuals in so we're good. Copy them back
            // into the original individuals
            job.copyIndividualsBack(state);

            // /// LAST STEP: LET OTHERS KNOW WE'RE DONE AND AVAILABLE FOR
            // ANOTHER JOB

            // we're all done! Yank the job from the queue so others think we're
            // available
            synchronized (jobs) {
                jobs.removeFirst();
            }
    ...
    /**
     * Adds a new jobs to the queue. This implies that the slave will be in
     * charge of executing this particular job.
     */
    ...
    /**
     * Reschedules the jobs in this job queue to other slaves in the system. It
     * assumes that the slave associated with this queue has already been
     * removed from the available slaves, such that it is not assigned its own
     * jobs.
     */
    // only called when we're shutting down, so we're not waiting for any
    // notification.
