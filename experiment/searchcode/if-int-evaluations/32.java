...
/**
 * This subclass of EvolutionState implements basic Steady-State Evolution and
 * (in distributed form) Asynchronous Evolution. The procedure is as follows. We
 * begin with an empty Population and one by one create new Indivdiuals and send
 * them off to be evaluated. In basic Steady-State Evolution the individuals are
 * immediately evaluated and we wait for them; but in Asynchronous Evolution the
 * individuals are evaluated for however long it takes and we don't wait for
 * them to finish. When individuals return they are added to the Population
 * until it is full. No duplicate individuals are allowed.
 * 
 * <p>
 ...
 * 
 * <p>
 * SteadyStateEvolutionState will run either for some N "generations" or for
 * some M evaluations of individuals. A "generation" is defined as a
 * Population's worth of evaluations. If you do not specify the number of
 * evaluations (the M), then SteadyStateEvolutionState will use the standard
 ...
 * <table>
 * <tr>
 * <td valign=top><tt>evaluations</tt><br>
 * <font size=-1>int &gt;= 1</font></td>
 * <td valign=top>(maximal number of evaluations to run.)</td>
 * </tr>
 * </table>
 * 
 * 
...
public class SteadyStateEvolutionState extends EvolutionState {
    ...
    /**
     * How many evaluations should we run for? If set to UNDEFINED (0), we run
     * for the number of generations instead.
     */
    public long numEvaluations;
    public static long UNDEFINED = 0;
    /**
     * how big is a generation? Set to the size of subpopulation 0 of the
     * initial population.
     */
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (!(breeder instanceof SteadyStateBreeder))
            state.output
                    .error("You've chosen to use Steady-State Evolution, but your breeder is not of the class SteadyStateBreeder.",
                            base);
        if (!(evaluator instanceof SteadyStateEvaluator))
            state.output
                    .error("You've chosen to use Steady-State Evolution, but your evaluator is not of the class SteadyStateEvaluator.",
                            base);
        if (!(exchanger instanceof SteadyStateExchangerForm))
            state.output
                    .error("You've chosen to use Steady-State Evolution, but your exchanger does not implement the SteadyStateExchangerForm.",
                            base);
    ...
    public void startFresh() {
        ...
        population = initializer.setupPopulation(this, 0); // unthreaded. We're
                                                           // NOT initializing
                                                           // here, just
                                                           // setting up.
    ...
    public int evolve() {
            ...
            if (partiallyFullSubpop) // is subpopulation full?
            {
                ...
                if (statistics instanceof SteadyStateStatisticsForm)
                    ((SteadyStateStatisticsForm) statistics).individualsEvaluatedStatistics(this,
                            new Individual[] { ind }, new Individual[] { deadInd }, new int[] { subpop },
        ...
        if ((numEvaluations > 0 && evaluations >= numEvaluations) || // using
                                                                     // numEvaluations
                (numEvaluations <= 0 && generationBoundary && generation == numGenerations - 1)) // not
                                                                                                 // using
                                                                                                 // numEvaluations
        {
            return R_FAILURE;
