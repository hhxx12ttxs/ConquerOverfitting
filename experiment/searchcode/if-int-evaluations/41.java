 ...
 * 
 * <p>
 * The MasterProblem is a special ECJ problem that performs evaluations by
 * sending them to a remote Slave process to be evaluated. As it implements both
 * the <i>SimpleProblemForm</i> and the <i>GroupedProblemForm</i> interfaces,
 * the MasterProblem can perform both traditional EC evaluations as well as
 * coevolutionary evaluations.
 * 
 * <p>
 * When a MasterProblem is specified by the Evaluator, the Problem is set up as
 * usual, but then the MasterProblem replaces it. The Problem is not garbage
 * collected -- instead, it's hung off the MasterProblem's <tt>problem</tt>
 * variable. In some sense the Problem is "pushed aside".
 * 
 * <p>
 * If the Evaluator begins by calling prepareToEvaluate(), and we're not doing
 * coevolution, then the MasterProblem does not evaluate individuals
 * immediately. Instead, it waits for at most <i>jobSize</i> individuals be
 * submitted via evaluate(), and then sends them all off in a group, called a
 * <i>job</i>, to the remote slave. In other situations (coevolution, or no
 * prepareToEvaluate()) the MasterProblem sends off individuals immediately.
 * 
 * <p>
 * It may be the case that no Slave has space in its queue to accept a new job
 * containing, among others, your new individual. In this case, calling
 * evaluate() will block until one comes available. You can avoid this by
 * testing for availability first by calling canEvaluate(). Note that
 * canEvaluate() and evaluate() together are not atomic and so you should not
 * rely on this facility if your system uses multiple threads.
 * 
 * <P>
 * When the individuals or their fitnesses return, they are immediately updated
 * in place. You have three options to wait for them:
 * 
 * <ul>
 * <li>
 * <p>
 * You can wait for all the individuals to finish evaluation by calling
 * finishEvaluating(). If you call this method before a job is entirely filled,
 * it will be sent in truncated format (which generally is perfectly fine). You
 * then block until all the jobs have been completed and the individuals
 * updated.
 * 
 * <li>
 * <p>
 * You can block until at least one individual is available, by calling
 * getNextEvaluatedIndividual(), which blocks and then returns the individual
 * that was just completed.
 * 
 * <li>
 * <p>
 * You can test in non-blocking fashion to see if an individual is available, by
 * calling evaluatedIndividualAvailable(). If this returns true, you may then
 * call getNextEvaluatedIndividual() to get the individual. Note that this isn't
 * atomic, so don't use it if you have multiple threads.
 * </ul>
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 * <tr>
 * <td valign=top><i>base.</i><tt>debug-info</tt><br>
 * <font size=-1>boolean</font></td>
 * <td valign=top>(whether the system should display information useful for
 * debugging purposes)<br>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>job-size</tt><br>
 ...
 * <font size=-1>boolean</font></td>
 * <td valign=top>(whether the communication with the slaves should be
 * compressed or not)<br>
 * </td>
 * </tr>
 * <tr>
 * <td valign=top><tt>eval.masterproblem.max-jobs-per-slave</tt><br>
 * <font size=-1>int</font></td>
 * <td valign=top>(the maximum load (number of jobs) per slave at any point in
 * time)<br>
 * </td>
 * </tr>
 * 
...
public class MasterProblem extends Problem implements SimpleProblemForm, GroupedProblemForm {
    ...
    void evaluate(EvolutionState state, Individual inds[], int[] subpopulations, int threadnum) {
        if (showDebugInfo)
            state.output.message(Thread.currentThread().getName() + "Starting a " + (batchMode ? "batched " : "")
                    + "SimpleProblemForm evaluation.");
        ...
        if (!batchMode)
            monitor.waitForAllSlavesToFinishEvaluating(state);
        if (showDebugInfo)
            state.output.message(Thread.currentThread().getName() + "Finished a " + (batchMode ? "batched " : "")
                    + "SimpleProblemForm evaluation.");
    }
     ...
     * (non-Javadoc)
     * 
     * @see ec.simple.SimpleProblemForm#describe(ec.EvolutionState,
     ...
     * 
     * @see
     * ec.coevolve.GroupedProblemForm#preprocessPopulation(ec.EvolutionState,
     * ec.Population)
     */
    public void preprocessPopulation(EvolutionState state, Population pop, boolean countVictoriesOnly) {
        if (!(problem instanceof GroupedProblemForm)) {
            state.output
                    .fatal("MasterProblem.preprocessPopulation(...) invoked, but the underlying Problem is not of GroupedProblemForm");
        }
     ...
     * 
     * @see
     * ec.coevolve.GroupedProblemForm#postprocessPopulation(ec.EvolutionState,
     * ec.Population)
     */
    public void postprocessPopulation(EvolutionState state, Population pop, boolean countVictoriesOnly) {
        if (!(problem instanceof GroupedProblemForm)) {
            state.output
                    .fatal("MasterProblem.postprocessPopulation(...) invoked, but the underlying Problem is not of GroupedProblemForm");
        }
    ...
    public void evaluate(EvolutionState state, Individual[] inds, boolean[] updateFitness, boolean countVictoriesOnly,
    ...
    /**
     * This will only return true if (1) the EvolutionState is a
     * SteadyStateEvolutionState and (2) an individual has returned from the
     * system. If you're not doing steady state evolution, you should not call
     * this method.
     */
     ...
     * (which will cause evaluatedIndividualAvailable() to return true), at
     * which time it returns the individual. You should only call this method if
     * you're doing steady state evolution -- otherwise, the method will block
     * forever.
     */
