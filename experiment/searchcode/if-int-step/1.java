 ...
 * Joăo Dias: 16/05/2006 - Added getPrecondition(Integer) method
 * Joăo Dias: 16/05/2006 - ID changed from int to Integer
 * Joăo Dias: 17/05/2006 - Added the clone() Method
 * Joăo Dias: 22/05/2006 - Added comments to each public method's header
 * Joăo Dias: 10/07/2006 - the class is now serializable
 * Joăo Dias: 12/07/2006 - Changes in groundable methods, the class now implements
 * 						   the IGroundable Interface, the old ground methods are
 * 					       deprecated 
 * Joăo Dias: 21/09/2006 - Added the attribute _agent which represents the name
 ...
 * Joăo Dias: 02/10/2006 - Added the method getEffect(Integer id) which is analog to method
 * 						   getPrecondition(Integer id)
 * Joăo Dias: 03/10/2006 - Removed the restriction that you can only use variables in step's
 ...
 * Joăo Dias: 04/10/2006 - Added the attribute selfExecutable that indicates if the action can
 * 						   be executable by the Agent or not.
 * 						 - Added corresponding SetSelfExecutable(boolean) method
 * Joăo Dias: 22/12/2006 - The UpdateEffectsProbability method now only updates the probabilities
