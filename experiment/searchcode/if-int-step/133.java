 ...
 * 
 * History: 
 * Joăo Dias: 11/01/2004 - File created
 * Joăo Dias: 16/05/2006 - Added the Method getStep(Integer)
 * Joăo Dias: 16/05/2006 - Completely Reimplemented the AddOrderingConstraint Method
 * 						   The old method had problems in detecting non-direct cycles.
 * 						   The new version uses OrderRelations
 * Joăo Dias: 16/05/2006 - Made changes according to the new definition of OpenPreconditions,
 * 						   CausalLinks, CausalConflicts
 * Joăo Dias: 16/05/2006 - Added Methods CheckCausalConflicts() and CheckCausalConflict(Step), 
 * 						   removed part of the code from AddStep Method to these ones.
 * Joăo Dias: 16/05/2006 - (Refactoring) Added Method CheckProtectionConstraints(Step) and moved 
 * 						   the code that implemented this functionality from the AddStep method
 * 						   to this one.
 * Joăo Dias: 16/05/2006 - (Refactoring) Renamed method CheckLinks() to UpdatePlan()
 * Joăo Dias: 16/05/2006 - Added the Compare(Step,Step) method, that compares two plan's 
 * 						   step and tell us which one must come first in the plan
 * Joăo Dias: 16/05/2006 - small changes in CheckRedundantStep. Now it receives a Step's ID.
 * Joăo Dias: 16/05/2006 - Changed RemoveOpenPrecondition, RemoveCausalLinks,  
 ...
 * Joăo Dias: 22/05/2006 - Removed constructor Plan(KnowledgeBase, ArrayList, Name) that was
 * 						   not being used
 * Joăo Dias: 22/05/2006 - Removed methods setStart(Step) and setFinish(Step) that were
 * 						   not being used
 * Joăo Dias: 22/05/2006 - Added comments to each public method's header
 * Joăo Dias: 10/07/2006 - the class is now serializable
 * Joăo Dias: 12/07/2006 - the class is now clonable, renamed method Clone() to clone()
 * Joăo Dias: 12/07/2006 - Replaced the deprecated Ground methods for the new ones.
 *                         Old Ground private method in the plan renamed to MakeGround
 * Joăo Dias: 15/07/2006 - Removed the KnowledgeBase from the Class fields since the KB is now
 * 						   a singleton that can be used anywhere without previous references.
 * Joăo Dias: 17/07/2006 - Removed the field Goal and the method getGoal() from the class. They 
 * 						   were not being used and caused problems with the plan serialization.
 * Joăo Dias: 22/07/2006 - Changed method GetStep(id). Altough less efficient, it allows us
 * 						   to remove steps from the plan without having to change all steps ID's
 * Joăo Dias: 26/09/2006 - Added method CheckProtectedConstraints()
 * Joăo Dias: 02/10/2006 - The UpdatePlan() method now updates more than one continuous planner flaw
 ...
 * Joăo Dias: 03/10/2006 - We no longer add to a plan's ProtectedConditions (conditions of InterestGoals)
 ...
 * Joăo Dias: 13/11/2006 - Added the method Compare(Integer, Integer) to the class.
 ...
 * Joăo Dias: 14/11/1006 - The method CheckRedundantStep now returns whether the step was removed from the plan or
 * 						   not (a boolean value)
 ...
 *						   such step.
 * Joăo Dias: 15/11/2006 - The method CheckRedundant step does not remove the step anymore, it just checks
 * 					       if the step is redundant. The removal is performed by the methods that use
 * 						   this function.
 * Joăo Dias: 22/11/2006 - We were forgetting to check for causal conflicts when a causal link was extended
 ...
 * Joăo Dias: 22/12/2006 - The method UnexecutedAction now may return actions of other agents that contain
 ...
 * Joăo Dias: 20/03/2007 - Solved a bug in MakeGround method when removing Inequality Constraints
 * 						    
 */
...
public class Plan implements Cloneable, Serializable
{
    ...
    /**
     * Checks if a given Step threatens any of the conditions protected
    ...
    /**
     * Checks if the Step just introduced in the plan caused any causal
        ...
        while (li.hasNext())
        {
                    ...
                    && !link.getSource().equals(step.getID()))
            {
                ...
                while (li2.hasNext())
                {
                    ...
                    if (eff.GetEffect().ThreatensCondition(cond))
                    {
                    	//the threat only exits if the step can occur between
                    	//the source and the destination
                    	if(Compare(step.getID(),link.getSource()) >= 0 &&
                    			Compare(step.getID(),link.getDestination()) <= 0)
                    	{
    ...
    /**
     * Checks if any of the plan's steps introduced a causal conflict
    ...
    public void UpdatePlan()
    {
        ...
        //Step anymore. The reason of this two-stage proccess is that if you have
            ...
            if(!link.getSource().equals(this._start.getID()))
            {
                ...
                else //if the condition is not grounded we must do another thing
                {
                	...
                	if(cond.GetValidBindings() != null) 
                	{
                		...
                		System.out.println("condition verified in start: " + cond);
                		linksToRemove.add(link);
        ...
        for(ListIterator li = linksToRemove.listIterator(); li.hasNext();)
        {
        	...
        	//the Start Step is never redundant, and futhermore we cannot remove it
        }
    }

    /**
     * Checks if a given Step is redundant and not needed anymore, because the CausalLinks
     * it establishes are not needed anymore.  
     * @param stepID - the ID in the plan of the step to remove
        ...
        while (li.hasNext())
        {
            ...
            if (HasNoneBefore(op.getID()))
            {
            			...
            			op.getAgent().toString().equals(AutobiographicalMemory.GetInstance().getSelf()))
            	{
            		...
            		{
            			System.out.println("The next action by self is not grounded: " + op.getName());
            			return null;
            		}
            	}
            	...
            	{
            		System.out.println("The next action does not have the preconditions verified: " + op.getName());
            		return null;
            	}
