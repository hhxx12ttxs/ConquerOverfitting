...
public class Manager4 implements Manager {
	Logger logger = Logger.getLogger(Manager4.class.toString());

	private System system;

	public Manager4(System system) {
		logger.info("Manager 4 is initlized");
		logger.info("4.	Алгоритмы «соседнего» назначения без пересылок «с упреждением».  В данном случае используется коммуникационная модель, когда только после формирования всех данных начинается пересылка для любой вычислительной работы.");
		this.system = system;
	}
	...
	public void loadTaskOnProc(Task task) {
		...
		for (int i = 0; i < parentTasks.size(); i++) {
			...
			for (int j = path.length - 1; j > 0; j--) {
				...
				if (j < path.length - 1) {
					Processor fromProc = system.procs.get(path[j + 1]);
					while (!fromProc.isFree(t, fromId)) {
						t++;
					}
				}
