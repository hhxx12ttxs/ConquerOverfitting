            ...
            while (i.hasNext())
            {
                ...
                for (int j = 0; j < len; j++)
                {
                    ...
                    if (propertyNode.getNodeType() == Node.ELEMENT_NODE)
                    {
                        ...
                        Text innerNode = (Text)((Element)propertyNode).getFirstChild();

                        if(innerNode != null)
                        {
                            nameVals.add(propertyNode.getNodeName());
                            nameVals.add(innerNode.getNodeValue());
                        }
                        else
                            isRecordOK = false;
                    }
...
//      if maximum value is not reached fire an event
                                          ...
                                          boolean caseSensitive)
    {
        ...
        for (int j = 0; j < len; j++)
        {
            ...
            if (propertyNode.getNodeType() == Node.ELEMENT_NODE)
            {
                String nodeName = ((Element)propertyNode).getNodeName();
                // Get nested TEXT node's value
                Text innerFirstChild = (Text)((Element)propertyNode).getFirstChild();
                
                String nodeValue =  innerFirstChild.getNodeValue();
