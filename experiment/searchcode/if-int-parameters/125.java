 ...
 * version of Koza-style "Subtree Crossover". Two individuals are selected, then
 * a single tree is chosen in each such that the two trees have the same
 * GPTreeConstraints. Then a random node is chosen in each tree such that the
 * two nodes have the same return type. If by swapping subtrees at these nodes
 * the two trees will not violate maximum depth constraints, then the trees
 * perform the swap, otherwise, they repeat the hunt for random nodes.
 * 
 * <p>
 * The pipeline tries at most <i>tries</i> times to a pair of random nodes BOTH
 * with valid swap constraints. If it cannot find any such pairs after
 * <i>tries</i> times, it uses the pair of its last attempt. If either of the
 * nodes in the pair is valid, that node gets substituted with the other node.
 * Otherwise an individual invalid node isn't changed at all (it's
 * "reproduced").
 * 
 * <p>
 * <b>Compatibility with constraints.</b> Since Koza-I/II only tries 1 time, and
 * then follows this policy, this is compatible with Koza. lil-gp either tries 1
 * time, or tries forever. Either way, this is compatible with lil-gp. My hacked
 * <a href="http://www.cs.umd.edu/users/seanl/gp/">lil-gp kernel</a> either
 * tries 1 time, <i>n</i> times, or forever. This is compatible as well.
 * 
 * <p>
 * This pipeline typically produces up to 2 new individuals (the two newly-
 * swapped individuals) per produce(...) call. If the system only needs a single
 * individual, the pipeline will throw one of the new individuals away. The user
 * can also have the pipeline always throw away the second new individual
 * instead of adding it to the population. In this case, the pipeline will only
 * typically produce 1 new individual per produce(...) call.
 * 
 * <p>
 ...
 * <td valign=top>(first tree for the crossover; if parameter doesn't exist,
 * tree is picked at random)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>tree.1</tt><br>
 * <font size=-1>0 &lt; int &lt; (num trees in individuals), if exists</font></td>
 * <td valign=top>(second tree for the crossover; if parameter doesn't exist,
 * tree is picked at random. This tree <b>must</b> have the same
 * GPTreeConstraints as <tt>tree.0</tt>, if <tt>tree.0</tt> is defined.)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>ns.</tt><i>n</i><br>
 * <font size=-1>classname, inherits and != GPNodeSelector,<br>
 * or String <tt>same<tt></font></td>
 * <td valign=top>(GPNodeSelector for parent <i>n</i> (n is 0 or 1) If, for
 * <tt>ns.1</tt> the value is <tt>same</tt>, then <tt>ns.1</tt> a copy of
 * whatever <tt>ns.0</tt> is. Note that the default version has no <i>n</i>)</td>
 * </tr>
 * 
 * <tr>
 ...
 * <tr>
 * <td valign=top><i>base</i>.<tt>ns.</tt><i>n</i><br>
 * <td>nodeselect<i>n</i> (<i>n</i> is 0 or 1)</td>
 * </tr>
 * </table>
 * 
 * 
