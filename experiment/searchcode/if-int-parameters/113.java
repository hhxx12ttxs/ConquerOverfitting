...
public class Yarrow extends RandomSource {
	...
	public Yarrow(File seed, String digest, String cipher,boolean updateSeed) {
        ...
        try {
            ...
            Logger.error(this, "Could not init pools trying to getInstance("+digest+"): "+e.toString());
	    e.printStackTrace();
	    ...
	    if(File.separatorChar == '/') {
	        ...
	        if(hwrng.exists() && hwrng.canRead()) {
		        try {
		            ...
		            Logger.notice(this, "Can't read /dev/hwrng even though exists and is readable: "+t.toString());
		        } finally {
		            try {
		            	if(fis != null)
		            		fis.close();
	        ...
	        try {
	            ...
	            Logger.notice(this, "Can't read /dev/urandom: "+t);
	        } finally {
	            try {
	            	if(fis != null)
	            		fis.close();
	        ...
	        if(canBlock) {
	            ...
	            try {
	                ...
	                Logger.notice(this, "Can't read /dev/random: "+t);
	            } finally {
		            try {
		            	if(fis != null)
		            		fis.close();
	...
	private int accept_entropy(long data, EntropySource source, int actualEntropy) {
		...
		if (performedPoolReseed && (seedfile != null)) {
			...
			Logger.debug(this, "Writing seedfile");
			write_seed(seedfile);
			Logger.debug(this, "Written seedfile");
		}
	...
	private void fast_pool_reseed() {
		...
		if(endTime - startTime > 5000)
			Logger.debug(this, "Fast pool reseed took "+(endTime-startTime)+"ms");
	}
