 ...
 * 
 * <p>
 * IntegerVectorSpecies can specify min/max numeric constraints on gene values
 ...
 * <font size=-1>long &gt;= <i>base</i>.min-gene.<i>i</i> (default=<i>base</i>.
 * <tt>max-gene</tt>)</font></td>
...
public class IntegerVectorSpecies extends VectorSpecies {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        for (int x = 0; x < genomeSize; x++) {
            if (!state.parameters.exists(base.push(P_MINGENE).push("" + x), base.push(P_MINGENE).push("" + x))) {
                ...
                minGenes[x] = state.parameters.getLongWithDefault(base.push(P_MINGENE).push("" + x),
                        base.push(P_MINGENE).push("" + x), minGene);
                foundStuff = true;
            }
            ...
            if (!state.parameters.exists(base.push(P_MAXGENE).push("" + x), base.push(P_MAXGENE).push("" + x))) {
                ...
                maxGenes[x] = state.parameters.getLongWithDefault(base.push(P_MAXGENE).push("" + x),
                        base.push(P_MAXGENE).push("" + x), maxGene);
                foundStuff = true;
            }
        }
    ...
    private void initializeGenomeSegmentsByStartIndices(final EvolutionState state, final Parameter base,
            final Parameter def, int numSegments, long minGene, long maxGene) {
        boolean warnedMin = false;
        boolean warnedMax = false;
        ...
        for (int i = numSegments - 1; i >= 0; i--) {
            ...
            {
                currentSegmentMinGeneValue = state.parameters.getLongWithDefault(base.push(P_SEGMENT).push("" + i)
            ...
            {
                currentSegmentMaxGeneValue = state.parameters.getLongWithDefault(base.push(P_SEGMENT).push("" + i)
