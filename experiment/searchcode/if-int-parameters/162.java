...
/**
 * A slight modification of the tournament selection procedure for use with
 * spatially-embedded EAs.
 * 
 * When selecting an individual, the SpatialTournamentSelection is told a
 * specific individual. It then picks N individuals at random which are within a
 * certain distance (the <i>neighborhood size</i>) of that individual. These
 * individuals then enter a tournament a-la standard Tournament Selection.
 * 
 * <p>
 * The method of picking individuals is either <tt>uniform</tt> (picking
 * individuals using the Space interface's getRandomIndividual(...)) or
 * <tt>random-walk</tt> (wandering <i>distance</i> steps at random). You can
 * also stipulate whether the original individual must be in the tournament.
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 ...
 * <td valign=top>(the neighborhood size)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>ind-competes</tt><br>
 * <font size=-1> bool = <tt>true</tt> or <tt>false</tt> (default)</font></td>
 * <td valign=top>(Do we include the base individual in the tournament?)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>type</tt><br>
 * <font size=-1>String: uniform (default) or random-walk</font></td>
 * <td valign=top>Method for selecting individuals in neighborhood</td>
 * </tr>
 * 
 * </table>
 * 
...
public class SpatialTournamentSelection extends TournamentSelection {
    /**
     * The size of the neighborhood from where parents are selected. Small
     * neighborhood sizes enforce a local selection pressure, while larger
     * values for this parameters allow further-away individuals to compete for
     * breeding as well.
     */
    ...
    /**
     * Some models assume an individual is always selected to compete for
     * breeding a child that would take its location in space. Other models
     * don't make this assumption. This parameter allows one to specify whether
     * an individual will be selected to compete with others for breeding a
     * child that will take its location in space. If the parameter value is not
     * specified, it is assumed to be false by default.
     */
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (neighborhoodSize < 1)
            state.output.fatal("Parameter not found, or its value is < 1.", base.push(P_N_SIZE),
                    defaultBase.push(P_N_SIZE));
    ...
    public int getRandomIndividual(int number, int subpopulation, EvolutionState state, int thread) {
        ...
        if (number == 0 && indCompetes) // Should we just return the individual?
            return index;
        else if (type == TYPE_UNIFORM) // Should we pick randomly in the space
        ...
        else // if (type == TYPE_RANDOM_WALK) // Should we do a random walk?
        {
            ...
            space.setIndex(thread, oldIndex); // just in case we weren't
                                              // supposed to mess around with
                                              // that
            return val;
        }
    }
