...
/**
 * GPNodeBuilder is a Prototype which defines the superclass for objects which
 * create ("grow") GP trees, whether for population initialization, subtree
 * mutation, or whatnot. It defines a single abstract method,
 ...
 * probabilities of various tree sizes, which the tree builder can use as it
 * sees fit (or totally ignore). There are two such facilities. First, the user
 * might specify a minimum and maximum range for tree sizes to be picked from;
 * trees would likely be picked uniformly from this range. Second, the user
 * might specify an array, <tt>num-sizes</tt> long, of probabilities of tree
 * sizes, in order to give a precise probability distribution.
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 ...
 * <td valign=top>(smallest valid size, see discussion above)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>max-size</tt><br>
 * <font size=-1>int &gt;= <tt>min-size</tt>, or undefined</font></td>
 * <td valign=top>(largest valid size, see discussion above)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>num-sizes</tt><br>
 * <font size=-1>int &gt;= 1, or underfined</font></td>
 * <td valign=top>(number of sizes in the size distribution, see discussion
 * above)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>size</tt>.<i>n</i><br>
 * <font size=-1>0.0 &lt;= float &lt;= 1.0</font>, or undefined</td>
 * <td valign=top>(probability of choosing size <i>n</i>. See discussion above)</td>
 * </tr>
 * </table>
 * 
 * 
...
public abstract class GPNodeBuilder implements Prototype {
     ...
     * swap-compatible with <i>type</i>. When you build a brand-new tree out of
     * GPNodes cloned from the prototypes stored in the GPNode[] arrays, you
     * must remember to call resetNode() on each cloned GPNode. This gives ERCs
     * a chance to randomize themselves and set themselves up.
     * 
     * <p>
     * requestedSize is an optional argument which differs based on the
     * GPNodeBuilder used. Typically it is set to a tree size that the calling
     * method wants the GPNodeBuilder to produce; the GPNodeBuilder is not
     * obligated to produce a tree of this size, but it should attempt to
     * interpret this argument as appropriate for the given algorithm. To
     * indicate that you don't care what size the tree should be, you can pass
     * NOSIZEGIVEN. However if the algorithm <i>requires</i> you to provide a
     * size, it will generate a fatal error to let you know.
     */
    ...
    public float[] sizeDistribution; /*
                                      * sizeDistribution[x] represents the
                                      * likelihood of size x appearing -- if
                                      * unused, it's null
                                      */

    /**
     * Returns true if some size distribution (either minSize and maxSize, or
     * sizeDistribution) is set up by the user in order to pick sizes randomly.
     */
    ...
    /**
     * Assuming that either minSize and maxSize, or sizeDistribution, is
     * defined, picks a random size from minSize...maxSize inclusive, or
     * randomly from sizeDistribution.
     */
    public int pickSize(final EvolutionState state, final int thread) {
        if (minSize > 0) {
            ...
            return RandomChoice.pickFromDistribution(sizeDistribution, state.random[thread].nextFloat()) + 1;
        } else
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (state.parameters.exists(base.push(P_MINSIZE), def.push(P_MINSIZE))) {
            ...
            if (minSize == 0)
                state.output.fatal("The GPNodeBuilder must have a min size >= 1.", base.push(P_MINSIZE),
                        def.push(P_MINSIZE));

            maxSize = state.parameters.getInt(base.push(P_MAXSIZE), def.push(P_MAXSIZE), 1);
            if (maxSize == 0)
                state.output.fatal("The GPNodeBuilder must have a max size >= 1.", base.push(P_MAXSIZE),
                        def.push(P_MAXSIZE));

            if (minSize > maxSize)
                state.output.fatal("The GPNodeBuilder must have min size <= max size.", base.push(P_MINSIZE),
                        def.push(P_MINSIZE));
        } else if (state.parameters.exists(base.push(P_MAXSIZE), def.push(P_MAXSIZE)))
            state.output.fatal("This GPNodeBuilder has a " + P_MAXSIZE + " but not a " + P_MINSIZE + ".",
        ...
        // load sizeDistribution
            ...
            if (state.parameters.exists(base.push(P_SIZE).push("0"), def.push(P_SIZE).push("0")))
                state.output.warning("GPNodeBuilder does not use size #0 in the distribution",
                        base.push(P_SIZE).push("0"), def.push(P_SIZE).push("0"));

            float sum = 0.0f;
            for (int x = 0; x < siz; x++) {
                sizeDistribution[x] = state.parameters.getFloat(base.push(P_SIZE).push("" + (x + 1)), def.push(P_SIZE)
                        .push("" + (x + 1)), 0.0f);
                if (sizeDistribution[x] < 0.0) {
                    state.output.warning("Distribution value #" + x + " negative or not defined, assumed to be 0.0",
                            base.push(P_SIZE).push("" + (x + 1)), def.push(P_SIZE).push("" + (x + 1)));
    ...
    public abstract GPNode newRootedTree(final EvolutionState state, final GPType type, final int thread,
            final GPNodeParent parent, final GPFunctionSet set, final int argposition, final int requestedSize);

    /**
     * Issues a warning that no terminal was found with a return type of the
     * given type, and that an algorithm had requested one. If fail is true,
     * then a fatal is issued rather than a warning. The warning takes the form
     * of a one-time big explanatory message, followed by a one-time-per-type
     * message.
     */
    protected void warnAboutNoTerminalWithType(GPType type, boolean fail, EvolutionState state) {
        ...
        state.output
                .warnOnce("A GPNodeBuilder has been requested at least once to generate a one-node tree with "
        ...
        if (fail)
            state.output
                    .fatal(""
                            + this.getClass()
                            + " can't find a terminal type-compatable with "
                            + type
    ...
    /**
     * If the given test is true, issues a warning that no terminal was found
     * with a return type of the given type, and that an algorithm had requested
     * one. If fail is true, then a fatal is issued rather than a warning. The
     * warning takes the form of a one-time big explanatory message, followed by
     * a one-time-per-type message. Returns the value of the test. This form
     * makes it easy to insert warnings into if-statements.
     */
    ...
    /**
     * Issues a warning that no nonterminal was found with a return type of the
     * given type, and that an algorithm had requested one. If fail is true,
     * then a fatal is issued rather than a warning. The warning takes the form
     * of a one-time big explanatory message, followed by a one-time-per-type
     * message.
    ...
    protected void warnAboutNonTerminalWithType(GPType type, boolean fail, EvolutionState state) {
        // big explanation -- appears only once
        state.output
                .warnOnce("A GPNodeBuilder has been requested at least once to generate a tree with "
        ...
        if (fail)
            state.output
                    .fatal(""
                            + this.getClass()
                            + " can't find a non-terminal type-compatable with "
                            + type
    ...
    /**
     * Issues a fatal error that no node (nonterminal or terminal) was found
     * with a return type of the given type, and that an algorithm had requested
     * one.
     */
    protected void errorAboutNoNodeWithType(GPType type, EvolutionState state) {
        state.output
                .fatal("" + this.getClass() + " could find no terminal or nonterminal type-compatable with " + type);
    }
}
