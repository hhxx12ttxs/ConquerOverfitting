...
/**
 * InternalCrossoverPipeline picks two subtrees from somewhere within an
 * individual, and crosses them over. Before doing so, it checks to make sure
 * that the subtrees come from trees with the same tree constraints, that the
 * subtrees are swap-compatible with each other, that the new individual does
 * not violate depth constraints, and that one subtree does not contain the
 * other. It tries <tt>tries</tt> times to find a valid subtree pair to cross
 * over. Failing this, it just copies the individual.
 * 
 * 
 * <p>
 ...
 * <font size=-1>int &gt;= 1</font></td>
 * <td valign=top>(maximum valid depth of the crossed-over individual's trees)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>ns.</tt>0<br>
 * <font size=-1>classname, inherits and != GPNodeSelector</font></td>
 * <td valign=top>(GPNodeSelector for subtree 0.</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>ns.</tt>1<br>
 * <font size=-1>classname, inherits and != GPNodeSelector,<br>
 * or String <tt>same<tt></font></td>
 * <td valign=top>(GPNodeSelector for subtree 1. If value is <tt>same</tt>, then
 * <tt>ns.1</tt> a copy of whatever <tt>ns.0</tt> is)</td>
 * </tr>
 * 
 * <tr>
...
public class InternalCrossoverPipeline extends GPBreedingPipeline {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        nodeselect0 = (GPNodeSelector) (state.parameters.getInstanceForParameter(p, d, GPNodeSelector.class));
...

    {
        ...
        for (int q = start; q < n + start; q++) {
            ...
            if (tree1 != TREE_UNFIXED && (tree1 < 0 || tree1 >= i.trees.length))
                // uh oh
                state.output
                        .fatal("Internal Crossover Pipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
            ...
            } else // need to copy it
            {
                ...
                for (int x = 0; x < j.trees.length; x++) {
                    j.trees[x] = (GPTree) (i.trees[x].lightClone()); // light
                                                                     // clone
                    j.trees[x].owner = j;
            ...
            if (tree1 == TREE_UNFIXED || tree2 == TREE_UNFIXED) {
                ...
                if (i.trees[t1].constraints(initializer) != i.trees[t2].constraints(initializer)) // uh
                                                                                                  // oh
                    state.output
