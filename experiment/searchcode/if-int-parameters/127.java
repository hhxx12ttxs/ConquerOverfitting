 ...
 * between subpopulations. IterPopulationExchange uses an arbitrary graph
 * topology for migrating individuals from subpopulations. The assumption is
 * that all subpopulations have the same representation and same task to solve,
 * otherwise the exchange between subpopulations does not make much sense.
 * 
 * <p>
 * InterPopulationExchange has a topology which is similar to the one used by
 ...
 * 
 * <p>
 * Individuals are sent from a subpopulation prior to breeding. They are stored
 * in a waiting area until after all subpopulations have bred; thereafter they
 * are added into the new subpopulation. This means that the subpopulation order
 * doesn't matter. Also note that it means that some individuals will be created
 * during breeding, and immediately killed to make way for the migrants. A
 * little wasteful, we know, but it's simpler that way.
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 ...
 * </table>
 * <p>
 * <i>Note:</i> For each subpopulation in your population, there <b>must</b> be
 * one exch.subpop... declaration set.
 * <table>
 * <tr>
 * <td valign=top><tt><i>base</i>.subpop.<i>n</i>.select</tt><br>
 ...
 * migrants to emigrate to other subpopulations. If not set, uses the default
 * parameter below.</td>
 * </tr>
 * <tr>
 ...
 * <font size=-1>classname, inherits and != ec.SelectionMethod</font></td>
 * <td valign=top>
 * <i>server</i>: Default parameter: the selection method used by a given
 * subpopulation for picking migrants to emigrate to other subpopulations.</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.subpop.<i>n</i>.select-to-die</tt><br>
 * <font size=-1>classname, inherits and != ec.SelectionMethod (Default is
 * random selection)</font></td>
 * <td valign=top>The selection method used by subpopulation #n for picking
 * individuals to be replaced by migrants. If not set, uses the default
 * parameter below.</td>
 * </tr>
 * <tr>
 ...
 * <td valign=top>
 * <i>server</i>: Default parameter: the selection method used by a given
 * subpopulation for picking individuals to be replaced by migrants.</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.subpop.<i>n</i>.mod</tt><br>
 * <font size=-1>int >= 1</font></td>
 * <td valign=top>The number of generations that subpopulation #n waits between
 * sending emigrants. If not set, uses the default parameter below.</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.mod</tt><br>
 * <font size=-1>int >= 1</font></td>
 * <td valign=top>
 * <i>server</i>: Default parameter: the number of generations that a given
 * subpopulation waits between sending emigrants.</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.subpop.<i>n</i>.start</tt><br>
 * <font size=-1>int >= 0</font></td>
 * <td valign=top>The generation when subpopulation #n begins sending emigrants.
 * If not set, uses the default parameter below.</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.start</tt><br>
 * <font size=-1>int >= 0</font></td>
 * <td valign=top>
 * <i>server</i>: Default parameter: the generation when a given subpopulation
 * begins sending emigrants.</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.subpop.<i>n</i>.size</tt><br>
 * <font size=-1>int >= 0</font></td>
 * <td valign=top>The number of emigrants sent at one time by generation #n. If
 * not set, uses the default parameter below.</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.subpop.<i>n</i>.num-dest</tt><br>
 * <font size=-1>int >= 0</font></td>
 * <td valign=top>The number of destination subpopulations for this
 * subpopulation.</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.subpop.<i>n</i>.dest.<i>m</i></tt><br>
 * <font size=-1>int >= 0</font></td>
 * <td valign=top>Subpopulation #n's destination #m is this subpopulation.</td>
 * </tr>
 * </table>
 * 
 * <p>
 ...
 * <tr>
 * <td valign=top><tt><i>base</i>.subpop.<i>n</i>.select</tt><br>
 * <td>selection method for subpopulation #n's migrants</td>
 * </tr>
 * </table>
 * 
 * 
...
public class InterPopulationExchange extends Exchanger {
     ...
     * first emigration from the current subpopulation
     */
    ...
    public void setup(final EvolutionState state, final Parameter _base) {
        ...
        // how many individuals (maximally) would each of the mailboxes have to
        // hold
        ...
        for (int i = 0; i < numsubpops; i++) {
            ...
            exchangeInformation[i].immigrantsSelectionMethod = (SelectionMethod) state.parameters
                    .getInstanceForParameter(p.push(P_SELECT_METHOD), base.push(P_SELECT_METHOD),
            ...
            if (state.parameters.exists(p.push(P_SELECT_TO_DIE_METHOD), base.push(P_SELECT_TO_DIE_METHOD)))
                exchangeInformation[i].indsToDieSelectionMethod = (SelectionMethod) state.parameters
                        .getInstanceForParameter(p.push(P_SELECT_TO_DIE_METHOD), base.push(P_SELECT_TO_DIE_METHOD),
                                ec.SelectionMethod.class);
            else
                // use RandomSelection
            ...
            for (int j = 0; j < exchangeInformation[i].numDest; j++) {
                ...
                if (exchangeInformation[i].destinations[j] == -1
                    ...
                    state.output.fatal("Parameter not found, or it has an incorrect value.", p.push(P_DEST)
                            .push("" + j));
                // update the maximum number of incoming individuals for the
                // destination island
        ...
        // calculate the maximum number of incoming individuals to be stored in
        // the mailbox
        int max = -1;
     ...
     * set up. It doesn't do anything, as this exchanger works on only 1
     * computer.
     */
    ...
    /**
     * Initializes contacts with other processes, if that's what you're doing.
     * Called after restarting from a checkpoint. It doesn't do anything, as
     * this exchanger works on only 1 computer.
     */
    ...
    public Population preBreedingExchangePopulation(EvolutionState state) {
        ...
        for (int i = 0; i < exchangeInformation.length; i++) {
            ...
            if ((state.generation >= exchangeInformation[i].offset)
                    && ((exchangeInformation[i].modulo == 0) || (((state.generation - exchangeInformation[i].offset) % exchangeInformation[i].modulo) == 0))) {
                ...
                for (int x = 0; x < exchangeInformation[i].numDest; x++) {
                    ...
                    if (chatty)
                        state.output.message("Sending the emigrants from subpopulation " + i + " to subpopulation "
                    ...
                    for (int y = 0; y < exchangeInformation[i].size; y++) // send
                                                                          // all
                                                                          // necesary
                                                                          // individuals
                    {
                        ...
                        // copy the individual to the mailbox of the destination
                        // subpopulation
                        immigrants[exchangeInformation[i].destinations[x]][nImmigrants[exchangeInformation[i].destinations[x]]] = state.population.subpops[i].individuals[index];
                        // increment the counter with the number of individuals
                        // in the mailbox
                        nImmigrants[exchangeInformation[i].destinations[x]]++;
                    }
                    exchangeInformation[i].immigrantsSelectionMethod.finishProducing(state, i, 0); // end
                                                                                                   // the
                                                                                                   // selection
                                                                                                   // step
                }
    ...
    public Population postBreedingExchangePopulation(EvolutionState state) {
        ...
        for (int x = 0; x < nImmigrants.length; x++) {
            ...
            if (nImmigrants[x] > 0 && chatty) {
                state.output.message("Immigrating " + nImmigrants[x] + " individuals from mailbox for subpopulation "
                        + x);
            }
            ...
            if (nImmigrants[x] >= state.population.subpops[x].individuals.length)
                state.output.fatal("Number of immigrants (" + nImmigrants[x] + ") is larger than subpopulation #" + x
                        + "'s size (" + len
                        + ").  This would cause an infinite loop in the selection-to-die procedure.");
            ...
            for (int y = 0; y < nImmigrants[x]; y++) {
                ...
                // reset the evaluated flag (the individuals are not evaluated
                // in the current island */
                state.population.subpops[x].individuals[indeces[y]].evaluated = false;

            }

            // reset the number of immigrants in the mailbox for the current
            // subpopulation
            // this doesn't need another synchronization, because the thread is
