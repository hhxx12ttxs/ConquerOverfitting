...
public class HMMScorer implements LocalTrigramScorer {
	...
	public HMMScorer(int len, int numNoCaps, int numCaps, double a, double b, double c, double d, double penalty) {
		System.out.println();
		System.out.println("**************************************************************************");
	...
	public void train(List<LabeledLocalTrigramContext> localTrigramContexts) {
		...
		for (int len = 1; len <= maxSuffixLen; len++) {
			for (String suffix : suffixCountsCaps[len].keySet()) {
				Counter<String> ProbTagBarSuffix = suffixCountsCaps[len].getCounter(suffix);
				double marginalSuffix = ProbTagBarSuffix.totalCount();
				for (String tag : ProbTagBarSuffix.keySet()) {
					double marginalTag = unigramCounts.getCount(tag);
					ProbTagBarSuffix.setCount(tag, ProbTagBarSuffix.getCount(tag) * marginalSuffix/marginalTag);
				}
			}
		}
		for (int len = 1; len <= maxSuffixLen; len++) {
			for (String suffix : suffixCountsNoCaps[len].keySet()) {
				String less;
				if (len > 1) {
					less = suffix.substring(1);
				} else {
				...
				Counter<String> ProbTagBarSuffix = suffixCountsNoCaps[len].getCounter(suffix);
				Counter<String> ProbTagBarLess = new Counter<String>(suffixCountsNoCaps[len - 1].getCounter(less));
				ProbTagBarLess.scale(theta[len - 1]); // no theta for suffixes
				// of length 0,
				ProbTagBarSuffix.incrementAll(ProbTagBarLess);
				ProbTagBarSuffix.scale(1 / (1 + theta[len - 1]));
