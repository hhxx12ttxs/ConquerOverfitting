...
abstract public class Animation {
	...
	static public void tapCardToggle (final CardPanel panel, final MWCardImpl parent) {
		new Animation(300) {
			protected void start () {
	...
	static public void displayHandToggle(final MWHandBeneath hand) {
		if (hand.locked && hand.displayed) return;
		
		new Animation(200) {
			protected void start () {
				hand.displayed = !hand.displayed;
			}

			protected void update (float percentage) {
				Dimension d = hand.getSize();
				percentage = hand.displayed ? percentage : 1 - percentage;
				int dy = (int)((SettingsManager.getManager().getCardSize().height + 10 - 40) * percentage);
	        	d.height = dy  + 40;
	        	WindowManager.getManager().getHandBeneath().setPreferredSize(d);
	        	WindowManager.getManager().getHandBeneath().revalidate();
			}
	...
	static public void showIcons(final MWCard card) {
		new Animation(500) {
			protected void start () {
			}
	...
	static public void hideCard(final CardPanel panel, final MWCard card) {
		new Animation(600) {
			protected void start () {
			}
