 ...
 * @author huliqing
 */
public class TaskArticlePrivateCounter implements Task{
    ...
    public boolean execute() {
        CounterEn ce = ArticleCounter.getInstance().findCounter(CounterType.article_private);
        if (ce.getLastDate() != null) {
            long diff = System.currentTimeMillis() - ce.getLastDate().getTime();
            if (diff < 86400000) { // 1000 * 60 * 60 * 24
                // ?????????
                logger.info("??????private article???????: " + (diff / 3600000) + " Hours????????.");
                return true;
            }
        }
        logger.info("????private article ???.");
        if (!isContinue) {
            // ?0,???lastArticleId
            count = 0;
            lastArticleId = 1L;
        ...
        while ((lastId = ArticleCounter.getInstance().countArticle(ArticleSecurity.PRIVATE, startId, store)) != null) {
            count += store.getValue();
            lastArticleId = lastId;
            startId = lastId + 1;
            store.clear();
        }
        ArticleCounter.getInstance().updatePrivateCounter(count);
        lastArticleId = null;
        isContinue = false;
        logger.info("??private article ?????,???" + count);
        return true;
    }
}
