...
public abstract class TwitterCursorBaseActivity extends TwitterListBaseActivity {
    ...
    /**
     * ??listView?? - ???? NOTE: ???listView#setAdapter????
     */
    ...
    protected void specialItemClicked(int position) {
        // ?? mTweetAdapter.getCount ? mTweetList.getCount???
        // ??????????????foot?head??????foot?head
        // ???????foot?head?????list.count = adapter.count + 2
        if (position == 0) {
            // ???Item(header)
    ...
    protected boolean _onCreate(Bundle savedInstanceState) {
        ...
        if (super._onCreate(savedInstanceState)) {
            ...
            // FIXME: ??????Followers???????????????Followers????
            // ??????@???????Follower???????review?refactoring
            // ???????????????
            //
