...
public class OperationUpdate extends AbstractSynchronize {
  ...
  TreeMap columnObjectTreeMap;
  String shadowTable, tableName;
  String[] primaryColumnNames, changedColumnNames, changedColumnValues, tableColumnNames;
  PreparedStatement psForInitialRecordInShadowTable, psForActualUpdateInTable,
  ...
  Object conisderedId, lastShadowUid;
  AbstractColumnObject[] primaryKeyColumnsObject;
  ArrayList voilationPkKeys;
  boolean NO_OPERATION = false;
  XMLElement xmlElement_NULL;
  AbstractDataBaseHandler dbHandler;
  MetaDataInfo mdi;
  BufferedWriter bw;
  String replicationType, transactionLogType;
    ...
    try {
      ...
      if (isFirstPass) {
        ...
        if (!tracer.recordFound) {
          ...
          if (tracer.type.equalsIgnoreCase(RepConstants.delete_operation)) {
            if (NO_OPERATION) {
              return;
            }
            if (repTable.isLocalServerWinner()) {
...
//              localChangedColumnsValues.add(newRecord);
              ...
              if (actualListOFColumns.size() > 0) {
                Object lastSyncId = getLastUIDFromShadowTable();
                String query = repTable.getPreparedStatement(
                    actualListOFColumns, primaryColumnNames);
...
// THIS RECORD SHOULD BE SENT TO REMOTE SERVER FOR UPDATE...
  ...
  private void primaryVoilation(Object[] conflictingPrimaryKeyValues, Tracer trForRollback) throws RepException, SQLException {
    Object lastShadowUidObj = getLastUIDFromShadowTable();
    if (! (lastShadowUidObj instanceof Integer)) {
      lastShadowUid = new Long( ( (Long) getLastUIDFromShadowTable()).longValue() +1);
    }
    else { //for firebird
      lastShadowUid = new Long( ( (Integer) lastShadowUidObj).longValue() + 1);
    ...
    if (tracer.type.equalsIgnoreCase(RepConstants.insert_operation)) { // Insert Case
    ...
    else if (tracer.type.equalsIgnoreCase(RepConstants.update_operation)) { // Update Case
