...
/**
 * Implement DBSplitter over date/time values. Make use of logic from
 * IntegerSplitter, since date/time are just longs in Java.
 */
...
public class DateSplitter extends IntegerSplitter {
	...
	public List<InputSplit> split(Configuration conf, ResultSet results,
		...
		if (sqlDataType == Types.TIMESTAMP) {
			...
			// lower-bound nanos.
			try {
				((java.sql.Timestamp) startDate).setNanos(results.getTimestamp(
						1).getNanos());
		...
		for (int i = 1; i < splitPoints.size(); i++) {
			...
			if (i == splitPoints.size() - 1) {
				if (sqlDataType == Types.TIMESTAMP) {
					...
					// upper-bound nanos.
					try {
						((java.sql.Timestamp) endDate).setNanos(results
								.getTimestamp(2).getNanos());
					} catch (NullPointerException npe) {
				...
				splits.add(new DataDrivenDBInputFormat.DataDrivenDBInputSplit(
						lowClausePrefix + dateToString(startDate), colName
