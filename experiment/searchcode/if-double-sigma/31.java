...
public class MaximumEntropyClassifier <I,F,L> implements ProbabilisticClassifier<I, L> {
	...
	public static class ObjectiveFunction <F,L> implements DifferentiableFunction {
		...
		private Pair<Double, double[]> calculate(double[] x) {
			...
			// sum over each training set data
			for(EncodedDatum datum : data) {
				// calculate P values
				double[] P = DoubleArrays.constantArray(0.0, numLabels);
				double totalP = 0;
				for(int labelIndex=0; labelIndex<numLabels; labelIndex++) {
					for(int i=0; i<datum.getNumActiveFeatures(); i++) {
						...
						double weight = x[indexLinearizer.getLinearIndex(featureIndex, labelIndex)];
						P[labelIndex] += weight * featureCount;
					}
					P[labelIndex] = Math.exp(P[labelIndex]);
					totalP += P[labelIndex];
				}
				...
				objective -= Math.log(P[datum.getLabelIndex()]);
				...
				for(int labelIndex=0; labelIndex<numLabels; labelIndex++) {
					for(int i=0; i<datum.getNumActiveFeatures(); i++) {
						...
						if(labelIndex == datum.getLabelIndex()) {
							derivatives[linearIndex] -= featureCount;
						}
						derivatives[linearIndex] += P[labelIndex] * featureCount;
					}
				} 
			}
			
			// penalize large weights
			final double alpha = 1.0 / 2 / sigma / sigma;
			for(int linearIndex=0; linearIndex<dimension; linearIndex++) {
				objective += alpha * x[linearIndex] * x[linearIndex];
				derivatives[linearIndex] += 2 * alpha * x[linearIndex]; 
			}
	...
	private static <F,L> double[] getLogProbabilities(EncodedDatum datum, double[] weights, Encoding<F, L> encoding, IndexLinearizer indexLinearizer) {
		int numLabels = encoding.getNumLabels();
		double[] logProbabilities = DoubleArrays.constantArray(Double.NEGATIVE_INFINITY, numLabels);
		
		// stores sum(w_ci*f_i,i=1..N) for each label c
		double[] weightedFeatureSum = DoubleArrays.constantArray(0.0, encoding.getNumLabels());
		for(int labelIndex=0; labelIndex<numLabels; labelIndex++) {
			for(int i=0; i<datum.getNumActiveFeatures(); i++) {
				int featureIndex = datum.getFeatureIndex(i);
				int linearIndex = indexLinearizer.getLinearIndex(featureIndex, labelIndex);
				weightedFeatureSum[labelIndex] += weights[linearIndex] * datum.getFeatureCount(i);
			}
		}
		...
		for(int labelIndex=0; labelIndex<numLabels;labelIndex++) {
			normalization += Math.exp(weightedFeatureSum[labelIndex]);
		}
		normalization = Math.log(normalization);
		
		// calculate log probability
		for(int labelIndex=0; labelIndex<numLabels; labelIndex++) {
			logProbabilities[labelIndex] = weightedFeatureSum[labelIndex] - normalization;
		}
