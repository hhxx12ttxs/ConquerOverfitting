...
//			System.out.println("weights: " + Arrays.toString(weights));
	...
	public static class ObjectiveFunction <F,L> implements DifferentiableFunction {
		...
		private Pair<Double, double[]> calculate(double[] w) {
			...
			for (int i=0; i<data.length; i++) {
				double[] lprobs = getLogProbabilities(data[i], w, encoding, indexLinearizer);
				double logTerm = lprobs[data[i].getLabelIndex()];
				logSum += logTerm;
			}
			objective = logSum;
			...
			for (int i=0; i<data.length; i++) {
				for (int j=0; j<data[i].featureIndexes.length; j++) {
					int linearIndex = indexLinearizer.getLinearIndex(data[i].featureIndexes[j], data[i].getLabelIndex());
					derivatives[linearIndex] = derivatives[linearIndex] + data[i].featureCounts[j];
				}
			}
			
			for (int i=0; i<data.length; i++) {
				double[]  logProbs = getLogProbabilities(data[i], w, encoding, indexLinearizer);
				for (int j=0; j<encoding.getNumLabels(); j++) {
					for (int k=0; k<data[i].featureIndexes.length; k++) {
						double term = data[i].featureCounts[k] * Math.exp(logProbs[j]);
						int linearIndex = indexLinearizer.getLinearIndex(data[i].featureIndexes[k], j);
						derivatives[linearIndex] = derivatives[linearIndex] - term;
					}
				}
			}
			
			// add in terms for regularization term
			double k = 1/(2*sigma*sigma);
			double normWeightsSq = 0.0;
			for (int i=0; i<w.length; i++) {
				normWeightsSq += w[i] * w[i];
			}
			objective = objective - k*normWeightsSq;
			
			for (int i=0; i<w.length; i++) {
				derivatives[i] = derivatives[i] - k*w[i];
			}
	 ...
	 */
	public static <F,L> double[] getLogProbabilities(EncodedDatum datum, double[] weights, Encoding<F, L> encoding, IndexLinearizer indexLinearizer) {
		double[] logProbs = new double[ encoding.getNumLabels()];
		double[] features = datum.featureCounts;
		int[] indices = datum.featureIndexes;
		for (int i=0; i<datum.getNumActiveFeatures(); i++) {
			for (int j=0; j<encoding.getNumLabels(); j++) {
					int linearIndex = indexLinearizer.getLinearIndex(indices[i], j);
					double vote = features[i] * weights[linearIndex];
					logProbs[j] = logProbs[j] + vote;
				}
		}
		
		double logDenom = logOfSum(logProbs);
		for (int i=0; i<logProbs.length; i++) {
			logProbs[i] = logProbs[i] - logDenom;
		}
		return logProbs;
	}
	...
	private Counter<L> getProbabilities(FeatureVector<F> featureVector) {
		...
		return logProbabilityArrayToProbabiltyCounter(logProbabilities);
	}

	private Counter<L> logProbabilityArrayToProbabiltyCounter(double[] logProbabilities) {
	...
	public static double logOfSum(double[] logs) {
		...
		double innerSum = 0;
		for (int i=0; i<logs.length; i++) {
			double term = Math.exp(logs[i] - logs[imax]);
			innerSum += term;
		}
		
		return logs[imax] + Math.log(innerSum);
	}
}
