...
public class RandomMatrix extends Matrix
{
    ...
    */
    ...
    public static RandomMatrix dirac(int m, int n, Matrix val_prob)
    {
        ...
        for (int i = 0; i < m; i++)
        {
            for (int j = 0; j < n; j++)
            {
                x = RandomVariable.dirac(val_prob.getColumnArrayCopy(0),
                        val_prob.getColumnArrayCopy(1));
                C[i][j] = x;
            }
        }
    ...
    */
     ...
     *  ! WARNING : this simulation technic can take a very long time !
     @param m    Number of rows.
     @param n    Number of columns.
     @param fun    Density methodName of the random variable.
     @param min    Min of the random variable.
     @param max    Max of the random variable.
     @return      A RandomMatrix.
     */
    public static RandomMatrix rejection(int m, int n, DoubleFunction fun,
    ...
    public static RandomMatrix sampleWithoutReplacement(int m, int n, Matrix B)
    {
        if ((m * n) > ((B.m) * (B.n)))
        {
            throw new IllegalArgumentException(
                "Matrix number of elements must be < " + ((B.m) * (B.n)));
        }
    ...
    */
    ...
    @return      A RandomMatrix.
    */
    public static RandomMatrix triangular(int m, int n, double min, double med,
        double max)
    {
        RandomMatrix X = new RandomMatrix(m, n);
        double[][] C = X.getArray();
