...
public class RecordFiller {
	...
	// this will fail for even moderately sized durations
	private Integer durationToInt(Duration d) {
		return 
			d.durationSeconds +
			d.durationMinutes * 60 +
			d.durationHours * 60 * 60 +
			d.durationDays * 24 * 60 * 60 +
			d.durationWeeks * 7 * 24 * 60 * 60 +
			d.durationMonths * 30 * 7 * 24 * 60 * 60 +
			d.durationYears * 360 * 30 * 7 * 24 * 60 * 60;
	}
	...
	private Integer normalizeFold(
		...
		switch(fun) {
			case Foldl:
				...
				// leftMost  is acc
				// rightMost is list to fold over
				
				// the idea is simply to construct a new term representing n
				// applications of lamE to succesive elements of the list and
				// then finally normalize the constructed exp
				// i.e.:
				//  app(app(lamE, app(app(lamE, leftMost), rightMost[0])), rightMost[1]) ...
				curAccIdx = leftMostIdx;
				for(Integer ix : exprMap.get(rightMostIdx).getListExps()) {
		...
		throws InferError, InferErrorFieldNotFound
	{
		...
		if(exp.isSetBinaryOpExp()) {
			...
			// emulate duration with secs!
			pv = new PoetsValue.IntV(durationToInt(exp.getDurationExp()));
		} else if(exp.isSetDoubleExp()) {
			pv = new PoetsValue.DoubleV(exp.getDoubleExp());
		...
		} else if(exp.isSetRefExp()) {
			RefVal rv = exp.getRefExp();
			pv = new PoetsValue.RefV(
					rv.getRecordName(), rv.getRefPointer());
		} else if(exp.isSetListExps()) {
