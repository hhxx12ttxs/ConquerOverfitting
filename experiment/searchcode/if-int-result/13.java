package pl.zgora.uz.wmie.fe.abstracts;
...
import pl.zgora.uz.wmie.fe.util.HibernateUtil;
...
public abstract class GenericDAO<T, Id extends Serializable> extends
	...
	/*TODO jesli polaczenia na tabelce sa nastepujace
	<FE_Table:column sort="yes" label="Identyfikator" databaseColumn="<%=Stanowisko.PROPERTY_ID %>" />
	<FE_Table:column sort="yes" label="dzial" databaseColumn="<%=Stanowisko.PROPERTY_OBJECT_PPRACOWNIKS+'.'+Pracownik.PROPERTY_OBJECT_DZIALS+'.'+Dzial.PROPERTY_NAZWA_DZIALU %>" />
	*/
	protected DataResult findByParams(ProjectionList projections,
		...
		try {
			...
			projections = HibernateUtil.projectionSupport(criteria,
					projections);
	...
	//TODO optymalizacja
	protected int getRowCount(ProjectionList projections, List<Criterion> criterions) throws Exception {
		Session session = null;
		int result = 0;
		try{
			session = getSession();
	...
	private int getRowCount(Criteria criteria, ProjectionList projectionList){
		...
		if(stringProjection.contains("distinct")){
			criteria.setProjection(Projections.countDistinct(stringProjection.split(" ")[1]));
		} else {
