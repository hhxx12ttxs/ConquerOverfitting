...
public class ByteUtils {
	...
	public static byte[] range(int from, int to, byte[] a) {
		if (to < 0 || to >= a.length) // was: (to < 0 || to > a.length)
			to = a.length - 1;
		if (to < from)
			return new byte[0];
	...
	public static String byteToHex(byte b){
		int i = b & 0xFF;
		String intAsHex = Integer.toHexString(i).toUpperCase();
	...
	public static int byteArrayToInt(byte[] bytes) {
		...
		for(int i = 0 ; i < bytes.length ; i = i + 3) {
			...
			if(k >= bytes.length){
				if(j >= bytes.length){
					result = result | mergeBytes(bytes[i], bytes[j]);
				} else {
					result = result | unsignedByteToInt(bytes[i]);
				}
			} else {
				result = result | mergeBytes(bytes[i], bytes[j], bytes[k]);
			}
		}
		return result;
	}
	...
	public static byte[] timeToA(int time) {
		if(time > 0xFFFF){
			time = 0xFFFF;
		}
		int lsb = time & 255;
		int msb = time >> 8;
		byte[] timeA = {(byte)msb, (byte)lsb};
		return timeA;
	}
