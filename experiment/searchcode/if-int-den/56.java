...
public class GraphicsContent extends BasicGame {
    ...
    /**
     * Die Grφίe eines Feldes Richtung X.
     * Fόr Zeichenkoordinatenbestimmungen durch multiplizieren wird aber die Halbe benφtigt!
     */
    public static final int FIELD_SIZE_X = 20;
    /**
     * Die Grφίe eines Feldes Richtung Y.
     * Fόr Zeichenkoordinatenbestimmungen durch multiplizieren wird aber die Halbe benφtigt!
     */
    public static final int FIELD_SIZE_Y = 15;
    /**
     * Die halbe Grφίe eines Feldes in X-Richtung
     */
    public static final int FIELD_HALF_X = FIELD_SIZE_X / 2;
    /**
     * Die halbe Grφίe eines Feldes in Y-Richtung
     */
    public static final double FIELD_HALF_Y = FIELD_SIZE_Y / 2.0;
    /**
     * Wie viele Pixel das tatsδchlich gezeichnete Feld von den Zeichenkoordinaten entfernt ist.
     */
    ...
    /**
     * Wie viele Pixel der Zeichenursprung fόr ein 1x1 Feld von dem Ursprung des Zuordungsfeldes entfernt ist.
     */
    ...
    /**
     * Wie viele Pixel der Zeichenursprung fόr ein 2x2 Feld von dem Rohkoordinatenfeld entfernt ist.
     */
    ...
    /**
     * Diesen Abstand muss man bei allen Zeichenoperationen reinrechnen, die mit prδzisen Pixeln arbeiten.
     */
    ...
    // Diese Klasse reprδsentiert den Tatsδchlichen GrafikINHALT von RogGraphics und RogMapEditor
    public GraphicsImage colModeImage;
    public int modi = -2; // Was gerendert werden soll, spezielle Ansichten fόr den Editor etc...
    ...
    public int sizeX; // Mapgrφίe
    public int sizeY;
    public int positionX; // Position, die angezeigt wird
    public int positionY;
    public int viewX; // Die Grφίe des Angezeigten ausschnitts
    public int viewY;
    public int realPixX; // Echte, ganze X-Pixel
    public int realPixY; // Echte, ganze Y-Pixel
    private HashMap<String, GraphicsImage> imgMap; // Enthδlt alle Bilder
    public HashMap<String, GraphicsImage> coloredImgMap;
    Image renderBackground = null; // Bodentextur, nur Bild-Auflφsung
    int rBvX; // Position wδhrend der letzen Bildberechnung
    int rBvY; // Position wδhrend der letzen Bildberechnung
    public boolean serverColMode = false;
    boolean useAntialising = false; // Kantenglδttung, normalerweise AUS // NUR IM NORMALEN RENDERMODE
    public List<Sprite> allList;
    public static boolean alwaysshowenergybars = false; // Energiebalken immer anzeigen, WC3 lδsst grόίen.
    Date initRun;
    ...
    private static final boolean beautyDraw = true;     // Schφner zeichnen durch umsortieren der allList, kostet Leistung
    ...
    public boolean saveMode = false;                    // Sicherer Grafikmodus ohne unsichere, weil zu Groίe Bilder
    public boolean coordMode = false;                          // Modus, der die Koordinaten der Felder mit anzeigt.
    public boolean idMode = false;                             // Modus, der die NetIds von Einheiten/Gebδuden und Ressourcen mit anzeigt.
    ...
    boolean updateBuildingsFow = false;                 // Lδsst den Gebδude-Fow neu erstellen
    ...
    public void paintComponent(Graphics g) {
        //Die echte, letzendlich gόltige paint-Methode, sollte nicht direkt aufgerufen werden
        if (modi == -2) {
            // 13Ducks-Logo
            ...
            // Sobald es einmal gezeichnet wurde kφnnen wir weiter laden
            initState = 12;
            modi = 0;
        } else if (modi == -1) {
            ...
            try {
                if (pauseMode) {
                    // Pause fόr diesen Frame freischalten:
...
//                }
                ...
                // Einheiten nach ihrer Y-Enfernung sortieren, erzeugt rδumlichen Eindruck.
                if (beautyDraw) {
                    try {
                        allListLock.lock();
...
//                    // Wenn sichs geδndert hat neu suchen
...
//                }
     ...
     * Schmeiίt SkyEffects raus, die fertig sind.
     ...
     * gemδί dem FOW gezeichnet werden sollen und sich nicht verstecken.
     * @param g
     */
     ...
     * Reine Scroll-άberprόfung, macht nichts mit FOW oder verstecken.
     * @param s
     * @return
     */
     ...
     * Reine Scroll-άberprόfung, macht nichts mit FOW.
     * @param s
     * @return
     */
...
//                    // Target (ziel)
//                    // Diese 2. Abfrage ist sinnvoll und notwendig, weil getMovingPosition am Ende der Bewegung das movingtarget lφscht
//                    if (unit.movingtarget != null) {
//                        int tx = (unit.movingtarget.getX() - positionX) * 20;
//                        int ty = (unit.movingtarget.getY() - positionY) * 15;
//                        // Zeichnen
//                        g2.setColor(new Color(3, 100, 0)); // Dunkelgrόn
...
//                        //Gebδude-Mitte finden:
//                        float bx = 0;
//                        float by = 0;
//                        //Z1
//                        //Einfach die Hδlfte als Mitte nehmen
...
//        g2.setLineWidth(1);
    ...
    /**
     * Zeichnet NetIds von allen Einheiten / Gebδuden / Ressourcen
...
//        }
    ...
    private void renderLoadScreen(Graphics g2) {
        ...
        int ly = realPixY / 50;
        int dx = realPixX / 2;      // Lδnge des Balkens
        int dy = realPixY / 32;
        ...
        if (launchError) {
            ...
            int edx = realPixX / 5 * 3;      // Lδnge des Balkens
            int edy = realPixY / 2;
            // Kasten zeichnen
            g2.setColor(Color.red);
...
//        // Murks, aber die position mόssen gerade sein...
//        if (positionX % 2 == 1) {
//            positionX--;
//        }
//        if (positionY % 2 == 1) {
...
//        // Bpxen kφnnen nur von links oben nach rechts unten gezogen werden - eventuell Koordinaten tauschen
//        if (dirX < 0 && dirY > 0) {
//            // Nur x Tauschen
...
//        g2.drawRect(this.boxselectionstart.width, this.boxselectionstart.height, dirX, dirY);
    ...
    private void renderBackground() {
        ...
        // Alte Methode - Groίe Background-Bilder im Speicher vertrδgt nicht jedes System
...
//        }
    ...
    private void renderCoords(Graphics g2) {
        ...
        g2.setColor(Color.black);
        g2.setLineWidth(1);
        for (int i = 0; i < viewX * 2; i += 2) { // Doppelt so lang, das Bildverhδltniss ist ja in der Regel nicht quadratisch
    ...
    public void setVisibleArea(int vX, int vY) {
        //Setzt die Grφίe des sichtbaren Bereichs
        viewX = vX;
        viewY = vY;
    ...
    // Lφscht die gesamte Fow-Map (όberschreibt sie mit 0 (unerkundet))
    ...
    /**
     * Setzt alles auf Sichtbar. Kann nicht rόckgδngig gemacht werden. Ist cheaten
     */
    ...
    public void startRender() {
        ...
        modi = 3;
        initRun = new Date();// Das mach ich jetzt genau ein mal, das muss in den Gebδudebau-Handling-Code...
//        try {Thread.sleep(100000);} catch (Exception ex) {}
    ...
    /**
     * Informiert die Minimap darόber, dass sich die derzeitige AnzeigePosition geδndert hat.
     */
    ...
    public int getModi() {
        // liefert den aktuellen renderModus zurόck
        return modi;
    }

    public void calcColoredMaps(Color[] colors) {
        // Berechnet die eingefδrbten Texturen
    ...
    public void enableAntialising() {
        // Schaltet die Kantenglδttung an
        useAntialising = true;
    }

    /**
     * Die Updates des FoW dόrfen nicht wδhrend der Bildberechnung sein, sonst flackert es.
     */
...
//        }
    ...
    private void cutSight(Building b) {
        ...
        // Diese Position als Startfeld όberhaupt zulδssig?
        if ((x + y) % 2 == 1) {
            y++;
        }
    ...
    /**
     * Setzt den Fow-Sichtlayer von sterbenden Einheitn auf Unit, damit die Gegend beim nδchsten Update auf erkundet gesetzt werden kann.
     * @param b
     */
...
//                        // Nix tun, ein Kreis kann ja den Maprand schneiden, da gibts natόrlich dann kein FoW-Raster...
//                    }
//                }
//            }
//        }
     ...
     * @param origin Mitte
     * @param building Fόr Gebδude (true) oder Einheit (false)
     */
...
//        }
    ...
    /**
     * Muss regelmδίig vom Grafikmodul aufgerufen werden (4 mal pro Sekunde oder so)
...
//            }
//        }
//        // Fόr alle Einheiten auf der Map
...
//        }
    ...
    public void render(GameContainer container, Graphics g) throws SlickException {
        if (initState == -1) {
            ...
            // Komponenten fόrs Hauptmenu laden
