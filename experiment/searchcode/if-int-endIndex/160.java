 ...
 */
package com.ironore.io;

import com.ironore.utils.Utils;


/**
 * Buffer that stores data as an array of characters. It expands automatically when it's
 * current capacity is exceeded, this happends when appending more data (characters in this
 * case).
 * <p/>
 * Capacity is the total size this buffer can hold.
 * Length is the current size the buffer has used up holding data.
 * 
 * @author tmjee
 */
public class CharArrayBuffer {
    ...
    /**
     * Compact the buffer, remove everything before this <code>index</code>, data/bytes under this
     * <code>index</code> will be included in the compacted buffer. The new buffer will
     * contains stuff starting from this <code>index</code>
     * 
     * Putting index < 0 will not do anything. 
     * 
     * Putting index > buffer's length will clear up everything in the buffer 
     * 
     * @param index
     */
    public void compact(int index) {
    	if (index >= 0) {
    		...
    		if (index < len) {
    			System.arraycopy(this.buffer, index, newBuffer, 0, len - index);
    			len = len - index;
    		}
    		else {
    			len = 0;
    ...
    public String substringTrimmed(int beginIndex, int endIndex) {
        ...
        while (beginIndex < endIndex && Utils.isWhitespace(this.buffer[beginIndex])) {
            beginIndex++;
        }
        while (endIndex > beginIndex && Utils.isWhitespace(this.buffer[endIndex - 1])) {
            endIndex--;
        }
        return new String(this.buffer, beginIndex, endIndex - beginIndex);
    }
