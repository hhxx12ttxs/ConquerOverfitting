		...
		synchronized (_myBuffer) {
			...
			if (length > theBytes.length) {
				System.err.println("readBytesUntil() byte buffer is" + " too small for the " + length
						+ " bytes up to and including char " + theLookUpByte);
				return -1;
			}
			// byte outgoing[] = new byte[length];
			System.arraycopy(_myBuffer, _myBufferIndex, theBytes, 0, length);
...
/*
 * class SerialMenuListener implements ItemListener { //public SerialMenuListener() { }
 * 
 * public void itemStateChanged(ItemEvent e) { int count = serialMenu.getItemCount(); for (int i = 0; i < count; i++) {
 * ((CheckboxMenuItem)serialMenu.getItem(i)).setState(false); } CheckboxMenuItem item = (CheckboxMenuItem)e.getSource();
 * item.setState(true); String name = item.getLabel(); //System.out.println(item.getLabel());
 * PdeBase.properties.put("serial.port", name); //System.out.println("set to " + get("serial.port")); } }
 */

/*
 * protected Vector buildPortList() { // get list of names for serial ports // have the default port checked (if present) Vector
 * list = new Vector();
 * 
 * //SerialMenuListener listener = new SerialMenuListener(); boolean problem = false;
 * 
 * // if this is failing, it may be because // lib/javax.comm.properties is missing. // java is weird about how it searches for
 * java.comm.properties // so it tends to be very fragile. i.e. quotes in the CLASSPATH // environment variable will hose things.
 * try { //System.out.println("building port list"); Enumeration portList = CommPortIdentifier.getPortIdentifiers(); while
 * (portList.hasMoreElements()) { CommPortIdentifier portId = (CommPortIdentifier) portList.nextElement();
 * //System.out.println(portId);
 * 
 * if (portId.getPortType() == CommPortIdentifier.PORT_SERIAL) { //if (portId.getName().equals(port)) { String name =
 * portId.getName(); //CheckboxMenuItem mi = //new CheckboxMenuItem(name, name.equals(defaultName));
 * 
 * //mi.addItemListener(listener); //serialMenu.add(mi); list.addElement(name); } } } catch (UnsatisfiedLinkError e) {
 * e.printStackTrace(); problem = true;
 * 
 * } catch (Exception e) { System.out.println("exception building serial menu"); e.printStackTrace(); }
 * 
 * //if (serialMenu.getItemCount() == 0) { //System.out.println("dimming serial menu"); //serialMenu.setEnabled(false); //}
 * 
 * // only warn them if this is the first time if (problem && PdeBase.firstTime) { JOptionPane.showMessageDialog(this, //frame,
 * "Serial port support not installed.\n" + "Check the readme for instructions\n" + "if you need to use the serial port.    ",
 * "Serial Port Warning", JOptionPane.WARNING_MESSAGE); } return list; }
 */
