...
public class HistogramGraph extends Display {
	...
	protected BarRenderer m_shapeR = new BarRenderer(2);
	...
	private void initializeAxes(String fieldName) {
		...
		DecimalFormat axisNumberFormat = new DecimalFormat();
		axisNumberFormat.setMaximumFractionDigits(2);
		axisNumberFormat.setMinimumFractionDigits(2);
		

		m_xLabels = new AxisLabelLayout("xlabels", m_xAxis, m_xlabB);
		m_xLabels.setNumberFormat(axisNumberFormat);
		m_vis.putAction("xlabels", m_xLabels);

		m_yLabels = new AxisLabelLayout("ylabels", m_yAxis, m_ylabB);
		m_yLabels.setNumberFormat(axisNumberFormat);
