...
public class GA extends TravelingSalesman
{
	...
	private static int MAX_CITIES = 9;
	...
	private static String[] cityName = {"Aberdeen",
        "Ayr",
        "Edinburgh",
        "Fort Williams",
        "Glasgow",
	...
	private static void initialiseThePopulation() 
	{
		...
		int chromosome, i, startCity,  refLength;
		...
		for (chromosome = 0; chromosome < populationSize; chromosome++) 
		{
			...
			for (i = 0; i < numberOfCities-1; i++) 
			{
				...
				startCity = 2;
				//endCity = 2;
				currentPopulation[chromosome][i] = referenceSolution[startCity];
				referenceSolution[startCity] = referenceSolution[refLength - 1];
				refLength--;
			}
		}
	}
	...
	private static int[] mutate(int[] child) {
		int g = getRandomNumberBetween(0, numberOfCities);
		int temp = child[g];
		child[g] = child[g + 1];
		child[g + 1] = temp;
		return child;
