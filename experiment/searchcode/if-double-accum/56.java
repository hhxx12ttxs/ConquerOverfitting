...
public class HBaseClient extends com.yahoo.ycsb.DB
{
  ...
  for (KeyValue kv : r.raw()) {
	...
	public int readMultiple(String table, Set<String> keys, Set<String> fields, List<Map<String,String>> result) {
		...
		final StringBuffer debugBuffer = (_debug ? new StringBuffer() : null);
		if (_debug) {
			debugBuffer.append("Doing read multiple from HBase columnfamily ");
			debugBuffer.append(_columnFamily);
			debugBuffer.append(" keys (");
		}

		final int numberOfGets = keys.size();
		final List<Row> gets = new ArrayList<Row>(numberOfGets);
		for (final Iterator<String> keysIterator = keys.iterator(); keysIterator.hasNext();) {
			...
			if (fields == null) {
				get.addFamily(_columnFamilyBytes);
			}
			else {
				for (final String field : fields) {
					get.addColumn(_columnFamilyBytes, Bytes.toBytes(field));
				}
			}
		}
		...
		final Result[] rows = new Result[numberOfGets];
		try {
			_hTable.batch(gets, rows);
		}
		...
		for (final Result row : rows) {
			final Map<String,String> rowResult = new HashMap<String,String>();
			for (final KeyValue keyValue : row.raw()) {
				rowResult.put(
