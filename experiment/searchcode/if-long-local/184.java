...
public class TaskTracker 
  ...
  public static float getCurTemp(){
      ...
      try
      {
              Process myProcess = Runtime.getRuntime().exec("sensors");
              ...
              ansSplit = ans.split("[+?]");;
              temperature = Float.parseFloat(ansSplit[1]);
              temperature += Float.parseFloat(ansSplit[5]);
              temperature += Float.parseFloat(ansSplit[9]);
              temperature += Float.parseFloat(ansSplit[13]);
      }
      catch (IOException ex)
      {
              LOG.error("Error in TaskTracker:getCurTemp " + ex);
      }
  ...
  public int getCurFreq(){
      int intFreq = -1;
      try{
              Process myProcess = Runtime.getRuntime().exec("python /home/tarek/hieule2/shenli/Java/sudo_readFreq.py");
              InputStreamReader myIStreamReader = new InputStreamReader(myProcess.getInputStream());
              String strFreq = "";
              int ch;
              while ((ch = myIStreamReader.read()) != -1){
              ...
              String [] strSplit = strFreq.split("[\n\r]");
              /*for(int i = 0 ; i < strSplit.length; ++i){
                      System.out.println(i + ": " + strSplit[i]);
              }*/


      //      System.out.println("I get : " + strSplit[strSplit.length - 1]);
              intFreq = Integer.parseInt(strSplit[strSplit.length - 1]);
      }
      catch(IOException ex){
              LOG.error("*shenli* Error in TaskTracker:getCurFreq, " +  ex.getMessage());
      }
      return intFreq;
  }
  
  public void setNewFreq(int frequency){
      try{
	    	  String cmd = "python /home/tarek/hieule2/shenli/Java/sudo_setFreq.py " + frequency;
	          ...
	          LOG.error("*shenli*" + strFreq);
    	  
              //Runtime.getRuntime().exec("python /home/tarek/hieule2/shenli/Java/sudo_setFreq.py " + frequency);
              LOG.error("*shenli* In host :" + localHostname + " frequency setted to :" + frequency);
              LOG.error("*shenli* " + localHostname + " Current frequency is :" + getCurFreq());
              
      }
      catch(IOException ex){
              LOG.error("*shenli*" + localHostname + " Error in TaskTracker:setNewFreq, " +  ex.getMessage());
      }
  }
  ...
  public void setFreq(int freq){
	  ...
	  try{
		  BufferedWriter myWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("/home/shen/workspace/test_output.txt", true)));
	  
		  myWriter.write("frequency on host " + localHostname + " is set to " + freq + "\n");
	  
		  myWriter.flush();
	  }
	  catch(Exception ex){
		  LOG.error("in TaskTracker:setFreq, file output eroor");
	  }
  }*/
  //End
  ...
  private HeartbeatResponse transmitHeartBeat(long now) throws IOException {
    ...
    //Shen Li Read the current Frequency and Temperature here
    ...
    //Shen Li Set new frequency here
    
    int newFreq = heartbeatResponse.getFrequency();
    LOG.error("*shenli* in TaskTracker:transmitHeatbeat, newFreq is " + newFreq + " " + heartbeatResponse.frequency);
    LOG.error("*shenLI* in TaskTracker:transmitHeatbeat, responseId is " + heartbeatResponse.responseId);
    LOG.error("*shenLI* in TaskTracker:transmitHeatbeat, responseInterval is" + heartbeatResponse.getHeartbeatInterval());
    if(newFreq > 0)
    	setNewFreq(newFreq);
