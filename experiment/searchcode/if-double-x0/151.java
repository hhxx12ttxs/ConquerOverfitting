...
public class Point3D implements Representable {
	...
	public Representable place(MODObjet mod) {
		Point3D p = this;
		/*
		 * p = p.homothetie(mod.homothetie().centre(),
		 * mod.homothetie().facteur()); Point3D r =
		 * mod.rotation().centre().homothetie(mod.homothetie().centre(),
		 * mod.homothetie().facteur()); p = p.rotation(mod.rotation().matrice(),
		 * r); p = p.plus(mod.translation().vecteur());
		 */
		return p;
	}
