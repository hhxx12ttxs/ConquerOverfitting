...
public class Block
{
    ...
    {
    	if (mod_XRay.on || mod_XRay.cavefinder || mod_XRay.lightmode == 2) return 1.3F;
    ...
    {
		if (mod_XRay.on) {
			for (int n : mod_XRay.blackList)
				if (n == blockID)
					return false;

			if (mod_XRay.blackList.length != 0) return true;
		} else if (mod_XRay.cavefinder) {
			for (int n : mod_XRay.blackList){
				if (n == 1) continue; //Skip stone
				if (n == blockID) return false;
			}
		}
    ...
    {
        entityplayer.func_25058_a(StatList.field_25159_y[blockID], 1);
    ...
    {
        return StatCollector.func_25200_a((new StringBuilder()).append(getBlockName()).append(".name").toString());
    }
    ...
    static 
    {
        ...
        blockBed = (new BlockBed(26)).setHardness(0.2F).setBlockName("bed");
        ...
        redstoneRepeaterIdle = (new BlockRedstoneRepeater(93, false)).setHardness(0.0F).setStepSound(soundWoodFootstep).setBlockName("diode");
        redstoneRepeaterActive = (new BlockRedstoneRepeater(94, true)).setHardness(0.0F).setLightValue(0.625F).setStepSound(soundWoodFootstep).setBlockName("diode");
