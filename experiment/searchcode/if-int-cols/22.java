...
public class ImageUtils {
    ...
    public static void main(String[] args) {
        ...
        // 方法二：按高度和宽度缩放
    	// ImageUtils.scale2("e:/abc.jpg", "e:/abc_scale2.jpg", 200, 100, true);//测试OK
        ...
        // 6-给图片添加图片水印：
    	ImageUtils.pressImage("e:/abc2.png", "e:/abc.jpg","e:/abc_pressImage.jpg", 1, -1, 0.6f);//测试OK
    }
    ...
    public final static void pressImage(String pressImg, String srcImageFile,String destImageFile,
        ...
        try {
            ...
            g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP,
                    alpha));
            // 在指定坐标绘制图片水印
            int onWidth = wideth - wideth_biao;
            int onHeight = height - height_biao;
            // 设置值大于源文件尺寸、或xy其中有一项为负数、则水印居中显示
            if(x>onWidth || y>onHeight || (x<0 && y>0) || (x>0 && y<0)){
            	onWidth = onWidth / 2;
            	onHeight = onHeight / 2;
            }else if(x<0 && y<0){ // 设置值xy都为负数、则从右下角显示水印
            	onWidth = onWidth + x;
            	onHeight = onHeight + y;
            }else{ // 设置值xy都为正数、则从左下角显示水印
            	onWidth = x;
            	onHeight = y;
            }
            g.drawImage(src_biao ,onWidth , onHeight, wideth_biao, height_biao, null);
            // 水印文件结束
            g.dispose();
            ImageIO.write((BufferedImage) image,  "JPEG", new File(destImageFile));
        } catch (Exception e) {
