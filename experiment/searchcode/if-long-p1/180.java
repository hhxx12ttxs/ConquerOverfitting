...
public class FVenda extends FVD implements PostListener, CarregaListener, FocusListener, ActionListener, InsertListener, DeleteListener, KeyListener, InterVenda, CancelListener {
	...
	private JButtonPad btBuscaOrc = new JButtonPad( "Or?amento", Icone.novo( "btVenda2.gif" ) );

	private JButtonPad btDevolucaoConserto = new JButtonPad( "Devolu??o", Icone.novo( "btRetorno.png" ) );
	...
	// Campo para guardar o c?digo do comissionado anterior
	private int codvendold = 0;
	
	// Campo flag para definir se os itens devem ser reprocessados para ajuste de comiss?o
	...
	private void montaTela() {
		...
		pnGImp.add( btDescIPI );// Agora o painel est? maior
		pnGImp.add( btAssocContr );// Agora o painel est? maior
		...
		lcCli.add( new GuardaCampo( txtCodmunicCli, "Codmunic", "C?d.munic.", ListaCampos.DB_SI, false ) );
		lcCli.add( new GuardaCampo( txtBairCli, "Baircli", "Bairro", ListaCampos.DB_SI, false ) );
		lcCli.add( new GuardaCampo( txtEndCli, "Endcli", "Endere?o", ListaCampos.DB_SI, false ) );
		lcCli.add( new GuardaCampo( txtNumCli, "Numcli", "N?m.end.", ListaCampos.DB_SI, false ) );

		lcCli.add( new GuardaCampo( txtSiglaUFEnt, "SiglaUFEnt", "UF Ent.", ListaCampos.DB_SI, false ) );
		lcCli.add( new GuardaCampo( txtCodmunicEnt, "CodmunicEnt", "C?d.munic.entr.", ListaCampos.DB_SI, false ) );
		lcCli.add( new GuardaCampo( txtBairEnt, "BairEnt", "Bairro entrega", ListaCampos.DB_SI, false ) );
		lcCli.add( new GuardaCampo( txtEndEnt, "EndEnt", "Endere?o entrega", ListaCampos.DB_SI, false ) );
		lcCli.add( new GuardaCampo( txtNumEnt, "NumEnt", "N?m.end.ent.", ListaCampos.DB_SI, false ) );
		...
		lcColeta.add( new GuardaCampo( txtTicket, "Ticket", "C?d.coleta", ListaCampos.DB_PK, false ) );
		...
		// lc para vinculo de ?tem de venda com ?tem de compra
		...
		// lc para vinculo de ?tem de venda com nota de remessa

		lcItRemessa.add( new GuardaCampo( txtCodVendaRemessa, "CodVendaVR", "C?d.venda", ListaCampos.DB_PK, false ) );
		lcItRemessa.add( new GuardaCampo( txtCodItVendaRemessa, "CodItVendaVR", "C?d.it.venda", ListaCampos.DB_PK, false ) );
		...
		btBuscaOrc.setToolTipText( "Busca or?amentos" );
		btDevolucaoConserto.setToolTipText( "Devolu??o de consertos" );
		btComplementar.setToolTipText( "Gera??o de nota complementar" );
		...
		lcTipoMov.add( new GuardaCampo( txtCodTranTipoMov, "CodTran", "C?d.transp.", ListaCampos.DB_SI, false ) );
		...
		//lcTipoMov.add( new GuardaCampo( txtOperTipoMov, "OperTipoMov", "Opera??o", ListaCampos.DB_SI, false ) );
		...
		adicCampo( txtCodTipoMov, 90, 20, 77, 20, "CodTipoMov", "C?d.tp.mov.", ListaCampos.DB_FK, txtDescTipoMov, true );
		adicDescFK( txtDescTipoMov, 170, 20, 207, 20, "DescTipoMov", "Descri??o do tipo de movimento" );
		...
		adicCampo( txtDtEmitVenda, 540, 20, 97, 20, "DtEmitVenda", "Data da emiss?o", ListaCampos.DB_SI, true );
		adicCampo( txtDtSaidaVenda, 640, 20, 97, 20, "DtSaidaVenda", "Data da sa?da", ListaCampos.DB_SI, true );

		adicCampo( txtCodCli, 7, 60, 80, 20, "CodCli", "C?d. cli.", ListaCampos.DB_FK, txtDescCli, true );
		adicDescFK( txtDescCli, 90, 60, 264, 20, "RazCli", "Raz?o social do cliente" );
		adicDescFK( txtSiglaUFCli, 357, 60, 20, 20, "SiglaUF", "UF" );

		adicCampo( txtCodPlanoPag, 380, 60, 77, 20, "CodPlanoPag", "C?d.p.pag.", ListaCampos.DB_FK, txtDescPlanoPag, true );
		adicDescFK( txtDescPlanoPag, 460, 60, 178, 20, "DescPlanoPag", "Descri??o do plano de pag." );
		adicCampo( txtPedCliVenda, 640, 60, 97, 20, "PedCliVenda", "N.ped.cli.", ListaCampos.DB_SI, false );
		adicCampoInvisivel( txtTicket, "ticket", "C?d.coleta", ListaCampos.DB_FK, false );
		...
		adicCampo( txtCodVend, 7, 20, 80, 20, "CodVend", "C?d.comis.", ListaCampos.DB_FK, txtNomeVend, true );
		...
		adicDB(rgLocalServico, 608, 15, 133, 40, "LocalServ", "Local do Servi?o", false);
		...
		// SwingParams.getPanelLabel( "Entrada via c?digo de barras", Color.RED );c
		...
		if ( (Boolean) oPrefs[ POS_PREFS.USAREFPROD.ordinal() ] ) {
			...
			adicCampoInvisivel( txtRefProd, "RefProd", "Refer?ncia", ListaCampos.DB_SI, true );
			...
			adicCampo( txtCodProd, 40, 20, 70, 20, "CodProd", "C?d.prod.", ListaCampos.DB_FK, txtDescProd, true );
		}

		adicDescFK( txtDescProd, 113, 20, 190, 20, "DescProd", "Descri??o do produto" );
		...
		adicCampoInvisivel( txtCodCompra, "codcompra", "C?d.Compra", ListaCampos.DB_FK, false );
		adicCampoInvisivel( txtCodItCompra, "coditcompra", "C?d.It.Compra", ListaCampos.DB_FK, false );

		adicCampoInvisivel( txtCodVendaRemessa, "codVendaVR", "C?d.remessa", ListaCampos.DB_FK, false );
		adicCampoInvisivel( txtCodItVendaRemessa, "codItVendaVR", "C?d.it.remessa", ListaCampos.DB_FK, false );
		adicCampoInvisivel( txtTipoVendaRemessa, "tipoVendaVR", "tipo remessa", ListaCampos.DB_FK, false );

		lbNumSerie = adicCampo( txtNumSerie, 7, 100, 150, 20, "NumSerieTmp", "N?mero de s?rie", ListaCampos.DB_FK, txtObsSerie, false );
		...
		// Bloqueia edi??o do pre?o de venda conforme prefer?ncias
	...
	private void initPinCabOrcamento() {
		...
		tpnCab.addTab( "Or?amentos", pinCabOrcamentos );
		pinCabOrcamentos.add( spOrcamentos, BorderLayout.CENTER );

		tabOrcamentos.adicColuna( "C?d.Orc" );
		...
		tabOrcamentos.adicColuna( "C?d.pag." );
		tabOrcamentos.adicColuna( "Descri??o do plano de pagamento" );
	...
	private void getCFOP() {
		...
		try {
			...
			// Incluido parametro com o c?digo do item fiscal
	...
	private void carregaComisIt(){
		...
		/**
		 * A ideia inicial era um sql unico com union e unico resultado por?m n?o consegui aplicar um order by
		 * que trabalha-se da forma a qual seria necess?ria
		 */
	...
	private void senhaCredito( boolean fechamento ) {
		...
		if ( fpw.OK ) {
			...
			if ( !Aplicativo.telaPrincipal.temTela( "Libera??o de cr?dito" ) ) {
				FLiberaCredito tela = new FLiberaCredito();
				Aplicativo.telaPrincipal.criatela( "Libera??o de cr?dito", tela, con );
	...
	private boolean senhaLiberaAtraso() {
		...
		FPassword fpw = new FPassword( this, FPassword.LIBERA_CRED, "Libera??o de t?tulo em atraso", con );
		fpw.execShow();
		result=fpw.OK;
		fpw.dispose();
		return result;
	...
	private boolean consultaCredito( BigDecimal vlradic, boolean fechamento ) {
		...
		try {
			...
			if ( mens.indexOf( "VENDA" ) > -1 ) {
				...
				if ( Funcoes.mensagemConfirma( this, " O valor da venda ultrapassa o limite de cr?dito pr?-estabelecido!\n\n " + mens + "\n\n" + "Deseja efetuar libera??o agora?" ) == JOptionPane.YES_OPTION ) {
					senhaCredito( fechamento );
				}

				Logger.gravaLogTxt( "", Aplicativo.getUsuario().getIdusu(), Logger.LGEB_BD, "Problema com limite de cr?dito." + mens );
			}
	...
	private void abreBuscaOrc() {
		...
		if ( !Aplicativo.telaPrincipal.temTela( "Busca or?amento" ) ) {
			// Se o tipo de emiss?o for diferente de 1, ent?o ambiente est? setado para conting?ncia.
	...
	private void fechaVenda() {
		...
		try {
			...
			if ( (Boolean) oPrefs[ POS_PREFS.CONS_CRED_FECHA.ordinal() ] ) { // Verifica se deve consultar cr?dito ;
			...
			if ( !consistSeq( tab.getDataVector() ) ){
				if( (Boolean) oPrefs[ POS_PREFS.BLOQSEQIVD.ordinal() ] ) {
					Funcoes.mensagemInforma( this, "Sequ?ncia de itens inv?lida !\nFavor ajustar em \"Arquivos => Ferramentas => Reorganiza??o de seq. de itens\"" );
					return;
				} else if ( Funcoes.mensagemConfirma( this, "Sequ?ncia de itens inv?lida !\nDeseja Continuar?" ) == JOptionPane.NO_OPTION ){
					return;						
				}
			}
			...
			if ((Boolean) oPrefs[POS_PREFS.CONSISTENDENTVD.ordinal()]) {
				// Condi??o para alertar sobre o endere?o de entrega
				...
				{
					impPedido = "S";
					Funcoes.mensagemInforma( this, "Endere?o de entrega ? diferente do endere?o do cliente !\nO pedido ser? impresso ap?s a emiss?o da NF !" );
				}
			...
			if ( sValores != null ) {
				...
				else if ( ( "S".equals( sValores[ DLFechaVenda.COL_RETDFV.IMPREC.ordinal() ] ) ) && ( lsParcRecibo != null ) && ( lsParcRecibo.size() > 0 ) ) { // Logica para impress?o do recibo.
					FRBoleto fBol = new FRBoleto( this );
					fBol.setConexao( con );
				...
				// Gerando informa??es fiscis complementares
						...
						if ( classped.equals( "QA" ) ) {
							...
							if( (assinatura != null) && (assinatura.size()>0) ) {
								...
								try {
									...
									Funcoes.mensagemInforma( this, "N?o foi poss?vel carregar imagem de assinatura, verifique os par?metros em prefer?ncias !\n" + ex.getMessage() );
								}
/*								} catch ( SQLException e ) {
									e.printStackTrace();
								}*/ 
							...
							if ( bVisualizar==TYPE_PRINT.VIEW ) {
								...
								try {
									...
									Funcoes.mensagemErro( this, "Erro na impress?o do pedido!" + err.getMessage(), true, con, err );
									err.printStackTrace();
								}
							}
				...
				if ( "T".equals( tipoimp ) ) {
						...
						for ( int i = 0; i < vDesc.size(); i++ ) {
							...
							if ( imp.pRow() == 0 ) {
								...
								imp.say( 1, ( rs.getString( "EndCli" ) != null ? rs.getString( "EndCli" ).trim() + " N?:" + ( rs.getString( "NumCli" ) != null ? rs.getString( "NumCli" ) : "" ) : "" ) + " " + ( rs.getString( "BairCli" ) != null ? rs.getString( "BairCli" ).trim() : "" )
										...
										+ ( rs.getString( "FaxCli" ) != null ? Funcoes.setMascara( rs.getString( "FaxCli" ), "####-####" ) : "" ) );// endere?o e telefone cliente
								imp.pulaLinha( 1, imp.comprimido() );
					...
					if ( bVisualizar==TYPE_PRINT.VIEW ) {
						...
						try {
							...
							Funcoes.mensagemErro( this, "Erro na impress?o de pedido!" + err.getMessage(), true, con, err );
						}
					}
	...
	public void abreVendaComplementar(){
		if ( !Aplicativo.telaPrincipal.temTela( "Gera nota fiscal complementar de sa?da" ) ) {
			FVendaComplementar tela = new FVendaComplementar( this, this );
			Aplicativo.telaPrincipal.criatela( "Gera nota fiscal complementar de sa?da", tela, con );
		}
	}
	...
	public void beforeInsert( InsertEvent ievt ) {
		...
		if ( ievt.getListaCampos() == lcCampos ) {
			...
			if (isBloqNfeAutoriz()) {
				Funcoes.mensagemInforma( this, "N?o ? permitido inser??o de itens em nota fiscal autorizada!" );
				ievt.cancela();
				return;
			}
	...
	public void afterInsert( InsertEvent ievt ) {
		...
		if ( ievt.getListaCampos() == lcCampos ) {
			...
			fatLucro = new BigDecimal(1);
			habilitaBotoes( true );
			// Define o subtipo padr?o de venda como NF = Nota fiscal
			txtSubtipoVenda.setVlrString( "NF" );
			// Define o padr?o da situa??o da nota complementar como N = N?o Complementar.
		...
		if ( pevt.getListaCampos() == lcCampos ) {
			...
			if ( (lcCampos.getStatus() == ListaCampos.LCS_INSERT) || (lcCampos.getStatus() == ListaCampos.LCS_EDIT) ){
				if (txtDtEmitVenda.getVlrDate() == null ) {
					Funcoes.mensagemErro( this, "A Data de Emiss?o ? obrigat?ria!");
					tpnCab.setSelectedIndex( 0 );
				...
				if (txtDtSaidaVenda.getVlrDate() == null ) {
					Funcoes.mensagemErro( this, "A Data de Sa?da/Entrega ? obrigat?ria!");
					tpnCab.setSelectedIndex( 0 );
				...
				if (txtDtEmitVenda.getVlrDate().after(txtDtSaidaVenda.getVlrDate())){
					Funcoes.mensagemErro( this, "A data de Sa?da n?o pode ser anterior ? data de Emiss?o!");
					tpnCab.setSelectedIndex( 0 );
					this.txtDtSaidaVenda.requestFocus();
					pevt.cancela();
					return;
					...
					if ( "N".equals( mensagem ) ) {
						...
						if (!"S".equals( Aplicativo.getUsuario().getLiberacredusu()) ) {
							if (!senhaLiberaAtraso() ) {
								Funcoes.mensagemInforma(this, "Usu?rio n?o tem permiss?o para libera??o da venda !");
								pevt.cancela();
								return;
							}
						}
			...
			if (isBloqNfeAutoriz()) {
				Funcoes.mensagemInforma( this, "Item n?o pode ser alterado.\nNota fiscal j? foi autorizada." );
				pevt.cancela();
				return;
			}
			if ( lcDet.getStatus() == ListaCampos.LCS_INSERT || lcDet.getStatus() == ListaCampos.LCS_EDIT ) {
				...
				if ( numComissionados > 0 ) {
					if ( !consisteComisObrig() ) {
						...
						mens.append( "N?o ? poss?vel inserir o item!\n" );
						mens.append( "Existem comissionados obrigat?rios n?o informados.\n" );
				...
				// Verifica??o de cr?dito

				if ( (Boolean) oPrefs[ POS_PREFS.CONS_CRED_ITEM.ordinal() ] ) { // Verifica se deve consultar cr?dito na inser??o do ?tem;
				...
				if ( ! ( txtCodItFisc.getVlrInteger() > 0 ) ) {
					if ( Funcoes.mensagemConfirma( this, "Este ?tem apresenta problemas nas regras de tributa??o.\n" + "? poss?vel que esse ?tem n?o seja impresso na nota fiscal!\n" + "Deseja continuar?\n" ) != 0 ) {
						pevt.cancela();
						return;
					}
				}
	...
	private void atualizaItens() {
		try {
		  ...
		  con.commit();
			...
			Funcoes.mensagemInforma( this, "Erro atualizando comiss?o !\n" + e.getMessage() );
		}
	}
	...
	public void beforeCarrega( CarregaEvent cevt ) {
		...
		/*
		 * if (lcCampos.getStatus() != ListaCampos.LCS_INSERT) { // Cancela os auto-incrementos que sobrep?em o que est? guardado na tabela venda if (cevt.getListaCampos() == lcVendedor) { lcVendedor.cancLerCampo(2, true); //Comiss?o do vendedor; } else if ( cevt.getListaCampos() == lcCli) {
		 * lcCli.cancLerCampo(2, true); //C?digo de Pagamento lcCli.cancLerCampo(3, true); //C?digo do Vendador } } else { if (cevt.getListaCampos() == lcVendedor) { //Ativa auto-incrementos lcVendedor.cancLerCampo(2, false); //Comiss?o do vendedor; } else if (cevt.getListaCampos() == cCli) {
		 * lcCli.cancLerCampo(2, false); //C?digo do Pagamento lcCli.cancLerCampo(3, false); //C?digo do Vendedor } } Por que faz a mesma coisa no if e no else?
		 */
	...
	public void afterCarrega( CarregaEvent cevt ) {
		...
		try {
			...
			// Desabilitada a feature devido a problema na edi??o do registro ap?s setar para readonly.
	...
	public void beforeDelete( DeleteEvent devt ) {
		if (devt.getListaCampos()==lcDet) {
			if (isBloqNfeAutoriz()) {
				Funcoes.mensagemInforma( this, "N?o ? permitido exclus?o de itens de nota fiscal autorizada !" );
				devt.cancela();
				return;
			}
		}
	...
	private void getTratTrib() {
		...
		try {
			if ( !NF_EMITIDA ) {
				...
				// Carregando campos para grava??o do item de classifica??o selecionado
	...
	private void geraDevolucaoServico() {
		...
		try {
			...
			if ( codtipomov == null || codtipomov == 0 ) {
				Funcoes.mensagemInforma( this, "N?o foi encontrado um tipo de movimento configurado para devolu??o de servi?os\nVerifique as prefer?ncias do m?dulo GMS." );
				return;
			}

			if ( codplanopag == null || codplanopag == 0 ) {
				Funcoes.mensagemInforma( this, "N?o foi encontrado um plano de pagemento sem valor financeiro configurado para devolu??o de servi?os\nVerifique as prefer?ncias gerais do sistema." );
				return;
			}

			// Vari?veis do detalhe
			Integer codprod = null;
			String refprod = null;
			BigDecimal qtditvenda = null;
			String numserie = null;

			// Query para busca do ?tem a ser devolvido (?tem coletado na ordem de servi?o)
			sql.append( "select " );
			...
			// Busca o or?amento que originou a venda atual
			sql.append( "from " );
			sql.append( "vdvendaorc vo " );
			// Verifica qual ordem de servi?o originou o or?amento
			...
			// Busca informa??es do item da ordem de servi?o vinculada ao or?amento
			...
			// Busca informa??es da ordem de servi?o vinculada ao or?amento
			...
			// Busca informa??es da nota fiscal de entrada vinculada ao ticket
			...
			// Busca informa??es do produto a ser devolvido
			...
			System.out.println( "Query devolu??o:" + sql.toString() );
			...
			if ( i == 0 ) {
				Funcoes.mensagemInforma( this, "N?o foi encontrada nenhuma ordem de servi?o para devolu??o!" );
				return;
			}

			// Inserindo cabe?alho (Venda)
			...
			if ( (codcompra==null) || (codcompra==0) ) {
				...
				obsvenda.append( "NOTA FISCAL ELETR?NICA DE ENTRADA NRO " );
			...
			Funcoes.mensagemErro( this, "Erro ao gera??o nota de devolu??o!\n" + e.getMessage() );
			e.printStackTrace();
		}
	}
	...
	private String recriaSqlWhereLcProdutos(){
		...
		try {
			...
			// Se o prefer?ncias estiver marcado para vender produtos de qualquer natureza, n?o adiciona filtro de tipos
			if ( ! (Boolean) oPrefs[ POS_PREFS.VDPRODQQCLAS.ordinal()]) {
				...
				// Regra para verificar se permite a venda de mat?ria prima.
	...
	//M?todo que bloqueia a edi??o dos campos de comiss?o no or?amento.
	...
	//M?todo que bloqueia a edi??o dos campos de comiss?o no item do or?amento.
