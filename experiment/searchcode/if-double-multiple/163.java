...
public class Transform implements Cloneable, Savable, Externalizable, ReadOnlyTransform {
    ...
    protected final Vector3 _scale = new Vector3(Vector3.UNIT_XYZ);
    ...
    public void setScale(final ReadOnlyVector3 scale) {
        ...
        _scale.set(scale);
        _identity = _identity && scale.equals(Vector3.UNIT_XYZ);
        _uniformScale = (Double.compare(scale.getX(), scale.getY()) == 0 && Double.compare(scale.getY(), scale.getZ()) == 0);
    }
    ...
    public void setScale(final double x, final double y, final double z) {
        ...
        _scale.set(x, y, z);
        _identity = false;
        _uniformScale = (Double.compare(x, y) == 0 && Double.compare(y, z) == 0);
    }
    ...
    public void setScale(final double uniformScale) {
        ...
        if (Double.compare(0.0, uniformScale) == 0) {
            throw new IllegalArgumentException("scale may not be ZERO.");
        }
    ...
    public Matrix4 getHomogeneousMatrix(final Matrix4 store) {
        ...
        if (_rotationMatrix) {
            ...
            result._data[0][1] = _scale.getX() * _matrix._data[1][0];
            result._data[0][2] = _scale.getX() * _matrix._data[2][0];
            result._data[0][3] = 0.0;
            result._data[1][0] = _scale.getY() * _matrix._data[0][1];
            result._data[1][1] = _scale.getY() * _matrix._data[1][1];
            result._data[1][2] = _scale.getY() * _matrix._data[2][1];
            result._data[1][3] = 0.0;
            result._data[2][0] = _scale.getZ() * _matrix._data[0][2];
            result._data[2][1] = _scale.getZ() * _matrix._data[1][2];
            result._data[2][2] = _scale.getZ() * _matrix._data[2][2];
            result._data[2][3] = 0.0;
        } else {
            result._data[0][0] = _matrix._data[0][0];
            result._data[0][1] = _matrix._data[1][0];
            result._data[0][2] = _matrix._data[2][0];
            result._data[0][3] = 0.0;
            result._data[1][0] = _matrix._data[0][1];
            result._data[1][1] = _matrix._data[1][1];
            result._data[1][2] = _matrix._data[2][1];
            result._data[1][3] = 0.0;
            result._data[2][0] = _matrix._data[0][2];
            result._data[2][1] = _matrix._data[1][2];
            result._data[2][2] = _matrix._data[2][2];
            result._data[2][3] = 0.0;
        }

        result._data[3][0] = _translation.getX();
        result._data[3][1] = _translation.getY();
        result._data[3][2] = _translation.getZ();
    ...
    public void write(final Ardor3DExporter e) throws IOException {
        ...
        capsule.write(_scale, "scale", new Vector3(Vector3.UNIT_XYZ));
    ...
    public void read(final Ardor3DImporter e) throws IOException {
        ...
        _scale.set((Vector3) capsule.readSavable("scale", new Vector3(Vector3.UNIT_XYZ)));
