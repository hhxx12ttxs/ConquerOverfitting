...
public abstract class CRUD extends Application {
    ...
    public static void data(String id, String field) throws Exception {
        ObjectType type = ObjectType.get(getControllerClass());
        notFoundIfNull(type);
        JPASupport object = type.findById(id);
        byte[] data = (byte[]) object.getClass().getField(field).get(object);
        if (data == null) {
            notFound();
        }
        InputStream inStream = new ByteArrayInputStream(data) ; 
    	renderBinary(inStream);
    }
    ...
    public static void changeItemState(String id, boolean done) {
        ObjectType type = ObjectType.get(getControllerClass());
        notFoundIfNull(type);
        JPASupport object = type.findById(id);
        //@@TODO FINISH HERE AND CHANGE METHOD NAME ?
    }

    public static void reorderItems(String newOrder) {
        ObjectType type = ObjectType.get(getControllerClass());
        notFoundIfNull(type);
        //@@TODO FINISH HERE
    }
