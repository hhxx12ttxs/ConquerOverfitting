...
/**
 * PTC2 implements the "Strongly-typed Probabilistic Tree Creation 2 (PTC2)"
 * algorithm described in
 * 
 * <p>
 * Luke, Sean. 2000. <i>Issues in Scaling Genetic Programming: Breeding
 * Strategies, Tree Generation, and Code Bloat.</i> Ph.D. Dissertation,
 ...
 * 
 * <p>
 * Luke, Sean. 2000. Two fast tree-creation algorithms for genetic programming.
 * In <i>IEEE Transactions on Evolutionary Computation</i> 4:3 (September 2000),
 * 274-283. IEEE.
 * 
 * <p>
 * Both can be found at <a
 * href="http://www.cs.gmu.edu/~sean/papers/">http://www.
 * cs.gmu.edu/~sean/papers/</a>
 * 
 * <p>
 * PTC2 requires that your function set to implement PTCFunctionSetForm. The
 * provided function set, PTCFunctionSet, does exactly this.
 * 
 * <p>
 * The Strongly-typed PTC2 algorithm roughly works as follows: the user provides
 * a requested tree size, and PTC2 attempts to build a tree of that size or that
 * size plus the maximum arity of a nonterminal in the function set. PTC2 works
 * roughly like this:
 * 
 * <ol>
 * <li>If the tree size requested is 1, pick a random terminal and return it.
 * <li>Else pick a random nonterminal as the root and put each of its unfilled
 * child positions into the queue <i>Q</i>.
 * <li>Loop until the size of <i>Q</i>, plus the size of the nodes in the tree
 * so far, equals or exceeds the requested tree size:
 * <ol>
 * <li>Remove a random position from <i>Q</i>.
 * <li>Fill the position with a random nonterminal <i>n</i>.
 * <li>Put each of </i>n's</i> unfilled child positions into <i>Q</i>.
 * </ol>
 * <li>For each position in <i>Q</i>, fill the position with a randomly-chosen
 * terminal.
 * </ol>
 * 
 * <p>
 * Generally speaking, PTC2 picks a random position in the horizon of the tree
 * (unfiled child node positions), fills it with a nonterminal, thus extending
 * the horizon, and repeats this until the number of nodes (nonterminals) in the
 * tree, plus the number of unfilled node positions, is >= the requested tree
 * size. Then the remaining horizon is filled with terminals.
 * 
 * <p>
 * The user-provided requested tree size is either provided directly to the PTC2
 * algorithm, or if the size is NOSIZEGIVEN, then PTC2 will pick one at random
 * from the GPNodeBuilder probability distribution system (using either
 * max-depth and min-depth, or using num-sizes).
 * 
 * <p>
 * PTC2 also has provisions for picking nonterminals with a certain probability
 * over other nonterminals of the same return type (and terminals over other
 * terminals likewise), hence its name. To change the probability of picking
 * various terminals or nonterminals, you modify your PTCFunctionSetForm
 * function set.
 * 
 * <p>
 * PTC2 further has a maximum depth, which you should set to some fairly big
 * value. If your maximum depth is small enough that PTC2 often creates trees
 * which bump up against it, then PTC2 will only generate terminals at that
 * depth position. If the depth is *really* small, it's possible that this means
 * PTC2 will generate trees smaller than you had requested.
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>max-depth</tt><br>
 * <font size=-1>int &gt;= 1</font></td>
 * <td valign=top>maximum allowable tree depth (usually a big value)</td>
 * </tr>
 * </table>
 * 
 * <p>
