...
public class Root {
	...
	public static double bisection(final Function f, double x1, double x2,
		...
		if (y1 * y2 > 0) { // y1 and y2 must have opposite sign
			Log
					.fine(count
							+ " bisection root - interval endpoints must have opposite sign");
		...
		Log.fine(count
				+ " bisection root trials made - no convergence achieved");
	 ...
	 * 
	 * @param a
	 *            double cubic term coefficient
	 * @param b
	 *            double quadratic term coefficient
	 * @param c
	 *            double linear term coefficient
	 * @param d
	 *            double constant term
	 * @return double[] an array containing the two roots.
	 */
	public static double[][] cubic(final double a, final double b,
		...
		if (D == 0) { // all roots are real and at least two are equal
			...
			D = Math.sqrt(D);
			double S = (R + D < 0) ? -Math.pow(-R - D, 1.0 / 3) : Math.pow(R
					+ D, 1.0 / 3);
			double T = (R - D < 0) ? -Math.pow(-R + D, 1.0 / 3) : Math.pow(R
					- D, 1.0 / 3);
			roots[0][0] = -A / 3 + S + T;
	...
	/**
	 * Central difference approximation to the derivative for use in Newton's
	 * mehtod.
	 * 
	 * @param f
	 *            Function
	 ...
	 * @return double
	 */
	final static double fxprime(final Function f, final double x,
			final double tol) {
	 ...
	 * 
	 * @param feqs
	 *            VectorFunction - the function containing n equations
	 * @param n
	 *            int - number of equations
	 * @param xx
	 *            double[] - the variable array at which the Jacobian is
	 *            calculated
	 * @param tol
	 *            double - the small change to find the derivatives
	 * @return double[][] J - the square matrix containing the Jacobian
	 */
	public static double[][] getJacobian(final VectorFunction feqs,
			final int n, final double xx[], final double tol) {
		// builds the Jacobian
		// xxp, xxm contain the varied parameter values to evaluate the
		// equations on
		// fp, fm comtain the transpose of the function evaluations needed in
		// the Jacobian
		...
		for (int i = 0; i < n; i++) { // f's here are built in transpose form
			fp[i] = feqs.evaluate(xxp[i], fp[i]); // i=1: f's at x+tol; i=2 f's
			// at y+tol, etc
			fm[i] = feqs.evaluate(xxm[i], fm[i]); // i=1: f's at x-tol; i=2 f's
			// at y-tol, etc
		}
		// Builds the Jacobian by the differences methods
		// becasue the f's above are in transpose form, we swap i, j in the
		// derivative
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				J[i][j] = (fp[j][i] - fm[j][i]) / tol / 2.; // the derivatives
				// df_i/dx_j
			}
		}
		return J;
	...
	/**
	 * Implements Newton's method for finding the root of a function. The
	 * derivative is calculated numerically using the central difference
	 * approximation.
	 * 
	 * @param f
	 *            Function the function
	 * @param x
	 *            double guess the root
	 * @param tol
	 *            double computation tolerance
	 * @return double the root or NaN if root not found.
	 */
	public static double newton(final Function f, double x, final double tol) {
		...
		while (count < MAX_ITERATIONS) {
			...
			double df = 0;
			try {
				// df = Derivative.romberg(f, x, Math.max(0.001,
				// 0.001*Math.abs(x)), tol/10);
				df = fxprime(f, x, tol);
		...
		Log.fine(count + " newton root trials made - no convergence achieved");
	 ...
	 * @return double the root or NaN if root not found.
	 */
	public static double newton(final Function f, final Function df, double x,
			final double tol) {
		int count = 0;
		while (count < MAX_ITERATIONS) {
			double xold = x; // save the old value to test for convergence
	...
	/**
	 * Implements Newton's method for finding the root but switches to the
	 * bisection method if the the estimate is not between xleft and xright.
	 * 
	 * Method contributed by: J E Hasbun
	 * 
	 * A Newton Raphson result is accepted if it is within the known bounds,
	 * else a bisection step is taken. Ref: Computational Physics by P. L.
	 * Devries (J. Wiley, 1993) input: [xleft,xright] is the interval wherein
	 * fx() has a root, icmax is the maximum iteration number, and tol is the
	 * tolerance level output: returns xbest as the value of the function
	 ...
	 */
	public static double newtonBisection(final Function f, double xleft,
			double xright, final double tol, final int icmax) {
		double rtest = 10 * tol;
		...
		if ((icount > icmax) || (rtest > tol)) {
			Log
					.fine(icmax
							+ " Newton and bisection trials made - no convergence achieved");
			return Double.NaN;
		}
		return xbest;
	}
	...
	/*
	 * Inputs feqs - the function containing n equations with n unknowns and
	 * whose zeros we seek xx - the array containing the guess to the solutions
	 * max - the maximum iteration number tol - the tolerance level
	 * 
	 * @return double the error
	 */
	public static double newtonMultivar(final VectorFunction feqs,
			final double xx[], final int max, final double tol) {
		int Ndim = xx.length;
		...
		err = 9999.;
		relerr = 9999.;
		// Use the Newton-Raphson method for systems of equations - employs the
		// Jacobian
		// Needs a good guess - use one found by a grid method if one is not
		// available
		while ((err > tol * 1.e-6) && (relerr > tol * 1.e-6)
				&& (Iterations < max)) {
			Iterations++;
			LUPDecomposition lu = new LUPDecomposition(getJacobian(feqs, Ndim,
					xx, tol / 100.));
	 ...
	 * @return double[] an array containing the two roots.
	 */
	public static double[][] quadratic(final double a, final double b,
			final double c) {
		...
		final double q = -0.5
				* (b + ((b < 0.0) ? -1.0 : 1.0) * Math.sqrt(disc));
		roots[0][0] = q / a;
		roots[1][0] = c / q;
		return roots;
	}
	 ...
	 * @return double[] an array containing the two roots.
	 */
	public static double[] quadraticReal(final double a, final double b,
			final double c) {
		final double roots[] = new double[2];
		final double q = -0.5
				* (b + ((b < 0.0) ? -1.0 : 1.0)
						* Math.sqrt(b * b - 4.0 * a * c));
		roots[0] = q / a;
		roots[1] = c / q;
		return roots;
	}
