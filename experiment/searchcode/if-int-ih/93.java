...
public class TimeWidgetSlider extends View {
	...
	// fields
	private final static Typeface tfMono = Typeface.create(Typeface.MONOSPACE,
			Typeface.NORMAL);
	...
	private BlurMaskFilter filterBlur = new BlurMaskFilter(0.5F,
	...
	// methods
	public TimeWidgetSlider(Context context, boolean b24HourMode,
			int iSliderType, int iWidth) {
		super(context);
	...
	private void initRectangles(int iWidth, int iHeight) {
		...
		rectBar.set(iFocusBorder, iFocusBorder + iSliderBorder + iSliderSpace,
				iWidth - iFocusBorder, iHeight - iSliderSpace - iSliderBorder
						- iFocusBorder);
		...
		rectCenterBox.left += ((int) rectCenterBox.width() >> 1)
				- (iBoxWidth >> 1);
	...
	private int getTotalHeight() {
		pt.setTextSize(fTextSize);
		return iSliderBorder + iSliderSpace + iBoxBorder + getTextHeight()
				+ iBoxBorder + iSliderSpace + iSliderBorder;
	}
	...
	private void drawBackground(Canvas canvas) {
		...
		canvas.drawRect(rectSlider.left, rectSlider.top + 1, rectSlider.right,
				rectSlider.bottom - 1, pt);
	...
	private boolean isCurrentBoxSelected(RectF rect) {
		if ((rect.left >= rectCenterBox.left)
				&& (rect.right <= rectCenterBox.right))
			return true;
		return false;
	}
	...
	private void drawValueBox(Canvas canvas, RectF rect, ValueBox value) {
		...
		final int iNumberWidth = (bSelected) ? value.iStrWidth
				: value.iStrWidthSmall;
		...
		pt.setColor((bSelected) ? iColorValueBoxTextSelected
				: iColorValueBoxText);
	...
	private void drawFrames(Canvas canvas) {
		...
		// slider frame
		canvas.drawLine(rectSlider.left, rectSlider.top, rectSlider.left,
				rectSlider.bottom, pt);
		canvas.drawLine(rectSlider.right - 1, rectSlider.top,
				rectSlider.right - 1, rectSlider.bottom, pt);

		// center box frame
		canvas.drawLine(rectCenterBox.left - 1, rectSlider.top,
				rectCenterBox.left - 1, rectSlider.bottom, pt);
		canvas.drawLine(rectCenterBox.right, rectSlider.top,
				rectCenterBox.right, rectSlider.bottom, pt);
	}
