	...
	public static class EdgeHandle implements CellHandle, Serializable {
		...
		public void mousePressed(MouseEvent event) {
			...
			// Detect hit on extra labels
					...
					&& GraphConstants.isMoveable(edge.getAllAttributes())) {
				Object[] extraLabels = GraphConstants.getExtraLabels(((DefaultEdge)edge.getCell()).getAttributes());
				for (int i = 0; i < extraLabelLocations.length; i++) {
					if (extraLabelLocations[i] != null
						...
						String label = extraLabels[i].toString();
						
						// Dirty check to see if the edge is a rebinding edge (i.e. does not have a dash pattern).
						// If it is not the case, making it a broadcast edge does not make sense
						// so do not allow it.
						if (GraphConstants.getDashPattern(((DefaultEdge)edge.getCell()).getAttributes()) != null) {
							if (label == "1") {
								extraLabels[i] = "*";
							} else {
								extraLabels[i] = "1";
						...
						GraphConstants.setExtraLabels(((DefaultEdge)edge.getCell()).getAttributes(), extraLabels);
