...
class TestException extends Exception {
  private static final long serialVersionUID = 4041599563246583422L;
...
public class RTest {
  public static void main(String[] args) {
    try {
      ...
      // but the one below allows us to see all output from R via
      // REngineStdOutput()
      // However, it won't succeed if the engine doesn't support callbacks, so
      ...
      if (args.length > 0 && args[0].equals("--debug")) { // --debug waits for
                                                          // <Enter> so a
                                                          // debugger can be
                                                          // attached
...

      {
        ...
        // regression: the inverse failed becasue Java screwed up the bits in
        // the NA value
      ...
      { // regression: object bit was not set for Java-side generated objects
        // before 0.5-3
        System.out.println("* Testing functionality of assembled S3 objects ...");
        // we have already assigned the data.frame in previous test, so we jsut
        // re-use it
      ...
      { // this test does a pull and push of a data frame. It will fail when the
        // S3 test above failed.
      ...
      }
      {
        ...
        eng.assign("m", mat);
        eng.parseAndEval("m<-matrix(m," + m + "," + n + ")", null, false); // don't
                                                                           // return
                                                                           // the
                                                                           // result
                                                                           // -
        ...
        eng.parseAndEval("crossprod(m,m)").asDoubleMatrix();
        System.out.println("PASSED");
      }

      { /*
         * NAs in character vectors are mapped to null references in String[]
         * and vice versa. JRI 0.5-0 and older incorrectly returned "NA" instead
         * of null
         */
...

      {
        ...
        if (x == null || !x.isEnvironment())
          throw new TestException("pull of an environemnt failed");
        REXPEnvironment e = (REXPEnvironment) x;
        e.assign("foo", new REXPString("bar"));
        x = e.get("foo");
        System.out.println("  get(\"foo\") = " + x);
        if (x == null || !x.isString() || !x.asString().equals("bar"))
          throw new TestException("assign/get in an environemnt failed");
        x = eng.newEnvironment(e, true);
        System.out.println("  eng.newEnvironment() = " + x);
        if (x == null || !x.isEnvironment())
          throw new TestException("Java-side environment creation failed");
        x = ((REXPEnvironment) x).parent(true);
        System.out.println("  parent = " + x);
        if (x == null || !x.isEnvironment())
          throw new TestException("parent environment pull failed");
        x = e.get("foo");
        System.out.println("  get(\"foo\",parent) = " + x);
        if (x == null || !x.isString() || !x.asString().equals("bar"))
          throw new TestException("get in the parent environemnt failed");
        System.out.println("  ");
        ...
        if (!(x instanceof REXPDouble) || x.asDouble() != 3.0)
          throw new TestException("eval within environment failed");
