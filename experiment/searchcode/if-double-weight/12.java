...
public class MultiSplitLayout implements LayoutManager {
  ...
  /**
   * Set the root of the tree of Split, Leaf, and Divider nodes that define this
   * layout. The model can be a Split node (the typical case) or a Leaf. The
   * default value of this property is a Leaf named "default".
   * 
  ...
  /**
   * Returns the width of Dividers in Split rows, and the height of Dividers in
   * Split columns.
   * 
   ...
   * Dividers define the bounds of the adjacent Split and Leaf nodes. Typically
   * this property is set to false after the (MultiSplitPane) user has dragged a
   * Divider.
   * 
   ...
   * <code>child</code>. After layoutContainer() recomputes the bounds of all of
   * the nodes in the model, it will set this child's bounds to the bounds of
   * the Leaf node with <code>name</code>. Note: if a component was already
  ...
  private void layoutShrink(Split split, Rectangle bounds) {
    ...
    /*
     * The bounds of the Split node root are set to be big enough to contain all
     * of its children. Since Leaf children can't be reduced below their
     * (corresponding java.awt.Component) minimum sizes, the size of the Split's
     * bounds maybe be larger than the bounds we were asked to fit within.
     */
  ...
  private void layoutGrow(Split split, Rectangle bounds) {
     ...
     * splitChild.getWeight()) Any extraWidth "left over" (that's availableWidth
     * in the loop below) is given to the last child. Note that Dividers always
     ...
     * bounds argument. Extra height is allocated to the to each child with a
     * non-zero weight: newHeight = currentHeight + (extraHeight
     * splitChild.getWeight()) Any extraHeight "left over" (that's
     * availableHeight in the loop below) is given to the last child. Note that
     * Dividers always have a weight of zero, and they're never the last child.
     */
    else {
      double y = bounds.getY();
   ...
   * Otherwise, just set the bounds of each Leaf/Split node so that it's to the
   * left of (for Split.isRowLayout() Split children) or directly above the
   * Divider that follows.
   * 
   * This pass sets the bounds of each Node in the layout model. It does not
   * resize any of the parent Container's (java.awt.Component) children. That's
   * done in the second pass, see layoutGrow() and layoutShrink().
   */
  private void layout1(Node root, Rectangle bounds) {
    if (root instanceof Leaf) {
      ...
      /*
       * Layout the Split's child Nodes' along the X axis. The bounds of each
       * child will have the same y coordinate and height as the layout1()
       * bounds argument.
       * 
       * Note: the column layout code - that's the "else" clause below this if,
       * is identical to the X axis (rowLayout) code below.
       */
      if (split.isRowLayout()) {
        double x = bounds.getX();
        while (splitChildren.hasNext()) {
      ...
      /*
       * Layout the Split's child Nodes' along the Y axis. The bounds of each
       * child will have the same x coordinate and width as the layout1() bounds
       * argument. The algorithm is identical to what's explained above, for the
       * X axis case.
       */
      else {
        double y = bounds.getY();
      ...
      /*
       * The bounds of the Split node root are set to be just big enough to
       * contain all of its children, but only along the axis it's allocating
       * space on. That's X for rows, Y for columns. The second pass of the
       * layout algorithm - see layoutShrink()/layoutGrow() allocates extra
       * space.
       */
      minimizeSplitBounds(split, bounds);
    }
   ...
   * model, and then set the bounds of each child component with a matching Leaf
   * Node.
   */
  ...
  public static abstract class Node {
     ...
     * sibling when the layout grows or how much to reduce when the layout
     * shrinks.
     * 
     ...
     * space to add to this sibling when the layout grows or how much to reduce
     * when the layout shrinks. If rowLayout is true then this node's width
     * grows or shrinks by (extraSpace * weight). If rowLayout is false, then
     * the node's height is changed. The default value of weight is 0.0.
     * 
  ...
  public static class Split extends Node {
    ...
    /**
     * Returns true if the this Split's children are to be laid out in a row:
     * all the same height, left edge equal to the previous Node's right edge.
     * If false, children are laid on in a column.
     * 
    ...
    /**
     * Set the rowLayout property. If true, all of this Split's children are to
     * be laid out in a row: all the same height, each node's left edge equal to
     * the previous Node's right edge. If false, children are laid on in a
    ...
    /**
     * Returns this Split node's children. The returned value is not a reference
     * to the Split's internal list of children
     * 
    ...
    /**
     * Set's the children property of this Split node. The parent of each new
     * child is set to this Split node, and the parent of each old child (if
     * any) is set to null. This method defensively copies the incoming List.
  ...
  public static class Divider extends Node {
    ...
    /**
     * Convenience method, returns true if the Divider's parent is a Split row
     * (a Split with isRowLayout() true), false otherwise. In other words if
     * this Divider's major axis is vertical, return true.
     * 
  ...
  /**
   * A convenience method that converts a string to a MultiSplitLayout model (a
   * tree of Nodes) using a a simple syntax. Nodes are represented by
   * parenthetical expressions whose first token is one of ROW/COLUMN/LEAF. ROW
   * and COLUMN specify horizontal and vertical Split nodes respectively, LEAF
   * specifies a Leaf node. A Leaf's name and weight can be specified with
   * attributes, name=<i>myLeafName</i> weight=<i>myLeafWeight</i>. Similarly, a
   * Split's weight can be specified with weight=<i>mySplitWeight</i>.
   * 
   * <p>
   ...
   * 
   * <p>
   * Dividers should not be included in the string, they're added automatcially
   * as needed. Because Leaf nodes often only need to specify a name, one can
   * specify a Leaf by just providing the name. The previous example can be
   * written like this:
   * 
   * <pre>
