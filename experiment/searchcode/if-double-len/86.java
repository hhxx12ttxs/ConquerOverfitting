...
public class Jarnal extends JApplet{
   ...
   public static void pipe(){
    ...
    private static File getConfDir() {
        String appSupport;
        try {
            appSupport = FileManager.findFolder(FileManager.kUserDomain, FileManager.OSTypeToInt("asup"), true);
        } catch(FileNotFoundException e) {
            throw new RuntimeException("Could not find Application Support directory.");
        }
        final File confDir = new File(appSupport + File.separator + "Jarnal");
        if(!confDir.exists() && !confDir.mkdirs())
            throw new RuntimeException("Could not create configuration directory: " + confDir);
        return confDir;
    }
    ...
    public static Jarnal newJarnal(String title){
      ...
      app.setAboutHandler(new AboutHandler() {
        public void handleAbout(AppEvent.AboutEvent e) {
          controller.jrnlPane.doAction("About");
        }
      });
      app.setOpenFileHandler(new OpenFilesHandler() {
        public void openFiles(AppEvent.OpenFilesEvent e) {
    	    HACK_fname = e.getFiles().get(0).getAbsolutePath();
          controller.jrnlPane.doAction("Open");
        }
      });
      app.setQuitHandler(new QuitHandler() {
        public void handleQuitRequestWith(AppEvent.QuitEvent e, QuitResponse response) {
          controller.jrnlPane.doAction("Exit");
        }
      });
...
//end of API
	...
	private String manageOpen(){
		...
		String temp;
	  temp = getFile("Open " + manageType, false);
