...
public class RtpStreamReceiver extends Thread {
	...
	private void init(SipdroidSocket socket) {
		String key = PreferenceManager.getDefaultSharedPreferences(
				...
				org.sipdroid.sipua.ui.Settings.PREF_SRTPKEY,
				org.sipdroid.sipua.ui.Settings.DEFAULT_PREF_SRTPKEY);
		boolean encSend = PreferenceManager.getDefaultSharedPreferences(
				Receiver.mContext).getBoolean(
				org.sipdroid.sipua.ui.Settings.PREF_SENDENC, false);
		boolean decRec = PreferenceManager.getDefaultSharedPreferences(
				Receiver.mContext).getBoolean(
				org.sipdroid.sipua.ui.Settings.PREF_RECIEVEDEC, false);
		if (socket != null)
			rtp_socket = new RtpSocket(socket, key, decRec);
	}
	...
	void cleanupBluetooth() {
		if (was_enabled && Integer.parseInt(Build.VERSION.SDK) == 8
				&& !Build.VERSION.RELEASE.equals("2.2.1")) {
		...
		case AudioManager.MODE_IN_CALL:
			...
			if (oldring > 0)
				setStreamVolume(
						...
						(int) (am.getStreamMaxVolume(AudioManager.STREAM_RING) * org.sipdroid.sipua.ui.Settings
								.getEarGain()), 0);
			track.setStereoVolume(AudioTrack.getMaxVolume()
					* org.sipdroid.sipua.ui.Settings.getEarGain(), AudioTrack
					.getMaxVolume()
					* org.sipdroid.sipua.ui.Settings.getEarGain());
			break;
		case AudioManager.MODE_NORMAL:
			track.setStereoVolume(AudioTrack.getMaxVolume(), AudioTrack
					.getMaxVolume());
			break;
		}
		setStreamVolume(stream(), PreferenceManager
				.getDefaultSharedPreferences(Receiver.mContext).getInt(
						"volume" + speakermode,
						am.getStreamMaxVolume(stream())
								* (speakermode == AudioManager.MODE_NORMAL ? 4
										: 3) / 4), 0);
	...
	void saveSettings() {
		if (!PreferenceManager.getDefaultSharedPreferences(Receiver.mContext)
			...
			edit.putInt(org.sipdroid.sipua.ui.Settings.PREF_OLDPOLICY,
					oldpolicy);
			edit.putInt(org.sipdroid.sipua.ui.Settings.PREF_OLDRING, am
	...
	public static void restoreSettings() {
		if (PreferenceManager.getDefaultSharedPreferences(Receiver.mContext)
			...
			Editor edit = PreferenceManager.getDefaultSharedPreferences(
					Receiver.mContext).edit();
			edit
					.putBoolean(org.sipdroid.sipua.ui.Settings.PREF_OLDVALID,
							false);
			edit.commit();
		...
		try {
			if (lock) {
				boolean lockNew = (keepon && Receiver.on_wlan)
						|| (InCallScreen.mSlidingCardManager != null && InCallScreen.mSlidingCardManager
								.isSlideInProgress())
						|| Receiver.call_state != UserAgent.UA_STATE_INCALL
						|| RtpStreamSender.delay != 0 || !InCallScreen.started;
		...
		while (running) {
			...
			if (running && timeout == 0) {
				...
				if (cnt <= 500 * mu || cnt2 >= 2 || headroom - jitter < len
					...
					len = p_type.codec.decode(buffer, lin, rtp_packet
							.getPayloadLength());
