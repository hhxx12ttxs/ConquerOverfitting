...
public class CCGraphics{
	...
	public static GL currentGL() {
		if(debug)return GLU.getCurrentGL();
		else return new DebugGL(GLU.getCurrentGL());
	}
...
//	private final CCCamera _myCamera;
	...
	private GL _myPlainGL;
	
	private DebugGL _myDebugGL;
	
	private TraceGL _myTraceGL2;
...
//	private float[] _myTextureXFactors;
//	private float[] _myTextureYFactors;

	/**
	 * Create a new CCGraphics at the specified size.
	 * <P/>
	 ...
	 * @param theApp the host applet
	 * @invisible
	 */
	public CCGraphics(final GL theGL){
		instance = this;
		gl = theGL;
		_myPlainGL = theGL;
		glu = new GLU();
...
//		gl.glGetIntegerv(GL.GL_MAX_LIGHTS, lights);
//		System.out.println("LIGHTS:"+lights.get());
		MAX_LIGHTS = 8;
...
//		_myTextureXFactors = new float[myTextureUnits];
//		_myTextureYFactors = new float[myTextureUnits];
//		
//		for(int i = 0; i < myTextureUnits;i++){
//			_myTextureXFactors[i] = 1;
//			_myTextureYFactors[i] = 1;
//		}
	}
	...
	public void checkError(final String theString){
		...
		case GL.GL_NO_ERROR:
			if(_myReportNoError)System.err.println(theString + " # NO ERROR REPORTED");
			return;
		case GL.GL_INVALID_ENUM:
			System.err.println(theString + " # INVALID ENUMERATION REPORTED. check for errors in OPENGL calls with constants.");
			return;
		case GL.GL_INVALID_VALUE:
			System.err.println(theString + "# INVALID VALUE REPORTED. check for errors with passed values that are out of a defined range.");
			return;
		case GL.GL_INVALID_OPERATION:
			System.err.println(theString + "# INVALID OPERATION REPORTED. check for function calls that are invalid in the current graphics state.");
			return;
		case GL.GL_STACK_OVERFLOW:
			System.err.println(theString + "# STACK OVERFLOW REPORTED. check for errors in matrix operations");
			return;
		case GL.GL_STACK_UNDERFLOW:
			System.err.println(theString + "# STACK UNDERFLOW REPORTED. check for errors  in matrix operations");
			return;
		case GL.GL_OUT_OF_MEMORY:
			System.err.println(theString + "# OUT OF MEMORY. not enough memory to execute the commands");
			return;
		case GL.GL_TABLE_TOO_LARGE:
			System.err.println(theString + "# TABLE TOO LARGE.");
			return;
		}
	}
	...
	public void beginDraw(){
		if(debug) {
			if(_myDebugGL == null)_myDebugGL = new DebugGL(_myPlainGL);
			gl = _myDebugGL;
		}else {
			gl = _myPlainGL;
		}
	...
	public CCColor color(){
		...
		gl.glGetFloatv(GL.GL_CURRENT_COLOR,myColor,0);
	...
	public static enum CCMatrixMode{
		 ...
		 */
		MODELVIEW(GL.GL_MODELVIEW,GL.GL_MODELVIEW_MATRIX),
		/**
		 * Applies subsequent matrix operations to the projection matrix stack.
		 */
		PROJECTION(GL.GL_PROJECTION,GL.GL_PROJECTION_MATRIX),
		/**
		 * Applies subsequent matrix operations to the texture matrix stack.
		 */
		TEXTURE(GL.GL_TEXTURE,GL.GL_TEXTURE_MATRIX);
	...
	public CCMatrixMode matrixMode() {
		switch(getInteger(GL.GL_MATRIX_MODE)) {
		case GL.GL_TEXTURE:
	...
	public void applyMatrix(final CCMatrix3f theMatrix) {
		applyMatrix(
	...
	public static enum CCAttributeMask{
		 ...
		 */
		ACCUM_BUFFER(GL.GL_ACCUM_BUFFER_BIT),
		/**
	...
	public static enum CCDrawMode{
		 ...
		 */
		LINES(GL.GL_LINES),
		LINES_ADJACENCY(GL.GL_LINES_ADJACENCY_EXT),
		LINE_STRIP(GL.GL_LINE_STRIP),
		LINE_STRIP_ADJACENCY(GL.GL_LINE_STRIP_ADJACENCY_EXT),
		LINE_LOOP(GL.GL_LINE_LOOP),
		TRIANGLES(GL.GL_TRIANGLES),
		TRIANGLES_ADJACENCY(GL.GL_TRIANGLES_ADJACENCY_EXT),
		TRIANGLE_STRIP(GL.GL_TRIANGLE_STRIP),
		TRIANGLE_STRIP_ADJACENCY(GL.GL_TRIANGLE_STRIP_ADJACENCY_EXT),
		...
		public static CCDrawMode valueOf(final int theGLID) {
			switch (theGLID) {
			case GL.GL_POINTS:return POINTS;
			case GL.GL_LINES:return LINES;
			case GL.GL_LINE_STRIP:return LINE_STRIP;
			case GL.GL_LINE_LOOP:return LINE_LOOP;
			case GL.GL_TRIANGLES:return TRIANGLES;
			case GL.GL_TRIANGLE_STRIP:return TRIANGLE_STRIP;
			case GL.GL_TRIANGLE_FAN:return TRIANGLE_FAN;
			case GL.GL_QUADS:return QUADS;
			case GL.GL_QUAD_STRIP:return QUAD_STRIP;
			case GL.GL_POLYGON:return POLYGON;
			}
			return POINTS;
		}
	}
	...
	static public final CCDrawMode POINTS = CCDrawMode.POINTS;	

	
	static public final CCDrawMode LINES = CCDrawMode.LINES;

	/**
	 * Draws a connected group of line segments from the first vertex to the last. 
	 * Vertices n and n+1 define line n. N-1 lines drawn.
	 */
	static public final CCDrawMode LINE_STRIP = CCDrawMode.LINE_STRIP;
	 ...
	 */
	static public final CCDrawMode LINE_LOOP = CCDrawMode.LINE_LOOP;
	 ...
	 */
	static public final CCDrawMode TRIANGLES = CCDrawMode.TRIANGLES;
	 ...
	 */
	static public final CCDrawMode TRIANGLE_STRIP = CCDrawMode.TRIANGLE_STRIP;

	/**
	 * Draws a connected group of triangles. One triangle is defined for each vertex 
	 * presented after the first two vertices. Vertices 1, n+1, and n+2 define 
	 * triangle n. N-2 triangles are drawn.
	 */
	static public final CCDrawMode TRIANGLE_FAN = CCDrawMode.TRIANGLE_FAN;
	...
	private void bezier(final FloatBuffer thePointBuffer, final int theNumberOfPoints){
		gl.glMap1f(GL.GL_MAP1_VERTEX_3,0.0f,bezierDetail,3,theNumberOfPoints,thePointBuffer);
		gl.glEnable(GL.GL_MAP1_VERTEX_3);
		gl.glMapGrid1d(bezierDetail,0,bezierDetail);
		gl.glEvalMesh1(GL.GL_LINE,0,bezierDetail);
	}
	 ...
	 * @param x1,y1,z1 int or float: coordinates for the first anchor point
	 * @param x2,y2,z2 int or float: coordinates for the first control point
	 * @param x3,y3,z3 int or float: coordinates for the second control point
	 * @param x4,y4,z4 int or float: coordinates for the second anchor point
	 */
	public void bezier(
	...
	public void endBezier(){
		gl.glMap1f(GL.GL_MAP1_VERTEX_3,0.0f,bezierDetail,3,31,FloatBuffer.wrap(_myBezierCoords));
		gl.glEnable(GL.GL_MAP1_VERTEX_3);
		gl.glMapGrid1d(bezierDetail,0,bezierDetail);
		gl.glEvalMesh1(GL.GL_LINE,0,bezierDetail);
	}
	 ...
	 * @param x1 int or float: x coordinate of the lines starting point
	 * @param y1 int or float: y coordinate of the lines starting point
	 * @param x2 int or float: x coordinate of the lines end point
	 * @param y2 int or float: y coordinate of the lines end point
	 */
	...
	/**
	 * @param z1 int or float: z coordinate of the lines starting point
	 * @param z2 int or float: z coordinate of the lines end point
	 */
	...
	/**
	 * @param v1 CCVector2f: vector with the x,y coordinates of the lines start point
	 * @param v2 CCVector2f: vector with the x,y coordinates of the lines end point
	 */
	...
	public float lineWidthMaximum(){
		return getFloatArray(GL.GL_LINE_WIDTH_RANGE, 2)[1];
	}
	...
	public float lineWidthMinimum(){
		return getFloatArray(GL.GL_LINE_WIDTH_RANGE, 2)[0];
	}
	...
	public float lineWidthGranularity(){
		return getFloat(GL.GL_LINE_WIDTH_GRANULARITY);
	}
	 ...
	 * @param theX int or float: the x coordinate of the point to draw
	 * @param theY int or float: the y coordinate of the point to draw
	...
	/**
	 * @param theZ int or float: z coordinate of the point to draw
	 */
	 ...
	 * @param thePointSize int or float: the size for points to draw
	...
	public float pointSizeMaximum(){
		return getFloat(GL.GL_POINT_SIZE_MAX);
	}

	public void pointSizeMaximum(final float thePointSize){
		gl.glPointParameterf(GL.GL_POINT_SIZE_MAX, thePointSize);
	}
	...
	public float pointSizeMinimum(){
		return getFloat(GL.GL_POINT_SIZE_MIN);
	}

	public void pointSizeMinimum(final float thePointSize){
		gl.glPointParameterf(GL.GL_POINT_SIZE_MIN, thePointSize);
	}
	
	public float[] pointSizeRange(){
		return getFloatArray(GL.GL_POINT_SIZE_RANGE, 2);
	}
	...
	public float pointSizeGranularity(){
		return getFloat(GL.GL_POINT_SIZE_GRANULARITY);
	}
	...
	public int textureUnits(){
		return getInteger(GL.GL_MAX_TEXTURE_IMAGE_UNITS);
	}
	...
	public void textureEnvironment(final CCTextureEnvironmentMode theMode){
		gl.glTexEnvi(GL.GL_TEXTURE_ENV, GL.GL_TEXTURE_ENV_MODE, theMode.glID);
	}
	...
	public static enum CCTextureGenMode{
		 ...
		 */
		OBJECT_LINEAR(GL.GL_OBJECT_LINEAR, GL.GL_OBJECT_PLANE),
		 ...
		 * </pre>
		 */
		EYE_LINEAR(GL.GL_EYE_LINEAR, GL.GL_EYE_PLANE),
		 ...
		 * less overhead.
		 */
		SPHERE_MAP(GL.GL_SPHERE_MAP,-1),
		NORMAL_MAP(GL.GL_NORMAL_MAP,-1),
		REFLECTION_MAP(GL.GL_REFLECTION_MAP,-1);
	...
	public static enum CCTextureGenCoord{
		S(GL.GL_S, GL.GL_TEXTURE_GEN_S),
		T(GL.GL_T, GL.GL_TEXTURE_GEN_T),
		R(GL.GL_R, GL.GL_TEXTURE_GEN_R),
		Q(GL.GL_Q, GL.GL_TEXTURE_GEN_Q);
...
//		texGen(CCTextureGenCoord.Q, theMode);
	...
	public void texGen(final CCTextureGenCoord theCoord, final CCTextureGenMode theMode, float...theData) {
		...
		gl.glTexGeni(theCoord.glID, GL.GL_TEXTURE_GEN_MODE, theMode.glID);
...
//			}
	...
	public void colorMaterial(final CCCullFace theFace, final CCColorMaterialMode theMode){
		...
		case EMISSION:
			...
			gl.glColorMaterial(_myColorMaterialFace.glID, GL.GL_EMISSION);
			break;
		case AMBIENT:
			gl.glEnable(GL.GL_COLOR_MATERIAL);
			gl.glColorMaterial(_myColorMaterialFace.glID, GL.GL_AMBIENT);
			break;
		case DIFFUSE:
			gl.glEnable(GL.GL_COLOR_MATERIAL);
			gl.glColorMaterial(_myColorMaterialFace.glID, GL.GL_DIFFUSE);
			break;
		case AMBIENT_AND_DIFFUSE:
			gl.glEnable(GL.GL_COLOR_MATERIAL);
			gl.glColorMaterial(_myColorMaterialFace.glID, GL.GL_AMBIENT_AND_DIFFUSE);
			break;
		case SPECULAR:
			gl.glEnable(GL.GL_COLOR_MATERIAL);
			gl.glColorMaterial(_myColorMaterialFace.glID, GL.GL_SPECULAR);
			break;
		}
	}
	...
	public void lightModelAmbient(final float theRed, final float theGreen, final float theBlue){
		...
		theColor[2] = theBlue;
		theColor[3] = 1.0f;
		gl.glLightModelfv(GL.GL_LIGHT_MODEL_AMBIENT, theColor, 0);
	}
	...
	public void lightModelAmbient(final int theRed, final int theGreen, final int theBlue){
		...
		theColor[2] = theBlue;
		theColor[3] = 255;
		gl.glLightModeliv(GL.GL_LIGHT_MODEL_AMBIENT, theColor, 0);
	}

	public void lightModelAmbient(final CCColor theColor){
		gl.glLightModelfv(GL.GL_LIGHT_MODEL_AMBIENT, theColor.array(), 0);
	}
	...
	public void lightModelTwoSide(final boolean theIsTwoSide){
		if(theIsTwoSide)gl.glLightModeli(GL.GL_LIGHT_MODEL_TWO_SIDE, 1);
		else gl.glLightModeli(GL.GL_LIGHT_MODEL_TWO_SIDE, 0);
	}
	...
	public void lightModelLocalViewer(final boolean theIsLocalViewer){
		if(theIsLocalViewer)gl.glLightModeli(GL.GL_LIGHT_MODEL_LOCAL_VIEWER, 1);
		else gl.glLightModeli(GL.GL_LIGHT_MODEL_LOCAL_VIEWER, 0);
	}
	...
	public void lightModelColorControl(final boolean theIsSingleColor){
		if(theIsSingleColor)gl.glLightModeli(GL.GL_LIGHT_MODEL_COLOR_CONTROL, GL.GL_SINGLE_COLOR);
		else gl.glLightModeli(GL.GL_LIGHT_MODEL_COLOR_CONTROL, GL.GL_SEPARATE_SPECULAR_COLOR);
	}
	...
	private CCFont<?> _myTextFont = CCFontIO.createGlutFont(CCGlutFontType.BITMAP_HELVETICA_10);

	/** The current text size (read-only) */
	private float _myTextSize;

	/** The current text leading (read-only) */
	private float _myTextLeading;
	 ...
	 */
	private char[] _myTextBuffer = new char[8 * 1024];

	private char[] _myTextWidthBuffer = new char[8 * 1024];
	
	
	/** The current text align (read-only) */
	private CCTextAlign _myTextAlign = CCTextAlign.LEFT;
	...
	public float textAscent(){
		return _myTextFont.ascent() * _myTextSize;
	}

	public float textDescent(){
		return _myTextFont.descent() * _myTextSize;
	}
	...
	public void textFont(final CCFont<?> theFont){
		_myTextFont = theFont;
		if(_myTextFont != null)textSize(theFont.size());
	}
	...
	public void textFont(final CCFont<?> theFont, final float theSize) {
		textFont(theFont);
		if(theFont != null)textSize(theSize);
	}
	...
	public void textSize(final float theSize){
		_myTextSize = theSize;
		_myTextLeading = (textAscent() + textDescent()) * 1.275f;
	}
	...
	public float textWidth(final char theChar){
		_myTextBuffer[0] = theChar;
		return _myTextFont.width(_myTextBuffer, 0, 1);
	}

	/**
	 * Return the width of a line of text. If the text has multiple lines,
	 * this returns the length of the longest line.
	 * @param theString
	 * @return
	 */
	public float textWidth(final String theString){
		final int myStringLength = theString.length();
		
		if (myStringLength > _myTextWidthBuffer.length){
			_myTextWidthBuffer = new char[myStringLength + 10];
		}
		theString.getChars(0, myStringLength, _myTextWidthBuffer, 0);
		...
		while (myIndex < myStringLength){
			if (_myTextWidthBuffer[myIndex] == '\n'){
				myWidth = Math.max(myWidth, _myTextFont.width(_myTextWidthBuffer, myStart, myIndex));
				myStart = myIndex + 1;
			}
			myIndex++;
		}
		...
		return myWidth * _myTextSize;
	}

	/**
	 * Handles placement of a text line, then calls textLinePlaced to actually
	 * render at the specific point.
	 */
	private void textLine(
		final char theTextBuffer[], 
		final int theStart, final int theStop, 
		float theX, final float theY, final float theZ
	){
		if(_myTextFont == null)throw new RuntimeException("Set a font using textFont befor writing text.");
		switch(_myTextAlign){
		case CENTER:
			theX -= _myTextFont.width(theTextBuffer, theStart, theStop) / 2f * _myTextSize;
			break;
		case RIGHT:
			theX -= _myTextFont.width(theTextBuffer, theStart, theStop) * _myTextSize;
			break;
		}
		...
		for (int index = theStart; index < theStop; index++){
			...
			final int myIndex = _myTextFont.index(myChar);
			
			theX += _myTextFont.drawChar(this,myIndex,_myTextSize,theX,theY,theZ);
			theX += _myTextFont.kerning(myLastIndex, myIndex) * _myTextSize;
	...
	private void textBlockLine(
		final char theTextBuffer[], 
		final int theStart, final int theStop, final float theWidth,
		float theX, final float theY, final float theZ
	){
		switch(_myTextAlign){
			case JUSTIFY:
				final float mySpace = (float)(theWidth - _myTextFont.width(theTextBuffer, theStart, theStop)* _myTextSize)/(theStop - theStart - 1);
				for (int index = theStart; index < theStop; index++){
					final char myChar = theTextBuffer[index];
					switch(myChar){
						case ' ':
							theX += _myTextFont.spaceWidth() * _myTextSize;
							break;
						default:
							final int myIndex = _myTextFont.index(myChar);
							theX += _myTextFont.drawChar(this,myIndex,_myTextSize,theX,theY,theZ);
				...
				break;
			default:
				textLine(theTextBuffer,theStart,theStop,theX,theY,theZ);
		}
	}
	...
	public void text(final char theChar){
		text(theChar, textX, textY, textZ);
	}
	...
	public void text(final char theChar, final float theX, final float theY, final float theZ){
		_myTextBuffer[0] = theChar;
		textLine(_myTextBuffer, 0, 1, theX, theY, theZ);
	}

	/**
	 * Write text where we just left off.
	 */
	public void text(final String theString){
		text(theString, textX, textY, textZ);
	}


	/**
	 * Draw a chunk of text. Newlines that are \n (Unix newline or linefeed
	 * char, ascii 10) are honored, but \r (carriage return, Windows and Mac
	 * OS) are ignored.
	 */
	public void text(final String theString, final float theX, float theY, final float theZ){
		final int myStringLength = theString.length();
		
		if (myStringLength > _myTextBuffer.length){
			_myTextBuffer = new char[myStringLength + 10];
		}
		theString.getChars(0, myStringLength, _myTextBuffer, 0);
		...
		while (myIndex < myStringLength){
			if (_myTextBuffer[myIndex] == '\n'){
				textLine(_myTextBuffer, myStart, myIndex, theX, theY,theZ);
				myStart = myIndex + 1;
				theY += _myTextLeading;
			}
			myIndex++;
		}
		_myTextFont.beginText(this);
		if (myStart < myStringLength){
			textLine(_myTextBuffer, myStart, myIndex, theX, theY,theZ);
		}
		_myTextFont.endText(this);
	}
	...
	/**
	 * Draw text in a box that is constrained to a particular size. The
	 * current rectMode() determines what the coordinates mean (whether
	 * x1/y1/x2/y2 or x/y/w/h). <P/> Note that the x,y coords of the start of
	 * the box will align with the *ascent* of the text, not the baseline, as
	 * is the case for the other text() functions. <P/> Newlines that are \n
	...
	public void text(
		...
		float myHorizontalRadius, myVerticalRadius;
		...
		switch (_myRectMode){
			case CORNER:
				...
				break;
			case RADIUS:
				myHorizontalRadius = theX2;
				myVerticalRadius = theY2;
				theX2 = theX1 + myHorizontalRadius;
				theY2 = theY1 + myVerticalRadius;
				theX1 -= myHorizontalRadius;
				theY1 -= myVerticalRadius;
				break;
			case CENTER:
				myHorizontalRadius = theX2 / 2.0f;
				myVerticalRadius = theY2 / 2.0f;
				theX2 = theX1 + myHorizontalRadius;
				theY2 = theY1 + myVerticalRadius;
				theX1 -= myHorizontalRadius;
				theY1 -= myVerticalRadius;
		...
		final float mySpaceWidth = _myTextFont.spaceWidth();
		float myRunningX = theX1; // boxX1;
		float myCurrentY = theY1; // boxY1;
		
		final float myBoxWidth = theX2 - theX1; // boxX2 - boxX1;

		// lineX is the position where the text starts, which is adjusted
		// to left/center/right based on the current textAlign
		float myLineX = theX1; // boxX1;
		
		if (_myTextAlign == CCTextAlign.CENTER){
			myLineX = myLineX + myBoxWidth / 2f;
		}else if (_myTextAlign == CCTextAlign.RIGHT){
			myLineX = theX2; // boxX2;
		}

		// ala illustrator, the text itself must fit inside the box
		myCurrentY += textAscent(); // ascent() * textSize;
		...
		while (index < myStringLength){
			if ((_myTextBuffer[index] == ' ') || (index == myStringLength - 1)){
				// boundary of a word
				float wordWidth = _myTextFont.width(_myTextBuffer, wordStart, index) * _myTextSize;

				if (myRunningX + wordWidth > theX2){ // boxX2) {
					if (myRunningX == theX1){ // boxX1) {
						...
						do{
							...
							wordWidth = _myTextFont.width(_myTextBuffer, wordStart, index) * _myTextSize;
						}while (wordWidth > myBoxWidth);
						
						textBlockLine(_myTextBuffer, lineStart, index,myBoxWidth, myLineX, myCurrentY, theZ);
					}else{
						// next word is too big, output current line
						// and advance to the next line
						textBlockLine(_myTextBuffer, lineStart, wordStop,myBoxWidth, myLineX, myCurrentY, theZ);
					...
					wordStart = index;
					wordStop = index;
					myRunningX = theX1; // boxX1;
					myCurrentY -= _myTextLeading;
				...
				lineStart = index + 1;
				wordStart = lineStart;
				myRunningX = theX1; // fix for bug 188
				myCurrentY -= _myTextLeading;
				// if (currentY > boxY2) return; // box is now full
				if (myCurrentY > theY2){
					_myTextFont.endText(this);
	 ...
	 * x/y/w/h). <P/> Note that the x,y coords of the start of the box will align
	 * with the *ascent* of the text, not the baseline, as is the case for the
	 * other text() functions. <P/> Newlines that are \n (Unix newline or
	 * linefeed char, ascii 10) are honored, and \r (carriage return, Windows and
	 * Mac OS) are ignored.
	 */
	public float textBoxHeight(String str, float x1, float x2){
		float hradius;
		switch (_myRectMode){
			case CORNER:
				x2 += x1;
		...
		if (length > _myTextBuffer.length){
			_myTextBuffer = new char[length + 10];
		}
		str.getChars(0, length, _myTextBuffer, 0);
		...
		while (index < length){
			if ((_myTextBuffer[index] == ' ') || (index == length - 1)){
				// boundary of a word
				float wordWidth = _myTextFont.width(_myTextBuffer, wordStart, index);
				...
				if (runningX + wordWidth > x2){ // boxX2) {
					if (runningX == x1){ // boxX1) {
						...
						do{
							...
							wordWidth = _myTextFont.width(_myTextBuffer, wordStart, index);
						...
						index = wordStop; // back that ass up
						while ((index < length) && (_myTextBuffer[index] == ' ')){
							index++;
						}
					}
					lineStart = index;
	...
	public void text(
		...
		text(theString,theX1,theY1,theX2,theY2,0);
	}
	...
	public void strokeJoin(int join){
		String msg = "strokeJoin() not available with ProGL";
	...
	public void strokeCap(CCStrokeCap cap){
		String msg = "strokeCap() not available with P";
		throw new RuntimeException(msg);
	}
	 ...
	 * quickly passed over to OpenGL. (fix from Willis Morse)
	 */
...
//					int temp = image.pixels[index];
//					// ignores alpha component, just sets it opaque
//					image.pixels[index] = (image.pixels[yindex] & 0xff000000) | ((image.pixels[yindex] >> 8) & 0x00ffffff);
...
//					// identical to updatePixels because only two
//					// components are being swapped
//					image.pixels[index] = (image.pixels[yindex] & 0xff000000) | ((image.pixels[yindex] << 16) & 0xff0000) | (image.pixels[yindex] & 0xff00)
...
//		if (_myImageMode == CORNERS){ // if CORNER, do nothing
//			w = (w - x);
//			h = (h - x);
//		}
//
...
//		gl.glDrawPixels(source.width, source.height, GL.GL_RGBA, GL.GL_UNSIGNED_BYTE, setBuffer);
...
//	}
	...
	public void copy(
		...
		gl.glCopyPixels(width/2+sx1, height/2 + sy1, sx2, sy2, GL.GL_COLOR);
		gl.glPixelZoom(1, 1);
	}
...
//	public void blend(final PImage src, final int sx, final int sy, final int dx, final int dy, final int mode){
//		beginBlend(mode);
//		set(dx, dy, src.get(sx, sy));
//		endBlend();
//	}
	 ...
	 * Now OPENGL based and much faster
	 */
//	public void blend(final PImage src, final int sx1, final int sy1, final int sx2, final int sy2, final int dx1, final int dy1, final int dx2, final int dy2, final int mode){
//		beginBlend(mode);
//		copy(src, sx1, sy1, sx2, sy2, dx1, dy1, dx2, dy2);
//		endBlend();
//	}
...
//	}
	 ...
	 * <p>
	 * As of revision 0100, this function requires an absolute path, in
	 * order to avoid confusion. To save inside the sketch folder, use the
	 * function savePath() from PApplet, or use saveFrame() instead. As of
	 * revision 0116, savePath() is not needed if this object has been
	 * created (as recommended) via createImage() or createGraphics() or one
	 * of its neighbors.
	 * <p>
	 * As of revision 0115, when using Java 1.4 and later, you can write to
	 * several formats besides tga and tiff. If Java 1.4 is installed and
	 * the extension used is supported (usually png, jpg, jpeg, bmp, and
	 * tiff), then those methods will be used to write the image. To get a
	 * list of the supported formats for writing, use: <BR>
	...
	public void save(){
		//save("data/export/screen-" + CCFormatUtil.nf(parent.frameCount, 4) + ".png");
	}
	...
	public void report(String where){
		...
		if (err == 0){
			...
			System.out.print(CCConversion.hex(err, 4) + "  ");
			switch (err){
				case GL.GL_INVALID_ENUM:
