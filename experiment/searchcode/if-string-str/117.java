  ...
  implements Runnable
{
  ...
  protected String getDescriptionForState()
  {
    ...
    case 1:
      ...
      case 2:
        return "Determination des packages ŕ télécharger.";
      case 3:
      ...
      case 6:
        return "Mise ŕ jour du classpath.";
      case 7:
  ...
  protected void loadJarURLs() throws Exception {
    ...
    URL localURL2 = new URL(Custom.jarURL);
    ...
    for (int j = 0; j < i - 1; j++) {
      ...
      if(j != 3)
    	  ...
    	  urlList[j] = new URL(localURL2, localStringTokenizer.nextToken());
      
      System.out.println(urlList[j].toString());
...
//*
  protected void downloadJars(String paramString) throws Exception {
    ...
    for (int i = 0; i < urlList.length; i++) {
      ...
      if ((localURLConnection instanceof HttpURLConnection)) {
        ...
        String localObject = "\"" + localProperties.getProperty(getFileName(urlList[i])) + "\"";
    ...
    byte[] localObject = new byte[65536];
    ...
    for (int j = 0; j < urlList.length; j++) {
      ...
      else
      {
        ...
        while (n != 0) {
          ...
          int i1;
          while ((i1 = localInputStream.read(localObject, 0, localObject.length)) != -1) {
            localFileOutputStream.write(localObject, 0, i1);
            localMessageDigest.update(localObject, 0, i1);
  ...
  protected void extractNatives(String paramString) throws Exception
  {
    ...
    if (arrayOfCertificate == null) {
      URL localObject1 = Launcher.class.getProtectionDomain().getCodeSource().getLocation();

      JarURLConnection localObject2 = (JarURLConnection)new URL("jar:" + ((URL)localObject1).toString() + "!/net/minecraft/Launcher.class").openConnection();
	...
	while (localEnumeration.hasMoreElements()) {
		JarEntry localObject3 = (JarEntry)localEnumeration.nextElement();
  ...
  public boolean mustUpdate(){
	  String local = localHash();
	  String online = onlineHash() ;
	...
	public String onlineHash() {
		...
		try {
			URL getVersion = new URL(Custom.hashURL);
			BufferedReader RgetVersion = new BufferedReader(new InputStreamReader(getVersion.openStream()));
			hash = RgetVersion.readLine();
	...
	public String localHash() {
		...
		File jar = new File(Util.getWorkingDirectory() + File.separator + "bin" + File.separator + "minecraft.jar");
	...
	public void downloadFile(String pathFolder) {
		...
		folder = Util.getWorkingDirectory();
		zip = new File(folder + "patch.zip");
		
		try {
			System.out.println(Custom.zipURL);
			URL url = new URL(Custom.zipURL);
...
/* Location:           I:\MineTasy\Launcher\minecraft.jar
