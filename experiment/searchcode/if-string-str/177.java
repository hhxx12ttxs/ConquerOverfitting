...
public class EReader extends Thread {
	...
	private final EClientSocket	  m_parent;
	...
	public EReader(final EClientSocket parent, final DataInputStream dis) {
		this("EReader", parent, dis);
	}
	
	protected EReader(final String name, final EClientSocket parent,
	...
	protected boolean processMsg(final int msgId) throws IOException {
		if (msgId == -1) { return false; }
		
		switch (msgId) {
			case TICK_PRICE: {
				final int version = readInt();
				final int tickerId = readInt();
				final int tickType = readInt();
				final double price = readDouble();
				int size = 0;
				if (version >= 2) {
					size = readInt();
				}
			...
			case TICK_OPTION_COMPUTATION: {
				...
				if (Math.abs(delta) > 1) { // -2 is the "not yet computed"
										   // indicator
				...
				if (version >= 6 || tickType == TickType.MODEL_OPTION) { // introduced
																		 ...
																		 // == 5
					optPrice = readDouble();
					if (optPrice < 0) { // -1 is the "not yet computed"
										// indicator
						optPrice = Double.MAX_VALUE;
					}
					pvDividend = readDouble();
					if (pvDividend < 0) { // -1 is the "not yet computed"
										  // indicator
				...
				if (version >= 6) {
					...
					undPrice = readDouble();
					if (undPrice < 0) { // -1 is the "not yet computed"
										// indicator
						undPrice = Double.MAX_VALUE;
					}
				}
				
				eWrapper().tickOptionComputation(tickerId, tickType,
			...
			case TICK_EFP: {
				...
				final int tickerId = readInt();
				final int tickType = readInt();
				final double basisPoints = readDouble();
				final String formattedBasisPoints = readStr();
				final double impliedFuturesPrice = readDouble();
				final int holdDays = readInt();
				final String futureExpiry = readStr();
				final double dividendImpact = readDouble();
				final double dividendsToExpiry = readDouble();
				eWrapper().tickEFP(tickerId, tickType, basisPoints,
				        formattedBasisPoints,
				                    impliedFuturesPrice, holdDays,
			...
			case ORDER_STATUS: {
				...
				final String status = readStr();
				final int filled = readInt();
				final int remaining = readInt();
				final double avgFillPrice = readDouble();
				...
				eWrapper().orderStatus(id, status, filled, remaining,
				        avgFillPrice,
			...
			case ACCT_VALUE: {
				final int version = readInt();
				final String key = readStr();
				final String val = readStr();
				final String cur = readStr();
			...
			case PORTFOLIO_VALUE: {
				...
				final double marketPrice = readDouble();
				final double marketValue = readDouble();
			...
			case ACCT_UPDATE_TIME: {
				final int version = readInt();
				final String timeStamp = readStr();
			...
			case ERR_MSG: {
				final int version = readInt();
				if (version < 2) {
					final String msg = readStr();
					m_parent.error(msg);
				} else {
					final int id = readInt();
					final int errorCode = readInt();
					final String errorMsg = readStr();
			...
			case OPEN_ORDER: {
				...
				if (version >= 20) {
					if (readBoolFromInt()) {
						final UnderComp underComp = new UnderComp();
				...
				if (version >= 21) {
					...
					if (!Util.StringIsEmpty(order.m_algoStrategy)) {
						final int algoParamsCount = readInt();
			...
			case NEXT_VALID_ID: {
				final int version = readInt();
				final int orderId = readInt();
				eWrapper().nextValidId(orderId);
				break;
			}
				
			case SCANNER_DATA: {
				final ContractDetails contract = new ContractDetails();
				final int version = readInt();
				final int tickerId = readInt();
				final int numberOfElements = readInt();
				for (int ctr = 0; ctr < numberOfElements; ctr++) {
					final int rank = readInt();
					if (version >= 3) {
					...
					final String distance = readStr();
					final String benchmark = readStr();
					final String projection = readStr();
					String legsStr = null;
					if (version >= 2) {
						legsStr = readStr();
					}
			...
			case MARKET_DEPTH: {
				...
				final int position = readInt();
				final int operation = readInt();
				final int side = readInt();
				final double price = readDouble();
				final int size = readInt();
			...
			case MARKET_DEPTH_L2: {
				...
				final int position = readInt();
				final String marketMaker = readStr();
				final int operation = readInt();
				final int side = readInt();
				final double price = readDouble();
				final int size = readInt();
			...
			case NEWS_BULLETINS: {
				final int version = readInt();
				final int newsMsgId = readInt();
				final int newsMsgType = readInt();
				final String newsMessage = readStr();
				final String originatingExch = readStr();
				
				eWrapper().updateNewsBulletin(newsMsgId, newsMsgType,
				        newsMessage, originatingExch);
				break;
			}
			case MANAGED_ACCTS: {
				final int version = readInt();
				final String accountsList = readStr();
			...
			case RECEIVE_FA: {
				final int version = readInt();
				final int faDataType = readInt();
				final String xml = readStr();
			...
			case HISTORICAL_DATA: {
				...
				final int itemCount = readInt();
				for (int ctr = 0; ctr < itemCount; ctr++) {
					final String date = readStr();
					final double open = readDouble();
					final double high = readDouble();
					final double low = readDouble();
					final double close = readDouble();
					final int volume = readInt();
					final double WAP = readDouble();
					final String hasGaps = readStr();
					int barCount = -1;
					if (version >= 3) {
						barCount = readInt();
					}
					eWrapper()
					        .historicalData(reqId, date, open, high, low,
					                    close, volume, barCount, WAP,
					                    Boolean.valueOf(hasGaps).booleanValue());
				}
				// send end of dataset marker
				eWrapper().historicalData(reqId, completedIndicator, -1, -1,
				        -1, -1, -1, -1, -1, false);
				break;
			}
			...
			case REAL_TIME_BARS: {
				...
				final double low = readDouble();
				final double close = readDouble();
				final long volume = readLong();
				final double wap = readDouble();
				final int count = readInt();
				eWrapper().realtimeBar(reqId, time, open, high, low, close,
				        volume, wap, count);
				break;
			}
			...
			case ACCT_DOWNLOAD_END: {
				/* int version = */readInt();
				final String accountName = readStr();
