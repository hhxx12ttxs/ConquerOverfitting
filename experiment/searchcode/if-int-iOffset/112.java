...
public class Mesh implements Disposable {
	...
	public void renderInstanced(ShaderProgram shader, int primitiveType, int numInstances) {
		renderInstanced(shader, primitiveType, 0, indices.getNumMaxIndices() > 0 ? getNumIndices() : getNumVertices(), numInstances, autoBind);
	}
	
	public void renderInstanced (ShaderProgram shader, int primitiveType, int offset, int count, int numInstances, boolean autoBind) {
		if (count == 0) return;
