...
public class ManualAttenuation extends javax.swing.JDialog implements WindowFocusListener {
	...
	Date saveSerTime;
	Vector<Integer> sliceInUse = null;
	Vector<Integer> sourceIdx = null;
	...
	void refresh() {
		...
		if( parentPet != null) {
			jSpinOffset.setValue(parentPet.petPipe.data1.shitOffset);
			if( attenMap != null) {
	...
	void offsetChanged() {
		...
		parentPet.petPipe.data1.shitOffset = i;
		parentPet.repaint();
	}
	...
	void doMapBuild() {
		...
		data1.pixFloat = new Vector<float []>();
		data1.zpos = new Vector<Float>();
		if( petPipe.data1.rescaleSlope == null) petData = false;
		if( petData) data1.rescaleSlope = new Vector<Double>();
		data1.numFrms = n;
		...
		for( i=0; i<n; i++) {
			pixImg = pixFAtten = pixFUAtten = null;
			pixAtten = pixUAtten = null;
			if( petPipe.data1.pixFloat != null) {
				pixFAtten = petPipe.data1.pixFloat.elementAt(i);
				pixFUAtten = upetPipe.data1.pixFloat.elementAt(i);
			} else {
				pixAtten = petPipe.data1.pixels.elementAt(i);
				pixUAtten = upetPipe.data1.pixels.elementAt(i);
			}
			zpos = petPipe.data1.zpos.elementAt(i);
			data1.zpos.add(zpos);
			aslope = uslope = 1.0;
			if( petData) {
				aslope = petPipe.data1.rescaleSlope.elementAt(i);
				uslope = upetPipe.data1.rescaleSlope.elementAt(i);
	...
	void anotherSmooth() {
		...
		for( i=0; i<n; i++) {
			...
			pixImg = attenMap.data1.pixFloat.elementAt(i);
			w1 = width1-1;
			for( j=0; j<heigh1; j++) {
				off0 = (j-1)*width1;
				off1 = j*width1;
			...
			attenMap.data1.pixFloat.setElementAt(smthImg, i);
		}
		numSmooth++;
	...
	JFijiPipe findSlices() {
		...
		sliceInUse = new Vector<Integer>();
		sourceIdx = new Vector<Integer>();
		int i, j, n, size1;
		...
		shitOffset = (float) (upetPipe.data1.shitOffset * upetPipe.data1.spacingBetweenSlices);
		for( i=0; i<n; i++) {
			zpos = upetPipe.data1.zpos.elementAt(i) + shitOffset;
			for( j=0; j<size1; j++) {
				zposMap = attenMap.data1.zpos.elementAt(j);
	...
	protected void doApplyMap() {
		...
		for( i=k=0; i<n; i++) {
			val1 = sliceInUse.elementAt(i);
			pixImg = attenMap.data1.pixFloat.elementAt(val1);
			val1 = sourceIdx.elementAt(i);
			pixFloat = null;
			pixels = null;
			if( upetPipe.data1.pixFloat != null) pixFloat = upetPipe.data1.pixFloat.elementAt(val1);
			else pixels = upetPipe.data1.pixels.elementAt(val1);
			factor2 = factor1 = 1.0;
			if( petData) {
				factor2 = getFactor(pixImg, pixels, factor1, size1, ucoef);
				currVal = upetPipe.data1.rescaleSlope.elementAt(val1);
				upetPipe.data1.rescaleSlope.setElementAt(currVal/factor2, val1);
			}
			factor1 *= factor2;
	...
	double fillSUV( JFijiPipe upetPipe) {
		...
		SUV = dlg1.calculateSUV(upetPipe, true);
		if( SUV > 0) return SUV;
