...
/**
 * The Texture Library manages all textures in the game.  Textures are pooled
 * and handed out to requesting parties via allocateTexture().  However, the
 * texture data itself is not immediately loaded at that time; it may have
 * already been loaded or it may be loaded in the future via a call to
 * loadTexture() or loadAllTextures().  This allows Texture objects to be
 * dispersed to various game systems and while the texture data itself is
 * streamed in or loaded as necessary.
 */
public class TextureLibrary extends BaseObject {
  // Textures are stored in a simple hash. This class implements its own
  // array-based hash rather than using HashMap for performance.
  ...
  /** 
   * Creates a Texture object that is mapped to the passed resource id.
   * If a texture has already been allocated for this id, the previously
   * allocated Texture object is returned.
   * @param resourceID
   * @return
   */
   ...
   * chaining mechanism: if the hash slot is occupied by some other entry,
   * the next empty array index is used. This is O(n) for the worst case
   * (every slot is a cache miss) but the average case is constant time. 
   * @param startIndex
   * @param key
   * @return
   */
