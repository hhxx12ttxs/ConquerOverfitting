...
public class ResolutionPanel extends JPanel {
	...
	private void collectFeatures() {
		...
		try {
			...
			Display.destroy();
			String adapter=vendor+"-"+renderer+"/"+canDoShadows+"/"+canDoBloom+"/"+canDoAA+"/"+canDoFBO;
			...
			if (hw.indexOf("intel")!=-1 || hw.indexOf("s3")!=-1 || hw.indexOf("sis")!=-1) {
				// Oh my god...it's intel or s3 or sis. Disable everything and it *might* work well...
				guessedAAValue=0;
				guessedShadowValue=0;
				if (hw.indexOf("sis")!=-1) {
					// If it's SIS, it may has problems with vertex arrays...disable them and use strips instead.
				...
				if (hw.indexOf("ati")!=-1 && hw.indexOf("nvidia")==-1) {
					...
					if ((number>=300 && number<=600) || (number>1000 && number<1800)) {
						...
						guessedShadowValue=0;
						if (number>1000) {
							Config.glShadowZBias=ZBIAS_OFFSET_ATI_1XX0; // But ATI...
						} else {
            ...
            if (isQuadro) {
              // A Quadro (usually some business crap, but at least fast enough for normal shadow quality in most cases)
              guessedAAValue=0;
              guessedShadowValue=3;
            }
					} 
	...
	public ResolutionPanel(JFrame parentFrame) {
		...
		Dimension size = new Dimension(305, 350);
		...
		Dimension size2 = new Dimension(305, 275);
		...
		options.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (!optionsVisible) {
					Dimension dim=new Dimension(320, 370+Math.max(0,pf.getInsets().bottom));
		...
		if (mody.length > 0) {
			for (int i = 0; i < mody.length; i++) {
				if (mody[i].width>=512 && mody[i].height>=480) {
					String key = mody[i].width + "*" + mody[i].height + "*"
							+ mody[i].bpp;
   ...
   private void load() {
	   ...
	   if (TalkBack.mayTalkBack()) {
		   int res=JOptionPane.showConfirmDialog(this, "Do you want to allow Robombs to transfer some configuration information and an\nin-game screen shot to a remote server to further improve the game in the future?\nNo personal data will be transfered and it happens only once.\nYou won't be bothered again! Thank you!", "Help to improve the game?", JOptionPane.YES_NO_OPTION);
		   if (res!=0) {
			   TalkBack.noTalkBack();
		   }
	   }
