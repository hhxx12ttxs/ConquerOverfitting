...
public class ScatterPlotViewPanel extends JPanel implements TPPModelEventListener, ComponentListener {
	...
	/**
	 * Find the indices of the nearest points to the given coordinates in data space. If no
	 * point is found then zero length array returned
	 */
	...
	/**
	 * Find the indices of the nearest axes to the given coordinates in data space. If no
	 * axis is found then zero length array returned
	 */
	public int[] findNearestAxes(java.awt.geom.Point2D.Double pt) {
		...
		for (int i = 0; i < spModel.getNumDataDimensions(); i++) {
			distance = pt.distance(new Point2D.Double(spModel.getProjection().get(i, 0), spModel.getProjection().get(i, 1)));
			if (distance < margin)
	 ...
	 * from data space to device space, and with markers of the given size (in
	 * pixels) If transform is null then use the default one. If markerSize=0
	...
	public void paintView(Graphics2D g2, AffineTransform transform, int width, int height) {
		...
		if (spModel != null && spModel.getData() != null) {
			...
			if (spModel.showAxes() && (numPointsSelected > 0)) {
				...
				Matrix attributeMeanSelected = mPointsSelected.times(spModel.getData()).times(
						1d / spModel.numPointsSelected());
				Matrix attributeMean = MatrixUtils.columnMeans(spModel.getData());
				...
				while (allSeries.hasNext()) {
					...
					if (nextSeries.hasNext()) {
						...
						while (nextSeries.hasNext()) {
							...
							g2.draw(line);
							g2.fill(MarkerFactory.buildArrowHead(line, markerRadius, true));
			...
			if (spModel.showTarget()) {
				...
				for (i = 0; i < spModel.getNumDataPoints(); i++) {
					...
					circle = new Ellipse2D.Double(x - markerRadius, y - markerRadius, markerRadius * 2,
							markerRadius * 2);
					g2.draw(circle);
				}
			}
			...
			if (spModel.showAxes()) {
				for (i = 0; i < spModel.getNumDataDimensions(); i++) {
					...
					if (numPointsSelected > 0)
						g2.setColor(spModel.getColours().getColorFromSpectrum(relativeMeanForSelected[i], 0, 1));
					...
					g2.draw(new Line2D.Double(0, 0, spModel.getProjection().get(i, 0), spModel.getProjection()
							.get(i, 1)));
