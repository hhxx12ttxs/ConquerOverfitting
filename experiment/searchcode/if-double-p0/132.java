...
public class SpaghettiFactory
{
    ...
    private static final Point2d INLET = new Point2d(0.999, 0.999);
	...
	/// BUILD BOX
	double e = 0.00;  
	Particle p00 = PS.createParticle(new Point2d(  e,  e), false);  p00.setPin(true);
	Particle p01 = PS.createParticle(new Point2d(  e,1-e), false);  p01.setPin(true);
	Particle p11 = PS.createParticle(new Point2d(1-e,1-e), false);  p11.setPin(true);
	Particle p10 = PS.createParticle(new Point2d(1-e,  e), false);  p10.setPin(true);
	PS.addForce(new SpringForce2Particle(p00, p01, Constants.STIFFNESS_STRETCH, PS));
	PS.addForce(new SpringForce2Particle(p00, p10, Constants.STIFFNESS_STRETCH, PS));
	PS.addForce(new SpringForce2Particle(p11, p10, Constants.STIFFNESS_STRETCH, PS));
	PS.addForce(new SpringForce2Particle(p11, p01, Constants.STIFFNESS_STRETCH, PS));
    ...
    {
	if(p0==null) {
	    p0 = PS.createParticle(new Point2d(INLET), false);
	    p1 = null;
	    P.add(p0);
	    p0.x.set(INLET);
	    p0.v.set(-speed, 0);
	...
	if(time - tLast > h/speed) {/// MAKE NEW PARTICLE:
	    Particle p = PS.createParticle(new Point2d(INLET.x+P.size()*h, INLET.y), false);
	    P.add(p);
	    p.x.set(INLET);
	    p.v.set(-speed, 0);
		...
		Force f = new SpringForce2Particle(p, p0, Constants.STIFFNESS_STRETCH, PS);
		PS.addForce(f);
		F.add(f);
		nEdges++;

		if(p1 != null) {// ADD BENDING FORCE:
		    f = new SpringForceBending(p, p0, p1, Constants.STIFFNESS_BEND, PS);
		    PS.addForce(f);
		    F.add(f);
		}
	    }
	...
	textRenderer.beginRendering(width, height);
	{
	    ...
	    textRenderer.draw(  "Particles: "+P.size()       , 20, height-54);
	    textRenderer.draw(  "Edges    : "+nEdges         , 20, height-74);
