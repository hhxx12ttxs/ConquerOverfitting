 ...
 * Date: $Date: 2008-01-25 20:26:38 +0900 (é, 25  1ć 2008) $
 *
...
public class Secure {
	...
	static Logger logger = Logger.getLogger(Secure.class.getName());
	...
	public Secure(VChannels channels) {
		...
		SessionManager.getInstance().get(channels.getSessionId()).mcs = McsLayer;
		rc4_dec = new RC4();
		rc4_enc = new RC4();
		rc4_update = new RC4();
		sha1 = new SHA1();
		...
		options = SessionManager.getInstance().getOption(channels.getSessionId());
	}
