...
class Client extends JFrame{
	...
	public Client(){
    	...
    	catch(Exception e){System.out.println("Error when generating RSA keyŁş"+e);}   
  	}
  	...
  	private void connectServer(String serverAddress){
		try { 
			...
			if(clientMsg.isBound()==true &&
				...
				msgShowArea.append("Successfully connected to the serverŁĄ"+"\n");
				new keyThread(clientRSA, clientDES).start();
				new recThread(clientMsg).start();
			}
			else msgShowArea.append("Connection FAILEDŁĄ"+"\n");
		} 
  	...
  	class keyThread extends Thread{
  		...
  		public void run(){
  			try {
				...
				msgShowArea.append("Client's Private KeyŁş" + byteToBinary(clientKeyRSA.getPrivateKey().getEncoded()) + "\n");	
				msgShowArea.append("Client's Public KeyŁş" + byteToBinary(clientKeyRSA.getPublicKey().getEncoded()) + "\n");
 				msgShowArea.append("Server's Public Key: " + byteToBinary(serverPBK.getEncoded()) + "\n");
				msgShowArea.append("DES KeyŁş" + byteToBinary(keyDES.getEncoded()) + "\n");	
 				...
 				while(true){
 					...
 					sData = SKey_DES.SEnc(keyDES, "DEC", eSData);
 		 			...
 		 			if(detailShow.isSelected()){
						msgShowArea.append("Encrypted message receivedŁş" + byteToBinary(eData) + "\n");
						msgShowArea.append("Encrypted signature receivedŁş" + byteToBinary(eSData) + "\n");
						msgShowArea.append("MD5withRSA signature infoŁş" + byteToBinary(sData) + "\n");	
					}
					msgShowArea.setCaretPosition(msgShowArea.getText().length());
  				}
 	 		}
 			catch(Exception e){System.out.println("Error when receiving messageŁş"+e);};
 		}
  	}
    	...
    	sendButton.addActionListener(new ActionListener(){
    		public void actionPerformed(ActionEvent e){
    			try {
					...
					if(detailShow.isSelected()){
						msgShowArea.append("Encrypted messageŁş" + byteToBinary(eData) + "\n");
						msgShowArea.append("MD5WithRSA signature infoŁş" + byteToBinary(sData) + "\n");
						msgShowArea.append("Encrypted signatureŁş" + byteToBinary(eSData) + "\n");	
					}	   
					msgShowArea.setCaretPosition(msgShowArea.getText().length());
