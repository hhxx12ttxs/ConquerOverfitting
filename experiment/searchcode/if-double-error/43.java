...
public class Polish_Spot_Location implements PlugInFilter {
    ImagePlus imp;
    static int hw = 6; // half of window size in pixels 
    ...
    static String RTSource = "work/OriginalSpots/";
    static String RTDestination = "work/PolishedSpots/";
	...
	if (tr >= 0 && tr < rt.getCounter()) {
	    ...
	    rt.setValue("cx", tr, rt.getValue("X", tr) - hw + par.a[2]);
	    rt.setValue("cy", tr, rt.getValue("Y", tr) - hw + par.a[3]);
	...
	for (int n = 1 ; n <= nSlices; n++) {
	    ...
	    rt.show("Results");
	    try {
		rt.saveAs(RTDestination+"PSpA"+String.format("%04d", n)+".txt");
	    } 
	    catch (Exception e) {
