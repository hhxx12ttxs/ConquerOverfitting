...
public class EClientSocket {
	...
	public static String faMsgTypeName(final int faDataType) {
		switch (faDataType) {
			case GROUPS:
				return "GROUPS";
			case PROFILES:
	...
	private final AnyWrapper	m_anyWrapper;	                                  // msg
																				  // handler
	private DataOutputStream	m_dos;	                                          // the
																				  // socket
																				  // output
																				  // stream
	private boolean	         m_connected;	                                      // true
																				  ...
																				  // are
																				  // connected
	private EReader	         m_reader;	                                          // thread
																				  // which
																				  // reads
																				  // msgs
																				  // from
	...
	public EClientSocket(final AnyWrapper anyWrapper) {
	...
	public synchronized void eConnect(String host, final int port,
	        final int clientId) {
		// already connected?
	...
	public synchronized void eConnect(final Socket socket, final int clientId)
		...
		// set client version
		send(EClientSocket.CLIENT_VERSION);
		...
		if (m_serverVersion < EClientSocket.SERVER_VERSION) {
			eDisconnect();
			m_anyWrapper.error(EClientErrors.NO_VALID_ID,
	...
	public synchronized void eDisconnect() {
		// not connected?
		if (m_dos == null) { return; }
		
		m_connected = false;
		m_serverVersion = 0;
		m_TwsTime = "";
		
		final DataOutputStream dos = m_dos;
		m_dos = null;
		
		final EReader reader = m_reader;
		m_reader = null;
	...
	public synchronized void cancelScannerSubscription(final int tickerId) {
		...
		try {
			send(EClientSocket.CANCEL_SCANNER_SUBSCRIPTION);
			send(VERSION);
			send(tickerId);
		} catch (final Exception e) {
	...
	public synchronized void reqScannerParameters() {
		...
		try {
			send(EClientSocket.REQ_SCANNER_PARAMETERS);
			send(VERSION);
		} catch (final Exception e) {
	...
	public synchronized void reqScannerSubscription(final int tickerId,
		...
		try {
			send(EClientSocket.REQ_SCANNER_SUBSCRIPTION);
			send(VERSION);
			send(tickerId);
	...
	public synchronized void reqMktData(final int tickerId,
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_SNAPSHOT_MKT_DATA &&
		        snapshot) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_UNDER_COMP) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_REQ_MKT_DATA_CONID) {
		...
		try {
			...
			send(EClientSocket.REQ_MKT_DATA);
			send(VERSION);
			send(tickerId);
			...
			if (m_serverVersion >= 8 &&
			        EClientSocket.BAG_SEC_TYPE
			                .equalsIgnoreCase(contract.m_secType)) {
	...
	public synchronized void cancelHistoricalData(final int tickerId) {
		...
		// send cancel mkt data msg
		try {
			send(EClientSocket.CANCEL_HISTORICAL_DATA);
			send(VERSION);
			send(tickerId);
		} catch (final Exception e) {
	...
	public void cancelRealTimeBars(final int tickerId) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_REAL_TIME_BARS) {
			error(EClientErrors.NO_VALID_ID,
		...
		// send cancel mkt data msg
		try {
			send(EClientSocket.CANCEL_REAL_TIME_BARS);
			send(VERSION);
			send(tickerId);
		} catch (final Exception e) {
	...
	public synchronized void reqHistoricalData(final int tickerId,
		...
		try {
			...
			send(EClientSocket.REQ_HISTORICAL_DATA);
			send(VERSION);
			send(tickerId);
			...
			if (EClientSocket.BAG_SEC_TYPE.equalsIgnoreCase(contract.m_secType)) {
				if (contract.m_comboLegs == null) {
					send(0);
				} else {
					send(contract.m_comboLegs.size());
	...
	public synchronized void reqRealTimeBars(final int tickerId,
	        final Contract contract, final int barSize,
		...
		try {
			// send req mkt data msg
			send(EClientSocket.REQ_REAL_TIME_BARS);
			send(VERSION);
			send(tickerId);
	...
	public synchronized void reqContractDetails(final int reqId,
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_SEC_ID_TYPE) {
			if (!EClientSocket.IsEmpty(contract.m_secIdType) ||
			        !EClientSocket.IsEmpty(contract.m_secId)) {
		...
		try {
			// send req mkt data msg
			send(EClientSocket.REQ_CONTRACT_DATA);
			send(VERSION);
			
			if (m_serverVersion >= EClientSocket.MIN_SERVER_VER_CONTRACT_DATA_CHAIN) {
				send(reqId);
			}
			
			// send contract fields
			if (m_serverVersion >= EClientSocket.MIN_SERVER_VER_CONTRACT_CONID) {
				send(contract.m_conId);
			}
			send(contract.m_symbol);
			send(contract.m_secType);
	...
	public synchronized void reqMktDepth(final int tickerId,
	        final Contract contract, final int numRows) {
		// not connected?
		if (!m_connected) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.NOT_CONNECTED, "");
			return;
		...
		try {
			// send req mkt data msg
			send(EClientSocket.REQ_MKT_DEPTH);
			send(VERSION);
			send(tickerId);
	...
	public synchronized void cancelMktData(final int tickerId) {
		...
		// send cancel mkt data msg
		try {
			send(EClientSocket.CANCEL_MKT_DATA);
			send(VERSION);
			send(tickerId);
		} catch (final Exception e) {
	...
	public synchronized void cancelMktDepth(final int tickerId) {
		...
		// send cancel mkt data msg
		try {
			send(EClientSocket.CANCEL_MKT_DEPTH);
			send(VERSION);
			send(tickerId);
		} catch (final Exception e) {
	...
	public synchronized void exerciseOptions(final int tickerId,
		...
		try {
			...
			send(EClientSocket.EXERCISE_OPTIONS);
			send(VERSION);
			send(tickerId);
	...
	public synchronized void placeOrder(final int id, final Contract contract,
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_SCALE_ORDERS) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_SSHORT_COMBO_LEGS) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_WHAT_IF_ORDERS) {
			if (order.m_whatIf) {
				error(id, EClientErrors.UPDATE_TWS,
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_SCALE_ORDERS2) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_ALGO_ORDERS) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_NOT_HELD) {
			if (order.m_notHeld) {
				error(id, EClientErrors.UPDATE_TWS,
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_PLACE_ORDER_CONID) {
			if (contract.m_conId > 0) {
				error(id, EClientErrors.UPDATE_TWS,
				        "  It does not support conId parameter.");
				return;
		...
		final int VERSION = m_serverVersion < EClientSocket.MIN_SERVER_VER_NOT_HELD ? 27
		        : 30;
		
		// send place order msg
		try {
			send(EClientSocket.PLACE_ORDER);
			send(VERSION);
			send(id);
			...
			if (m_serverVersion >= 19) {
				...
				// server version 26
				final double lower = m_serverVersion == 26 &&
				        order.m_orderType.equals("VOL")
				        ? Double.MAX_VALUE
				        : order.m_stockRangeLower;
				final double upper = m_serverVersion == 26 &&
				        order.m_orderType.equals("VOL")
				        ? Double.MAX_VALUE
			...
			if (m_serverVersion >= 26) { // Volatility orders
				...
				if (m_serverVersion == 26) {
					// Volatility orders had specific watermark price attribs in
					// server version 26
					final double lower = order.m_orderType.equals("VOL") ? order.m_stockRangeLower
					        : Double.MAX_VALUE;
					final double upper = order.m_orderType.equals("VOL") ? order.m_stockRangeUpper
					        : Double.MAX_VALUE;
					sendMax(lower);
					sendMax(upper);
				}
			...
			if (m_serverVersion >= EClientSocket.MIN_SERVER_VER_PTA_ORDERS) {
			...
			if (m_serverVersion >= EClientSocket.MIN_SERVER_VER_ALGO_ORDERS) {
				...
				if (!EClientSocket.IsEmpty(order.m_algoStrategy)) {
					final java.util.Vector algoParams = order.m_algoParams;
					final int algoParamsCount = algoParams == null ? 0
					        : algoParams.size();
					send(algoParamsCount);
					if (algoParamsCount > 0) {
						for (int i = 0; i < algoParamsCount; ++i) {
							final TagValue tagValue = (TagValue) algoParams
							        .get(i);
							send(tagValue.m_tag);
	...
	public synchronized void reqAccountUpdates(final boolean subscribe,
		...
		try {
			send(EClientSocket.REQ_ACCOUNT_DATA);
			send(VERSION);
			send(subscribe);
	...
	public synchronized void reqExecutions(final int reqId,
		...
		// send cancel order msg
		try {
			send(EClientSocket.REQ_EXECUTIONS);
			send(VERSION);
			
			if (m_serverVersion >= EClientSocket.MIN_SERVER_VER_EXECUTION_DATA_CHAIN) {
				send(reqId);
			}
	...
	public synchronized void cancelOrder(final int id) {
		...
		// send cancel order msg
		try {
			send(EClientSocket.CANCEL_ORDER);
			send(VERSION);
			send(id);
		} catch (final Exception e) {
	...
	public synchronized void reqOpenOrders() {
		...
		// send cancel order msg
		try {
			send(EClientSocket.REQ_OPEN_ORDERS);
			send(VERSION);
		} catch (final Exception e) {
	...
	public synchronized void reqNewsBulletins(final boolean allMsgs) {
		...
		try {
			send(EClientSocket.REQ_NEWS_BULLETINS);
			send(VERSION);
			send(allMsgs);
		} catch (final Exception e) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_CORDER,
	...
	public synchronized void cancelNewsBulletins() {
		...
		// send cancel order msg
		try {
			send(EClientSocket.CANCEL_NEWS_BULLETINS);
			send(VERSION);
		} catch (final Exception e) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_CORDER,
			        "" + e);
	...
	public synchronized void setServerLogLevel(final int logLevel) {
		...
		try {
			send(EClientSocket.SET_SERVER_LOGLEVEL);
			send(VERSION);
			send(logLevel);
		} catch (final Exception e) {
			error(EClientErrors.NO_VALID_ID,
	...
	public synchronized void reqAutoOpenOrders(final boolean bAutoBind) {
		...
		try {
			send(EClientSocket.REQ_AUTO_OPEN_ORDERS);
			send(VERSION);
			send(bAutoBind);
		} catch (final Exception e) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_OORDER,
	...
	public synchronized void reqAllOpenOrders() {
		...
		try {
			send(EClientSocket.REQ_ALL_OPEN_ORDERS);
			send(VERSION);
		} catch (final Exception e) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_OORDER,
			        "" + e);
	...
	public synchronized void reqManagedAccts() {
		...
		try {
			send(EClientSocket.REQ_MANAGED_ACCTS);
			send(VERSION);
		} catch (final Exception e) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_OORDER,
			        "" + e);
	...
	public synchronized void reqCurrentTime() {
		...
		try {
			send(EClientSocket.REQ_CURRENT_TIME);
			send(VERSION);
		} catch (final Exception e) {
			error(EClientErrors.NO_VALID_ID,
	...
	public synchronized void reqFundamentalData(final int reqId,
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_FUNDAMENTAL_DATA) {
			error(reqId, EClientErrors.UPDATE_TWS,
		...
		try {
			...
			send(EClientSocket.REQ_FUNDAMENTAL_DATA);
			send(VERSION);
			send(reqId);
	...
	public synchronized void cancelFundamentalData(final int reqId) {
		...
		try {
			// send req mkt data msg
			send(EClientSocket.CANCEL_FUNDAMENTAL_DATA);
			send(VERSION);
			send(reqId);
		} catch (final Exception e) {
	...
	public synchronized void calculateImpliedVolatility(final int reqId,
	        final Contract contract,
	        final double optionPrice, final double underPrice) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_REQ_CALC_IMPLIED_VOLAT) {
			error(reqId,
			        EClientErrors.UPDATE_TWS,
		...
		try {
			...
			send(EClientSocket.REQ_CALC_IMPLIED_VOLAT);
			send(VERSION);
			send(reqId);
	...
	public synchronized void cancelCalculateImpliedVolatility(final int reqId) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_CANCEL_CALC_IMPLIED_VOLAT) {
			error(reqId,
			        EClientErrors.UPDATE_TWS,
		...
		try {
			...
			send(EClientSocket.CANCEL_CALC_IMPLIED_VOLAT);
			send(VERSION);
			send(reqId);
		} catch (final Exception e) {
	...
	public synchronized void calculateOptionPrice(final int reqId,
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_REQ_CALC_OPTION_PRICE) {
			error(reqId, EClientErrors.UPDATE_TWS,
		...
		try {
			...
			send(EClientSocket.REQ_CALC_OPTION_PRICE);
			send(VERSION);
			send(reqId);
	...
	public synchronized void cancelCalculateOptionPrice(final int reqId) {
		...
		if (m_serverVersion < EClientSocket.MIN_SERVER_VER_CANCEL_CALC_OPTION_PRICE) {
			error(reqId,
			        EClientErrors.UPDATE_TWS,
		...
		try {
			...
			send(EClientSocket.CANCEL_CALC_OPTION_PRICE);
			send(VERSION);
			send(reqId);
		} catch (final Exception e) {
	...
	protected synchronized void error(final String err) {
		m_anyWrapper.error(err);
	}
	
	protected synchronized void error(final int id, final int errorCode,
	...
	private static boolean is(final String str) {
	...
	private static boolean isNull(final String str) {
		...
		return !EClientSocket.is(str);
	}
	
	private void error(final int id, final EClientErrors.CodeMsgPair pair,
	        final String tail) {
		error(id, pair.code(), pair.msg() + tail);
	}
	
	protected void send(final String str) throws IOException {
		// write string to data buffer; writer thread will
		// write it to socket
		if (!EClientSocket.IsEmpty(str)) {
	...
	private void sendEOL() throws IOException {
		m_dos.write(EClientSocket.EOL);
	}
	
	protected void send(final int val) throws IOException {
		send(String.valueOf(val));
	}
	
	protected void send(final char val) throws IOException {
		m_dos.write(val);
		sendEOL();
	}
	
	protected void send(final double val) throws IOException {
		send(String.valueOf(val));
	}
	
	protected void send(final long val) throws IOException {
		send(String.valueOf(val));
	}
	
	private void sendMax(final double val) throws IOException {
	...
	private void sendMax(final int val) throws IOException {
	...
	protected void send(final boolean val) throws IOException {
		send(val ? 1 : 0);
	}
