...
public class Polygon
{
	 ...
	 * 
	 * @param count
	 *            the length of the polygon vertex
	 * @param xa
	 ...
	 */
	private boolean testInside( int count, double[] xa, double[] ya, double x,
			double y )
	{
		boolean c = false;
	...
	private boolean testWithinLine(Point pStart, Point pEnd, Point p)
	{
		...
		boolean c = ( p.x >= pStart.x && p.x <= pEnd.x || p.x <= pStart.x
				&& p.x >= pEnd.x )
				&& ( p.y >= pStart.y && p.y <= pEnd.y || p.y <= pStart.y
						&& p.y >= pEnd.y );
		if ( c )
		{
	...
	private boolean testLineIntersect( Point p1, Point p2, Point q1, Point q2 )
	{
		...
		return ( ( ccw( p1, p2, q1 ) != ccw( p1, p2, q2 ) ) && ( ccw( q1,
				q2,	p1 ) != ccw( q1, q2, p2 ) ) );
	}
 ...
 pg.get( i ), pg.get( 0 ) ) )
	...
	public boolean contains( Point p )
	{
		...
		return testInside( count, xa, ya, p.getX( ), p.getY( ) )
				|| testWithinLine( p );
	}
