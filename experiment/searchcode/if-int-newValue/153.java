...
public class TileEntityReactorRedNetPort extends TileEntityReactorPart implements ITickableMultiblockPart, INeighborUpdatableEntity, IRedNetOmniNode {
	...
	IRedNetOmniNode redNetConnectable;
	...
	protected TileEntity getMappedTileEntity(int channel) {
		...
		if (!this.getWorldObj().checkChunksExist(coord.x, coord.y, coord.z, coord.x, coord.y, coord.z)) {
			return null;
		}

		return this.getWorldObj().getTileEntity(coord.x, coord.y, coord.z);
	}
	...
	protected void setControlRodInsertion(int channel, CoordTriplet coordTriplet, int newValue) {
		...
		if (!this.getWorldObj().checkChunksExist(coordTriplet.x, coordTriplet.y, coordTriplet.z,
				coordTriplet.x, coordTriplet.y, coordTriplet.z)) {
			return;
		}

		TileEntity te = this.getWorldObj().getTileEntity(coordTriplet.x, coordTriplet.y, coordTriplet.z);
	...
	protected void decodeSettings(NBTTagCompound source) {
		NBTTagList tagArray = source.getTagList("redNetConfig", 0);
		...
		if (out == ForgeDirection.UNKNOWN) {
			...
			if (!(b instanceof BlockReactorPart)) {
				if (b instanceof IRedNetNetworkContainer) {
					...
					redNetConnectable = (IRedNetOmniNode) b;
				}
			}
		}
	}
	...
	// Static Helpers
	public static boolean circuitTypeHasSubSetting(CircuitType circuitType) {
