...
/**
 * Class Integral defines various integration algorithms. This class cannot be
...
public final class Integral {
	...
	static private final class FunctionRate implements ODE {
		...
		private FunctionRate(final Function _f, final double start) {
			state = new double[2];
			state[0] = 0; // integral
	 ...
	 * @param start
	 *            double start of integral
	 * @param stop
	 *            double end of integral
	 * @param tol
	 ...
	 * @return double[][]
	 */
	static public double[][] fillArray(final Function f, final double start,
			final double stop, final double tol, final double[][] data) {
		ODE ode = new FunctionRate(f, start);
		ODEAdaptiveSolver ode_method = new ODEMultistepSolver(ode); // must be a
		// fixed
		// step size
		// algorithm
	 ...
	 */
	static public double[][] fillArray(final Function f, final double start,
			final double stop, final double tol, final int n) {
	 ...
	 * @return the integral
	 */
	static public double ode(final Function f, final double start,
			final double stop, final double tol) {
		ODE ode = new FunctionRate(f, start);
		RK45MultiStep ode_method = new RK45MultiStep(ode); // mimics a fixed
		// size solver
		// ODEMultistepSolver ode_method= new ODEMultistepSolver(ode); // mimics
		// a fixed size solver
		ode_method.setTolerance(tol);
	...
	/**
	 * Integrates the function using Romberg's algorithm based on Richardson's
	 * deferred approach.
	 * 
	 * @param f
	 *            the function
	 ...
	 * @return the integral
	 */
	static public double romberg(final Function f, final double a,
			final double b, int n, final double tol) {
		if (a == b) {
			return (0);
		}
		if (tol <= 0) {
		...
		for (int j = 1; j < MAX_ITERATIONS; j++) {
			...
			if (Util
					.relativePrecision(Math.abs(coef[j] - coef[j - 1]), coef[j]) < tol) {
	...
	/**
	 * Uses Simpson's rule to find the area of an array representing a function
	 * that's been evaluated at N intervals of size h, where N is an odd
	 * integer.
	 * 
	 * Example of usage: int N=27; x=new double[N]; f=new double[N]; double a=0,
	 * b=5, h=(b-a)/(N-1); for (int i=0; i< N;i++){ x[i]=a+i*h;
	 * f[i]=x[i]*Math.exp(x[i]); } double sum=Simp.Simp(f,h);
	 * 
	 * Results: sum=594.6615858178942
	 * 
	 * @Author J E Hasbun 2007
	 */
	public static double simpson(final double f[], final double h) {
	 ...
	 * @return the integral
	 */
	public static double simpson(final Function f, final double start,
			final double stop, final int n) throws IllegalArgumentException {
		if (n % 2 != 0) {
		...
		return h
				/ 3.0
				* (f.evaluate(start) + 4.0 * sumOdd + 2.0 * sumEven + f
						.evaluate(stop));
	}
	 ...
	 * @return the integral
	 */
	public static double simpson(Function f, double start, double stop, int n,
			double tol) {
		...
		do {
			...
			n /= 2;
				...
				|| ((iterations < MAX_ITERATIONS) && (Util.relativePrecision(
						Math.abs(result - oldResult), result) > tol)));
		return sign * result;
	}
	 ...
	 * @return the integral
	 */
	public static double trapezoidal(Function f, double start, double stop,
			int n, double tol) {
		double step = stop - start;
		int sign = (step < 0) ? -1 : 1;
		if (sign < 1) {
