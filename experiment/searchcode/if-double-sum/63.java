...
/**
 * RandomChoice organizes arrays of floats into distributions which can be used
 * to pick randomly from. You can provide three kinds of arrays:
 * 
 * <ul>
 * <li>An array of floats
 * <li>An array of doubles
 * <li>An array of arbitrary objects, plus a RandomChoiceChooser which knows how
 * to get and set the appropriate "float" value of objects in this array.
 * </ul>
 * 
 * <p>
 * Before the RandomChoice can pick randomly from your array, it must first
 * organize it. It does this by doing the following. First, it normalizes the
 * values in the array. Then it modifies them to their sums. That is, each item
 * i in the array is set to the sum of the original values for items 0...i. If
 * you cannot allow your objects to be modified, then this is not the class for
 * you.
 * 
 * <p>
 * An array is valid if (1) it has no negative values and (2) not all of its
 ...
 * RandomChoice pick random items from the array and return their indexes to
 * you. You do this by calling pickFromDistribution(), passing it a random
 * floating point value between 0.0 and 1.0. You call organizeDistribution()
 * only once; after which you may call pickFromDistribution() as many times as
 * you like. You should not modify the array thereafter.
 * 
 * @author Sean Luke
 * @version 1.0
 */
...
public class RandomChoice {
    ...
    /**
     * Normalizes probabilities, then converts them into continuing sums. This
     * prepares them for being usable in pickFromDistribution. If the
     * probabilities are all 0, then selection is uniform, unless allowAllZeros
     * is false, in which case an ArithmeticException is thrown. If any of them
     * are negative, or if the distribution is empty, then an
     * ArithmeticException is thrown. For example, {0.6, 0.4, 0.2, 0.8} -> {0.3,
     * 0.2, 0.1, 0.4} -> {0.3, 0.5, 0.6, 1.0}
     */
    ...
    /**
     * Normalizes the probabilities associated with an array of objects, then
     * converts them into continuing sums. This prepares them for being usable
     * in pickFromDistribution. If the probabilities are all 0, then selection
     * is uniform, unless allowAllZeros is false, in which case an
     * ArithmeticException is thrown. If any of them are negative, or if the
     * distribution is empty, then an ArithmeticException is thrown. For
     * example, {0.6, 0.4, 0.2, 0.8} -> {0.3, 0.2, 0.1, 0.4} -> {0.3, 0.5, 0.6,
     * 1.0} The probabilities are retrieved and set using chooser.
     */

    public static void organizeDistribution(final Object[] objs, final RandomChoiceChooser chooser,
            final boolean allowAllZeros) {
        // first normalize
        double sum = 0.0;
        ...
        for (int x = 0; x < objs.length; x++)
            chooser.setProbability(objs[x], (float) (chooser.getProbability(objs[x]) / sum));
        ...
        for (x = objs.length - 1; x > 0; x--)
            if (chooser.getProbability(objs[x]) == chooser.getProbability(objs[x - 1])) // we're
                                                                                        // 0.0
                chooser.setProbability(objs[x], 1.0f);
            else
                break;
    ...
    public static void organizeDistribution(final Object[] objs, final RandomChoiceChooserD chooser,
        ...
        for (int x = 0; x < objs.length; x++)
            chooser.setProbability(objs[x], (double) (chooser.getProbability(objs[x]) / sum));
    ...
    private static final int exemptZeroes(final float[] probabilities, int index) {
        ...
        if (probabilities[index] == 0.0f) // I need to scan forward because I'm
                                          // in a left-trail
        // scan forward
        {
    ...
    private static final int exemptZeroes(final double[] probabilities, int index) {
        // System.out.println(index);
        if (probabilities[index] == 0.0) // I need to scan forward because I'm
                                         // in a left-trail
        // scan forward
        {
    ...
    private static final int exemptZeroes(final Object[] objs, final RandomChoiceChooser chooser, int index) {
        // System.out.println(index);
        if (chooser.getProbability(objs[index]) == 0.0f) // I need to scan
                                                         // forward because
                                                         // I'm in a
                                                         // left-trail
        // scan forward
    ...
    private static final int exemptZeroes(final Object[] objs, final RandomChoiceChooserD chooser, int index) {
        // System.out.println(index);
        if (chooser.getProbability(objs[index]) == 0.0) // I need to scan
                                                        // forward because I'm
                                                        // in a left-trail
        // scan forward
        {
    ...
    /**
     * Picks a random item from an array of probabilities, normalized and summed
     * as follows: For example, if four probabilities are {0.3, 0.2, 0.1, 0.4},
     * then they should get normalized and summed by the outside owners as:
     * {0.3, 0.5, 0.6, 1.0}. If probabilities.length < CHECKBOUNDARY, then a
     * linear search is used, else a binary search is used.
     */
    ...
    /**
     * Picks a random item from an array of objects, each with an associated
     * probability that is accessed by taking an object and passing it to
     * chooser.getProbability(obj). The objects' probabilities are normalized
     * and summed as follows: For example, if four probabilities are {0.3, 0.2,
     * 0.1, 0.4}, then they should get normalized and summed by the outside
     * owners as: {0.3, 0.5, 0.6, 1.0}. If probabilities.length < CHECKBOUNDARY,
