...
public class Tester {
	...
	public static double esEnaPlusEnaAlgorithm(int dimension, int maxEvaluation, int funType) {
	    ...
	    int num_eval = 1;
	    int succ = 0;
	    double varianceOne = 5.0; // črepinek = 5.0 ; original je 1.0 
	    int everyK = 0;
	    ...
	    while (num_eval < maxEvaluation) {
	        ...
	        for(int i=0; i<one.length;i++) 
	        	oneplus[i] = one[i];
	       // oneplus = one.clone(); ^ gor navrh črepinek izboljava =)
...
//		test randomWalk
		...
		for(int funkcija = 1; funkcija < 5; funkcija++) { // testiramo funkcije Sphere, Rastrigin, SumSquares in Zakharov
			for(int dim : testneDimenzije) { // testiramo na dimenzijah 2, 5, 10, 30, 50
				...
				for(int eval : testnaOvrednotenja) { // testiramo na več kombinacijah evaluacij in sicer dimenzija*1000 ter dimenzija*2000
					//						System.out.println(eval*dim);
					for(int zagon = 0; zagon < 30; zagon++) { // vsako izmed vseh monih kombinacij poganjamo 30-krat
					...
					System.out.println("Povprečje metode RadomWalk s funkcijo " + funkcija + " pri tevilu evaluacij " + eval + " in dimenziji " + dim + " je: " + average);
		...
		for(int funkcija = 1; funkcija < 5; funkcija++) { // testiramo funkcije Sphere, Rastrigin, SumSquares in Zakharov
			for(int dim : testneDimenzije) { // testiramo na dimenzijah 2, 5, 10, 30, 50
				...
				for(int eval : testnaOvrednotenja) { // testiramo na več kombinacijah evaluacij in sicer dimenzija*1000 ter dimenzija*2000
					...
					System.out.println("Povprečje metode ES(1+1) s funkcijo " + funkcija + " pri tevilu evaluacij " + eval + " in dimenziji " + dim + " je: " + average);
