...
public abstract class FourierTransform
{
  ...
  public void linAverages(int numAvg)
  {
    ...
    {
      CCSoundIO.error("The number of averages for this transform can be at most "
          + spectrum.length / 2 + ".");
    }
    else
    {
  ...
  public void logAverages(int minBandwidth, int bandsPerOctave)
  {
    ...
    while ((nyq /= 2) > minBandwidth)
      octaves++;
    CCSoundIO.debug("Number of octaves = " + octaves);
  ...
  public void window(int which)
  {
    if (which < 0 || which > 1)
    {
      CCSoundIO.error("Invalid window type.");
    }
    else
    {
      whichWindow = which;
   ...
   *          the buffer to analyze
   */
  public void forward(CCAudioBuffer buffer)
  {
   ...
   *          the buffer to place the result of the inverse transform in
   */
  public void inverse(CCAudioBuffer buffer)
  {
