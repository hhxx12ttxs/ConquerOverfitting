...
public abstract class JavaEditor extends AbstractDecoratedTextEditor implements IViewPartInputProvider {
	...
	protected void setSelection(ISourceReference reference, boolean moveCursor) {
		...
		if (reference != null) {
			...
			try {
				...
				} else if (reference instanceof NJSDocOutlineNode && range instanceof SourceRange) {
					NJSDocOutlineNode node = (NJSDocOutlineNode) reference;
					ASTNode astNode = node.getASTNode();
					
					if (astNode != null && new SourceRange(astNode).covers((SourceRange)range)) {
						if (astNode instanceof FunctionDeclaration) {
							FunctionDeclaration fd = (FunctionDeclaration) astNode;
							if (fd.getName() != null) {
								offset = fd.getName().getStartPosition();
								length = fd.getName().getLength();
							} else if (fd.getJavadoc() == null) {
								offset = fd.getStartPosition();
								length = 8; // "function"
							} else {
								range = new SourceRange(getNodePos(astNode), 0);
							}
						}
					}
					
					if (offset < 0 && sourceViewer != null) {
						try {
							IRegion r = sourceViewer.getDocument().getLineInformationOfOffset(range.getOffset());
	...
	public void setSelection(NJSDocOutlineNode element) {
		if (element != null) {
	...
	/**
	 * Use heuristics to find the offset of the best line for this node.
	 * 
	 * @return The character offset of the node in the file. If the node is a
	 *         function, the offset of the line that the signature is on is
	 *         preferred.
	 */
	static int getNodePos(ASTNode astNode) {
		if (astNode == null) {
			return -1;
		}
		...
		if (astNode instanceof FunctionDeclaration) {
			...
			// Decreasingly accurate heuristics for the finding the character
			// offset of the line that the 'function' keyword is on. Another
			// approach is to have the AST node store this information too
			if (fd.getName() != null) {
				return fd.getName().getStartPosition();
			}
			
			for (Object o : fd.parameters()) {
				if (o instanceof ASTNode) {
					return ((ASTNode) o).getStartPosition();
				}
			}
			
			JSdoc jd = fd.getJavadoc();
	
			if (jd != null) {
				int a = astNode.getStartPosition() + jd.getLength() + 2;
	
				if (fd.getBody() != null) {
					return (int) Math.floor((a + fd.getBody().getStartPosition()) / 2.0);
				}
			...
			if (fd.getBody() != null) {
				return fd.getBody().getStartPosition() - 11; // 11 = length of "function(){"
			}
		}
