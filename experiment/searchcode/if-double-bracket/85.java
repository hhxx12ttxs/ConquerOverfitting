...
public class BiDiReorder {
    ...
    /**
     * Converts a "logical" string into a "visual" string
     * 
     * @param  text The input string
     * @return The converted string
     */
    public static String reorder(String text) {
        return reorder(text, null);
    }
     ...
     * 
     * @param  text The input string
     * @param  rtl  An optional parameter that is set to true if the
     *              input is RTL or mixed
     * @return The converted string
     */
    public static String reorder(String text, boolean[] rtl) {
        char[] inputChars = text.toCharArray();
        if (Bidi.requiresBidi(inputChars, 0, inputChars.length)) { 
            Object[] inputA = new Object[text.length()];
            byte[] levelsA = new byte[text.length()];
            
            Bidi bidi = new Bidi(text, Bidi.DIRECTION_DEFAULT_LEFT_TO_RIGHT);
            for (int i = 0; i < text.length(); ++i) {
                final char c = inputChars[i];
                levelsA[i] = (byte) bidi.getLevelAt(i);
                inputA[i] = ((levelsA[i] & 1) != 0) ? getMirror(c) : c;
            }
            Bidi.reorderVisually(levelsA, 0, inputA, 0, inputA.length);
            StringBuilder result = new StringBuilder();
            for (int i = 0; i < inputA.length; ++i)
                result.append(inputA[i]);
            if (rtl != null) rtl[0] = !bidi.isLeftToRight();//isRightToLeft();
            return result.toString();
        }
        if (rtl != null) rtl[0] = false;
        return text;
    }
     ...
     * 
     * @param  text The input string
     * @param width The maximal width to draw before trimming occurs
     * @param paint The paint used for the text (e.g. size, style)
     * @param wrap  A optional token used to indicate that the text has been trimmed
     * @return The converted string
     */
    public static String reorder(String text, float width, Paint paint, String wrap) {
        return reorder(text, width, paint, wrap, null);
    }
    ...
    public static String reorder(String text, float width, Paint paint, String wrap, boolean[] rtl) {
        ...
        int nFitChars;
        text = reorder(text, rtl);
        
        if (rtl[0]) {
            nFitChars = paint.breakText(text, false, width, null);
            if (nFitChars < text.length()) {
                if (wrap != null) {
                    nFitChars = paint.breakText(text, false, width - paint.measureText(wrap), null);
                    text = wrap + text.substring(text.length() - nFitChars);
                } else {
                    text =        text.substring(text.length() - nFitChars);
                }
            }
        } else { // LTR
            nFitChars = paint.breakText(text, true, width, null);
            if (nFitChars < text.length()) {
                if (wrap != null) {
                    nFitChars = paint.breakText(text, true, width - paint.measureText(wrap), null);
                    text = text.substring(0, nFitChars) + wrap;
                } else {
                    text = text.substring(0, nFitChars);
                }
            }
        }
        return text;
     ...
     * mirrored horizontally in text that is laid out from right to left. For
     ...
     * return that character. Otherwise the original character is returned.
     * 
     * @param ch The input character.
     * @return the mirrored character if exists, otherwise the input character is returned.
     */
    public static char getMirror(char ch) {
        if (Character.isMirrored(ch)) {
            for (int i = 0; i < MIRROR_CODES.length; i += 2) {
                if (MIRROR_CODES[i] == ch) {
                    return MIRROR_CODES[i+1];
                }
            }
        }
        return ch;
    ...
    /**
     * An array of characters that have the Bidi_Mirrored=True property
     * value, for which there is another Unicode character that typically
     * has a glyph that is the mirror image of the original character's glyph.
     * 
     * Taken from http://unicode.org/Public/UNIDATA/BidiMirroring.txt
     */
    private static char[] MIRROR_CODES = {
