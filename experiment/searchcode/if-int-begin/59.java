...
package cc.creativecomputing.sound.spi.javasound;

import org.tritonus.share.sampled.FloatSampleBuffer;

import cc.creativecomputing.sound.CCAudioSignal;
import cc.creativecomputing.sound.CCBufferedAudio;
import cc.creativecomputing.sound.CCSoundIO;
import cc.creativecomputing.sound.CCTriggerable;


class SampleSignal implements CCAudioSignal, CCTriggerable {
	...
	public void generate(float[] signal) {
		...
		for (int i = 0; i < marks.length; i++) {
			...
			// + marks[i]);
			int j, k;
			for (j = begin, k = 0; j < buffer.getSampleCount() && k < signal.length; j++, k++) {
	...
	public float[] getChannel(int channelNumber) {
		if (channelNumber == CCBufferedAudio.LEFT) {
			return buffer.getChannel(0);
		} else if (channelNumber == CCBufferedAudio.RIGHT) {
