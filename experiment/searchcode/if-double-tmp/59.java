...
/**
 * Implementations of various center-pivot QuickSort routines in Java, and (if
 * you really want 'em) Insertion Sort routines as well. This code is derived
 * from the QuickSort example in the <a href=
 * "ftp://ftp.prenhall.com/pub/esm/computer_science.s-041/shaffer/ds/code/JAVA/code/javacode.zip"
 * > source code </a> accompanying <i>A Practical Introduction to Data
 * Structures and Algorithm Analysis, Java Edition</i>, by Clifford Shaffer.
 * Here's the original header:
 * 
 * <p>
 * Source code example for "A Practical Introduction to Data Structures and
 * Algorithm Analysis" by Clifford A. Shaffer, Prentice Hall, 1998. Copyright
 * 1998 by Clifford A. Shaffer
 * 
 * <p>
 * Sorting main function for testing correctness of sort algorithm. To use:
 * <sortname> [+/-] <size_of_array> <threshold> + means increasing values, -
 * means decreasing value and no parameter means random values; where
 * <size_of_array> controls the number of objects to be sorted; and <threshold>
 * controls the threshold parameter for certain sorts, e.g., cutoff point for
 * quicksort sublists.
 * 
 * @author Sean Luke
 * @version 1.0
 ...
 * The original code
 * 
 * // Non-Recursive QuickSort static public void qsort(Elem[] array) {
 * qsort(array, 0, array.length-1); }
 * 
 * static public int MAXSTACKSIZE = 1000;
 * 
 * // Insertion Sort static void inssort(Elem[] array) { Elem tmp;
 * 
 * for (int i=1; i<array.length; i++) // Insert i'th record for (int j=i; (j>0)
 * && (array[j].key()<array[j-1].key()); j--) DSUtil.swap(array, j, j-1); }
 * 
 * static private void qsort_h(Elem[] array, int oi, int oj) { int[] stack = new
 * int[MAXSTACKSIZE]; // Stack for array bounds int listsize = oj-oi+1; int top
 * = -1; int pivot; int pivotindex, l, r; Elem tmp;
 * 
 * stack[++top] = oi; // Initialize stack stack[++top] = oj;
 * 
 * while (top > 0) // While there are unprocessed subarrays { // Pop stack int j
 * = stack[top--]; int i = stack[top--];
 * 
 * // Findpivot pivotindex = (i+j)/2; pivot = array[pivotindex].key();
 * DSUtil.swap(array, pivotindex, j); // Stick pivot at end // Partition l =
 * i-1; r = j; do { while (array[++l].key() < pivot); while ((r!=0) &&
 * (array[--r].key() > pivot)); DSutil.swap(array, l, r); } while (l < r);
 * DSUtil.swap(array, l, r); // Undo final swap DSUtil.swap(array, l, j); // Put
 * pivot value in place
 * 
 * // Put new subarrays onto stack if they are small if ((l-i) > THRESHOLD) //
 * Left partition { stack[++top] = i; stack[++top] = l-1; } if ((j-l) >
 * THRESHOLD) // Right partition { stack[++top] = l+1; stack[++top] = j; } }
 * inssort(array); // Final Insertion Sort }
 */
