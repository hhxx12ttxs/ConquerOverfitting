...
public final class DialRenderer
{
	 ...
	 */
	DialRenderer( ChartWithoutAxes dct, Dial dial, SeriesRenderingHints srh,
			Palette pa ) throws ChartException
	{
		xs = dial.getXServer( );
		...
		insCA = goFactory.scaleInsets( goFactory.createInsets( 2, 2, 2, 2 ),
				dScale );
		dSafeSpacing *= dScale;
	}
	...
	{
		return ds.getDial( ).isSetStartAngle( ) ? ds.getDial( ).getStartAngle( )
				: 0;
	}

	final double getDialStopAngle( )
	{
		return ds.getDial( ).isSetStopAngle( ) ? ds.getDial( ).getStopAngle( )
				: 180;
	}
			...
			Scale sc, Bounds bo, BigDecimal divisor ) throws ChartException
	{
		...
		double dFullRadius = Math.min( boCA.getWidth( ) / 2d,
				boCA.getHeight( ) / 2d );

		// pre-compute an intial scale.
		AutoScale isc = computeScale( ds.getDial( ).getLabel( ),
				new DataSetIterator( new double[]{
		...
		final boolean bFullDisc = dStopAngle < 0
				|| dStopAngle > 180
				|| dStartAngle < 0
				|| dStartAngle > 180;
		...
		if ( dRadius > 0
				&& ( dRadius + extraSpacing + dSafeSpacing * 2 ) < dFullRadius )
		{
		...
		// Dial actual bounds
		Bounds dialBounds = goFactory.createBounds( fullBounds.getLeft( )
				+ extraSpacing
				+ dSafeSpacing, fullBounds.getTop( )
				+ extraSpacing
				+ dSafeSpacing, fullBounds.getWidth( )
				- 2
				* ( extraSpacing + dSafeSpacing ), fullBounds.getHeight( )
				- 2
				* ( extraSpacing + dSafeSpacing ) );
			...
			throws ChartException
	{
		...
		Bounds dialBounds = goFactory.createBounds( fullBounds.getLeft( )
				...
				+ dSafeSpacing, fullBounds.getWidth( )
				- 2
				* ( dExtraSpacing + dSafeSpacing ), fullBounds.getHeight( )
				- 2
				* ( dExtraSpacing + dSafeSpacing ) );
		...
		if ( !bDialSuperimposed || bFirstSeries )
		{
			...
			are.setStyle( Math.abs( dStopAngle - dStartAngle ) < 360 ? ArcRenderEvent.SECTOR
					: ArcRenderEvent.OPEN );
			idr.drawArc( are );
		}
		...
		for ( int i = 0; i < dialComponent.getDialRegions( ).size( ); i++ )
		{
					...
					|| ( Math.min( drStartValue, drEndValue ) >= ( ascMaxValue ) ) || ( drStartValue == drEndValue ) ) )
			{
				...
				are.setInnerRadius( dregion.isSetInnerRadius( ) ? Math.min( dregion.getInnerRadius( )
						* dScale,
						dDialRadius )
						: 0 );
				if ( dregion.isSetOuterRadius( ) )
				{
					...
					{
						are.setOuterRadius( Math.min( dregion.getOuterRadius( )
								* dScale, dDialRadius ) );
					}
				}
				else
		...
		final LineRenderEvent lre = ( (EventObjectCache) idr ).getEventObject( StructureSource.createUnknown( dialComponent ),
		...
		if ( !bDialSuperimposed || bFirstSeries )
		{
							...
							.isVisible( ) )
			{
				...
				for ( int i = 0; i < asc.getTickCount( ); i++ )
				{
					...
					if ( dialComponent.getMajorGrid( )
							.getTickAttributes( )
							.isVisible( ) )
					{
						lre.setLineAttributes( dialComponent.getMajorGrid( )
								.getTickAttributes( ) );

						switch ( dialComponent.getMajorGrid( )
								.getTickStyle( )
								.getValue( ) )
						{
							case TickStyle.ABOVE :
								start.set( xc + dDialRadius * Math.cos( mtr ),
										yc - dDialRadius * Math.sin( mtr ) );
								end.set( xc
										+ ( dDialRadius + getTickSize() )
										* Math.cos( mtr ), yc
										- ( dDialRadius + getTickSize() )
										* Math.sin( mtr ) );
								break;
					...
					if ( dialComponent.getMinorGrid( )
							.getTickAttributes( )
							.isVisible( )
							&& i != asc.getTickCount( ) - 1 )
					{
						lre.setLineAttributes( dialComponent.getMinorGrid( )
								.getTickAttributes( ) );
						...
						for ( int j = 0; j < sc.getMinorGridsPerUnit( ) - 1; j++ )
						{
							...
							if ( ( inverseScale && mtd + minortd >= transformAngle( tickCoord.getCoordinate( i + 1 ) ) )
									|| ( !inverseScale && mtd - minortd <= transformAngle( tickCoord.getCoordinate( i + 1 ) ) ) )
							{
							...
							double minortr = Math.toRadians( inverseScale ? ( mtd + minortd )
									: ( mtd - minortd ) );

							switch ( dialComponent.getMinorGrid( )
									.getTickStyle( )
									.getValue( ) )
							{
								case TickStyle.ABOVE :
		...
		if ( !bDialSuperimposed || ( bDialSuperimposed && bFirstSeries ) )
		{
			...
			{
				final TextRenderEvent tre = ( (EventObjectCache) idr ).getEventObject( StructureSource.createUnknown( dialComponent.getLabel( ) ),
				...
				for ( int i = 0; i < tickCoord.size( ); i++ )
				{
					...
					double tickAngle = dStartAngle
							+ ( tickCoord.getNormalizedCoordinate( i ) )
							/ ( tickCoord.getEnd( ) - tickCoord.getStart( ) )
							* ( dStopAngle - dStartAngle );
		...
		Fill fl = bCategoryStyle ? ( FillUtil.getPaletteFill( pa.getEntries( ),
				iValueValidIndex ) )
		...
		final LineRenderEvent lreNeedle = ( (EventObjectCache) idr ).getEventObject( WrappedStructureSource.createSeriesDataPoint( ds, dphValue ),
				LineRenderEvent.class );
		lreNeedle.setLineAttributes( lia );
		lreNeedle.setStart( goFactory.createLocation( dialBounds.getLeft( )
				+ dialBounds.getWidth( )
				/ 2, dialBounds.getTop( ) + dialBounds.getHeight( ) / 2 ) );
		...
		lreNeedle.setEnd( goFactory.createLocation( lreNeedle.getStart( )
				.getX( )
				+ xOffset, lre.getStart( ).getY( ) - yOffset ) );
		...
		if ( dial.isInteractivityEnabled( ) )
		{
			...
			{
				final StructureSource iSource = WrappedStructureSource.createSeriesDataPoint( ds,
						dphValue );
		...
		// Needle decorator
		renderNeedleDecorator( idr,
				needleComponent,
				lreNeedle.getStart( ),
				lreNeedle.getEnd( ),
				lreNeedle.getLineAttributes( ),
				fl,
				needleAngle,
				dialBounds.getWidth( ) / 2d,
				dphValue );
	...
	private void renderNeedleDecorator( IDeviceRenderer idr, Needle ndl,
			Location startPoint, Location endPoint, LineAttributes lia,
			Fill fill, double angle, double dialRadius, DataPointHints dph )
			throws ChartException
	{
		...
		switch ( ndl.getDecorator( ).getValue( ) )
		{
			case LineDecorator.ARROW :
				final PolygonRenderEvent pre = ( (EventObjectCache) idr ).getEventObject( WrappedStructureSource.createSeriesDataPoint( ds, dph ),
						PolygonRenderEvent.class );

				Location[] loa = new Location[3];
				loa[0] = endPoint.copyInstance( );
				loa[1] = goFactory.createLocation( endPoint.getX( )
						- 2
						* decSize
						* Math.cos( Math.toRadians( angle - 30 ) ),
						endPoint.getY( )
								+ 2
								* decSize
								* Math.sin( Math.toRadians( angle - 30 ) ) );
				loa[2] = goFactory.createLocation( endPoint.getX( )
						- 2
						* decSize
						* Math.sin( Math.toRadians( 60 - angle ) ),
						endPoint.getY( )
								+ 2
								* decSize
								* Math.cos( Math.toRadians( 60 - angle ) ) );
				pre.setPoints( loa );
			...
			case LineDecorator.CIRCLE :
				...
				final OvalRenderEvent ore = ( (EventObjectCache) idr ).getEventObject( WrappedStructureSource.createSeriesDataPoint( ds, dph ),
						OvalRenderEvent.class );

				double radius = Math.sqrt( Math.pow( endPoint.getX( )
						- startPoint.getX( ), 2 )
				...
				Bounds bo = goFactory.createBounds( x - decSize,
						y - decSize,
						2 * decSize,
						2 * decSize );
				ore.setBounds( bo );
	...
	private Size computeLabelSize( AutoScale sc, Label la )
			throws ChartException
	{
		if ( !la.isVisible( ) )
		{
			...
			throws ChartException
	{
		...
		final Integer oStepNumber = scModel.isSetStepNumber( )
				? Integer.valueOf( scModel.getStepNumber( ) ) : null;
		...
		ScaleInfo info = new ScaleInfo( dial.getComputations( ),
				IConstants.LINEAR
	...
	/**
	 * Checks all labels for any overlap for a given axis' scale, SPECIFICALLY
	 * ONLY used for meter scale(numeric,linear).
	 * 
	 * @param la
	 ...
	 * @return
	 */
	private boolean checkFit( AutoScale sc, IDisplayServer xs, Label la,
			int iLabelLocation, double fullRadius ) throws ChartException
	{
		...
		if ( iLabelLocation == IConstants.ABOVE
				|| iLabelLocation == IConstants.BELOW )
		{
		...
		else if ( iLabelLocation == IConstants.LEFT
				|| iLabelLocation == IConstants.RIGHT )
		{
		...
		if ( ( sc.getType( ) & ( IConstants.NUMERICAL | IConstants.LINEAR ) ) == ( IConstants.NUMERICAL | IConstants.LINEAR ) )
		{
			...
			for ( int i = 0; i < angles.size( ); i++ )
			{
				...
				{
					sText = ValueFormatter.format( nde,
							sc.getFormatSpecifier( ),
							sc.getRunTimeContext( ).getULocale( ),
							df );
				}
				catch ( ChartException dfex )
				{
