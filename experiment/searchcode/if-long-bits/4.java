...
public final class Bits
{
    ...
    {
        if ( CHECK ) check(start, finish) ;
        bits = clearInt(bits, start, finish) ;
    ...
    { 
        if ( CHECK ) check(start, finish) ;
        return setInt(bits, start, finish) ;
    }
    ...
    {
        if ( CHECK ) check(bitIndex) ;
        return testInt(bits, isSet, bitIndex) ;
    }
    ...
    {
        if ( CHECK ) check(start, finish) ;
        return testInt(bits, value, start, finish) ;
    }
    ...
    {
        if ( CHECK ) check(start, finish) ;
        return accessInt(bits, start, finish) ; 
    }
    ...
    {
        if ( CHECK ) check(start, finish) ;
        return clearInt(bits, start, finish) ;
    }
    ...
    {
        if ( CHECK ) check(start, finish) ;
        return maskZeroInt(start, finish) ;
    }
    
    private static final
    long clearInt(long bits, int start, int finish)
    {
        long mask = maskZeroInt(start, finish) ;
        bits = bits & mask ;
        return bits ;
    }

    private static final
    long setInt(long bits, int bitIndex)
    { 
        long mask = maskInt(bitIndex) ;
        return bits | mask ;
    }

    private static final
    long setInt(long bits, int start, int finish)
    { 
        long mask = maskInt(start, finish) ;
        return bits | mask ;
    }

    private static
    boolean testInt(long bits, boolean isSet, int bitIndex)
    {
        return isSet == accessInt(bits, bitIndex) ;
    }

    private static
    boolean testInt(long bits, long value, int start, int finish)
    {
        long v = accessInt(bits, start, finish) ;
        return v == value ;
    }
    ...
    private static final
    boolean accessInt(long bits, int bitIndex)
    {
        long mask = maskInt(bitIndex) ;
        return (bits & mask) != 0L ;
    }
    ...
    private static final
    long accessInt(long bits, int start, int finish)
    {
        // Two ways:
//        long mask = maskInt(start, finish) ;
//        return bits & mask ;
    ...
    private static final
    long maskInt(int bitIndex)
    {
        return 1L << bitIndex ;
    }

    private static final
    long maskInt(int start, int finish)
    {
    //        long mask = 0 ;
    //        if ( finish == Long.SIZE )
...
//        return mask >>> start << start ;                  // Clear the bottom bits
    ...
    private static final
    long maskZeroInt(int start, int finish)
    {
