...
public class TokenizerJSON implements Tokenizer
{
    ...
    //                }
                ...
                case Chars.CH_SEMICOLON:  reader.readChar() ; token.setType(TokenType.SEMICOLON) ; return token ;
                case Chars.CH_COMMA:      reader.readChar() ; token.setType(TokenType.COMMA) ;     return token ;
                case Chars.CH_LBRACE:     reader.readChar() ; token.setType(TokenType.LBRACE) ;    return token ;
                case Chars.CH_RBRACE:     reader.readChar() ; token.setType(TokenType.RBRACE) ;    return token ;
                case Chars.CH_LPAREN:     reader.readChar() ; token.setType(TokenType.LPAREN) ;    return token ;
                case Chars.CH_RPAREN:     reader.readChar() ; token.setType(TokenType.RPAREN) ;    return token ;
                case Chars.CH_LBRACKET:   reader.readChar() ; token.setType(TokenType.LBRACKET) ;  return token ;
                case Chars.CH_RBRACKET:   reader.readChar() ; token.setType(TokenType.RBRACKET) ;  return token ;
    
                // Some interesting characters
                case Chars.CH_COLON:      reader.readChar() ; token.setType(TokenType.COLON) ; return token ;
    //            case CH_UNDERSCORE: reader.readChar() ; token.setType(TokenType.UNDERSCORE) ; return token ;
                case Chars.CH_LT:         reader.readChar() ; token.setType(TokenType.LT) ; return token ;
                case Chars.CH_GT:         reader.readChar() ; token.setType(TokenType.GT) ; return token ;
                // GE, LE
            }
    ...
    private void skip()
    {
        ...
        for ( ;; )
        {
            ...
            if ( ch == Chars.CH_HASH )
            {
                ...
                {
                    ch = reader.peekChar() ;
                    if ( ch == Chars.EOF || isNewlineChar(ch) )
                        break ;
                    reader.readChar() ;
                }
            }
