...
/**
 * ADFContext is the object pushed onto an ADF stack which represents the
 * current context of an ADM or ADF function call, that is, how to get the
 * argument values that argument_terminals need to return.
 * 
 * <p>
 * <i>adf</i> contains the relevant ADF/ADM node. If it's an ADF function call,
 * then <i>arguments[]</i> contains the evaluated arguments to the ADF. If it's
 * an ADM function call, then <i>arguments[]</i> is set to false.
 * 
 * <p>
 * You set up the ADFContext object for a given ADF or ADM node with the
 * prepareADF(...) and prepareADM(...) functions.
 * 
 * <p>
 * To evaluate an argument number from an ADFContext, call evaluate(...), and
 * the results are evaluated and copied into input.
 * 
 * <p>
 ...
 * <td valign=top>(the ADFContext's basic GPData type)</td>
 * </tr>
 * </table>
 * 
 * @author Sean Luke
...
public class ADFContext implements Prototype {
    ...
    /**
     * An array of GPData nodes (none of the null, when it's used) holding an
     * ADF's arguments' return results
     */
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (state.parameters.exists(p, d)) {
            /*
             * arg_proto = (GPData) (state.parameters.getInstanceForParameter(
             * p,d,GPData.class)); arg_proto.setup(state,p);
             */
            state.output
                    .warning(
                            "ADF Data is deprecated -- this parameter is no longer used.  Instead, we directly use the GPData.",
                            p, d);
        }
        // else
        // snarf it from eval.problem.data, hacked up from the Problem's data
        // type,
    ...
    public void evaluate(final EvolutionState state, final int thread, final GPData input, final ADFStack stack,
            final GPIndividual individual, final Problem problem, final int argument) {
    ...
    /**
     * Increases arguments to accommodate space if necessary. Sets adf to a. You
     * need to then fill out the arguments yourself.
     */
