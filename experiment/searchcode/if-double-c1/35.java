...
/**
 * FFTReal computes the discrete Fourier coefficients a[0], ...., a[N/2] and
 * b[1], ...., b[N/2 - 1] of the discrete partial Fourier sum a[0] +
 * a[1]*cos(N/2*omega*x) + Sum (k=1,2,...,N/2-1) (a[2*k] * cos(k * omega * x) +
 * a[2*k+1] * sin(k * omega * x)) given real functional values y[0], ....,
 * y[N-1].
 * 
...
public class FFTReal {
	 ...
	 *            int
	 */
	private void shuffle(final double data[], final int sign) {
		int nh = n / 2;
		int nq = n / 4;
		if (n == 6)
			nq = 2;
	...
	/**
	 * Computes the Fast Fourier Transform of the data leaving the result in
	 * data.
	 * 
	 * The given array is returned after it has been transformed.
	 * 
