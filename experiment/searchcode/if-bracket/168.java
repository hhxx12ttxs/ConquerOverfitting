...
class PFormatter extends Formatter {
    ...
    private ConstantString pattern;
    ...
    public MaverickString format(MaverickString result, MaverickString status, ConstantString input) { 
        if (input.MATCH(pattern).equals(ConstantString.ONE)) {
            result.set(input);
        } else {
            result.clear();
        }
