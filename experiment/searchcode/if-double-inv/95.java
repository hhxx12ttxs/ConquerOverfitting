...
public class Transaction implements Runnable {
    ...
    public boolean buy(Player player, String[] args) {
        ...
        if (DynamicEconomy.useRegions) {
            ...
            if (DynamicEconomy.useRegionFlags) {
                ...
                String[] regionBannedItems = DynamicEconomy.regionConfig.getString(node + ".banned-purchase-items", "")
                        .split(",");
                ...
                if (DynamicEconomy.groupControl) {
                    List<String> allowedGroups = DynamicEconomy.regionConfig.getStringList(node
                            + ".allowed-purchase-groups");
                    boolean inRegion = Item.isItemInRegionGroup(allowedGroups, itemName);

                    if (inRegion == false) {
                        color.sendColouredMessage(player, DynamicEconomy.prefix + "&2You cannot buy &f" + itemName
                                + "&2 in this region.");
                        Utility.writeToLog(stringPlay + " attempted to buy " + itemName + " in region " + reg
                                + ", but it's not in any allowed item groups.");
                        return false;
                    }
                }
            }
        ...
        if (DynamicEconomy.groupControl) {
            if (Item.canBuy(player, itemName) == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2You are not permitted to purchase &f"
                        + itemName);
        ...
        if (args.length == 1) {
            ...
            if (args[1].equalsIgnoreCase("all")) {
                if (DynamicEconomy.usestock) {
                    ...
                    Utility.writeToLog(stringPlay + " attempted to buy 'all' of '" + itemName
                            + "', but stock is disabled.");
                }
            } else {
                try {
        ...
        if (DynamicEconomy.usePercentVelocity) {
            ...
            /*
             * for (int x = 0; x < purchaseAmount; x++) { totalCost +=
             * itemPrice; if (itemPrice == itemCeiling) { itemPrice =
             * itemCeiling; } else { itemPrice = itemPrice + (itemPrice *
             * itemVelocity); if (itemPrice > itemCeiling) { itemPrice =
             * itemCeiling; } } //System.out.println(x + ": Price - $" +
             * itemPrice +", totalCost - " + totalCost); }
             */
            ...
            for (int x = 0; x < purchaseAmount; x++) {
                ...
                // System.out.println(x + ": Price - $" + itemPrice
                // +", totalCost - " + totalCost);
            }
        }
        ...
        if (balance < totalCost) {
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Your balance: &f"
                    + DynamicEconomy.currencySymbol + decFormat.format(balance) + "   &2Your order total: &f"
                    + DynamicEconomy.currencySymbol + decFormat.format(totalCost));
            Utility.writeToLog(stringPlay + " attempted to buy " + decFormat.format(purchaseAmount) + " of '"
                    + itemName + "' for " + decFormat.format(totalCost) + " but could not afford it.");
            return false;
            ...
            if (itemStock < purchaseAmount) {
                ...
                color.sendColouredMessage(player,
                        DynamicEconomy.prefix + "&2Your request: &f" + decFormat.format(purchaseAmount)
                                + "   &2Current Stock: &f" + itemStock);
                Utility.writeToLog(stringPlay + " attempted to buy " + decFormat.format(purchaseAmount) + " of '"
                        + itemName + "' but there was only " + itemStock + " remaining");
                return false;
            }
        }
        ...
        color.sendColouredMessage(player, DynamicEconomy.prefix + "&fYou bought " + purchaseAmount + " &2of "
                + itemName + "&f + " + decFormat.format(percentTax) + "&2% tax = &f" + DynamicEconomy.currencySymbol
                + totalCost + " &2TOTAL");
        ...
        if (oldPrice != newPrice) {
            if (DynamicEconomy.globalNotify) {
                for (Player p : Bukkit.getServer().getOnlinePlayers()) {
                    if (!Utility.isQuiet(p)) {
                        color.sendColouredMessage(p, DynamicEconomy.prefix + "&2New Price of &f" + itemName
            ...
            Utility.writeToLog(DynamicEconomy.prefix + " New price of " + itemName + " changed dynamically to "
                    + newPrice + "(+" + change + ")");
        }
    ...
    public boolean sellInventory(Player player) {
        ...
        for (ItemStack item : contents) {
            ...
            if (dur == 0 || itemID >= 256 && itemID <= 258 || itemID >= 267 && itemID <= 279 || itemID >= 298
                    && itemID <= 317 || itemID >= 283 && itemID <= 286 || itemID >= 290 && itemID <= 294) {
            ...
            if (DynamicEconomy.groupControl) {
                if (Item.canSell(player, itemName) == false) {
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2You are not permitted to sell &f"
                            + itemName);
            ...
            if (DynamicEconomy.useRegions) {
                ...
                if (DynamicEconomy.useRegionFlags) {
                    ...
                    String[] regionBannedItems = DynamicEconomy.regionConfig.getString(node + ".banned-sale-items", "")
                            .split(",");
                    ...
                    if (DynamicEconomy.groupControl) {
                        ...
                        if (inRegion == false) {
                            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2You cannot sell &f" + itemName
                                    + "&2 in this region.");
                            Utility.writeToLog(player.getName() + " attempted to sell " + itemName + " in region "
                                    + reg + ", but it's not in any allowed item groups.");
                            continue Inv;
                        }
                    }
                }
            ...
            if (itemID >= 256 && itemID <= 258 || itemID >= 267 && itemID <= 279 || itemID >= 298 && itemID <= 317
                    || itemID >= 283 && itemID <= 286 || itemID >= 290 && itemID <= 294) {
        ...
        color.sendColouredMessage(player,
                DynamicEconomy.prefix + "&fYou sold&f your inventory &2 - " + decFormat.format(percentTax)
                        + "&2% tax = &f" + DynamicEconomy.currencySymbol + totalSale + " &2TOTAL");
    ...
    public boolean sell(Player player, String[] args) {
            ...
            if (DynamicEconomy.useRegionFlags == true) {
                ...
                if (DynamicEconomy.groupControl) {
                    ...
                    if (inRegion == false) {
                        color.sendColouredMessage(player, DynamicEconomy.prefix + "&2You cannot sell &f" + itemName
                                + "&2 in this region.");
                        Utility.writeToLog(stringPlay + " attempted to sell " + itemName + " in region " + reg
                                + ", but it's not in any allowed item groups.");
                        return false;
                    }
                }
        ...
        if (saleAmount > userAmount && !isAll) {
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2You do not have &f" + saleAmount + " "
                    + itemName);
            Utility.writeToLog(stringPlay + " attempted to sell " + saleAmount + " of " + itemName
                    + ", but didn't have that many.");
            return false;
        }
        ...
        if (itemID >= 256 && itemID <= 258 || itemID >= 267 && itemID <= 279 || itemID >= 298 && itemID <= 317
            ...
            for (int x = 0; x < saleAmount; x++) {
                ...
                // color.sendColouredMessage(player, DynamicEconomy.prefix +
                // "&fYou sold &2" + saleAmount + "&f of &2" + itemName +
                // "&f - " + percentTax + "&2% tax = &f$" + totalSale +
                // " &2TOTAL" );
                Utility.writeToLog(stringPlay + " sold a '" + itemName + "' at " + percentDur + "% durability for "
                        + indivsale);
            }
            percentTax = tax * 100;
            ...
            totalSale = Double.valueOf(decFormat.format(totalSale));
            color.sendColouredMessage(player,
                    DynamicEconomy.prefix + "&2TOTAL SALE (with " + decFormat.format(percentTax) + "% tax): &f"
                            + DynamicEconomy.currencySymbol + totalSale);
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&fYou sold &2" + saleAmount + "&f of &2"
                    + itemName + "&f - " + decFormat.format(percentTax) + "&2% tax = &f"
                    + DynamicEconomy.currencySymbol + totalSale + " &2TOTAL");
            Utility.writeToLog(stringPlay + " succesfully sold " + saleAmount + " of '" + itemName + "' for "
                    + totalSale);
        }
    ...
    public void addStock(Player player, String[] args) {
        ...
        if (args.length < 2 || args.length > 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/addStock [Item] [AdditionalStock]");
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Previous Stock: &f" + currentStock
                    + "&2 | New Stock: &f" + newStock);
            Utility.writeToLog(stringPlay + " added " + addStock + " stock of " + item + " for a new stock total of "
                    + newStock);
    ...
    public void removeStock(Player player, String[] args) {
        ...
        if (args.length < 2 || args.length > 2) {
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Previous Stock: &f" + currentStock
                    + "&2 | New Stock: &f" + newStock);
            Utility.writeToLog(stringPlay + " removed " + removeStock + " stock of " + item
                    + " for a new stock total of " + newStock);
            dataSigns.checkForUpdates(item, newStock - currentStock, 0);
    ...
    public void curTaxes(Player player, String[] args) {
        ...
        if (args.length > 0) {
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&fSales Tax: &2" + DynamicEconomy.salestax * 100
                    + "%");
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&fPurchase Tax: &2" + DynamicEconomy.purchasetax
                    * 100 + "%");
    ...
    public void setTaxes(Player player, String[] args) {
        ...
        if (args.length != 3) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/settax [region] [sale|purchase] [amount]");
            ...
            if (args[1].equalsIgnoreCase("sale") == false && args[1].equalsIgnoreCase("purchase") == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix
                        + "&2This is an invalid tax name. Use either &fsale or &fpurchase");
            ...
            if (region.equalsIgnoreCase("GLOBAL")) {
                ...
                if (DynamicEconomy.regionConfig.contains("regions." + region) == false) {
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Region &f" + region
                            + "&2 doesn't exist.");
                    Utility.writeToLog(stringPlay + " tried to set tax of region '" + region
                            + "', which doesn't exist.");
                    return;
                }
            ...
            decFormat.applyPattern("###.###");
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2" + args[0] + " " + args[1] + "tax set to &f"
                    + decFormat.format(tax * 100) + "%");
    ...
    public void run() {
        ...
        // itemConfig.getConfigurationSection("STONE").getParent();
                    ...
                    if (DynamicEconomy.globalNotify) {
                        for (Player p : Bukkit.getServer().getOnlinePlayers()) {
                            if (!Utility.isQuiet(p)) {
                                ...
                                color.sendColouredMessage(p, DynamicEconomy.prefix + "&2New Price of &f" + items[x]
                                        + "&2 is &f" + DynamicEconomy.currencySymbol + price + "&2 ( +"
                                        + DynamicEconomy.overTimePriceInflationPercent * 100 + "% )");
                            }
                        }
                    }
