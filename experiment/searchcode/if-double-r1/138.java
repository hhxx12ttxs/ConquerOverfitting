...
public class Inspector{
   ...
   * was there inexact comparison done when it was not allowed?
  ...
  /**
   * Set up the parameters for inexact test: report failure if tolerance 
   * is set below 0
   * @param tolerance the desired tolerance for the inexact tests
   * @return
   */
  ...
  /**
   * Set up the parameters for exact test: report failure if inexact comparisons
   * are made
   * @return true
   */
  ...
  * 
  * @param obj1 an <code>Iterable</code> object
  * @param obj2 an <code>Iterable</code> object
  ...
  */
   ...
   * 
   * @param obj1 a <code>Set</code> object
   * @param obj2 a <code>Set</code> object
  ...
  * 
  * @param obj1 a <code>Traversal</code> object
  * @param obj2 a <code>Traversal</code> object
  ...
  */
  ...
  /**
   * <P>Invoked by <CODE>{@link isSame isSame}</CODE> method after the 
   * hashmap that records seen objects has been cleared. </P>
   * 
   * <P>Compare the two given objects for extensional equality.<P>
   * <P>Consider inexact numbers (types <code>double</code>, 
   * <code>float</code>, <code>Double</code>, <code>Float</code>) 
   ...
   * <P>Use the <code>String equals</code> method to compare two
   * objects of the type <code>String</code>.
   * <P>Compare two objects in the same class that implements the <code>Set</code>
   * interface by their size and by matching the elements using the 
   ...
   * throw a given <code>Exception</code> with the given message.</P>
   * 
   * @param obj1 typically the actual value
   * @param obj2 typically the expected value
   * @return true if the two given object are the same
   */
  ...
  private <T> boolean isSamePrivate(T obj1, T obj2){
    ...
    if (sameClass){
      ...
      Integer i2match = this.hashmap.get(i1*i2);
     
      if ((i2match != null) && (i2match.equals(i1 + i1*i2)))
        return true;
    
      //System.out.println("Hash codes -- i1: " + i1 + "  i2: " + i2);
      this.hashmap.put(i1*i2, i1 + i1*i2);
      ...
      /** handle the Set objects in the Java Collection library
       * by comparing their size, and data items 
       * -- here we use the equals method when comparing the set elements */
      if ((obj1 instanceof Set) && (obj2 instanceof Set)
          && obj1.getClass().getName().startsWith("java.util"))
        return isSameSet((Set)obj1, (Set)obj2); 
      
      /** handle Iterable objects in the Java Collection library 
       * by comparing their size, then comparing the elements pairwise */
      if ((obj1 instanceof Iterable) && (obj2 instanceof Iterable)
      	   && obj1.getClass().getName().startsWith("java.util"))
        return isSameIterablePrivate((Iterable<T>)obj1, (Iterable<T>)obj2);     
      
      /** handle the Map objects in the Java Collection library 
      ...
      try{
        for ( ; i < Array.getLength(r1.sampleDeclaredFields); i++){
          sameValues = sameValues && 
            isSamePrivate(r1.sampleDeclaredFields[i].get(obj1),
   ...
   * Measure absolute tolerance, if one of the numbers is exact zero.
  ...
  protected boolean isSameDouble(double d1, double d2){
    if (d1 - d2 == 0.0)
      return true;
    	...
    	else   // d1, d2 are non-zero
      //return (Math.abs(d1 - d2) / (Math.max (Math.abs(d1), Math.abs(d2))))
      return Math.abs(d1 - d2) / (Math.abs ((d1 + d2)/2))
              < TOLERANCE;   	
    }
  }
   ...
   * data elements in the same order.
   * @param obj1 the first <code>Traversal</code> dataset
   * @param obj2 the second <code>Traversal</code> dataset
   * @return true is the two datasets are extensionally equal 
   */
   ...
   * @param obj1 the first <code>Set</code> dataset
   * @param obj2 the second <code>Set</code> dataset
  ...
  protected <T> boolean isSameSetPrivate(Set<T> obj1, Set<T> obj2){
    ...
    for (T item1 : obj1){
      ...
      // make sure each item of the first set matches an item in the second set
      for (T item2 : obj2){
        // now compare the corresponding values using 'equals' as required
