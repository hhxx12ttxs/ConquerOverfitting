...
public class PrintfFormat {
  ...
  private class ConversionSpecification {
     ...
     * double.
     * @param x the double value to be formatted.
     * @param eChar the char to be formatted.
     */
  ...
  */
  public static void main(String[] args) { 
      if (0 == args.length) { 
          System.out.println("\nUsage: com.stc.jcs.PrintfFormat <format string> <data ...>");
          System.exit(0);
      }
      
      PrintfFormat fmt = new PrintfFormat(args[0]);
      int nDatum = args.length - 1;
      if (nDatum > 0) { 
          Object[] data = new Object[nDatum];
          for (int i = 1; i < args.length; i++) { 
              boolean notcast = true;
              // Try interpretting as a float
              if (notcast && args[i].indexOf('.') != -1) { 
                  try { 
                      data[i - 1] = (Object) new Float(args[i]);
                      notcast = false;
              ...
              if (notcast) { 
                  try { 
                      data[i - 1] = (Object) new Integer(args[i]);
                      notcast = false;
                  } catch (NumberFormatException ex) { 
                      ex.printStackTrace();
                  }
              ...
              if (notcast) { 
                  data[i - 1] = (Object) args[i];
              }
          }
          System.out.println("\"" + args[0] + "\" = " + fmt.sprintf(data));
      }
  }
}
