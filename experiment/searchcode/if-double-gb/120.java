...
public class Common {
	...
	public static void extract(String... names) {
		for (String name : names) {
			...
			try {
				...
				System.out.println("[iConomy] Default setup file written: "
						+ name);
			} catch (Exception e) {
			} finally {
				try {
	...
	public static String formatted(String amount, List<String> maj,
			List<String> min) {
		...
		if (Constants.Nodes.AllowMinor.getBoolean()) {
			...
			if (Constants.Nodes.isSplit.getBoolean()) {
				...
				try {
					if (Integer.valueOf(fpieces[1]) != 0
							&& Integer.valueOf(fpieces[0]) != 0)
						formatted = pieces[0] + " " + major + ", " + pieces[1]
								+ " " + minor;
					...
					if (Long.valueOf(fpieces[1]) != 0
							&& Long.valueOf(fpieces[0]) != 0)
						formatted = pieces[0] + " " + major + ", " + pieces[1]
								+ " " + minor;
	...
	public static String readableProfile(long time) {
		int i = 0;
		String[] units = new String[] { "ms", "s", "m", "hr", "day", "week",
				"mnth", "yr" };
		int[] metric = new int[] { 1000, 60, 60, 24, 7, 30, 12 };
		long current = TimeUnit.MILLISECONDS
				.convert(time, TimeUnit.NANOSECONDS);
