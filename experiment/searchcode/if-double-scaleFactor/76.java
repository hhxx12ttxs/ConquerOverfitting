...
;
  ...
  public static void writeHttpRequest(OutputStream stream, String method, String host, String urlPath, Dictionary headers, String content) {
    ...
    if (content != null) {
      ...
      headers.insertAt(StringWrapper.wrapString("Content-Length"), StringWrapper.wrapString(Native.integerToString(((long)(content.length())))));
    }
    if (headers != null) {
        ...
        if (!Stella.stringEqlP(userInfo, "")) {
          ...
          { InputStream in = null;
            ...
            { Object [] caller_MV_returnarray = new Object[1];
              ...
              in = Stella.openNetworkStream(host, ((!Stella.stringEqlP(port, "")) ? ((int)(Native.stringToInteger(port))) : 80), caller_MV_returnarray);
              out = ((OutputStream)(caller_MV_returnarray[0]));
            }
  ...
  public static Stella_Object withInputUrl(Cons binding, Cons body) {
    { Symbol var = ((Symbol)(binding.value));
      ...
      return (Cons.list$(Cons.cons(Utilities.SYM_STELLA_LET, Cons.cons(Cons.cons(Cons.cons(var, Cons.list$(Cons.cons(Utilities.SYM_STELLA_INPUT_STREAM, Cons.cons(Utilities.SYM_STELLA_NULL, Cons.cons(Stella.NIL, Stella.NIL))))), Stella.NIL), Cons.cons(Cons.list$(Cons.cons(Utilities.SYM_STELLA_UNWIND_PROTECT, Cons.cons(Cons.list$(Cons.cons(Utilities.SYM_STELLA_PROGN, Cons.cons(Cons.list$(Cons.cons(Utilities.SYM_STELLA_SETQ, Cons.cons(var, Cons.cons(Cons.cons(Cons.list$(Cons.cons(Utilities.SYM_UTILITIES_OPEN_URL_STREAM, Cons.cons(binding.rest.value, Cons.cons(Stella.NIL, Stella.NIL)))), Stella.NIL), Stella.NIL)))), Cons.cons(body.concatenate(Stella.NIL, Stella.NIL), Stella.NIL)))), Cons.cons(Cons.list$(Cons.cons(Utilities.SYM_STELLA_WHEN, Cons.cons(Cons.list$(Cons.cons(Utilities.SYM_STELLA_DEFINEDp, Cons.cons(var, Cons.cons(Stella.NIL, Stella.NIL)))), Cons.cons(Cons.list$(Cons.cons(Utilities.SYM_STELLA_FREE, Cons.cons(var, Cons.cons(Stella.NIL, Stella.NIL)))), Cons.cons(Stella.NIL, Stella.NIL))))), Cons.cons(Stella.NIL, Stella.NIL))))), Cons.cons(Stella.NIL, Stella.NIL))))));
    }
  }
