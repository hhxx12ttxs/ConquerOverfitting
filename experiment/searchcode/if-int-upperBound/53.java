 ...
 * 
 * <p>
 * CompetitiveEvaluator is a Evaluator which performs <i>competitive fitness
 * evaluations</i>. Competitive fitness is where individuals' fitness is
 * determined by testing them against other members of the same subpopulation.
 * Competitive fitness topologies differ from co-evolution topologies in that
 * co-evolution is a term I generally reserve for multiple sbupopulations which
 * breed separately but compete against other subpopulations during evaluation
 * time. Individuals are evaluated regardless of whether or not they've been
 * evaluated in the past.
 * 
 * <p>
 * Your Problem is responsible for setting up the fitness appropriately.
 * CompetitiveEvaluator expects to use Problems which adhere to the
 * GroupedProblemForm interface, which defines a new evaluate(...) function,
 * plus a preprocess(...) and postprocess(...) function.
 * 
 * <p>
 * This competitive fitness evaluator is single-threaded -- maybe we'll hack in
 * multithreading later. And it only has two individuals competing during any
 * fitness evaluation. The order of individuals in the subpopulation will be
 * changed during the evaluation process. There are seven evaluation topologies
 * presently supported:
 * 
 * <p>
 * <dl>
 * <dt><b>Single Elimination Tournament</b>
 * <dd>
 * All members of the population are paired up and evaluated. In each pair, the
 * "winner" is the individual which winds up with the superior fitness. If
 * neither fitness is superior, then the "winner" is picked at random. Then all
 * the winners are paired up and evaluated, and so on, just like in a single
 * elimination tournament. It is important that the <b>population size be a
 * <i>power of two</i></b>, else some individuals will not have the same number
 * of "wins" as others and may lose the tournament as a result.
 * 
 * <dt><b>Round Robin</b>
 * <dd>
 * Every member of the population are paired up and evaluated with all other
 * members of the population, not not including the member itself (we might add
 * in self-play as a future later if people ask for it, it's easy to hack in).
 * 
 * <dt><b>K-Random-Opponents-One-Way</b>
 * <dd>
 * Each individual's fitness is calculated based on K competitions against
 * random opponents. For details, see
 * "A Comparison of Two Competitive Fitness Functions" by Liviu Panait and Sean
 * Luke in the Proceedings of GECCO 2002.
 * 
 * <dt><b>K-Random-Opponents-Two-Way</b>
 * <dd>
 * Each individual's fitness is calculated based on K competitions against
 * random opponents. The advantage of this method over
 * <b>K-Random-Opponents-One-Way</b> is a reduced number of competitions (when I
 * competes against J, both I's and J's fitnesses are updated, while in the
 * previous method only one of the individuals has its fitness updated). For
 * details, see "A Comparison of Two Competitive Fitness Functions" by Liviu
 * Panait and Sean Luke in the Proceedings of GECCO 2002.
 * </dl>
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 * <tr>
 * <td valign=top><i>base.</i><tt>style</tt><br>
 ...
 * <i>rand-1-way</i> (K-Random-Opponents, each game counts for only one of the
 * players)<br>
 ...
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>group-size</tt><br>
 * <font size=-1> int</font></td>
 * <td valign=top>(how many individuals per group, used in rand-1-way</i> and
 * <i>rand-2-way</i> tournaments)<br>
 * <i>group-size</i> &gt;= 1 for <i>rand-1-way</i> or <i>rand-2-way</i><br>
 * </td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>over-eval</tt><br>
 * <font size=-1> bool = <tt>true</tt> or <tt>false</tt> (default)</font></td>
 * <td valign=top>(if the tournament style leads to an individual playing more
 * games than others (as can be the case for rand-2-way), should the extra games
 * be used for his fitness evaluatiuon?)</td>
 * </tr>
 * 
 * </table>
 * 
...
public class CompetitiveEvaluator extends Evaluator {
    ...
    /**
     * An evaluator that performs coevolutionary evaluation. Like
     * SimpleEvaluator, it applies evolution pipelines, one per thread, to
     * various subchunks of a new population.
     */
        ...
        for (int y = 0; y < state.evalthreads; y++) {
            ...
            if (y < state.evalthreads - 1) // not last one
                ...
                numinds[y] = state.population.subpops[0].individuals.length
                        ...
                        (state.population.subpops[0].individuals.length - (state.population.subpops[0].individuals.length / state.evalthreads)
                                * state.evalthreads);
            // figure from
    ...
    public void evalSingleElimination(final EvolutionState state, final Individual[] individuals, final int subpop,
            final GroupedProblemForm prob) {

        // for a single-elimination tournament, the subpop[0] size must be 2^n
        // for
        ...
        // the bottom half will be winners. Then we cut our array in half and
        // repeat.
        while (len > 1) {
            ...
            for (int x = 0; x < len / 2; x++) {
                // if the second individual is better, than we switch them
                // around
    ...
    public void evalRoundRobin(final EvolutionState state, int[] from, int[] numinds, final Individual[] individuals,
            int subpop, final GroupedProblemForm prob) {
     ...
     * @param prob
     */
    public void evalRoundRobinPopChunk(final EvolutionState state, int from, int numinds, int threadnum,
            final Individual[] individuals, int subpop, final GroupedProblemForm prob) {
        Individual[] competition = new Individual[2];
        int[] subpops = new int[] { subpop, subpop };
        boolean[] updates = new boolean[2];
        updates[0] = updates[1] = true;
    ...
    public void evalNRandomOneWay(final EvolutionState state, int[] from, int[] numinds,
            final Individual[] individuals, int subpop, final GroupedProblemForm prob) {
        if (state.evalthreads == 1)
    ...
    public void evalNRandomOneWayPopChunk(final EvolutionState state, int from, int numinds, int threadnum,
            final Individual[] individuals, final int subpop, final GroupedProblemForm prob) {
        Individual[] queue = new Individual[individuals.length];
        int len = queue.length;
        System.arraycopy(individuals, 0, queue, 0, len);
    ...
    public void evalNRandomTwoWay(final EvolutionState state, int[] from, int[] numinds,
            final Individual[] individuals, int subpop, final GroupedProblemForm prob) {
        if (state.evalthreads == 1)
    ...
    public void evalNRandomTwoWayPopChunk(final EvolutionState state, int from, int numinds, int threadnum,
            final Individual[] individuals, final int subpop, final GroupedProblemForm prob) {
        ...
        for (int x = from; x < upperBound; x++) {
            ...
            // not for the opponents' (unless allowOverEvaluations is set to
            // true)

            // if true, it means that he has to play against all opponents with
            // greater index
            ...
            } else // here he has to play against a selection of the opponents
                   // with greater index
            {
                // we can use the queue structure because we'll just rearrange
                // the indexes
                // but we should make sure we also rearrange the other vectors
                ...
                for (int y = 0; groupSize > queue[x].nOpponentsMet; y++) {
                    ...
                    // swap the players (such that a player will not be
                    // considered twice)
            ...
            // if true, it means that the current player needs to play some
            // games with other players with lower indexes.
            // this is an unfortunate situation, since all those players have
            // already had their groupSize games for the evaluation
            if (queue[x].nOpponentsMet < groupSize) {
                for (int y = queue[x].nOpponentsMet; y < groupSize; y++) {
                    // select a random opponent with smaller index (don't even
                    // care for duplicates)
                    int index;
                    if (x > 0) // if x is 0, then there are no players with
                               // smaller index, therefore pick a random one
...
// used by the K-Random-Opponents-One-Way and K-Random-Opponents-Two-Ways
// evaluations
...
// used by the Single-Elimination-Tournament, (Double-Elimination-Tournament and
// World-Cup) evaluations
