 ...
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.reevalate-elites.<i>i</i></tt><br>
...
public class SimpleBreeder extends Breeder {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        int size = state.parameters.getInt(p, null, 1); // if size is wrong,
                                                        // we'll let Population
                                                        // complain about it --
                                                        // for us, we'll just
                                                        // make 0-sized arrays
                                                        // and drop out.
        ...
        for (int x = 0; x < size; x++) {
            ...
            if (elite[x] < 0)
                state.output.error("The number of elites for subpopulation " + x + " must be >= 0", base.push(P_ELITE)
                        .push("" + x));
    ...
    /**
     * Elites are often stored in the top part of the subpopulation; this
     * function returns what part of the subpopulation contains individuals to
     * replace with newly-bred ones (up to but not including the elites).
     */
    ...
    protected void breedPopChunk(Population newpop, EvolutionState state, int[] numinds, int[] from, int threadnum) {
        // System.out.println("Breeding: " + numinds[0] + " Starting at: " +
        // from[0]);
        for (int subpop = 0; subpop < newpop.subpops.length; subpop++) {
            ...
            if (!bp.produces(state, newpop, subpop, threadnum))
                state.output.fatal("The Breeding Pipeline of subpopulation " + subpop
                        ...
                        + newpop.subpops[subpop].species.getClass().getName() + " or fitness "
    ...
    protected void loadElites(EvolutionState state, Population newpop) {
        ...
        for (int x = 0; x < state.population.subpops.length; x++)
            if (elite[x] > state.population.subpops[x].individuals.length)
                state.output.error("The number of elites for subpopulation " + x
                        + " exceeds the actual size of the subpopulation", new Parameter(EvolutionState.P_BREEDER)
                        .push(P_ELITE).push("" + x));
        state.output.exitIfErrors();
        ...
        for (int sub = 0; sub < state.population.subpops.length; sub++)
            // if the number of elites is 1, then we handle this by just finding
            // the best one.
            if (elite[sub] == 1) {
                int best = 0;
