...
public class LZMAEncoder
{
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Init() { SevenZip.RangeCoderEncoder.InitBitModels(m_Encoders); }



			public void Encode(SevenZip.RangeCoderEncoder rangeEncoder, byte symbol) throws IOException
			{
				int context = 1;
				for (int i = 7; i >= 0; i--)
				{
			...
			public void EncodeMatched(SevenZip.RangeCoderEncoder rangeEncoder, byte matchByte, byte symbol) throws IOException
			{
				int context = 1;
				boolean same = true;
				for (int i = 7; i >= 0; i--)
			...
			public int GetPrice(boolean matchMode, byte matchByte, byte symbol)
			{
				...
				if (matchMode)
				{
					for (; i >= 0; i--)
					{
						int matchBit = (matchByte >> i) & 1;
						int bit = (symbol >> i) & 1;
						price += SevenZip.RangeCoderEncoder.GetPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
						context = (context << 1) | bit;
						if (matchBit != bit)
						{
							i--;
				...
				{
					int bit = (symbol >> i) & 1;
					price += SevenZip.RangeCoderEncoder.GetPrice(m_Encoders[context], bit);
					context = (context << 1) | bit;
				}
				return price;
			}
	...
	class LenEncoder
	{
		...
		{
			SevenZip.RangeCoderEncoder.InitBitModels(_choice);
		...
		public void Encode(SevenZip.RangeCoderEncoder rangeEncoder, int symbol, int posState) throws IOException
		{
		...
		{
			int a0 = SevenZip.RangeCoderEncoder.GetPrice0(_choice[0]);
			int a1 = SevenZip.RangeCoderEncoder.GetPrice1(_choice[0]);
			int b0 = a1 + SevenZip.RangeCoderEncoder.GetPrice0(_choice[1]);
			int b1 = a1 + SevenZip.RangeCoderEncoder.GetPrice1(_choice[1]);
			int i = 0;
	...
	SevenZip.BinTree _matchFinder = null;
	SevenZip.RangeCoderEncoder _rangeEncoder = new SevenZip.RangeCoderEncoder();
	...
	void Create()
	{
		...
		{
			SevenZip.BinTree bt = new SevenZip.BinTree();
			int numHashBytes = 4;
	...
	void Init()
	{
		...
		SevenZip.RangeCoderEncoder.InitBitModels(_isMatch);
		SevenZip.RangeCoderEncoder.InitBitModels(_isRep0Long);
		SevenZip.RangeCoderEncoder.InitBitModels(_isRep);
		SevenZip.RangeCoderEncoder.InitBitModels(_isRepG0);
		SevenZip.RangeCoderEncoder.InitBitModels(_isRepG1);
		SevenZip.RangeCoderEncoder.InitBitModels(_isRepG2);
		SevenZip.RangeCoderEncoder.InitBitModels(_posEncoders);
	...
	{
		return SevenZip.RangeCoderEncoder.GetPrice0(_isRepG0[state]) +
				SevenZip.RangeCoderEncoder.GetPrice0(_isRep0Long[(state << LZMABase.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetPureRepPrice(int repIndex, int state, int posState)
	{
		...
		if (repIndex == 0)
		{
			price = SevenZip.RangeCoderEncoder.GetPrice0(_isRepG0[state]);
			price += SevenZip.RangeCoderEncoder.GetPrice1(_isRep0Long[(state << LZMABase.kNumPosStatesBitsMax) + posState]);
		}
		else
		{
			price = SevenZip.RangeCoderEncoder.GetPrice1(_isRepG0[state]);
			if (repIndex == 1)
				price += SevenZip.RangeCoderEncoder.GetPrice0(_isRepG1[state]);
			else
			{
				price += SevenZip.RangeCoderEncoder.GetPrice1(_isRepG1[state]);
				price += SevenZip.RangeCoderEncoder.GetPrice(_isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	int GetOptimum(int position) throws IOException
	{
		...
		_optimum[1].Price = SevenZip.RangeCoderEncoder.GetPrice0(_isMatch[(_state << LZMABase.kNumPosStatesBitsMax) + posState]) +
				_literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!LZMABase.StateIsCharState(_state), matchByte, currentByte);
		_optimum[1].MakeAsChar();

		int matchPrice = SevenZip.RangeCoderEncoder.GetPrice1(_isMatch[(_state << LZMABase.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + SevenZip.RangeCoderEncoder.GetPrice1(_isRep[_state]);
		...
		int normalMatchPrice = matchPrice + SevenZip.RangeCoderEncoder.GetPrice0(_isRep[_state]);
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice +
				SevenZip.RangeCoderEncoder.GetPrice0(_isMatch[(state << LZMABase.kNumPosStatesBitsMax) + posState]) +
			...
			matchPrice = curPrice + SevenZip.RangeCoderEncoder.GetPrice1(_isMatch[(state << LZMABase.kNumPosStatesBitsMax) + posState]);
			repMatchPrice = matchPrice + SevenZip.RangeCoderEncoder.GetPrice1(_isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price +
						SevenZip.RangeCoderEncoder.GetPrice1(_isMatch[(state2 << LZMABase.kNumPosStatesBitsMax) + posStateNext]) +
						SevenZip.RangeCoderEncoder.GetPrice1(_isRep[state2]);
					{
				...
				if (lenTest < numAvailableBytesFull)
				{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice =
								...
								SevenZip.RangeCoderEncoder.GetPrice0(_isMatch[(state2 << LZMABase.kNumPosStatesBitsMax) + posStateNext]) +
						...
						int nextMatchPrice = curAndLenCharPrice + SevenZip.RangeCoderEncoder.GetPrice1(_isMatch[(state2 << LZMABase.kNumPosStatesBitsMax) + posStateNext]);
						int nextRepMatchPrice = nextMatchPrice + SevenZip.RangeCoderEncoder.GetPrice1(_isRep[state2]);
			...
			if (newLen >= startLen)
			{
				normalMatchPrice = matchPrice + SevenZip.RangeCoderEncoder.GetPrice0(_isRep[state]);
	...
	void FillDistancesPrices()
	{
		...
		for (int lenToPosState = 0; lenToPosState < LZMABase.kNumLenToPosStates; lenToPosState++)
		{
			...
			for (posSlot = LZMABase.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
				_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - LZMABase.kNumAlignBits) << SevenZip.RangeCoderEncoder.kNumBitPriceShiftBits);
