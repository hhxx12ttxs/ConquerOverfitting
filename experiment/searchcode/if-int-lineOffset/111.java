...
/* CVS MESSAGES:
 *
 * $Id: VRTFormatUtils.java 9045 2006-11-28 11:49:25Z nacho $
 * $Log$
 ...
 * *** empty log message ***
 *
 * Revision 1.1  2006/07/31 10:43:25  jorpiell
 * Algunos m?todos necesarios que calculan valores para construir el fichero VRT
 *
 *
 */
/**
 * This class has some utils to create a VRT file.
 * It has methods to obtaint the VRT parameters that
...
public class VRTFormatUtils {
	 ...
	 * Image height
	 * @return
	 * ImageOffSet parameter value
	 */
	public static String getImageOffset(int dataSize,
			int headerSize,
			String interLeaving,
			int band,
			int numBands,
		...
		}else if (interLeaving.equals(VRTFormatOptions.INTERLEAVING_BAND)){
			imageOffSet = width * height;
			
		}else if (interLeaving.equals(VRTFormatOptions.INTERLEAVING_LINE)){
	...
	public static String getImageOffset(VRTFileCreator fileCreator,int band){
		return getImageOffset(fileCreator.getDataSize(),
				fileCreator.getHeaderSize(),
				fileCreator.getInterleaving(),
				band,
				fileCreator.getBands(),
				fileCreator.getImageWidth(),
				fileCreator.getImageHeight());
	}
	 ...
	 * ImageOffSet parameter value
	 */
	public static String getLineOffset(int dataSize,
			String interLeaving,
			int numBands,
			int width){
	...
	public static String getLineOffset(VRTFileCreator fileCreator){
		return getLineOffset(fileCreator.getDataSize(),
				fileCreator.getInterleaving(),
				fileCreator.getBands(),
				fileCreator.getImageWidth());
	}
	 ...
	 * ImageOffSet parameter value
	 */
	public static String getPixelOffset(int dataSize,
			String interLeaving,
			int numBands){
	...
	public static String getPixelOffset(VRTFileCreator fileCreator){
		return getPixelOffset(fileCreator.getDataSize(),
				fileCreator.getInterleaving(),
				fileCreator.getBands());
	}
}
