...
import au.com.angry.geom.Point;
...
public class Fortune
{
    ...
    public Fortune(double minDistanceBetweenSites)
    {
        siteidx = 0;
        sites = null;
    ...
    /**
     * @param points Array of points for each site
     ...
     * @return a list of triangles making op the voronoi graph
     */
    public List<Triangle> generateVoronoi(Point[] points,
                                          double minX, double maxX, double minY, double maxY)
    {
        ...
        {
            triangles.add(new Triangle(e.site1.coord, e.site2.coord, new Point(e.x1, e.y1), new Point(e.x2, e.y2)));
            triangles.add(new Triangle(e.site2.coord, e.site1.coord, new Point(e.x2, e.y2), new Point(e.x1, e.y1)));
        }
...
//        double[] xValues = new double[nsites];
//        double[] yValues = new double[nsites];
...
//        }
    ...
    private void sortNode(final Point[] sites)
    {
        int i;
        this.sites = new Site[nsites];
        xmin = sites[0].x;
        ymin = sites[0].y;
        double xmax = xmin;
        double ymax = ymin;
        for (i = 0; i < nsites; i++)
        {
            this.sites[i] = new Site(sites[i], i);
            xmin = Math.min(xmin, sites[i].x);
            xmax = Math.max(xmax, sites[i].x);
            ymin = Math.min(ymin, sites[i].y);
            ymax = Math.max(ymax, sites[i].y);
        }
        qsort(this.sites);
        deltay = ymax - ymin;
        deltax = xmax - xmin;
    ...
    private Point PQ_min()
    {
        while (PQhash[PQmin].PQnext == null)
            PQmin += 1;
        return new Point(PQhash[PQmin].PQnext.vertex.coord.x, PQhash[PQmin].PQnext.ystar);
    }
    ...
    private Site intersect(Halfedge el1, Halfedge el2)
    {
        ...
        return new Site(new Point(xint, yint), 0);
    }
