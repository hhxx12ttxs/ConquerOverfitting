...
holesForShells.add(new ArrayList());
	...
	public static FShape jts_to_java2d(Geometry geo) {
		...
		try {
			if (geo instanceof Point) {
				shpNew = new FPoint2D(null, null, ((Point) geo).getX(), ((Point) geo).getY());
			}
			...
			if (geo instanceof Polygon) {
				shpNew = new FPolygon2D(null, null, toShape((Polygon) geo));
			}

			if (geo instanceof MultiPolygon) {
				shpNew = new FPolygon2D(null, null, toShape((MultiPolygon) geo));
			}

			if (geo instanceof LineString) {
				shpNew = new FPolyline2D(null, null, toShape((LineString) geo));
			}

			if (geo instanceof MultiLineString) {
				shpNew = new FPolyline2D(null, null, toShape((MultiLineString) geo));
			}
	...
	public static FPoint2D coordinate2FPoint2D(Coordinate coord) {
		return new FPoint2D(null, null, coord.x, coord.y); //,coord.z);
	}
    ...
    public static FShape transformToInts(IGeometry gp, AffineTransform at) {
        ...
        switch (gp.getGeometryType())
        {
            ...
            case FShape.POINT + FShape.Z:
                shp = new FPoint2D(null, null, ptDst.getX(), ptDst.getY());
                break;

            case FShape.LINE:
            case FShape.LINE + FShape.Z:
            case FShape.ARC:
            	shp = new FPolyline2D(null, null, newGp);
                break;
            ...
            case FShape.ELLIPSE:
                ...
                shp = new FPolygon2D(null, null, newGp);
                break;
        }
        return shp;
    }
