   ...
   public void doParse(Parser parser, ParserContext context) {
      ...
      if (tok == ParserContext.OPCHARS &&
          ...
          if (tok != ParserContext.IDENTIFIER)
             throw new ParseException("Expected the summation variable as the first argument of " + name + ".", context);
          String varName = context.tokenString;
          tok = context.next();
          if (tok != ParserContext.OPCHARS || ! context.tokenString.equals(","))
             throw new ParseException("Exprected a comma after the index variable, " + varName +".", context);
          parser.parseExpression(context);
          tok = context.next();
          if (tok != ParserContext.OPCHARS || ! context.tokenString.equals(","))
             throw new ParseException("Exprected a comma after the lower limit expression for " + name + ".", context);
          parser.parseExpression(context);
          tok = context.next();
          if (tok != ParserContext.OPCHARS || ! context.tokenString.equals(","))
             throw new ParseException("Exprected a comma after the upper limit expression for " + name + ".", context);
          ...
          if (tok != ParserContext.OPCHARS || ! context.tokenString.equals(close))
             throw new ParseException("Expected a \"" + close + "\" at the end of the paramter list for " + name + ".", context);
         ...
         throw new ParseException("Parentheses required around parameters of summation.", context);
   } // end doParse()
