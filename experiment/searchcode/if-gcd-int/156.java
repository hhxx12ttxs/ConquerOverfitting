...
public class NormalExpression implements Comparable,Comparator<NormalExpression> {
    ...
    ( Expression e1, BinaryOperator op, Expression e2 ) {
        Expression e = new BinaryExpression(
            ( Expression )e1.clone(), op, ( Expression )e2.clone() );
        return simplify( e, "SIMP" );
    }
    ...
    public static Expression negate( Expression e ) {
        Expression e1 = new UnaryExpression(
            UnaryOperator.LOGICAL_NEGATION, ( Expression )e.clone() );
...
//------------------------------------------------------------------------------
    ...
    ( Expression e, List<Identifier> id_list ) {
        NormalExpression ne = ( new NormalExpression( e ) ).simplify( "SIMP" );
        ...
        if ( !ne.op.equals( OP.ADD ) )
            return ( Tools.containsSymbols( se, id_set ) )? ret: se;

        for ( Object o : ne.list ) {
            Expression curr = ( ( NormalExpression )o ).getExpression();
            if ( Tools.containsSymbols( curr, id_set ) )
                continue;
            ret = add( ret, curr );
        }
        return ret;
    ...
    public static Expression getCoefficient( Expression e, Identifier id ) {
        ...
        if ( !Tools.containsSymbol( se, id.getSymbol() ) )
        ...
        if ( ne.op.equals( OP.MUL ) ) {
            ...
            while ( iter.hasNext() ) {
                Expression curr = ( ( NormalExpression )iter.next() ).getExpression();
                if ( curr.equals( id ) ) {
                    iter.remove();
                    break;
                }
            ...
            return ( Tools.containsSymbol( ret, id.getSymbol() ) )? null: ret;
        }
     ...
     */
    public static List<Expression> getCoefficient
    ( Expression e, List<Identifier> id_list ) {
     ...
     */
    public static boolean isAffine( Expression e, List<Identifier> id_list ) {
     ...
     * For now, assume the given constraints holds when this method is called. We can
     * use range information to assure this assumption in the future.
     */
    ...
    ( Expression lb, Expression ub, Expression e, Identifier id ) {
        ...
        Expression e0 = ( Expression )e.clone(), lb0 = lb, ub0 = ub;
        if ( !lb.equals( one ) ) {
    ...
    // More general method is to use Bernoulli Numbers - for now just use template.
    ...
    public static List<Expression> getPolynomialCoef( Expression e, Identifier id ) {
        ...
        if ( e instanceof Identifier ) {
            ...
            for ( Object o : ne.list ) {
                ...
                if ( term.equals( id ) )
                    order++;
                else if ( Tools.containsSymbol( term, id.getSymbol() ) )
        ...
        } else if ( !Tools.containsSymbol( e, id.getSymbol() ) )
            ret.add( ( Expression )e.clone() );
        else
            return null;
    ...
    public static void runtest( Program prog ) {
        ...
        while ( iter.hasNext() ) {
            ...
            if ( t instanceof CommaExpression ) {
                ...
                Tools.printlnStatus( "ID = "+id_list+", EXPR = "+e, 0 );
                for ( Identifier id : id_list )
                    Tools.printlnStatus( "  COEF["+id+"] = "+getCoefficient( e, id ), 0 );
                Tools.printlnStatus( "  CONST   = "+getConstantCoefficient( e, id_list ), 0 );
                Tools.printlnStatus( "  COEF    = "+getCoefficient( e, id_list ), 0 );
                Tools.printlnStatus( "  VARS    = "+getVariableList( e ), 0 );
                Tools.printlnStatus( "  AFFINE  = "+isAffine( e, id_list ), 0 );
                Tools.printStatus( "  POLY["+id_list.getFirst()+"] = ", 0 );
                System.out.println( getPolynomialCoef( e, id_list.getFirst() ) );
                if ( id_list.size() == 3 ) {
                    Tools.printStatus( "  CLOSED  = ", 0 );
                    System.out.println(
                        getClosedFormSum( id_list.get( 1 ), id_list.get( 2 ), e, id_list.get( 0 ) ) );
                }
                if ( id_list.size() == 2 ) {
                    Tools.printStatus( "  CLOSED  = ", 0 );
                    System.out.println(
                        getClosedFormSum( one, id_list.get( 1 ), e, id_list.get( 0 ) ) );
                }
