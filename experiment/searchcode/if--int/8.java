...
public class ServeurJeu implements ConstantesServeurJeu, EcouteurDeJeu, Runnable
{
        ...
        throws JeuEnCoursException, AucunePlaceDisponibleException
	{
        try
        {
            // Ajout du joueur ŕ l'ensemble des joueurs
            jeuServeur.ajouterJoueur(joueur);
            
            // Log
            log("Nouveau joueur ! ID : " + joueur.getId());
            
            // On vérifie que l'ID passé en paramętre soit bien unique
    		if (clients.containsKey(joueur.getId()))
    		{
    			log("ERROR : Le client " + joueur.getId() + " est déjŕ dans la partie");
    		...
    		else
    		{
    		    ...
    		    // On inscrit le joueur ŕ la partie
	...
	public synchronized int lancerVague(Joueur joueur, int nbCreatures, int typeCreature)
	{
	    ...
	    // TODO les créatures disponibles dervaient ętre dans le terrain
	    ...
	    synchronized (joueur)
        {
		    ...
		    if( (int) argentApresAchat >= 0)
		    {
	            ...
	            catch (ArgentInsuffisantException e)
                {
	                ...
	                // c'est pas trčs propre mais j'en avais besoins pour 
                }
	...
	public synchronized int ameliorerTour(Joueur joueur, int idTour)
	{
		...
		// Récupération de la tour ŕ améliorer
	...
	public synchronized int vendreTour(Joueur joueur, int tourCible)
	{
		...
		// Repérage de la tour ŕ supprimer
	...
	/**
	 * Envoi un message texte ŕ l'ensemble des clients connectés.
	 * 
	 * @param IDFrom
	 *            L'ID de l'expéditeur.
	 * @param message
	 *            Le message ŕ envoyer.
	 * @throws CanalException 
	...
	/**
	 * Envoi un message texte ŕ un client en particulier.
	 * 
	 * @param IDFrom
	 *            L'ID de l'expéditeur
	 * @param IDTo
	...
	{
		log("Le joueur " + idJoueur + " désire envoyer un message ŕ " + IDTo
				+ "(" + message + ")");
	 ...
	 * 
	 * @param message le message ŕ diffuser
	 * @throws CanalException 
	 */
    ...
    { 
        // si il est pas déjŕ deconnecte ?
        if(clients.containsKey(joueur.getId()))
        {
            clients.remove(joueur.getId());
            
            // l'hote met fin ŕ la partie
