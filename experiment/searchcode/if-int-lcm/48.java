...
import saasppcf.crypto.design.HomomorphicCryptosystem;
...
public class PaillierCryptosystem implements HomomorphicCryptosystem {
	/**
	...
	public PaillierCryptosystem(BigInteger n, int bitSize) {
		random = new Random();//new SecureRandom(BigInteger.valueOf(System.nanoTime()).toByteArray());
		this.n = n;
	 ...
	 * @param bitSize
	 */
	public PaillierCryptosystem(PaillierPublicKeyDO publicKey) {
		//random = new SecureRandom(BigInteger.valueOf(System.nanoTime()).toByteArray());
		random = new Random();
		/*
		if(publicKey.random == null) {
			random = new Random();//new SecureRandom(BigInteger.valueOf(System.nanoTime()).toByteArray());
		}
		else {
			random = publicKey.random;
		}
		*/
	 ...
	 * @param bitSize
	 */
	public PaillierCryptosystem(PaillierPrivateKeyDO privateKey) {
		//random = new SecureRandom(BigInteger.valueOf(System.nanoTime()).toByteArray());
		random = new Random();
	...
	protected void generateKeys() {
    	...
    	//chose a random prime number of bitSize/2 and ensure that n's bit size is bitSize
    		//p = new BigInteger(bitSize/2, certainty, rnd);
	...
	public PaillierPublicKeyDO getPublicKey() {
		return new PaillierPublicKeyDO(n,halfN,nSquared,bitSize);
	}
	
	public PaillierPrivateKeyDO getPrivateKey() {
		if(canDecrypt) {
			return new PaillierPrivateKeyDO(n,halfN,nSquared,bitSize,lambda,mu);
		}
		else {
			return null;
		}
	...
	protected BigInteger randomInZStarN() {
    	...
    	do
        {
    		...
    		//r = BigInteger.probablePrime(bitSize, rnd); //TODO: This method is preferred to the BigInteger constructor, see Javadoc for BigInteger
        } while (!isInZStarN(r));
    	return r;
    }
	...
	private boolean isInZN(BigInteger input) {
    	//return !(/*input.compareTo(BigInteger.ZERO) < 0 ||*/ input.compareTo(halfN) >= 0); //can accept negative inputs in n/2 field.
