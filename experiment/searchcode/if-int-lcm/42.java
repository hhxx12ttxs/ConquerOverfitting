...
public class Symbolic extends SimpleExpression {
    ...
    public static List<Object> getVariablesOnLHS( Expression e ) {
        ...
        //System.out.println("[LHS] = "+se.getExpression());
    ...
    private static List<Expression> getVariablesOnLHS( SimpleExpression se ) {
        ...
        if ( lhs.getOP() == ID ) {
            if ( !Tools.containsExpr( rhs.getExpression(), lhs.getExpression() ) )
            ...
            for ( SimpleExpression child : lhs.getChildren() ) {
                ...
                //System.out.println("[LHS.new_rhs] = "+new_rhs.getExpression()+" id = "+id);
                SimpleExpression new_se =
                    new SimpleExpression( child, se.getOP(), new_rhs );
                //System.out.println("[LHS.new_se] = "+new_se.getExpression());
                if ( !Tools.containsExpr( new_rhs.getExpression(),child.getExprRef() ) )
    ...
    ( Expression e, List<Identifier> ids ) {
        ...
        if ( se.getOP() == ADD ) {
            ...
            ret = Tools.containsExprs( expr, ids )? new IntegerLiteral( 0 ): expr;
    ...
    public static Expression getCoefficient( Expression e, Identifier id ) {
        ...
        else if ( !Tools.containsExpr( expr, id ) )
            ret = new IntegerLiteral( 0 );
    ...
    ( Identifier id, Expression lb, Expression ub, Expression e ) {
        ...
        if ( !lb.equals( one ) ) {
            ...
            e = Symbolic.simplify( Tools.replaceSymbol( e, id.getSymbol(), new_id ) );
        }
    ...
    public static Expression getClosedFormSum( Identifier id, Expression lb,
        ...
        if ( se.getOP() == DIV ) {
            ...
            if ( Tools.containsSymbol( divider.getExpression(), id.getSymbol() ) )
                return null;
    ...
    public static List<Expression> getPolynomialCoef( Expression e, Identifier id ) {
        ...
        case MUL: {
            ...
            for ( SimpleExpression child : se.getChildren() ) {
                ...
                if ( child_expr.equals( id ) )
                    skipped_order++;
                else if ( Tools.containsExpr( child_expr, id ) ) {
                    ret = null;
                    break;
                } else
        ...
        case ADD:
            ...
            break;
        default:
            if ( Tools.containsExpr( e, id ) )
                ret = null;
            else
                ret.add( ( Expression )e.clone() );
        }
        //Tools.printlnStatus("[POLY] "+e+" on "+id+" = "+ret, 1);
        return ret;
    }
    ...
    private static Expression unary( UnaryOperator op, Expression e ) {
        Expression expr = new UnaryExpression( op, ( Expression )e.clone() );
    	...
    	public static void runtest(Program prog)
    	{
    		...
    		while ( iter.hasNext() )
    		{
    			...
    			else if ( t instanceof AssignmentExpression )
    			{
    				...
    				Tools.printlnStatus("  VARS    = "+getVariables(e), 0);
    				...
    				if ( id_list.size() == 3 ) {
    					...
    					System.out.println(getClosedFormSum(
    						id_list.get(0), id_list.get(1), id_list.get(2), e));
    				}
    				if ( id_list.size() == 2 ) {
    					Tools.printStatus("  CLOSED  = ", 0);
    					System.out.println(getClosedFormSum(
    						id_list.get(0), new IntegerLiteral(1), id_list.get(1), e));
    				}
    			}
