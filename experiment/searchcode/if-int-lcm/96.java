...
import net.sf.colossus.ai.objectives.ThirdObjectiveHelper;
...
public class ExperimentalAI extends SimpleAI // NO_UCD
{
    ...
    public List<CritterMove> battleMove()
    {
        ...
        {
            IObjectiveHelper helper = new ThirdObjectiveHelper(client, this,
                variant);
        ...
        final BattleHex hex, final Legion legion, final int turn)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // warning : bec.TITAN_FORWARD_EARLY_PENALTY is negative
                float val = ((5.f - turn) / 3.f) *
                        bec.TITAN_FORWARD_EARLY_PENALTY *
                        (6.f - rangeToClosestOpponent( hex));
                if (turn >= 5)
                {
                    val = 0.f;
                }
                value.add(Math.round(val), "Progressive TitanForwardEarlyPenalty");
                for (int i = 0; i < 6; i++)
                {
                    BattleHex neighbor = hex.getNeighbor(i);
                    if (neighbor == null /* Edge of the map */
        ...
        Map<BattleHex, Integer> strikeMap, ValueRecorder value)
    {
        ...
        if (listObjectives != null)
        {
            for (TacticalObjective to : listObjectives)
            {
                ...
                temp.setScale(temp.getScale() * to.getPriority());
                value.add(temp);
            }
        }
    ...
    public void initBattle()
    {
        ...
        if (client.getMyEngagedLegion() != null)
        {
            /* TODO: Objectives are never updated during Battle, which is
    ...
    public void cleanupBattle()
    {
        ...
        if (listObjectives != null)
        {
            for (TacticalObjective to : listObjectives)
            {
                LOGGER.info("Objective (" + client.getMyEngagedLegion().
                        getMarkerId() + "): " + to.getDescription() + " -> "
