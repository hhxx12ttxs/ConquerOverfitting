...
public BufferedImage writeGlyph(String prefix)
{
  ...
  toret=toret.getSubimage(0, 0, matrix.length, matrix[0].length);
    ...
    imageHelpers.writeImage(toret, "/usr/glyphs/"+prefix+Thread.currentThread().getId()+".jpg");
    return toret;
}
public BufferedImage writeGlyph(String prefix, matrixBlob mb2)
{
    BufferedImage toret=imageHelpers.readAsBufferedImage("/usr/blankimg.jpg");
...
//    toret=toret.getSubimage(0, 0, matrix.length+mb2.matrix.length, matrix[0].length+mb2.matrix[0].length);
    imageHelpers.writeImage(toret, "/usr/glyphs/"+prefix+System.currentTimeMillis()+Thread.currentThread().getId()+".jpg");
    return toret;
}
