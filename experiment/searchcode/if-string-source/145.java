...
import org.gbif.api.model.vocabulary.OccurrenceStatus;
import org.gbif.api.model.vocabulary.ThreatStatus;
import org.gbif.checklistbank.api.model.vocabulary.CitesAppendix;
import org.gbif.checklistbank.api.model.vocabulary.EstablishmentMeans;
...
public class Distribution extends NameUsageComponent {
   ...
   * <blockqoute>
   * <p>
   * <i>Example:</i> TDWG:AGS-TF
   * <i>Example:</i> ISO3166:AR
   * <i>Example:</i> WOEID:564721
   * </p>
   * </blockquote>
   *
  ...
  /**
   * ISO3166 alpha 2 (3 is permissable) country codes the area belongs to or as an alternative for a lcoationID if the
   * area is a country.
   * <blockquote>
   * <p>
   * <i>Examples:</i> "AR" - "US"
   ...
   * <p>
   * <i>Example:</i>
   * "Euro+Med Plantbase - the information resource for Euro-Mediterranean plant diversity (2006-).
  ...
  @Override
  public int hashCode() {
    return Objects.hashCode(super.hashCode(), locality, country, status, lifeStage, temporal, threatStatus,
      establishmentMeans, appendixCites, source, startDayOfYear, endDayOfYear);
  }

  @Override
  public String toString() {
    return Objects.toStringHelper(this).add("super", super.toString()).add("locality", locality)
      .add("country", country).add("status", status).add("lifeStage", lifeStage).add("temporal", temporal)
      .add("threatStatus", threatStatus).add("establishmentMeans", establishmentMeans)
      .add("appendixCites", appendixCites).add("source", source).add("startDayOfYear", startDayOfYear)
      .add("endDayOfYear", endDayOfYear).add("locationId", locationId).add("remarks", remarks).toString();
  }
  ...
  public boolean equals(Object object) {
    if (object instanceof Distribution) {
      ...
      return Objects.equal(this.locality, that.locality) && Objects.equal(this.country, that.country)
        && Objects.equal(this.status, that.status) && Objects.equal(this.lifeStage, that.lifeStage)
        && Objects.equal(this.temporal, that.temporal) && Objects.equal(this.threatStatus, that.threatStatus)
        && Objects.equal(this.establishmentMeans, that.establishmentMeans)
        && Objects.equal(this.appendixCites, that.appendixCites) && Objects.equal(this.source, that.source)
