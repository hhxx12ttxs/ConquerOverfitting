...
public class Atom10FeedBuilder extends FeedBuilder {
	...
	public String addUpdatedTime(String source, Date updatedTime) {
		...
		return source.replace(FeedConstant.UPDATED_TIME_PLACEHOLDER,
				DateUtils.formatDateAsISO8601(updatedTime));
	}
	...
	protected Element buildRoot() {
		Element feed = this.feedDocument.createElementNS(
				FeedConstant.ATOM_10_NAMESPACE, "feed");
		if (!Utils.isNullOrEmpty(this.source.getLanguage()))
			feed.setAttribute("xml:lang", this.source.getLanguage());
	...
	protected void buildMetadata(Element root) {
		...
		if (!Utils.isNullOrEmpty(this.source.getDescription()))
			XMLUtils.appendSimpleElement(root, "subtitle",
		...
		if (!Utils.isNullOrEmpty(this.source.getImage()))
			XMLUtils.appendSimpleElement(root, "icon", this.source.getImage());

		XMLUtils.appendSimpleElement(root, "updated",
				FeedConstant.UPDATED_TIME_PLACEHOLDER);
		Element generator = XMLUtils.appendSimpleElement(root, "generator",
	...
	protected Element buildEntry(Element root, Entry entry) {
		...
		Element contentNode = XMLUtils.appendSimpleElement(entryNode,
				"content", entry.getContent());
		contentNode.setAttribute("type", "html");
		Element authorNode = XMLUtils.appendSimpleElement(entryNode, "author",
				null);
		XMLUtils.appendSimpleElement(authorNode, "name", entry.getAuthor());
		XMLUtils.appendSimpleElement(entryNode, "updated",
				DateUtils.formatDateAsISO8601(entry.getUpdatedTime()));
