	...
	implements AstroConstants
{
	...
	/**
	 * Get the ?T value for this Instant.
	 * 
	 * @return				?T in seconds.
	 */
	public double get?T() {
	...
	public static double utToTd(double jd) {
		...
		// ?T is TDT - UT1 in seconds.
		double ?T = calculateDeltaT((int) ymd[0], ymd[1] + ymd[2] / 31.0);
		return jd + ?T / SECS_PER_DAY;
	}
	...
	public static double[] julianToYmd(double jd) {
		...
		if (z < 2299161)
			...
			double ? = floor((z - 1867216.25) / 36524.25);
			a = z + 1 + ? - floor(? / 4.0);
		}
	 ...
	 * @param	?			The observer's geographical longitude in radians;
	 * 						west longitudes negative, east positive.
	 * @return				The local sideral time in decimal hours.
	 */
	@Deprecated
	public static double gstToLst(double GST, double ?) {
		double H = Math.toDegrees(?) / 15;
		double L = (GST + H) % 24;
		if (L < 0)
			L += 24;
		return L;
	 ...
	 */
	@Deprecated
	public static double lstToGst(double LST, double ?) {
		double H = Math.toDegrees(?) / 15;
		double G = (LST - H) % 24;
		if (G < 0)
			G += 24;
	...
	/**
	 * Calculate an estimate of the value of ?T, ie TD - UT in seconds,
	 * for a given moment in time.
	 * 
	 * This method gives a reasonably good value over the period
	 * -1999 to +3000.
	 * 
	 * Reference:	Polynomial Expressions for Delta T (?T)
	 *				Espenak and Meeus
	 *				http://eclipse.gsfc.nasa.gov/SEcat5/deltatpoly.html
	 * 
	 * @param	year		The year for which we want ?T.
	 * @param	month		The fractional month; 0.5 = mid-Jan.  Does not
	 * 						have to be terribly accurate, given the precision
	 * 						to which the variation in ?T is known.
	 * @return				The value of ?T for the given year and month.
	 */
	public static double calculateDeltaT(int year, double month) {
		...
		if (year < -500) {
			...
			// formula at that epoch.  The value for ?T is given by a
			...
			double t2 = t * t;
			double t3 = t2 * t;
			?T = 120.0 - 0.9808 * t - 0.01532 * t2 + t3 / 7129.0;
		} else if (year < 1800) {
			double t = y - 1700.0;
			double t2 = t * t;
			double t3 = t2 * t;
			double t4 = t3 * t;
			?T = 8.83 + 0.1603 * t - 0.0059285 * t2
			...
			double t6 = t5 * t;
			double t7 = t6 * t;
			?T = 13.72 - 0.332447 * t + 0.0068612 * t2 + 0.0041116 * t3
			...
			double t4 = t3 * t;
			double t5 = t4 * t;
			?T = 7.62 + 0.5737 * t - 0.251754 * t2 + 0.01680668 * t3
			...
			double t3 = t2 * t;
			double t4 = t3 * t;
			?T = -2.79 + 1.494119 * t - 0.0598939 * t2
			...
			double t2 = t * t;
			double t3 = t2 * t;
			?T = 29.07 + 0.407 * t - t2 / 233.0 + t3 / 2547.0;
		} else if (year < 1986) {
			double t = y - 1975.0;
			double t2 = t * t;
			double t3 = t2 * t;
			?T = 45.45 + 1.067 * t - t2 / 260.0 - t3 / 718.0;
		} else if (year < 2005) {
			double t = y - 2000.0;
			double t2 = t * t;
			double t3 = t2 * t;
			double t4 = t3 * t;
			double t5 = t4 * t;
			?T = 63.86 + 0.3345 * t - 0.060374 * t2 + 0.0017275 * t3
					+ 0.000651814 * t4 + 0.00002373599 * t5;
		} else if (year < 2050) {
			// This expression is derived from estimated values of ?T
			...
			double u = (y - 1820.0) / 100.0;
			double u2 = u * u;
			?T = -20.0 + 32.0 * u2 - 0.5628 * (2150.0 - y);
		} else {
			double u = (y - 1820.0) / 100.0;
			double u2 = u * u;
			?T = -20.0 + 32.0 * u2;
	...
	// The value for ?T at this instant, in seconds.
