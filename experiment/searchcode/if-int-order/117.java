...
public class OrderSafePropertiesTest {
    ...
    private void verifyPropertiesContent(Properties p) {
        assertThat(p.getProperty("_check_1")).isEqualTo("ćřĺĆŘĹ");
        ...
        assertThat(p.getProperty("_check_6")).isEqualTo("??");
        assertThat(p.getProperty("_check_7.????????????")).isEqualTo("????????????");
        assertThat(p.getProperty("_check_8")).isEqualTo("?");// Unicode Character 'CYRILLIC SMALL LETTER HA' (U+0445)

        // test from Lyubomir Ivanov
        final String cyrillic_bulgarian_caps  = "??????????????????????????????";
        final String cyrillic_bulgarian_small = "??????????????????????????????";
        final String cyrillic_bulgarian_old   = "??????";
        final String cyrillic_russian         = "????";
        final String cyrillic_serbian         = "??????????";
