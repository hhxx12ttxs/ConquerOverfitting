...
/**
 * Class representing 64-bit decimal number. It provides fixed point arithmetic
 * and can be also used in Lucene full text search engine to convert number to
 * strings with proper lexicographical order (so range operator in Lucene query
 * can be used for numeric fields). This class supports operations only for
...
public class Decimal extends Number implements Comparable<Decimal>, IValue {
	...
	private void checkFormat(Decimal x) {
		...
		if ((byte) value != (byte) x.value) {
			throw new IllegalArgumentException(
					"Decimal numbers have different format");
		}
	}
	...
	private static void checkArguments(boolean cond) {
		if (!cond) {
			throw new IllegalArgumentException(
					"Invalid decimal width or precision");
		}
	}

	/**
	 * Constructor of decimal number from scaled value with given width and
	 * precision
	 * 
	...
	public Decimal(long value, int nIntegerDigits, int nFractionDigits) {
		checkArguments(nIntegerDigits >= 0 && nFractionDigits >= 0
				&& nIntegerDigits + nFractionDigits <= 16);
	...
	/**
	 * Constructor of decimal number from floating point value with given width
	 * and precision
	 * 
	...
	public Decimal(double value, int nIntegerDigits, int nFractionDigits) {
		this((long) (value * ipow(10, nFractionDigits)), nIntegerDigits,
				nFractionDigits);
	}

	/**
	 * Constructor of decimal number from string representation with given width
	 * and precision
	 * 
	...
	public Decimal(String value, int nIntegerDigits, int nFractionDigits) {
		...
		if (dot >= 0) {
			...
			this.value = (sign
					* (Long.parseLong(intPart) * scale + Long.parseLong(value
							.substring(dot + 1))) << 8)
					| (nIntegerDigits << 4) | nFractionDigits;
		}
		else {
			this.value = (sign * Long.parseLong(intPart) * scale << 8)
					| (nIntegerDigits << 4) | nFractionDigits;
		}
	}
	...
	public Decimal(String value) {
		...
		if (dot >= 0) {
			...
			nFractionDigits = 0;
			scale = 1;
			this.value = (sign * Long.parseLong(intPart) << 8)
					| (nIntegerDigits << 4) | nFractionDigits;
		}
		checkArguments(nIntegerDigits >= 0 && nFractionDigits >= 0
				&& nIntegerDigits + nFractionDigits <= 16);
	...
	/**
	 * Subtract two decimal numbers. The values should have the same with and
	 * precision.
	 * 
	...
	/**
	 * Multiply two decimal numbers. The values should have the same with and
	 * precision.
	 * 
	...
	/**
	 * Divide two decimal numbers. The values should have the same with and
	 * precision.
	 * 
	...
	public boolean equals(Object o) {
		return o instanceof Decimal ? ((Decimal) o).value == value
				: o instanceof Number ? equals(create(((Number) o)
						.doubleValue()))
						: o instanceof String ? equals(create((String) o))
								: false;
	}
	...
	public Decimal abs() {
		Decimal result = new Decimal();
		result.value = value < 0 ? ((-(value >> 8) << 8) | (value & 0xFF))
				: value;
		result.maxValue = maxValue;
		result.scale = scale;
		return result;
	...
	public String toString() {
		...
		return Long.toString((value >> 8) / scale)
				+ "."
				+ pad((value < 0 ? -(value >> 8) : (value >> 8)) % scale,
						getFractionDigits(), '0');
	}
	...
	public String toLexicographicString() {
		calculateScale();
		return pad(((value >> 8) + maxValue) / scale, getIntegerDigits() + 1,
				'0')
				+ "."
