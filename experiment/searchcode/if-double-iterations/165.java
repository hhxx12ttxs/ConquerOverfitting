...
public class FiveFold {
	...
	public static void main ( String argu [] ) {
		...
		double total = 0 ; 
		for ( int i = 0 ; i < IT ; i ++ ) {
			TestResult result = ( new FiveFold ()).test() ;
	...
	void readIn () {   //   if mode = 0 then it is the 2 class mode ; else it is the 4 class mode,
		...
		try {
			...
			for ( i = 0 ; i < totalPic ; i ++ ) {
				...
				//imageCorpus [ thisClass ].add ( (thisClass%classes==0?"-1":"+1") + " " + scanner.nextLine().trim() );  // for 2 class classifiacation
				imageCorpus [ thisClass ].add ( (thisClass%classes) + " " + scanner.nextLine().trim() );  // for multiclass classification
			}
	...
	public TestResult test () {  
		...
		classifier.lambda = 10000000.0 ; 
		
		randomGenSet () ;	
		ITERATIONS = 5 ;
		result = new double[ ITERATIONS ] ;	
		for ( i = 0 ; i  < 5 ; i ++ ){
			randomTest ( i, classifier ) ;
	...
	void randomTest ( int whichFive, Classifier classifier ) {
