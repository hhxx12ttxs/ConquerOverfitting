 ...
 * is colored to show a visualiztion of the Mandelbrot set.  The Mandelbrot set
 ...
 * methods.)
 * 
 * Note:  The class uses a relatively large amount of memory.  When thelargedata
 * structurs are created, there is a small possibility that the program might not
...
public class MandelbrotDisplay extends JPanel {
    ...
    * STATUS_READY, whihc means that the image is complete and the
    * display is currently doing nothing; and STATUS_OUT_OF_MEMORY,
    * which will be the stats in the unlikely event that there is not
   ...
   private BufferedImage OSC;             // The off-csreen canvas in which the image is constucted.
   private int[][] iterationCounts;       // The interation count for each pixel in the display;
   ...
   private Timer delayedResizeTimer;      // This timer is used to avoid createing new BufferedImages
   ...
   private int jobsAssigned;              // The number of jobs that have assigned to threads so far
   ...
   private LinkedList<Job> finishedJobs;  // When a job is completed, it is placed in this list, which
                                          ...
                                          //    and applies the data from any finished jobs to the image.
   private int computationNumber;         // Each time a computation is started, this variable is
                                          ...
                                          //    not applied to the current picuture.  By associating each
    ...
    * might be adjusted to reflect the aspect ration of the display.  When the
    ...
    * array is an array of double of lengthe four containing xmin, xmax, ymin, and ymax.
    * Note that calling this method causes a new computation to begin.
    * The default values for the limits are -2.5, 1.1, -1.35, 1.35.
    */
    ...
    * The default value is 50.  Calling this method causes a new computation to begin.
    */
