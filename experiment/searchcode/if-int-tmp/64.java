...
// The values that change during interpretation
        ...
        CallFrame cloneFrozen()
        {
            ...
            CallFrame copy;
            throw new UnsupportedOperationException("Interpreter.java cloneFrozen()");
            // TODO Need to fix clone!!!
//            try {
//                copy = (CallFrame)clone();
//                copy = new CallFrame();
//            } catch (Exception ex){ //CloneNotSupportedException ex) {
    ...
    case Token.SHNE : {
      ...
      shallow_compare: {
            ...
            if (rhs == DBL_MRK) {
                ...
                if (lhs == DBL_MRK) {
                    ldbl = sDbl[stackTop];
                } else if (org.not.java.Number.isNumber(lhs)) {
                    ldbl = org.not.java.Number.doubleValue(lhs);
                } else {
                    valBln = false;
                ...
                if (rhs == DBL_MRK) {
                    rdbl = sDbl[stackTop + 1];
                } else if (org.not.java.Number.isNumber(rhs)) {
                    rdbl = org.not.java.Number.doubleValue(rhs);
                } else {
                    valBln = false;
                    break shallow_compare;
                }
...
"RHINO USAGE WARNING: onExit terminated with exception");
    ...
    private static boolean stack_boolean(CallFrame frame, int i)
    {
        ...
        if (x == Boolean.TRUE) {
            ...
            return false;
        } else if (org.not.java.Number.isNumber(x)) {
            double d = org.not.java.Number.doubleValue(x);
            return (d == d && d != 0.0);
                              ...
                              Context cx)
    {
        ...
        if (rhs == UniqueTag.DOUBLE_MARK) {
            ...
            if (lhs instanceof Scriptable || rhs instanceof Scriptable) {
                ...
                double lDbl = (org.not.java.Number.isNumber(lhs))
                    ? org.not.java.Number.doubleValue(lhs) : ScriptRuntime.toNumber(lhs);
                double rDbl = (rhs instanceof Number)
                    ? org.not.java.Number.doubleValue(rhs) : ScriptRuntime.toNumber(rhs);
