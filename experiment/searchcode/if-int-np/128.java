...
public class PointerLocationView extends View implements InputDeviceListener {
    ...
    // QEEXO START
    static Map<TouchKind, Paint> TouchKindBrushes = new HashMap<TouchKind, Paint>();
    ...
    static {
        TouchKindBrushes = new HashMap<TouchKind, Paint>();
		Paint paint;

		paint = new Paint();
		paint.setColor(Color.RED);
		TouchKindBrushes.put(TouchKind.PAD, paint);

		paint = new Paint();
		paint.setColor(Color.GREEN);
		TouchKindBrushes.put(TouchKind.NAIL, paint);

		paint = new Paint();
		paint.setColor(Color.BLUE);
		TouchKindBrushes.put(TouchKind.KNUCKLE, paint);

		paint = new Paint();
		paint.setColor(Color.YELLOW);
		TouchKindBrushes.put(TouchKind.STYLUS_1, paint);

		paint = new Paint();
		paint.setColor(Color.CYAN);
		TouchKindBrushes.put(TouchKind.ERASER, paint);

		paint = new Paint();
		paint.setColor(Color.GRAY);
		TouchKindBrushes.put(TouchKind.UNKNOWN, paint);

		paint = new Paint();
		paint.setColor(Color.parseColor("#990000"));
		TouchKindBrushes.put(TouchKind.TIP, paint);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mActivePointerId >= 0) {
            ...
            canvas.drawText(mText.clear()
                    // QEEXO START
                    .append(TouchKind.values()[ps.touchKind].toString().toLowerCase()).append("(" + ps.touchKind + ")")
                    // QEEXO END
        ...
        for (int p = 0; p < NP; p++) {
            ...
            // QEEXO START
            Paint touchKindPaint = paintBrushForTouchKind(ps.touchKind);
            mPathPaint.setColor(touchKindPaint.getColor());
            mPaint.setColor(touchKindPaint.getColor());
            // QEEXO END
            for (int i=0; i < N; i++) {
    ...
    public void addPointerEvent(MotionEvent event) {
        ...
        for (int i = 0; i < NI; i++) {
            ...
            if (ps != null) {
                ...
                // QEEXO START
                ps.touchKind = (int) event.getAxisValue(MotionEvent.AXIS_GAS, i);
                // QEEXO END
            }
        }
    ...
    // QEEXO START
    public static Paint paintBrushForTouchKind(Integer touchKind) {
		Paint ret = TouchKindBrushes.get(TouchKind.values()[touchKind]);
        if(ret == null)
            return DefaultPaintBrush;
        return ret;
    }
    // QEEXO END
}
