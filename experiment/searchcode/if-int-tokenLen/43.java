...
public final class CaptureActivity extends Activity
	...
	private static final int ABOUT_ID = Menu.FIRST + 10 + 1;
	private static final int BOXES_ID = Menu.FIRST + 10 + 2;
	...
	ArrayList<BoxesManager> getBoxesManagers() {
		return boxesManagers;
	}
	...
	// menu.add(0, BOXES_ID, 0, "Kastid").setIcon(
	 ...
	 */
	boolean handleOcrDecode(ArrayList<OcrResult> obj) {
		lastResult = obj;
		...
		boxesfinderView.setVisibility(View.GONE);
		...
		for(OcrResult ocrResult:obj){
			Log.i(TAG,"ocrResult iter:"+ocrResult.getText());
			ImageView newBiv=new ImageView(bitmapImageView.getContext());
			newBiv.setImageDrawable(bitmapImageView.getDrawable());
			newBiv.setLayoutParams(bitmapImageView.getLayoutParams());
			newBiv.setAdjustViewBounds(true);
			newBiv.setScaleType(bitmapImageView.getScaleType());
			newBiv.setId((int)(Math.random() * 100));
			imageViewParent.addView(newBiv);
    
			TextView newOtv= new TextView(ocrResultTextView.getContext());
			newOtv.setLayoutParams(ocrResultTextView.getLayoutParams());
			imageViewParent.addView(newOtv);
			if (ocrResult.getText() == null ){
				Log.e(TAG, "ocrResult.getText() == null");
				ocrResult.setText("");
			}
			lastBitmap = ocrResult.getBitmap();
			if (lastBitmap == null) {
				newBiv.setImageBitmap(BitmapFactory.decodeResource(
						getResources(), R.drawable.ic_launcher));
			} else {
				newBiv.setImageBitmap(lastBitmap);
			}
			...
			newOtv.setText(ocrResult.getText());
...
//		}
	 ...
	 *            Object representing successful OCR results
	 */
	void handleOcrContinuousDecode(ArrayList<OcrResult> ocrResults) {
		Log.i(TAG, "handleOcrContinuousDecode");
		OcrResult ocrResult=ocrResults.get(0);
		lastResult = ocrResults;

		// Send an OcrResultText object to the ViewfinderView for text rendering
		/*
		 * boxesfinderView.addResultText(new OcrResultText(ocrResult.getText(),
	...
	void handleOcrContinuousDecode(OcrResultFailure obj) {
		lastResult = null;
		/*
		 * boxesfinderView.removeResultText();
		 */
	...
	private void resetStatusView() {
		...
		boxesfinderView.setVisibility(View.VISIBLE);
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		if (boxesManagers != null) {
			for (BoxesManager box : boxesManagers) {
				menu.add(1, boxesManagers.indexOf(box), 0, box.name);
				i++;
			}
		}
		return true;
	...
	private void newBoxItem() {
		boxesManager = new BoxesManager(getApplication(), "BOX_"
				+ boxesManagers.size(), null, getCameraManager());
		boxesManagers.add(boxesManager);
		boxesfinderView.setBoxesManager(boxesManager);
		boxesfinderView.setBoxesList(boxesManagers);
		boxesfinderView.drawViewfinder();
