...
public class Decoder {
    ...
    public static Integer getAttributeResource(byte[] xml, String att) {
        ...
        while (off < xml.length) {
            ...
            if (tag0 == startTag) { // XML START TAG
                ...
                for (int ii = 0; ii < numbAttrs; ii++) {
                    ...
                    if (attrName.equalsIgnoreCase(att))
                        return attrResId;
                    String attrValue = attrValueSi != -1 ? compXmlString(xml, sitOff, stOff,
                            attrValueSi) : "resourceID 0x" + Integer.toHexString(attrResId);
                ...
                // ret.append("</" + name + ">");
                // prtIndent(indent,
                // "</"+name+">  (line "+startTagLineNo+"-"+lineNo+")");
                ...
                // Logger.LogInfo("  Unrecognized tag code '"+Integer.toHexString(tag0)
                // +"' at offset "+off);
                break;
            }
        } // end of while loop scanning tags and attributes of XML tree
          // Logger.LogInfo("    end at offset "+off);
        return -1;
    ...
    private static void prtIndent(int indent, String str) {
        // Logger.LogInfo(spaces.substring(0, Math.min(indent*2,
        // spaces.length()))+str);
    }
    ...
    private static int LEW(byte[] arr, int off) {
        return arr[off + 3] << 24 & 0xff000000 | arr[off + 2] << 16 & 0xff0000 | arr[off + 1] << 8
                & 0xff00 | arr[off] & 0xFF;
    } // end of LEW
