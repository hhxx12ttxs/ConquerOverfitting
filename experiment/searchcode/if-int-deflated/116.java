 ...
 * this class are unsynchronized and {@link #entries}/{@link #getEntry}
 * enumerate/return {@link ZipEntry} instances which are shared with this class
 * rather than clones of them. This is used within TrueZIP in order to benefit
 * from the slightly better performance.
 * 
 * @author Christian Schlichtherle
 * @version $Id: BasicZipOutputStream.java,v 1.6 2010/08/30 22:03:04
 *          christian_schlichtherle Exp $
 * @since TrueZIP 6.4
 * @see ZipOutputStream
 */
public class BasicZipOutputStream2 extends FilterOutputStream {
	 ...
	 * compatible files. This is {@value} for compatibility with Sun's JDK
	 * implementation. Note that you should use &quot;IBM437&quot; for ordinary
	 * ZIP files instead.
	 */
	...
	/**
	 * The list of ZIP entries started to be written so far. Maps entry names to
	 * zip entries.
	 */
	...
	/**
	 * Whether or not we need to deflate the current entry. This can be used
	 * together with the {@code DEFLATED} method to write already compressed
	 * entry data into the ZIP file.
	 */
	...
	private static LEDataOutputStream toLEDataOutputStream(OutputStream out) {
		return out instanceof LEDataOutputStream ? (LEDataOutputStream) out
	...
	/**
	 * Returns an enumeration of the ZIP entries written so far. Note that the
	 * enumerated entries are shared with this class. It is illegal to put more
	 * entries into this ZIP output stream concurrently or modify the state of
	...
	/**
	 * Returns the {@link ZipEntry} for the given name or {@code null} if no
	 * entry with that name exists. Note that the returned entry is shared with
	 * this class. It is illegal to change its state!
	 * 
	 * @param name
	...
	/**
	 * Returns the default compression method for subsequent entries. This
	 * property is only used if a {@link ZipEntry} does not specify a
	 * compression method.
	 * 
	 * @see #setMethod
	...
	/**
	 * Sets the default compression method for subsequent entries. This property
	 * is only used if a {@link ZipEntry} does not specify a compression method.
	 * <p>
	 * Legal values are {@link ZipEntry#STORED} (uncompressed) and
	 * {@link ZipEntry#DEFLATED} (compressed). The initial value is
	...
	/**
	 * Returns the total number of (compressed) bytes this stream has written to
	...
	/**
	 * Returns {@code true} if and only if this {@code BasicZipOutputStream} is
	 * currently writing a ZIP entry.
	 */
	...
	/**
	 * Equivalent to {@link #putNextEntry(ZipEntry, boolean) putNextEntry(entry,
	 * true)}.
	 */
	public final void putNextEntry(final ZipEntry entry) throws IOException {
		putNextEntry(entry, true);
	}

	/**
	 * Starts writing the next ZIP entry to the underlying stream. Note that if
	 * two or more entries with the same name are written consecutively to this
	 * stream, the last entry written will shadow all other entries, i.e. all of
	 * them are written to the ZIP compatible file (and hence require space),
	 * but only the last will be accessible from the central directory. This is
	 * unlike the genuine {@link java.util.zip.ZipOutputStream
	 ...
	 * 
	 * @param entry
	 *            The ZIP entry to write.
	 * @param deflate
	 *            Whether or not the entry data should be deflated. This should
	 *            be set to {@code false} if and only if you are writing data
	 *            which has been read from a ZIP archive file and has not been
	 *            inflated again. The entries' properties CRC, compressed size
	 *            and uncompressed size must be set appropriately.
	 * @throws ZipException
	 *             If and only if writing the entry is impossible because the
	 *             resulting file would not comply to the ZIP file format
	 *             specification.
	 * @throws IOException
	...
	public void putNextEntry(final ZipEntry entry, final boolean deflate)
		...
		/*
		 * if (entries.get(name) != null) throw new ZipException(name +
		 * " (duplicate entry)");
		 */

		{
			final long size = entry.getNameLength(charset)
					+ entry.getExtraLength() + entry.getCommentLength(charset);
			if (size > UShort.MAX_VALUE)
				throw new ZipException(entry.getName()
						+ ": sum of name, extra fields and comment too long: "
						+ size);
		}
	...
	private void writeLocalFileHeader() throws IOException {
		...
		= crc == ZipEntry.UNKNOWN || csize == ZipEntry.UNKNOWN
				|| size == ZipEntry.UNKNOWN;
		final boolean zip64 // ZIP64 extensions?
		= csize >= UInt.MAX_VALUE || size >= UInt.MAX_VALUE
				|| offset >= UInt.MAX_VALUE || ZIP.ZIP64_EXT;

		// Compose General Purpose Bit Flag.
		// See appendix D of PKWARE's ZIP File Format Specification.
		final boolean utf8 = ZIP.UTF8.equalsIgnoreCase(charset);
		final int general = (dd ? (1 << 3) : 0) | (utf8 ? (1 << 11) : 0);
	...
	public void closeEntry() throws IOException {
		...
		case ZIP.STORED:
			...
			if (expectedCrc != entry.getCrc()) {
				throw new ZipException(entry.getName() + ": bad CRC-32: 0x"
						+ Long.toHexString(entry.getCrc()) + " expected: 0x"
			...
			if (entry.getSize() != entrySize) {
				throw new ZipException(entry.getName()
						+ ": bad Uncompressed Size: " + entry.getSize()
						+ " expected: " + entrySize);
			}
			break;
	 ...
	 * this ZIP output stream with putNextEntry(...) and the like. When you
	 * finally close the stream, the central directory will contain <em>all</em>
	 * entries written.</li>
	 * </ul>
	 * 
	 * @throws ZipException
	...
	private void writeCentralFileHeader(final ZipEntry entry)
		...
		final boolean zip64 // ZIP64 extensions?
		= csize32 >= UInt.MAX_VALUE || size32 >= UInt.MAX_VALUE
				|| offset32 >= UInt.MAX_VALUE || ZIP.ZIP64_EXT;
	...
	private void writeEndOfCentralDirectory() throws IOException {
		...
		final boolean cdEntriesZip64 = cdEntries > UShort.MAX_VALUE
				|| ZIP.ZIP64_EXT;
		final boolean cdSizeZip64 = cdSize > UInt.MAX_VALUE || ZIP.ZIP64_EXT;
		final boolean cdOffsetZip64 = cdOffset > UInt.MAX_VALUE
				|| ZIP.ZIP64_EXT;
		final int cdEntries16 = cdEntriesZip64 ? UShort.MAX_VALUE
				: (int) cdEntries;
		final long cdSize32 = cdSizeZip64 ? UInt.MAX_VALUE : cdSize;
		final long cdOffset32 = cdOffsetZip64 ? UInt.MAX_VALUE : cdOffset;
		final boolean zip64 // ZIP64 extensions?
		= cdEntriesZip64 || cdSizeZip64 || cdOffsetZip64;

		if (zip64) {
			final long zip64eocdrOffset // relative offset of the zip64 end of
	 ...
	 * with the stream. This closes the open output stream writing to this ZIP
	 * file, if any.
	 * 
	 * @throws IOException
	 *             On any I/O related issue.
