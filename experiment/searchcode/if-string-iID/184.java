 ...
 * @verson v1.0.0
 */
public class PaidQuery extends JPanel {

    private static final Logger log = LoggerFactory.getLogger(PaidQuery.class);
    ...
    public PaidQuery(MainApp mainApp) {
    ...
    private void addDataPanel() {
        ...
        if (!init && !"".equals(jtfId.getText())) {
            try {
                ...
                JOptionPane.showMessageDialog(null, "还款单号必须是数字");
                return;
            }
        }

        int oid = -1;
        if (!init && !"".equals(jtfOid.getText())) {
            try {
                oid = Integer.parseInt(jtfOid.getText());
            } catch (NumberFormatException e) {
        ...
        if (!MyFactory.getResourceService().hasRight(user, Resource.CENSORED)) {
            ...
            columnNames = new String[]{"还款单号", "销售单号", "货号", "货品", "买家", "还款", "折扣",
                    "审核状态", "还款日期", "操作"};
        }
