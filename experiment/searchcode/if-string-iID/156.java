...
public class MyGraphModel extends mxGraphModel {
	...
	private Map<FieldMeta, List<TranslationVO>> mpTransation;
	...
	public MyGraphModel(mxGraphComponent graphComponent, EntityRelationshipModelEditPanel panel, MainFrame mf) {
		...
		this.mf = mf;
		mpTransation = new HashMap<FieldMeta, List<TranslationVO>>();
	}
					...
					if(cell.getValue() != null && cell.getValue() instanceof String) {
						...
						if(sEntity.length() == 0) {
							...
							while(blnNotSet) {
								...
								for(EntityMeta voMeta : MetaProvider.getInstance().getAllEntities()) {
									if(voMeta.getEntityName().equals(sEntity)){
							...
							if(sEntity.length() == 0)
								...
								voEntity.setEntityName(sEntity);
								voEntity.setDbTable("V_EO_" + NuclosWizardUtils.replace(sEntity));
								...
								for(LocaleInfo info : LocaleDelegate.getInstance().getAllLocales(false)){
									...
									TranslationVO vo = new TranslationVO(info.locale, info.country, info.language, mpValues);
									lstTranslation.add(vo);
								}
								toEntity.setTranslation(lstTranslation);
								
								String sResult = MetaDataDelegate.getInstance().createOrModifyEntity(null, toEntity, null, new ArrayList<FieldMetaTransport>(), true, null, null);
								
								EntityMeta voEntityCreated = MetaProvider.getInstance().getEntity(UID.parseUID(sEntity));
	...
	public JPopupMenu createRelationPopupMenu(final mxCell cell, boolean delete) {
		...
		i1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				panel.setEntity((EntityMeta)target.getValue());	
				panel.setEntitySource((EntityMeta)source.getValue());
		...
		i2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				NucletFieldMeta vo = new NucletFieldMeta();
			    ...
			    for(LocaleInfo voLocale : LocaleDelegate.getInstance().getAllLocales(false)) {
					...
					TranslationVO translation = new TranslationVO(voLocale.locale, voLocale.country, voLocale.language, map);
		...
		i4.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if(!blnFound) {
						GenerationCollectController gcc = (GenerationCollectController)NuclosCollectControllerFactory.getInstance().newMasterDataCollectController(E.GENERATION.getUID(), null, null);
						MasterDataVO vo = new MasterDataVO(E.GENERATION.getUID(), null, null, null, null, null, null);
						vo.setFieldValue(E.GENERATION.sourceModule.getUID(), sSourceModule);
						vo.setFieldValue(E.GENERATION.targetModule.getUID(), sTargetModule);
		...
		iWizard.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(cell.getValue() != null && cell.getValue() instanceof EntityMeta) {
					UID iId = ((EntityMeta)cell.getValue()).getUID();
					if(iId != null) {
						final EntityMeta vo = MetaProvider.getInstance().getEntity(iId);
	...
	public Map<FieldMeta, List<TranslationVO>> getTranslation() {
