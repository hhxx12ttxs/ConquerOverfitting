...
/**
 * <hr />
 * This is a copy of java.util.Random implemented <u><b><i>all synchronization removed</i></b></u>.
 * It relies on external things such as setting to be threadlocal to keep the seed safe. Also, this
 * randomizer is not serializable.
 * <p />
 * TODO: retype this from <i>Knuth Vol. 3</i> to avoid any potential licensing issues.
 * <p />
 * 
 * @author anonymous
 *         <hr />
 ...
 *         uses a 48-bit seed, which is modified using a linear congruential formula. (See Donald
 *         Knuth, <i>The Art of Computer Programming, Volume 3</i>, Section 3.2.1.)
 *         <p>
 *         If two instances of {@code Random} are created with the same seed, and the same sequence
 *         of method calls is made for each, they will generate and return identical sequences of
 *         numbers. In order to guarantee this property, particular algorithms are specified for the
 *         class {@code Random}. Java implementations must use all the algorithms shown here for the
 *         class {@code Random}, for the sake of absolute portability of Java code. However,
 *         subclasses of class {@code Random} are permitted to use other algorithms, so long as they
 *         adhere to the general contracts for all the methods.
 *         <p>
 *         The algorithms implemented by class {@code Random} use a {@code protected} utility method
...
public class UnsynchronizedJavaRandom implements RandomNumberGenerator {
     ...
     * generator to a value very likely to be distinct from any other invocation of this
     * constructor.
     */
    ...
    /**
     * Creates a new random number generator using a single {@code long} seed. The seed is the
     ...
     * contract of {@code setSeed} is that it alters the state of this random number generator
     * object so as to be in exactly the same state as if it had just been created with the argument
     * {@code seed} as a seed. The method {@code setSeed} is implemented by class {@code Random} by
     ...
     * 
     * <p>
     * The implementation of {@code setSeed} by class {@code Random} happens to use only 48 bits of
     * the given seed. In general, however, an overriding method may use all 64 bits of the
     * {@code long} argument as a seed value.
     * 
     * @param seed
     *            the initial seed
     */
    ...
    /**
     * Generates the next pseudorandom number. Subclasses should override this, as this is used by
     * all other methods.
     * 
     * <p>
     * The general contract of {@code next} is that it returns an {@code int} value and if the
     * argument {@code bits} is between {@code 1} and {@code 32} (inclusive), then that many
     * low-order bits of the returned value will be (approximately) independently chosen bit values,
     * each of which is (approximately) equally likely to be {@code 0} or {@code 1}. The method
     * {@code next} is implemented by class {@code Random} by atomically updating the seed to
     * 
     * <pre>
     ...
     * described by Donald E. Knuth in <i>The Art of Computer Programming,</i> Volume 3:
     ...
     * value is pseudorandomly generated and returned. All 2<font size="-1"><sup>32 </sup></font>
     ...
     * pseudorandomly generated and returned. All {@code n} possible {@code int} values are produced
     * with (approximately) equal probability. The method {@code nextInt(int n)} is implemented by
     ...
     * @return the next pseudorandom, uniformly distributed {@code int} value between {@code 0}
     *         (inclusive) and {@code n} (exclusive) from this random number generator's sequence
     ...
     * number generator's sequence. The general contract of {@code nextLong} is that one
     * {@code long} value is pseudorandomly generated and returned.
     * 
     * <p>
     ...
     * </pre>
     * 
     * Because class {@code Random} uses a seed with only 48 bits, this algorithm will not return
     * all possible {@code long} values.
     * 
     ...
     * 
     * <p>
     * The general contract of {@code nextFloat} is that one {@code float} value, chosen
     * (approximately) uniformly from the range {@code 0.0f} (inclusive) to {@code 1.0f}
     * (exclusive), is pseudorandomly generated and returned. All 2<font
     * size="-1"><sup>24</sup></font> possible {@code float} values of the form
     * <i>m&nbsp;x&nbsp</i>2<font size="-1"><sup>-24</sup></font>, where <i>m</i> is a positive
     * integer less than 2<font size="-1"><sup>24</sup> </font>, are produced with (approximately)
     * equal probability.
     * 
     * <p>
     ...
     * The hedge "approximately" is used in the foregoing description only because the next method
     * is only approximately an unbiased source of independently chosen bits. If it were a perfect
     * source of randomly chosen bits, then the algorithm shown would choose {@code float} values
     ...
     * 
     * <p>
     * The general contract of {@code nextDouble} is that one {@code double} value, chosen
     * (approximately) uniformly from the range {@code 0.0d} (inclusive) to {@code 1.0d}
     ...
     * 
     * <p>
     * The hedge "approximately" is used in the foregoing description only because the {@code next}
     ...
     * nonuniformity because of the bias in the rounding of floating-point numbers: it was three
     * times as likely that the low-order bit of the significand would be 0 than that it would be 1!
     ...
     * sequence.
     * <p>
     * The general contract of {@code nextGaussian} is that one {@code double} value, chosen from
     * (approximately) the usual normal distribution with mean {@code 0.0} and standard deviation
     * {@code 1.0}, is pseudorandomly generated and returned.
     * 
     * <p>
     * The method {@code nextGaussian} is implemented by class {@code Random} as if by a threadsafe
     ...
     * generates two independent values at the cost of only one call to {@code StrictMath.log} and
     * one call to {@code StrictMath.sqrt}.
     * 
     * @return the next pseudorandom, Gaussian ("normally") distributed {@code double} value with
     *         mean {@code 0.0} and standard deviation {@code 1.0} from this random number
