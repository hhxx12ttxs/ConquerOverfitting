...
public class GeodeticCalculator {
	...
	public GlobalCoordinates calculateEndingGlobalCoordinates(
		...
		double phi2 = Math
				.atan2(
						...
						(1.0 - f)
								* Math.sqrt(sin2Alpha
										+ Math.pow(sinU1 * sinSigma - cosU1 * cosSigma * cosAlpha1,
												2.0)));
		...
		// eq. 12
		double alpha2 = Math.atan2(sinAlpha, -sinU1 * sinSigma + cosU1 * cosSigma
				* cosAlpha1);
	...
	public GeodeticCurve calculateGeodeticCurve(Ellipsoid ellipsoid,
		...
		// else, it converged, so do the math
			...
			// eq. 20
			radians = Math.atan2(cosU2 * Math.sin(lambda), (cosU1sinU2 - sinU1cosU2
					* Math.cos(lambda)));
			if (radians < 0.0)
				radians += TwoPi;
			...
			// eq. 21
			radians = Math.atan2(cosU1 * Math.sin(lambda), (-sinU1cosU2 + cosU1sinU2
					* Math.cos(lambda)))
					+ Math.PI;
			if (radians < 0.0)
				radians += TwoPi;
