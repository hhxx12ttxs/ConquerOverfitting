...
public abstract class CSharpCCParserInternals extends CSharpCCGlobals {
	...
	static protected void compare(Token t, String id1, String id2) {
		if (!id2.equals(id1)) {
			CSharpCCErrors.parse_error(t, "Name " + id2 + " must be the same as that used at PARSER_BEGIN (" + id1 + ")");
		}
	}
	...
	static protected void setinsertionpoint(Token t, int no) {
		...
		if (no == 1) {
			if (insertionpoint1set) {
				CSharpCCErrors.parse_error(t, "Multiple declaration of parser class.");
			} else {
	...
	static protected void insertionpointerrors(Token t) {
		...
		if (!insertionpoint1set || !insertionpoint2set) {
			CSharpCCErrors.parse_error(t, "Parser class has not been defined between PARSER_BEGIN and PARSER_END.");
		}
	}
	...
	static protected void addregexpr(TokenProduction p) {
		...
		if (Options.B("USER_TOKEN_MANAGER")) {
			if (p.lexStates == null || p.lexStates.length != 1 || !p.lexStates[0].equals("DEFAULT")) {
				CSharpCCErrors.warning(p, "Ignoring lexical state specifications since option USER_TOKEN_MANAGER has been set to true.");
			}
		}
		if (p.lexStates == null) {
			return;
		...
		for (int i = 0; i < p.lexStates.length; i++) {
			for (int j = 0; j < i; j++) {
				if (p.lexStates[i].equals(p.lexStates[j])) {
					CSharpCCErrors.parse_error(p, "Multiple occurrence of \"" + p.lexStates[i] + "\" in lexical state list.");
				}
			}
	...
	static protected void add_token_manager_decls(Token t, java.util.Vector decls) {
		if (token_mgr_decls != null) {
			CSharpCCErrors.parse_error(t, "Multiple occurrence of \"TOKEN_MGR_DECLS\".");
		} else {
			token_mgr_decls = decls;
			if (Options.B("USER_TOKEN_MANAGER")) {
				CSharpCCErrors.warning(t, "Ignoring declarations in \"TOKEN_MGR_DECLS\" since option USER_TOKEN_MANAGER has been set to true.");
			}
		}
	}
	...
	static protected String remove_escapes_and_quotes(Token t, String str) {
		...
		while (index < str.length() - 1) {
			...
			if (ch == 'u') {
				...
				CSharpCCErrors.parse_error(t, "Encountered non-hex character '" + ch + "' at position " + index + " of string - Unicode escape must have 4 hex digits after it.");
				return retval;
			}
			CSharpCCErrors.parse_error(t, "Illegal escape sequence '\\" + ch + "' at position " + index + " of string.");
			return retval;
		}
		return retval;
	}
	...
	static protected char character_descriptor_assign(Token t, String s) {
		if (s.length() != 1) {
			CSharpCCErrors.parse_error(t, "String in character list may contain only one character.");
			return ' ';
		} else {
			return s.charAt(0);
		}
	...
	static protected char character_descriptor_assign(Token t, String s, String left) {
		if (s.length() != 1) {
			...
			CSharpCCErrors.parse_error(t, "Right end of character range \'" + s + "\' has a lower ordinal value than the left end of character range \'" + left + "\'.");
	...
	static protected void makeTryBlock(
	  ...
	  java.util.Vector finallyblk
		...
		if (catchblks.size() == 0 && finallyblk == null) {
			CSharpCCErrors.parse_error(tryLoc, "Try block must contain at least one catch or finally block.");
			result = nestedExp;
			return;
		}
