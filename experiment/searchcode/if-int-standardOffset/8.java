 ...
 * applications. The time zone object is instantiated from a TimeZoneData
 * object, which is made from a JSON string that contains all the data needed
 * for the specified time zone. Applications can instantiate a time zone
 * statically, in which case the data could be retrieved from the
 * {@link com.google.gwt.i18n.client.constants.TimeZoneConstants
 * TimeZoneConstants} class. Applications can also choose to instantiate from a
 * string obtained from a server. The time zone string contains locale specific
 * data. If the application only uses a short representation, the English data
 * will usually satisfy the user's need. In the case that only the time zone
 * offset is known, there is a decent fallback that only uses the time zone
...
public class TimeZone {
    ...
    /**
     * In GMT representation, +/- has reverse sign of time zone offset. when
     * offset == 480, it should output GMT-08:00.
     */
    ...
    /**
     * This factory method provides a decent fallback to create a time zone
     * object just based on a given time zone offset.
     * 
     ...
     * contains the time zone information for desired time zone. Applications
     * can get such a string from the TimeZoneConstants class, or it can request
     * the string from the server. Either way, the application obtains the
     * original string from the data provided in the TimeZoneConstant.properties
     * file, which was carefully prepared from CLDR and Olson time zone
     * database.
     * 
     * @param tzJSON
