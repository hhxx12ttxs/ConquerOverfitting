...
import uk.ac.rhul.cs.dice.golem.conbine.agent.anac.TheNegotiatorAgent.TheNegotiator;
...
public class TimeManager {
	...
	private TheNegotiator theNegotiator;
	 ...
	 */
	public TimeManager(double discount, BidsCollection bidsCollection, TheNegotiator theNegotiator) {
		//super();
		//this.timeline = timeline;
		this.bidsCollection = bidsCollection;
		this.discount = discount;
		this.theNegotiator=theNegotiator;
		//System.out.printf("totalTime : %s ", theNegotiator.getDeadline()/1000);
		totalTime = theNegotiator.getDeadline()/1000;
		endPhases1 = new double[3];
		endPhases1[0]=  (totalTime* 0.8)/ totalTime;
		endPhases1[1] =  (totalTime* 0.1) / totalTime;
		endPhases1[2] = (totalTime* 0.05) / totalTime;
		// no discounts
	...
	public double getThreshold(double time) {
		//System.out.printf("phase:%s ", getPhase(time));
	...
	public int getMovesLeft() {
		...
		if (queue.isEmpty()) {
			...
			for (int i = 0; i < queueSize; i++) {
			 ...
			 //System.out.printf("getTime():%s ", getTime());
			 //System.out.printf("total:%s ", total);
			 //System.out.printf("queueSize:%s ", queueSize);
			movesLeft = (int)Math.floor((1.0 - getTime()) / (total / (double)queueSize));
			//System.out.printf("movesLefte:%s ", movesLeft);
		}
		return movesLeft;
	}
	...
	public void calculatePropArray() {
		...
		//System.out.println("posBids.size()"+posBids.size());
		for (int i = 0; i < posBids.size(); i++) {
			...
			if (util >= max - rangeStep && util <= max) {
				propArray[0]++;
				//System.out.println("propArray[0]"+propArray[0]);
			} else if (util >= max - 2 * rangeStep && util < max - rangeStep) {
				propArray[1]++;
				//System.out.println("propArray[1]"+propArray[1]);
			} else if (util >= max - 3 * rangeStep && util < max - 2 * rangeStep) {
				propArray[2]++;
				//System.out.println("propArray[3]"+propArray[3]);
			}
		}
		...
		//System.out.println("bidsCol.get(propArray[0]).getUtility():"+bidsCol.get(propArray[0]).getUtility());
		maxThresArray2[1] = bidsCol.get(propArray[0] ).getUtility(); // -1 to correct for array offset of zero
		//System.out.println("bidsCol.get(propArray[0] + propArray[1] ).getUtility():"+bidsCol.get(propArray[0] + propArray[1] ).getUtility());
		maxThresArray2[2] = bidsCol.get(propArray[0] + propArray[1] ).getUtility();
	...
	public double getTime() {
		//System.out.printf("what is the time:"+ theNegotiator.getNormalisedTime(theNegotiator.getStartTime()));
		return  theNegotiator.getNormalisedTime(theNegotiator.getStartTime());
	}
}
