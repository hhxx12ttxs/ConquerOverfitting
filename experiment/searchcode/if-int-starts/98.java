...
public class RStringLiteral extends RegularExpression {
	...
	public static void DumpStrLiteralImages(java.io.PrintWriter ostr) {
		...
		ostr.println("public static readonly string[] mccstrLiteralImages = {");
		...
		for (i = 0; i < allImages.length; i++) {
			...
			String toPrint = "\"";
			toPrint += CSharpCCGlobals.add_escapes(image);
/*			for (int j = 0; j < image.length(); j++) {
				if (image.charAt(j) <= 0xff) {
					// toPrint += ("\\" + Integer.toOctalString((int)image.charAt(j)));
					// toPrint +=
					//	("\\x" + Integer.toHexString((int) image.charAt(j)));
					// Antonello: this hack is to avoid the printing of non-escaped quotes...
					char c = image.charAt(j);
					if (c == '"')
						toPrint += "\\";
					toPrint += c;
				} else {
	...
	static void DumpNullStrLiterals(java.io.PrintWriter ostr) {
		...
		if (NfaState.generatedStates != 0)
			ostr.println("   return mccMoveNfa" + LexGen.lexStateSuffix + "(" + NfaState.InitStateName() + ", 0);");
		else
	...
	static String GetLabel(int kind) {
		...
		if (re instanceof RStringLiteral)
			return " \"" + CSharpCCGlobals.add_escapes(((RStringLiteral)re).image) + "\"";
	...
	static void DumpStartWithStates(java.io.PrintWriter ostr) {
		ostr.println((Options.B("STATIC") ? "static " : "") + "private int " +
										  "mccStartNfaWithStates" + LexGen.lexStateSuffix + "(int pos, int kind, int state) {");
		ostr.println("   mccmatchedKind = kind;");
		ostr.println("   mccmatchedPos = pos;");
		...
		ostr.println("   catch(System.IO.IOException) { return pos + 1; }");
		...
		ostr.println("   return mccMoveNfa" + LexGen.lexStateSuffix + "(state, pos + 1);");
		ostr.println("}");
	}
	...
	static void DumpDfaCode(java.io.PrintWriter ostr) {
		...
		if (maxLen == 0) {
			ostr.println((Options.B("STATIC") ? "static " : "") + "private int " +
				 "mccMoveStringLiteralDfa0" + LexGen.lexStateSuffix + "()");
		...
		for (i = 0; i < maxLen; i++) {
			...
			ostr.print((Options.B("STATIC") ? "static " : "") + "private int " +
				   "mccMoveStringLiteralDfa" + i + LexGen.lexStateSuffix + "(");
			...
			if (i != 0) {
				if (i > 1) {
					...
					ostr.println(") == 0L)");
					if (!LexGen.mixed[LexGen.lexStateIndex] && NfaState.generatedStates != 0) {
						ostr.print("      return mccStartNfa" + LexGen.lexStateSuffix +
							   "(" + (i - 2) + ", ");
						for (j = 0; j < maxLongsReqd - 1; j++)
							if (i <= maxLenForActive[j] + 1)
								ostr.print("old" + j + ", ");
						...
						ostr.println("      return mccMoveNfa" + LexGen.lexStateSuffix + "(" + NfaState.InitStateName() + ", " + (i - 1) + ");");
					else
				...
				if (i != 0 && Options.B("DEBUG_TOKEN_MANAGER")) {
					...
					ostr.println("   if (mccmatchedKind != 0 && mccmatchedKind != Int32.MaxValue)");
					ostr.println("      debugStream.WriteLine(\"   Currently matched the first \" + (mccmatchedPos + 1) + \" characters as a \" + tokenImage[mccmatchedKind] + \" token.\");");
					...
					for (int vecs = 0; vecs < maxStrKind / 64 + 1; vecs++) {
						if (i <= maxLenForActive[vecs]) {
							ostr.println(" + ");
							ostr.print("         mccKindsForBitVector(" + vecs + ", ");
				...
				ostr.println("   try { curChar = input_stream.ReadChar(); }");
				ostr.println("   catch(System.IO.IOException) {");

				if (!LexGen.mixed[LexGen.lexStateIndex] && NfaState.generatedStates != 0) {
					ostr.print("      mccStopStringLiteralDfa" + LexGen.lexStateSuffix + "(" + (i - 1) + ", ");
			...
			Outer:
					...
					for (j = 0; j < maxLongsReqd; j++) {
						...
						for (k = 0; k < 64; k++) {
							...
							if (i != 0) {
								...
								//	     " & 0x" + Long.toHexString(1L << k) +
								//	     "L) != 0L)");
								// modified CSharpCC code:
								ostr.println("((active" + j + " & " + Long.toString(1L << k) + "L) != 0L)");
							}
							...
							intermediateMatchedPos[(j * 64 + k)][i] == i) {
								CSharpCCErrors.warning(" \"" +
									CSharpCCGlobals.add_escapes(allImages[j * 64 + k]) +
							...
							if (!subString[(j * 64 + k)]) {
								...
								if (stateSetName != -1) {
									ostr.println(prefix + "return mccStartNfaWithStates" +
										 LexGen.lexStateSuffix + "(" + i +
										 ", " + kindToPrint + ", " + stateSetName + ");");
								} else
									ostr.println(prefix + "return mccStopAtPos" + "(" + i + ", " + kindToPrint + ");");
							} else {
								if ((LexGen.initMatch[LexGen.lexStateIndex] != 0 &&
									 LexGen.initMatch[LexGen.lexStateIndex] != Integer.MAX_VALUE) ||
									i != 0) {
									ostr.println("         {");
									ostr.println(prefix + "mccmatchedKind = " +
										 kindToPrint + ";");
									ostr.println(prefix + "mccmatchedPos = " + i + ";");
									ostr.println("         }");
								} else
									ostr.println(prefix + "mccmatchedKind = " +
										 kindToPrint + ";");
				...
				if (info.validKindCnt != 0) {
					...
					if (i == 0) {
						...
						ostr.print("mccMoveStringLiteralDfa" + (i + 1) +
						...
						for (j = 0; j < maxLongsReqd - 1; j++)
							if ((i + 1) <= maxLenForActive[j]) {
								...
								//	   "L");
								// modified CSharpCC code:
								ostr.print(Long.toString(info.validKinds[j]) + "L");
							}
						...
						for (j = 0; j < maxLongsReqd - 1; j++)
							if ((i + 1) <= maxLenForActive[j] + 1) {
								...
								if (info.validKinds[j] != 0L)
									...
									//	       Long.toHexString(info.validKinds[
									//	    j]) + "L");
									// modified CSharpCC code:
									ostr.print("active" + j + ", " + Long.toString(info.validKinds[j]) + "L");
								else
...
upto and including this position for the matched string. */
						...
						ostr.print("   return mccStartNfa" + LexGen.lexStateSuffix + "(" + (i - 1) + ", ");
						for (k = 0; k < maxLongsReqd - 1; k++)
							if (i <= maxLenForActive[k])
								ostr.print("active" + k + ", ");
							else
						...
						ostr.println("   return mccMoveNfa" + LexGen.lexStateSuffix + "(" + NfaState.InitStateName() + ", " + i + ");");
					else
						ostr.println("   return " + (i + 1) + ";");
				}
			}
	...
	static void GenerateNfaStartStates(java.io.PrintWriter ostr,
		...
		int i, j, kind, mccmatchedPos = 0;
		int maxKindsReqd = maxStrKind / 64 + 1;
		long[] actives;
		Vector newStates = new Vector();
		Vector oldStates = null, mcctmpStates;
		...
		for (i = 0; i < maxStrKind; i++) {
			...
			try {
				...
				CSharpCCErrors.semantic_error("Error cloning state vector");
			}
			...
			for (j = 0; j < image.length(); j++) {
				if (oldStates == null || oldStates.size() <= 0) {
					...
					mccmatchedPos = intermediateMatchedPos[i][j] = intermediateMatchedPos[i][j - 1];
				} else {
					...
					if (GetStrKind(image.substring(0, j + 1)) < kind) {
						...
						mccmatchedPos = intermediateMatchedPos[i][j] = j;
					} else if (j == 0)
				...
				mcctmpStates = oldStates;
				oldStates = newStates;
				(newStates = mcctmpStates).removeAllElements();

				if (statesForPos[j] == null)
					statesForPos[j] = new Hashtable();

				if ((actives = ((long[])statesForPos[j].get(kind + ", " +
					mccmatchedPos + ", " + stateSetString))) == null) {
					actives = new long[maxKindsReqd];
					statesForPos[j].put(kind + ", " + mccmatchedPos + ", " +
	...
	static void DumpNfaStartStatesCode(Hashtable[] statesForPos,
		...
		ostr.print("private" + (Options.B("STATIC") ? " static" : "") + " int mccStopStringLiteralDfa" +
		...
		for (i = 0; i < maxLen - 1; i++) {
			...
			while (e.hasMoreElements()) {
				...
				for (int j = 0; j < maxKindsReqd; j++) {
					...
					// modified CSharpCC code:
					ostr.print("(active" + j + " & " + Long.toString(actives[j]) + "L) != 0L");
				}
				...
				if (condGenerated) {
					...
					int mccmatchedPos = Integer.parseInt(
					...
					if (!kindStr.equals(String.valueOf(Integer.MAX_VALUE))) {
						if (i == 0) {
							ostr.println("            mccmatchedKind = " + kindStr + ";");

							if ((LexGen.initMatch[LexGen.lexStateIndex] != 0 &&
							 LexGen.initMatch[LexGen.lexStateIndex] != Integer.MAX_VALUE))
								ostr.println("            mccmatchedPos = 0;");
						} else if (i == mccmatchedPos) {
							if (subStringAtPos[i]) {
								ostr.println("            if (mccmatchedPos != " + i + ") {");
								ostr.println("               mccmatchedKind = " + kindStr + ";");
								ostr.println("               mccmatchedPos = " + i + ";");
								ostr.println("            }");
							} else {
								ostr.println("            mccmatchedKind = " + kindStr + ";");
								ostr.println("            mccmatchedPos = " + i + ";");
							...
							if (mccmatchedPos > 0)
								ostr.println("            if (mccmatchedPos < " + mccmatchedPos + ")");
							else
								ostr.println("            if (mccmatchedPos == 0)");
							ostr.println("            {");
							ostr.println("               mccmatchedKind = " + kindStr + ";");
							ostr.println("               mccmatchedPos = " + mccmatchedPos + ";");
							ostr.println("            }");
						}
					}
		...
		ostr.print("private" + (Options.B("STATIC") ? " static" : "") + " int mccStartNfa" +
			   LexGen.lexStateSuffix + "(int pos, ");
		for (i = 0; i < maxKindsReqd - 1; i++)
			ostr.print("long active" + i + ", ");
		ostr.println("long active" + i + ")\n{");
		...
		if (LexGen.mixed[LexGen.lexStateIndex]) {
			if (NfaState.generatedStates != 0)
				ostr.println("   return mccMoveNfa" + LexGen.lexStateSuffix + "(" + NfaState.InitStateName() + ", pos + 1);");
			else
				ostr.println("   return pos + 1;");
		...
		ostr.print("   return mccMoveNfa" + LexGen.lexStateSuffix + "(" +
			   "mccStopStringLiteralDfa" + LexGen.lexStateSuffix + "(pos, ");
		for (i = 0; i < maxKindsReqd - 1; i++)
			ostr.print("active" + i + ", ");
		ostr.print("active" + i + ")");
		ostr.println(", pos + 1);");
