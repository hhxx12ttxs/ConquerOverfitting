...
public class VariableTraceAgent {
    ...
    public static void premain(String agentArgs, Instrumentation inst) throws Exception{
        ...
        if (targetClassName.length() < 1 || targetLineNum == -1 || targetVariables.length == 0){
            throw new Exception("Wrong Agent Args");
        }
        inst.addTransformer(new AddPrintTransformer(targetClassName, targetLineNum, targetVariables));
    }
}
